...
Type objective_function<Type>::operator()()
{
  ...
  if (model_type == 93) {
    ...
    // Missing-predictor mi() 2-point sum for a binary predictor; runs after all
    // mu/sigma random-effect contributions and the final log_sigma clamp, so
    // eta1/eta0 and the response density agree with what the main loop below
    // will use. mu is identity-linked here, so the mi() update lands on mu(i)
    // directly rather than on a separate eta_mu.
    if (has_mi == 1 && mi_family == 1) {
      vector<Type> mi_eta = X_mi * beta_mi;
      vector<Type> mi_probability(mi_eta.size());
      vector<Type> mi_x_full(mi_x.size());
      ...
      for (int i = 0; i < mi_x.size(); ++i) {
        ...
        if (mi_observed(i) == 1) {
          ...
          Type eta1 = mu(i) + beta_mu(mi_col) * (Type(1.0) - X_mu(i, mi_col));
          Type eta0 = mu(i) + beta_mu(mi_col) * (Type(0.0) - X_mu(i, mi_col));
          Type log_y1 = observed_y(i) == 1 ?
    ...
    // mu/sigma random-effect contributions and the final log_sigma clamp, so
    // eta1/eta0 and the response density agree with what the main loop below
    // will use. mu is computed AFTER this block (not before), so REPORT(mu)
    // reflects the mi()-imputed rows too.
    if (has_mi == 1 && mi_family == 1) {
      vector<Type> mi_eta = X_mi * beta_mi;
      vector<Type> mi_probability(mi_eta.size());
      vector<Type> mi_x_full(mi_x.size());
