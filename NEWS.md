...
* `drmTMB()` now fits Gaussian location-scale models with fixed effects, random intercepts, independent numeric random slopes, and ordinary correlated random intercept-slope blocks in the `mu` formula, such as `bf(y ~ x1 + (1 | id) + (0 + x1 | id), sigma ~ x1)` and `bf(y ~ x1 + (1 + x1 | id), sigma ~ x1)`.
* Gaussian `mu` random-effect correlations from ordinary correlated blocks are exposed as `corpars$mu`, keeping them separate from residual bivariate `rho12`.
