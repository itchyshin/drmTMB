...
## Zero-one-beta `coi` random intercept at point-fit recovery

* `zero_one_beta()` now admits the exact complete-response ML-Laplace route
  `bf(y ~ x, sigma ~ 1, zoi ~ 1, coi ~ 1 + (1 | id))`. Independent objective
  and gradient checks, current-source compatibility tests, and a retained
  `M = 64`, 50-observations-per-group recovery rung support population-level
  point recovery only. Sparse observed zeroes or ones can weakly identify
  individual conditional modes, so inspect within-group atom counts. Direct
  profiles, intervals, coverage, `coi` slopes, simultaneous or structured atom
  effects, missing responses, REML, and AGHQ remain unavailable.
...
* `student()`, `lognormal()`, `Gamma(link = "log")`, `beta()`, `beta_binomial()`, and `truncated_nbinom2()` now support ordinary unlabelled independent numeric `mu` random slopes such as `(0 + x | id)` beside their first ordinary `mu` random-intercept slices. Historical boundary note, superseded for the exact zero-one-beta ordinary `zoi` q1 intercept, same-raw-symbol `zoi` slope, and `coi` q1 intercept gates: other zero-one-beta random effects, correlated slopes, labelled covariance, broad structured effects, and unsupported shape/inflation neighbours remain planned.
...
* `zero_one_beta()` now fits the first fixed-effect route for continuous proportions on `[0, 1]` with structural exact 0 or 1 outcomes. The interior beta component uses `mu` and public scale `sigma`; `zoi` models the probability of an exact boundary outcome; `coi` models the probability that a boundary outcome is exactly 1; and `fitted()` returns the unconditional mean `(1 - zoi) * mu + zoi * coi`. Historical boundary note, superseded for the exact ordinary `zoi` q1 intercept, same-raw-symbol `zoi` slope, and `coi` q1 intercept gates: structured effects, other atom random effects, denominator syntax, known covariance, and bivariate bounded-response models remain planned or blocked.
...
* Shape and inflation random-effect boundaries now have random-slope-specific test coverage. Historical boundary note, superseded for the exact ordinary zero-one-beta same-raw-symbol `zoi` q1 slope: Student-t `nu`, zero-inflation `zi`, hurdle `hu`, `coi`, and all broader or mismatched `zoi` random slopes still error before fitting.
