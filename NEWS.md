...
* `drmTMB()` now fits Gaussian location-scale models with fixed effects, random intercepts, and simple numeric random slopes in the `mu` formula, such as `bf(y ~ x1 + (1 | id) + (0 + x1 | id), sigma ~ x1)`.
