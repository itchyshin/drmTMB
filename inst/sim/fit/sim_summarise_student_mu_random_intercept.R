phase18_summarise_student_mu_ri_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) ||
      !identical(truth$surface, "student_mu_random_intercept")
  ) {
    stop(
      "`truth` must be a Student-t mu random-intercept truth object.",
      call. = FALSE
    )
  }
  ...
  out <- data.frame(
    ...
    parameter_class = c(
      ...
      rep("fixed_nu", length(nu_est)),
      rep("random_sd", length(sd_est))
    ),
    truth = unname(truth_value),
    estimate = unname(estimate),
    std.error = unname(phase18_student_mu_ri_std_error(fit, parameter)),
  ...
  phase18_student_mu_ri_attach_profiles(
    out,
    fit = fit,
...
phase18_student_mu_ri_attach_profiles <- function(
  summary,
  fit,
  profile_random_sd,
  profile_level
...
phase18_student_mu_ri_std_error <- function(fit, parameter) {
