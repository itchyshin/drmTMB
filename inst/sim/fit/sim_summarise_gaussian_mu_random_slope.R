phase18_summarise_gaussian_mu_rs_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) ||
      !identical(truth$surface, "gaussian_mu_random_slope")
  ) {
    stop(
      "`truth` must be a Gaussian mu random-slope truth object.",
      call. = FALSE
    )
  }
  ...
  estimate <- c(mu_est, sigma_est, sd_est, cor_est)
  truth_value <- c(mu_truth, sigma_truth, sd_truth, cor_truth)
  parameter <- c(
  ...
  data.frame(
    ...
    parameter_class = c(
      ...
      rep("random_correlation", length(cor_est))
    ),
    truth = unname(truth_value),
    estimate = unname(estimate),
    std.error = unname(phase18_gaussian_mu_rs_std_error(fit, parameter)),
...
phase18_gaussian_mu_rs_std_error <- function(fit, parameter) {
