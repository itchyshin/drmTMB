phase18_summarise_biv_gaussian_q4_location_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) || !identical(truth$surface, "biv_gaussian_q4_location")
  ) {
    stop(
      "`truth` must be a bivariate Gaussian q4 location truth object.",
      call. = FALSE
    )
  }
  ...
  data.frame(
    ...
    std.error = unname(
      phase18_biv_gaussian_q4_location_std_error(fit, parameter)
    ),
...
phase18_biv_gaussian_q4_location_std_error <- function(fit, parameter) {
