phase18_summarise_poisson_mu_re_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) ||
      !identical(truth$surface, "poisson_mu_random_effect")
  ) {
    stop(
      "`truth` must be a Poisson mu random-effect truth object.",
      call. = FALSE
    )
  }
  ...
  out <- data.frame(
    ...
    std.error = unname(phase18_poisson_mu_re_std_error(fit, parameter)),
  ...
  phase18_poisson_mu_re_attach_profiles(
    out,
    fit = fit,
...
phase18_poisson_mu_re_attach_profiles <- function(
  summary,
  fit,
  profile_random_sd,
  profile_level
...
phase18_poisson_mu_re_std_error <- function(fit, parameter) {
