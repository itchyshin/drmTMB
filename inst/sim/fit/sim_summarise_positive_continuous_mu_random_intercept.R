phase18_summarise_positive_continuous_mu_ri_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) ||
      !identical(truth$surface, "positive_continuous_mu_random_intercept")
  ) {
    stop(
      "`truth` must be a positive-continuous mu random-intercept truth object.",
      call. = FALSE
    )
  }
  ...
  out <- data.frame(
    ...
    std.error = unname(
      phase18_positive_continuous_mu_ri_std_error(fit, parameter)
    ),
  ...
  phase18_positive_continuous_mu_ri_attach_profiles(
    out,
    fit = fit,
...
phase18_positive_continuous_mu_ri_attach_profiles <- function(
  summary,
  fit,
  profile_random_sd,
  profile_level
...
phase18_positive_continuous_mu_ri_std_error <- function(fit, parameter) {
