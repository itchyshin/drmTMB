phase18_summarise_relmat_mu_slope_fit <- function(
  fit,
  truth,
  ...
  if (!is.list(truth) || !identical(truth$surface, "relmat_mu_slope")) {
    stop("`truth` must be a relmat mu slope truth object.", call. = FALSE)
  }
  ...
  data.frame(
    ...
    parameter_class = c(
      ...
      "residual_sigma",
      rep("relmat_sd", length(sd_est))
    ),
    truth = unname(truth_value),
    estimate = unname(estimate),
    std.error = unname(phase18_relmat_mu_slope_std_error(fit, parameter)),
...
phase18_relmat_mu_slope_std_error <- function(fit, parameter) {
