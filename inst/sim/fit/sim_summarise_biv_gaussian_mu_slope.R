phase18_summarise_biv_gaussian_mu_slope_fit <- function(
  fit,
  truth,
  ...
  if (!is.list(truth) || !identical(truth$surface, "biv_gaussian_mu_slope")) {
    stop(
      "`truth` must be a bivariate Gaussian mu-slope truth object.",
      call. = FALSE
    )
  }
  ...
  sd_truth <- truth$sd_mu
  slope_cor_truth <- truth$rho_slope
  ...
  slope_cor_est <- fit$corpars$mu[names(slope_cor_truth)]
  ...
  parameter <- c(
    ...
    paste0("cor:mu:", names(slope_cor_est)),
    "rho12"
  )
  ...
  data.frame(
    ...
    parameter_class = c(
      ...
      rep("random_correlation", length(slope_cor_est)),
      "residual_rho12"
    ),
    truth = unname(truth_value),
    estimate = unname(estimate),
    std.error = unname(phase18_biv_gaussian_mu_slope_std_error(fit, parameter)),
...
phase18_biv_gaussian_mu_slope_std_error <- function(fit, parameter) {
