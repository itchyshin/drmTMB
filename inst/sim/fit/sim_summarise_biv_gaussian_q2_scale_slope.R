phase18_summarise_biv_gaussian_q2_scale_slope_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) || !identical(truth$surface, "biv_gaussian_q2_scale_slope")
  ) {
    stop(
      "`truth` must be a bivariate Gaussian q2 scale slope truth object.",
      call. = FALSE
    )
  }
  ...
  data.frame(
    ...
    std.error = unname(
      phase18_biv_gaussian_q2_scale_slope_std_error(fit, parameter)
    ),
...
phase18_biv_gaussian_q2_scale_slope_std_error <- function(fit, parameter) {
