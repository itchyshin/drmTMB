phase18_summarise_nbinom2_mu_re_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) ||
      !identical(truth$surface, "nbinom2_mu_random_effect")
  ) {
    stop("`truth` must be an NB2 mu random-effect truth object.", call. = FALSE)
  }
  ...
  out <- data.frame(
    ...
    std.error = unname(phase18_nbinom2_mu_re_std_error(fit, parameter)),
