phase18_summarise_biv_gaussian_mu_sigma_slope_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) ||
      !identical(truth$surface, "biv_gaussian_mu_sigma_slope")
  ) {
    stop(
      "`truth` must be a bivariate Gaussian mu/sigma slope truth object.",
      call. = FALSE
    )
  }
  ...
  cor_est <- fit$corpars$mu_sigma[names(cor_truth)]
  ...
  parameter <- c(
    ...
    paste0("cor:mu_sigma:", names(cor_est)),
    "rho12"
  )
  ...
  data.frame(
    ...
    std.error = unname(
      phase18_biv_gaussian_mu_sigma_slope_std_error(fit, parameter)
    ),
...
phase18_biv_gaussian_mu_sigma_slope_std_error <- function(fit, parameter) {
