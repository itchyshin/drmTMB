phase18_summarise_biv_gaussian_q6_location_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) || !identical(truth$surface, "biv_gaussian_q6_location")
  ) {
    stop(
      "`truth` must be a bivariate Gaussian q6 location truth object.",
      call. = FALSE
    )
  }
  ...
  data.frame(
    ...
    std.error = unname(
      phase18_biv_gaussian_q6_location_std_error(fit, parameter)
    ),
...
phase18_biv_gaussian_q6_location_std_error <- function(fit, parameter) {
