phase18_summarise_proportion_fe_fit <- function(
  fit,
  truth,
  ...
  if (
    !is.list(truth) ||
      !identical(truth$surface, "proportion_fixed_effect")
  ) {
    stop(
      "`truth` must be a proportion fixed-effect truth object.",
      call. = FALSE
    )
  }

  truth_value <- c(truth$beta_mu, truth$beta_sigma)
  names(truth_value) <- c(
  ...
  std_error <- phase18_proportion_fe_fixed_effect_se(fit, names(estimate))
...
phase18_proportion_fe_fixed_effect_se <- function(fit, parameter) {
