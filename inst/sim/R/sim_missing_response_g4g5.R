...
# conflated.
# Design provenance (#982).
#
# The centring switch below changes the DGP but NOT the frozen target manifest --
# `truth` is identical either way -- so the manifest hash cannot distinguish a v1
# artifact from a v2 one.  Nor can the seed: a seed fixes the RNG stream, not what
# is done with it.  A campaign was in fact generated by an ad-hoc driver that set
# the option outside the repository, and its artifact carried nothing to say so.
#
# So every record is stamped with the design state at GENERATION time, on the
# compute node, and the reconcilers refuse to merge receipts that disagree.  This
# cannot be applied retrospectively: records written before this stamp existed
# cannot be authenticated, only inferred about.
.mr_design_state <- function() {
  paste0("centre_random_effects=", isTRUE(.mr_centre_random_effects()))
}

# Stamp the design state on a record frame.  Called from every path that produces
# a G4 or G5 record, including the failure paths -- a record that escaped unstamped
# would be exactly the untraceable case this guards against.
.mr_stamp_design <- function(records) {
  records$design_state <- .mr_design_state()
  records
}
...
mr_g4_add_mask_receipt <- function(records, fixture, route_id) {
  ...
  records <- .mr_stamp_design(records)
...
mr_g4_reconcile_checkpoints <- function(paths) {
  ...
  all_records <- mr_g4g5_check_design_agreement(all_records, "G4 reconciliation")
...
mr_g5_run_attempt <- function(cell, seed, replicate, trace = TRUE) {
  ...
  if (inherits(fixture, "error")) {
    ...
    out$replicate <- as.integer(replicate)
    # This path returns before mr_g4_add_mask_receipt(), so it stamps its own
    # design state -- otherwise a fixture failure would be the one unlabelled record.
    out <- .mr_stamp_design(out)
...
# Refuse to reconcile receipts generated under different designs (#982).
#
# Fail closed on BOTH sides: disagreeing states are a silent conflation of two
# experiments, and a MISSING state means the record predates design stamping and
# cannot be authenticated at all.  Either is a hard stop -- the alternative is an
# artifact whose provenance can only be inferred from its own conclusions.
mr_g4g5_check_design_agreement <- function(records, what = "campaign") {
  if (!is.data.frame(records) || !nrow(records)) return(invisible(records))

  # DISAGREEMENT is a hard stop: merging two designs silently conflates two
  # experiments, and no downstream reader could ever separate them again.
  if ("design_state" %in% names(records)) {
    states <- unique(as.character(records$design_state))
    states <- states[!is.na(states) & nzchar(states)]
    if (length(states) > 1L) {
      stop(sprintf("%s mixes %d designs and cannot be reconciled: %s",
                   what, length(states), paste(sort(states), collapse = " | ")),
           call. = FALSE)
    }
    if (length(states) == 1L && !anyNA(records$design_state)) {
      return(invisible(records))
    }
  }

  # MISSING is NOT a hard stop -- it would deny access to already-computed
  # evidence, which destroys value rather than protecting it. Instead the records
  # are marked UNAUTHENTICATED so the taint travels with the artifact and any
  # claim built on it. Records written before design stamping existed cannot be
  # authenticated; they can only be inferred about, and the marker says so.
  warning(sprintf(
    "%s contains records with no `design_state`: they predate design stamping (#982) and are marked UNAUTHENTICATED. Their design can only be inferred, never verified.",
    what), call. = FALSE)
  if (!"design_state" %in% names(records)) {
    records$design_state <- "UNAUTHENTICATED"
  } else {
    unset <- is.na(records$design_state) | !nzchar(as.character(records$design_state))
    records$design_state[unset] <- "UNAUTHENTICATED"
  }
  invisible(records)
}
...
mr_g5_reconcile_checkpoints <- function(paths, registry) {
  ...
  all_records <- do.call(rbind, records)
  all_records <- mr_g4g5_check_design_agreement(all_records, "G5 reconciliation")
