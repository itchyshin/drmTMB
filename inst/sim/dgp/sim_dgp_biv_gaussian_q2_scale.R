phase18_biv_gaussian_q2_scale_conditions <- function(
  n_id = c(48L),
  n_each = c(8L),
...
phase18_biv_gaussian_q2_scale_sd_names <- function() {
  c(
    "sigma1:(1 | p | id)",
    "sigma2:(1 | p | id)"
  )
...
phase18_biv_gaussian_q2_scale_cor_name <- function() {
  "cor(sigma1:(Intercept),sigma2:(Intercept) | p | id)"
}

phase18_dgp_biv_gaussian_q2_scale <- function(
  n_id,
  n_each,
  beta_mu1 = c("(Intercept)" = 0.20, x = 0.45),
  beta_mu2 = c("(Intercept)" = -0.10, x = -0.35),
  sigma = c(sigma1 = 0.38, sigma2 = 0.52),
  sd_sigma = c(
    "sigma1:(1 | p | id)" = 0.28,
    "sigma2:(1 | p | id)" = 0.34
  ),
  ...
  sd_sigma <- phase18_named_pair(
    sd_sigma,
    phase18_biv_gaussian_q2_scale_sd_names(),
    "sd_sigma"
  )
  if (any(sd_sigma <= 0)) {
  ...
  cor_sigma <- stats::setNames(
    cor_sigma,
    phase18_biv_gaussian_q2_scale_cor_name()
  )
  ...
  draw <- function() {
    ...
    log_sigma1 <- log(sigma[["sigma1"]]) + b[id, 1L]
    log_sigma2 <- log(sigma[["sigma2"]]) + b[id, 2L]
