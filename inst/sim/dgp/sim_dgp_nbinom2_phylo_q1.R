phase18_nbinom2_phylo_q1_conditions <- function(
  n_species = c(20L, 40L),
...
phase18_dgp_nbinom2_phylo_q1 <- function(
  n_species,
  n_per_species,
  beta_mu = c("(Intercept)" = log(2.5), x = -0.20),
  beta_sigma = c("(Intercept)" = log(0.55), z = 0.15),
  sd_phylo = 0.35,
  tree_shape = "balanced",
  seed = NULL,
  ...
  tree_shape <- phase18_nbinom2_phylo_q1_tree_shape(tree_shape)

  draw <- function() {
    tree <- phase18_nbinom2_phylo_q1_tree(n_species, tree_shape)
    covariance <- phase18_nbinom2_phylo_q1_tip_correlation(tree)
    ...
    z <- rep(seq(-0.8, 0.8, length.out = n_per_species), times = n_species)
    eta_mu <- unname(
      beta_mu[["(Intercept)"]] +
        beta_mu[["x"]] * x +
    ...
    mu <- exp(eta_mu)
    sigma <- exp(eta_sigma)
    count <- as.integer(stats::rnbinom(length(mu), size = 1 / sigma^2, mu = mu))
    ...
    attr(dat, "truth") <- list(
      ...
      comparator_sd = stats::setNames(sd_phylo, "(1 | species)"),
      tree = tree,
      tree_shape = tree_shape,
      n_species = n_species,
...
phase18_nbinom2_phylo_q1_tree <- function(n_species, tree_shape = "balanced") {
  assert_positive_whole_number(n_species, "n_species")
  if (n_species < 2L) {
...
phase18_nbinom2_phylo_q1_tip_correlation <- function(tree) {
  covariance <- drmTMB:::drm_phylo_tip_covariance(tree)
  diag_sd <- sqrt(diag(covariance))
  covariance / outer(diag_sd, diag_sd)
}

phase18_nbinom2_phylo_q1_tree_shape <- function(tree_shape) {
