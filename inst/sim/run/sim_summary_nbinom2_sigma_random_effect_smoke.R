phase18_summarise_nbinom2_sigma_re_smoke <- function(
  ...
  run <- phase18_run_nbinom2_sigma_re_smoke(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The NB2 sigma random-effect smoke run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  profile_targets <- phase18_nbinom2_sigma_re_profile_targets(run$summary)
  profile_status_rows <- run$summary[
    run$summary$parameter_class == "random_sd",
    ,
    drop = FALSE
  ]
...
phase18_nbinom2_sigma_re_profile_targets <- function(summary) {
