phase18_write_truncated_nbinom2_mu_ri_grid_outputs <- function(
  output_dir,
  ...
  paths <- list(
    aggregate_csv = file.path(table_dir, "truncated-nb2-mu-ri-aggregate.csv"),
    replicate_csv = file.path(table_dir, "truncated-nb2-mu-ri-replicates.csv"),
    manifest_csv = file.path(table_dir, "truncated-nb2-mu-ri-manifest.csv"),
    failures_csv = file.path(table_dir, "truncated-nb2-mu-ri-failures.csv"),
    wald_intervals_csv = file.path(
      table_dir,
      "truncated-nb2-mu-ri-wald-intervals.csv"
    ),
    wald_coverage_csv = file.path(
      table_dir,
      "truncated-nb2-mu-ri-wald-coverage.csv"
    ),
    profile_intervals_csv = file.path(
      table_dir,
      "truncated-nb2-mu-ri-profile-intervals.csv"
    ),
    profile_coverage_csv = file.path(
      table_dir,
      "truncated-nb2-mu-ri-profile-coverage.csv"
    )
  )
  phase18_assert_truncated_nbinom2_mu_ri_grid_overwrite(paths, overwrite)
  ...
  list(
    surface = "truncated_nbinom2_mu_random_intercept_grid",
...
phase18_assert_truncated_nbinom2_mu_ri_grid_overwrite <- function(
  paths,
  overwrite
) {
  ...
  if (!overwrite && length(existing) > 0L) {
    stop(
      "Truncated NB2 mu random-intercept grid output already exists: ",
