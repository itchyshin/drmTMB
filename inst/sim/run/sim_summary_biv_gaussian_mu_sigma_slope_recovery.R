phase18_summarise_biv_gaussian_mu_sigma_slope_recovery <- function(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The bivariate Gaussian mu/sigma slope recovery run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  # Wald coverage is meaningful only for fixed-effect endpoints that carry a
  # standard error. The mu-slope SD, sigma-slope SD, and their derived
  # same-response correlation have no Wald standard error, so they remain
  # unavailable until a profile or bootstrap lane supplies interval evidence.
  ...
  list(
    surface = "biv_gaussian_mu_sigma_slope_recovery",
    run = run,
    aggregate = aggregate,
