phase18_write_binomial_fe_grid_outputs <- function(
  output_dir,
  ...
  paths <- list(
    aggregate_csv = file.path(table_dir, "binomial-fe-aggregate.csv"),
    replicate_csv = file.path(table_dir, "binomial-fe-replicates.csv"),
    manifest_csv = file.path(table_dir, "binomial-fe-manifest.csv"),
    failures_csv = file.path(table_dir, "binomial-fe-failures.csv"),
    wald_intervals_csv = file.path(table_dir, "binomial-fe-wald-intervals.csv"),
    wald_coverage_csv = file.path(table_dir, "binomial-fe-wald-coverage.csv"),
    comparator_parity_csv = file.path(
      table_dir,
      "binomial-fe-glm-parity.csv"
    )
  )
  phase18_assert_binomial_fe_grid_overwrite(paths, overwrite)
  ...
  utils::write.csv(
    summary$comparator_parity,
    paths$comparator_parity_csv,
    row.names = FALSE
  )

  list(
    surface = "binomial_fixed_effect_grid",
    output_dir = output_dir,
    result_dir = result_dir,
    table_dir = table_dir,
    paths = paths,
    artifact_manifest = phase18_grid_artifact_manifest(
      "binomial_fixed_effect_grid",
      paths
    ),
    summary = summary
  )
