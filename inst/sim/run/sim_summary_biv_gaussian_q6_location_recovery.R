phase18_summarise_biv_gaussian_q6_location_recovery <- function(
  ...
  run <- phase18_run_biv_gaussian_q6_location_smoke(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The bivariate Gaussian q6 location recovery run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  # q=6 location random-effect SDs and the fifteen derived location-location
  # correlations have no Wald standard error, so they fall out of the coverage
  # table as unusable rather than being reported as interval-ready. This keeps
  # the q6 correlations at derived_interval_unavailable.
  ...
  list(
    surface = "biv_gaussian_q6_location_recovery",
    run = run,
    aggregate = aggregate,
