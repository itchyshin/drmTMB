phase18_write_biv_gaussian_q6_location_recovery_grid_outputs <- function(
  output_dir,
  ...
  prefix <- "biv-gaussian-q6-location-recovery"
  paths <- list(
  ...
  phase18_assert_biv_gaussian_q6_location_recovery_grid_overwrite(
    paths,
    overwrite
  )

  summary <- phase18_summarise_biv_gaussian_q6_location_recovery(
  ...
  list(
    surface = "biv_gaussian_q6_location_recovery_grid",
    output_dir = output_dir,
    result_dir = result_dir,
    table_dir = table_dir,
    paths = paths,
    artifact_manifest = phase18_grid_artifact_manifest(
      "biv_gaussian_q6_location_recovery_grid",
      paths
    ),
    summary = summary
  )
...
phase18_assert_biv_gaussian_q6_location_recovery_grid_overwrite <- function(
  paths,
  overwrite
) {
  ...
  if (!overwrite && length(existing) > 0L) {
    stop(
      "Bivariate Gaussian q6 location recovery grid output already exists: ",
