phase18_summarise_bounded_response_mu_ri_smoke <- function(
  ...
  run <- phase18_run_bounded_response_mu_ri_smoke(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The bounded-response mu random-intercept smoke run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  profile_intervals <- phase18_bounded_response_mu_ri_profile_intervals(
    run$summary
  )
...
phase18_bounded_response_mu_ri_profile_intervals <- function(summary) {
