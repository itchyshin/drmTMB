phase18_write_biv_gaussian_mu_sigma_slope_recovery_grid_outputs <- function(
  output_dir,
  ...
  phase18_assert_biv_gaussian_mu_sigma_slope_recovery_grid_overwrite(
    paths,
    overwrite
  )

  summary <- phase18_summarise_biv_gaussian_mu_sigma_slope_recovery(
  ...
  list(
    surface = "biv_gaussian_mu_sigma_slope_recovery_grid",
    output_dir = output_dir,
    result_dir = result_dir,
    table_dir = table_dir,
    paths = paths,
    artifact_manifest = phase18_grid_artifact_manifest(
      "biv_gaussian_mu_sigma_slope_recovery_grid",
      paths
    ),
    summary = summary
  )
...
phase18_assert_biv_gaussian_mu_sigma_slope_recovery_grid_overwrite <- function(
  paths,
  overwrite
) {
  ...
  if (!overwrite && length(existing) > 0L) {
    stop(
      "Bivariate Gaussian mu/sigma slope recovery grid output already exists: ",
