phase18_summarise_biv_gaussian_q8_endpoint_recovery <- function(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The bivariate Gaussian q8 endpoint recovery run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  # The q8 smoke/recovery runner deliberately fits with se = FALSE so the
  # artifact lane stays tractable. These tables make interval unavailability
  # explicit instead of promoting q8 coverage.
  ...
  list(
    surface = "biv_gaussian_q8_endpoint_recovery",
    run = run,
    aggregate = aggregate,
