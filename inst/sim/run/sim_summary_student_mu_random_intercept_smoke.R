phase18_summarise_student_mu_ri_smoke <- function(
  ...
  run <- phase18_run_student_mu_ri_smoke(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The Student-t mu random-intercept smoke run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  interval_scale <- ifelse(
    run$summary$parameter_class %in% c("fixed_mu", "fixed_sigma", "fixed_nu"),
    "formula_coefficient",
    "public_sd"
  )
  ...
  profile_intervals <- phase18_student_mu_ri_profile_intervals(run$summary)
...
phase18_student_mu_ri_profile_intervals <- function(summary) {
