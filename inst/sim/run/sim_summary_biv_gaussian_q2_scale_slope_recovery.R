phase18_summarise_biv_gaussian_q2_scale_slope_recovery <- function(
  ...
  run <- phase18_run_biv_gaussian_q2_scale_slope_smoke(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The bivariate Gaussian q2 scale slope recovery run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  # random-effect SD and derived scale-scale correlation rows have no Wald
  # standard error, so they fall out of the coverage table as unusable rather
  # than being reported as interval-ready. This keeps the derived scale-scale
  # correlation at derived_interval_unavailable, consistent with the q8 gate.
  ...
  list(
    surface = "biv_gaussian_q2_scale_slope_recovery",
    run = run,
    aggregate = aggregate,
