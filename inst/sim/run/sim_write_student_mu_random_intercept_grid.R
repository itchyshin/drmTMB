phase18_write_student_mu_ri_grid_outputs <- function(
  output_dir,
  ...
  paths <- list(
    aggregate_csv = file.path(table_dir, "student-mu-ri-aggregate.csv"),
    replicate_csv = file.path(table_dir, "student-mu-ri-replicates.csv"),
    manifest_csv = file.path(table_dir, "student-mu-ri-manifest.csv"),
    failures_csv = file.path(table_dir, "student-mu-ri-failures.csv"),
    wald_intervals_csv = file.path(
      table_dir,
      "student-mu-ri-wald-intervals.csv"
    ),
    wald_coverage_csv = file.path(
      table_dir,
      "student-mu-ri-wald-coverage.csv"
    ),
    profile_intervals_csv = file.path(
      table_dir,
      "student-mu-ri-profile-intervals.csv"
    ),
    profile_coverage_csv = file.path(
      table_dir,
      "student-mu-ri-profile-coverage.csv"
    )
  )
  phase18_assert_student_mu_ri_grid_overwrite(paths, overwrite)
  ...
  list(
    surface = "student_mu_random_intercept_grid",
...
phase18_assert_student_mu_ri_grid_overwrite <- function(paths, overwrite) {
  path_values <- unlist(paths, use.names = FALSE)
  existing <- path_values[file.exists(path_values)]
  if (!overwrite && length(existing) > 0L) {
    stop(
      "Student-t mu random-intercept grid output already exists: ",
