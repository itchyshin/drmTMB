phase18_summarise_biv_gaussian_q2_scale_recovery <- function(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The bivariate Gaussian q2 scale recovery run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  list(
    surface = "biv_gaussian_q2_scale_recovery",
    run = run,
    aggregate = aggregate,
