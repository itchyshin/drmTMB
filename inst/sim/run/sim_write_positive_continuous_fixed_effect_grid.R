phase18_write_positive_continuous_fe_grid_outputs <- function(
  output_dir,
  ...
  paths <- list(
    aggregate_csv = file.path(
      table_dir,
      "positive-continuous-fe-aggregate.csv"
    ),
    replicate_csv = file.path(
      table_dir,
      "positive-continuous-fe-replicates.csv"
    ),
    manifest_csv = file.path(
      table_dir,
      "positive-continuous-fe-manifest.csv"
    ),
    failures_csv = file.path(
      table_dir,
      "positive-continuous-fe-failures.csv"
    ),
    wald_intervals_csv = file.path(
      table_dir,
      "positive-continuous-fe-wald-intervals.csv"
    ),
    wald_coverage_csv = file.path(
      table_dir,
      "positive-continuous-fe-wald-coverage.csv"
    )
  )
  phase18_assert_positive_continuous_fe_grid_overwrite(paths, overwrite)
  ...
  list(
    surface = "positive_continuous_fixed_effect_grid",
...
phase18_assert_positive_continuous_fe_grid_overwrite <- function(
  paths,
  overwrite
) {
  ...
  if (!overwrite && length(existing) > 0L) {
    stop(
      "Positive-continuous fixed-effect grid output already exists: ",
