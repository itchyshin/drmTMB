phase18_summarise_biv_gaussian_q4_location_recovery <- function(
  ...
  run <- phase18_run_biv_gaussian_q4_location_smoke(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The bivariate Gaussian q4 location recovery run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  # q=4 location random-effect SDs and the six derived location-location
  ...
  # the q4 correlations at derived_interval_unavailable.
  ...
  list(
    surface = "biv_gaussian_q4_location_recovery",
    run = run,
    aggregate = aggregate,
