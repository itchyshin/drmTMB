phase18_write_tweedie_fe_grid_outputs <- function(
  output_dir,
  ...
  paths <- list(
    aggregate_csv = file.path(table_dir, "tweedie-fe-aggregate.csv"),
    replicate_csv = file.path(table_dir, "tweedie-fe-replicates.csv"),
    manifest_csv = file.path(table_dir, "tweedie-fe-manifest.csv"),
    failures_csv = file.path(table_dir, "tweedie-fe-failures.csv"),
    wald_intervals_csv = file.path(
      table_dir,
      "tweedie-fe-wald-intervals.csv"
    ),
    wald_coverage_csv = file.path(
      table_dir,
      "tweedie-fe-wald-coverage.csv"
    )
  )
  phase18_assert_tweedie_fe_grid_overwrite(paths, overwrite)
  ...
  list(
    surface = "tweedie_fixed_effect_grid",
