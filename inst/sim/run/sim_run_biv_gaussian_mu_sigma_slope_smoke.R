phase18_dgp_biv_gaussian_mu_sigma_slope_cell <- function(
  cell,
  seed,
  cell_id,
  replicate
  ...
  phase18_dgp_biv_gaussian_mu_sigma_slope(
    n_id = cell$n_id[[1L]],
    ...
    sd_mu = c(
      "mu1:(0 + x | p | id)" = cell$sd_mu1[[1L]]
    ),
    sd_sigma = c(
      "sigma1:(0 + x | p | id)" = cell$sd_sigma1[[1L]]
    ),
    cor_mu_sigma = cell$cor_mu1_sigma1[[1L]],
...
phase18_fit_biv_gaussian_mu_sigma_slope <- function(data, cell) {
  phase18_assert_one_row_data_frame(cell, "cell")
  drmTMB(
    bf(
      mu1 = y1 ~ x + (0 + x | p | id),
...
phase18_run_biv_gaussian_mu_sigma_slope_smoke <- function(
  ...
  results <- phase18_run_replicates(
    ...
    dgp_fun = phase18_dgp_biv_gaussian_mu_sigma_slope_cell,
    fit_fun = phase18_fit_biv_gaussian_mu_sigma_slope,
    summarise_fun = phase18_summarise_biv_gaussian_mu_sigma_slope_fit,
