phase18_write_proportion_fe_grid_outputs <- function(
  output_dir,
  ...
  paths <- list(
    aggregate_csv = file.path(table_dir, "proportion-fe-aggregate.csv"),
    replicate_csv = file.path(table_dir, "proportion-fe-replicates.csv"),
    manifest_csv = file.path(table_dir, "proportion-fe-manifest.csv"),
    failures_csv = file.path(table_dir, "proportion-fe-failures.csv"),
    wald_intervals_csv = file.path(
      table_dir,
      "proportion-fe-wald-intervals.csv"
    ),
    wald_coverage_csv = file.path(
      table_dir,
      "proportion-fe-wald-coverage.csv"
    )
  )
  phase18_assert_proportion_fe_grid_overwrite(paths, overwrite)
  ...
  list(
    surface = "proportion_fixed_effect_grid",
...
phase18_assert_proportion_fe_grid_overwrite <- function(paths, overwrite) {
  path_values <- unlist(paths, use.names = FALSE)
  existing <- path_values[file.exists(path_values)]
  if (!overwrite && length(existing) > 0L) {
    stop(
      "Proportion fixed-effect grid output already exists: ",
