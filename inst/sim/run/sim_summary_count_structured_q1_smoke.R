phase18_summarise_count_structured_q1_smoke <- function(
  ...
  run <- phase18_run_count_structured_q1_smoke(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The count structured q1 smoke run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
  ...
  profile_targets <- phase18_count_structured_q1_profile_targets(run$summary)
  profile_status_rows <- run$summary[
    run$summary$parameter_class == "structured_sd",
    ,
    drop = FALSE
  ]
...
phase18_count_structured_q1_profile_targets <- function(summary) {
  ...
  out <- summary[
    summary$parameter_class == "structured_sd",
    c(
      "surface",
      "family",
      "structured_type",
