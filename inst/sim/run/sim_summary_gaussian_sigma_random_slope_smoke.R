phase18_summarise_gaussian_sigma_rs_smoke <- function(
  ...
  run <- phase18_run_gaussian_sigma_rs_smoke(
  ...
  if (nrow(run$summary) == 0L) {
    stop(
      "The Gaussian sigma random-slope smoke run produced no summaries.",
      call. = FALSE
    )
  }
  if (is.null(by)) {
