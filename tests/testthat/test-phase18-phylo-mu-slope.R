...
test_that("Phase 18 phylo mu slope DGP is seeded and self-describing", {
  ...
  dat <- phase18_dgp_phylo_mu_slope(
    ...
    n_each = 6L,
    seed = 244L,
    cell_id = "phylo_mu_slope_001",
    replicate = 1L
  )
  again <- phase18_dgp_phylo_mu_slope(
    n_tip = 8L,
    n_each = 6L,
    seed = 244L,
    cell_id = "phylo_mu_slope_001",
  ...
  expect_named(
    dat,
    c("y", "x", "species", "mu", "sigma", "cell_id", "replicate")
  )
  expect_identical(truth$surface, "phylo_mu_slope")
  ...
  expect_equal(row.names(truth$A), paste0("sp_", seq_len(8L)))
  expect_named(truth$phylo_intercept, paste0("sp_", seq_len(8L)))
  expect_named(truth$phylo_slope, paste0("sp_", seq_len(8L)))
  expect_length(truth$phylo_intercept, 8L)
  expect_length(truth$phylo_slope, 8L)

  fit <- phase18_fit_phylo_mu_slope(dat, conditions)
...
test_that("Phase 18 phylo mu slope smoke runner summarises output", {
  source_phase18_phylo_mu_slope()

  result_dir <- tempfile("phase18-phylo-mu-slope-")
  ...
  out <- phase18_summarise_phylo_mu_slope_smoke(
  ...
  expect_identical(out$surface, "phylo_mu_slope")
  ...
  expect_setequal(
    ...
    c("fixed_mu", "residual_sigma", "phylo_sd")
  )
  expect_equal(
...
test_that("Phase 18 phylo mu slope grid writer creates artifacts", {
  source_phase18_phylo_mu_slope()

  output_dir <- tempfile("phase18-phylo-mu-slope-grid-")
  ...
  expect_equal(out$surface, "phylo_mu_slope_grid")
...
test_that("Phase 18 phylo mu slope helpers reject malformed inputs", {
  source_phase18_phylo_mu_slope()

  expect_error(
    phase18_dgp_phylo_mu_slope(1L, 5L),
    "at least 2"
  )
  expect_error(
    phase18_dgp_phylo_mu_slope(6L, 5L),
    "power of two"
  )
  expect_error(
    phase18_dgp_phylo_mu_slope(8L, 1L),
    "at least 2"
  )
  expect_error(
    phase18_dgp_phylo_mu_slope(8L, 5L, sd = c(0.4, -0.2)),
    "positive"
  )
  expect_error(
    phase18_dgp_phylo_mu_slope_cell(
