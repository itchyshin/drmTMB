# S6 A7 / #962: missing-PREDICTOR mi() with a Gamma mean-CV response
# (model_type 5). One binary missing predictor. The 2-point-sum density is
# the shared drm_response_log_density leaf (shape = 1/sigma^2,
# scale = mu * sigma^2). Ported from the held 2026-08-10 P3 leaf work
# (claude/mi-response-leaves @ 110633584); lognormal is not this slice.
...
test_that("binary mi() predictor works with a Gamma response likelihood", {
  ...
  expect_equal(fit$missing_data$version, "MD-gamma-mi")
...
test_that("Gamma-response mi() recovers the mean, CV, and predictor model under MCAR", {
  set.seed(13)
  n <- 3000
  z <- rnorm(n)
...
test_that("Gamma-response mi() recovers under outcome-dependent MAR", {
  set.seed(21)
  n <- 3000
  z <- rnorm(n)
  x <- rbinom(n, 1, stats::plogis(0.3 + 0.8 * z))
  ...
  expect_equal(unname(coef(fit, "sigma")), log(cv), tolerance = 0.20)
  expect_equal(unname(coef(fit, "mi_x")), c(0.3, 0.8), tolerance = 0.20)
})

test_that("Gamma-response mi() still refuses a non-binary predictor", {
  dat <- missing_predictor_gamma_response_data()
