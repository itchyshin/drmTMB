source_phase18_biv_gaussian_q4_location_recovery <- function() {
  env <- parent.frame()
  files <- c(
    ...
    "sim/run/sim_summary_biv_gaussian_q4_location_recovery.R",
    "sim/run/sim_write_biv_gaussian_q4_location_recovery_grid.R"
  )
  for (file in files) {
...
test_that("Phase 18 q4 location recovery reports bias, MCSE, and coverage", {
  source_phase18_biv_gaussian_q4_location_recovery()

  result_dir <- tempfile("phase18-biv-gaussian-q4-location-recovery-")
  ...
  out <- phase18_summarise_biv_gaussian_q4_location_recovery(
  ...
  expect_identical(out$surface, "biv_gaussian_q4_location_recovery")

  # The recovery aggregate keeps the same 17 estimands as the smoke lane, now
...
test_that("Phase 18 q4 location recovery leaves SD and correlations Wald-unavailable", {
  source_phase18_biv_gaussian_q4_location_recovery()
  ...
  # The four q=4 location random-effect SDs and the six derived location-location
  # correlations have no Wald standard error, so their interval endpoints stay
  # NA rather than being reported as interval-ready.
...
test_that("Phase 18 q4 location recovery grid writer emits coverage tables", {
  source_phase18_biv_gaussian_q4_location_recovery()

  output_dir <- tempfile("phase18-biv-gaussian-q4-location-recovery-grid-")
  withr::defer(unlink(output_dir, recursive = TRUE))

  out <- phase18_write_biv_gaussian_q4_location_recovery_grid_outputs(
  ...
  expect_identical(out$surface, "biv_gaussian_q4_location_recovery_grid")
  ...
  expect_error(
    phase18_write_biv_gaussian_q4_location_recovery_grid_outputs(
      output_dir = output_dir,
      n_rep = 4L,
      master_seed = 20260629L,
      overwrite = FALSE
