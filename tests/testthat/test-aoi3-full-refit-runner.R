test_that("AOI-3 private full-refit runner keeps its frozen execution boundary", {
  ...
  supervisor <- test_path("..", "..", "tools", "run-aoi3-bernoulli-nb2-supervised.py")
  ...
  expect_true(file.exists(supervisor))
  ...
  expect_match(text, "run-startup.csv", fixed = TRUE)
  expect_match(text, "runner-events.csv", fixed = TRUE)
  expect_match(text, "outer_checkpoint_retained", fixed = TRUE)
  expect_match(text, "write_csv_atomic", fixed = TRUE)
  expect_match(text, "record_runner_event(\"run_complete\")", fixed = TRUE)
  ...
  expect_match(supervisor_text, "subprocess.run", fixed = TRUE)
  expect_match(supervisor_text, "outer_wall_time_exceeded", fixed = TRUE)
  expect_match(supervisor_text, "supervisor-events.csv", fixed = TRUE)
