# P3: missing-PREDICTOR mi() modelling with a lognormal response
# (model_type 4). One binary missing predictor; the response density in the
# mi() 2-point sum is the shared drm_response_log_density leaf, carrying the
# log-scale dispersion sigma (identity-link location on log(y)).

# Lognormal log-density in the exact C++ parameterization: dnorm(log(y), mu,
# sigma) minus the log(y) Jacobian for the log(y) change of variables.
lognormal_response_log_density <- function(y, eta, log_sigma) {
  sigma <- exp(log_sigma)
  stats::dnorm(log(y), mean = eta, sd = sigma, log = TRUE) - log(y)
}
...
missing_predictor_lognormal_response_data <- function() {
  ...
  # require log(y) to be truly normal, only y to be strictly positive.
...
test_that("lognormal-response mi() recovers the location, dispersion, and predictor model", {
  set.seed(17)
  n <- 3000
  z <- rnorm(n)
  ...
  eta <- 0.4 + 0.5 * z + 0.7 * x
  sigma_true <- 0.4
  y <- stats::rlnorm(n, meanlog = eta, sdlog = sigma_true)
  ...
  expect_equal(unname(coef(fit, "sigma")), log(sigma_true), tolerance = 0.15)
