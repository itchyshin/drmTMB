...
test_that("Phase 18 first-wave status report renders missing artifacts by default", {
  skip_if_not_installed("rmarkdown")
  skip_if_not(rmarkdown::pandoc_available())
  ...
  output_dir <- tempfile("phase18-first-wave-status-missing-")
  dir.create(output_dir)
  withr::defer(unlink(output_dir, recursive = TRUE))
  ...
  out <- rmarkdown::render(
    ...
    params = list(
      artifact_manifest_csv = artifact_manifest_csv,
      artifact_status_csv = artifact_status_csv,
      notes = "non-strict missing artifact smoke"
    )
  )

  expect_true(file.exists(out))
  html <- paste(readLines(out, warn = FALSE), collapse = "\n")
  expect_true(grepl("non-strict missing artifact smoke", html, fixed = TRUE))
  expect_true(grepl("student_shape_grid", html, fixed = TRUE))
  expect_true(grepl("student-bootstrap.csv", html, fixed = TRUE))
})
