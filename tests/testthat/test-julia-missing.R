...
test_that("engine='julia' fits Gaussian response='include' (observed-data, design kept)", {
  ...
  skip_if_not(dir.exists(drm_miss_jl_path()), "DRM.jl engine path not available")
  ...
  res <- tryCatch(
    {
      ...
      callr::r(
        function(pkg, jl_path) {
          ...
          set.seed(1L); n <- 60L; x <- stats::rnorm(n)
          y <- 0.3 + 0.5 * x + stats::rnorm(n) * exp(0.1 * x)
          y[1:6] <- NA
          dat <- data.frame(y = y, x = x)
          fj <- drmTMB::drmTMB(
            drmTMB::bf(y ~ x, sigma ~ x), family = stats::gaussian(),
            data = dat, engine = "julia",
            missing = drmTMB::miss_control(response = "include")
          )
          list(ll = as.numeric(stats::logLik(fj)), nobs = stats::nobs(fj))
        },
        args = list(pkg = pkg, jl_path = drm_miss_jl_path()), error = "error"
      )
    },
    error = function(e) testthat::skip(paste("julia missing round-trip unavailable:", conditionMessage(e)))
  )
...
test_that("engine='julia' rejects response='include' for non-Gaussian families", {
  set.seed(1L); n <- 40L; x <- stats::rnorm(n)
  y <- stats::rpois(n, exp(0.2 + 0.3 * x)); y[1:4] <- NA
  dat <- data.frame(y = y, x = x)
  expect_error(
    drmTMB(bf(y ~ x), family = stats::poisson(), data = dat,
           engine = "julia", missing = miss_control(response = "include")),
    "missing"
  )
})
