source_phase18_biv_gaussian_q2_scale_recovery <- function() {
  env <- parent.frame()
  files <- c(
...
test_that("Phase 18 q2 scale recovery summary reports bias, MCSE, and coverage", {
  source_phase18_biv_gaussian_q2_scale_recovery()

  result_dir <- tempfile("phase18-biv-gaussian-q2-scale-recovery-")
  ...
  out <- phase18_summarise_biv_gaussian_q2_scale_recovery(
  ...
  expect_identical(out$surface, "biv_gaussian_q2_scale_recovery")

  # Recovery aggregate keeps the same ten estimands as the smoke lane, now with
...
test_that("Phase 18 q2 scale recovery leaves SD and correlation Wald-unavailable", {
  source_phase18_biv_gaussian_q2_scale_recovery()
...
test_that("Phase 18 q2 scale recovery grid writer emits coverage tables", {
  source_phase18_biv_gaussian_q2_scale_recovery()

  output_dir <- tempfile("phase18-biv-gaussian-q2-scale-recovery-grid-")
  ...
  expect_identical(out$surface, "biv_gaussian_q2_scale_recovery_grid")
  ...
  expect_error(
    phase18_write_biv_gaussian_q2_scale_recovery_grid_outputs(
      output_dir = output_dir,
      n_rep = 4L,
      master_seed = 20260626L,
      overwrite = FALSE
