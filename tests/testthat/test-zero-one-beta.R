...
new_zero_one_beta_coi_random_intercept_data <- function(
  seed = 2026081701L,
  n_id = 32L,
  n_each = 50L,
  sd_coi = 0.45
...
zoib_coi_random_effect_nll <- function(fit, par) {
  ...
  term <- d$coi_re_term[index] + 1L
  eta_mu <- as.vector(d$X_mu %*% par$beta_mu)
  log_sigma <- as.vector(d$X_sigma %*% par$beta_sigma)
  eta_zoi <- as.vector(d$X_zi %*% par$beta_zoi)
  eta_coi <- as.vector(d$X_nu %*% par$beta_coi) +
    d$coi_re_value[, 1L] * exp(par$log_sd_coi[term]) * u_coi[index]
  mu <- 1e-12 + (1 - 2e-12) * stats::plogis(eta_mu)
  if (identical(as.integer(d$use_logsigma_clamp), 1L)) {
    log_sigma <- softclamp_logsigma_drm(log_sigma, d$logsigma_clamp)
  }
  sigma <- exp(log_sigma)
  -sum(stats::dnorm(u_coi, log = TRUE)) -
    sum(d$weights * dzoibeta_drm(
      d$y, mu, sigma, stats::plogis(eta_zoi), stats::plogis(eta_coi), log = TRUE
    ))
}
...
test_that("zero-one-beta phylo sigma response mask has oracle and recovery evidence", {
  skip_if_not_installed("ape")
  set.seed(2026081816L)
  n_tip <- 64L
  n_each <- 60L
  tree <- ape::stree(n_tip, type = "balanced")
  tree$edge.length <- rep(1, nrow(tree$edge))
  ...
  precision <- dense_zoib_phylo_precision(tree)
  latent <- as.numeric(t(chol(solve(precision$Q))) %*% stats::rnorm(nrow(precision$Q), sd = .55))
  names(latent) <- tree$tip.label
  ...
  observed <- !is.na(dat$y)
  formula <- bf(y ~ x, sigma ~ phylo(1 | species, tree = tree), zoi ~ 1, coi ~ 1)
  ...
  probe <- obj$par + seq(-.025, .025, length.out = length(obj$par))
  oracle_fn <- function(v) zoib_sigma_phylo_nll(fit_masked, obj$env$parList(v), tree, dat$species)
  expect_equal(fit_masked$opt$convergence, 0L)
  expect_equal(fit_observed$opt$convergence, 0L)
  expect_equal(nobs(fit_masked), sum(observed))
  expect_equal(obj$fn(probe), oracle_fn(probe), tolerance = 1e-8)
...
test_that("zero-one-beta animal mu response mask has oracle and recovery evidence", {
  sim <- new_zero_one_beta_animal_data(
    seed = 2026081812L, n_group = 64L, n_each = 50L
  )
  dat <- sim$data
  dat$y[seq(1L, nrow(dat), by = 50L)] <- NA_real_
  observed <- !is.na(dat$y)
  Ainv <- sim$Ainv
  formula <- bf(y ~ x + animal(1 | species, Ainv = Ainv))
  fit_masked <- drmTMB(
    formula, family = zero_one_beta(), data = dat,
    missing = miss_control(response = "include"), control = drm_control(se = FALSE)
  )
  ...
  probe <- obj$par + seq(-.025, .025, length.out = length(obj$par))
  oracle_fn <- function(v) zoib_animal_nll(
    fit_masked, obj$env$parList(v), Ainv, dat$species
  )
...
test_that("zero-one-beta admits only the exact relmat K q1 sigma gate", {
  set.seed(2026074201L); n <- 16L; labels <- paste0("sp", seq_len(n)); Q <- diag(2, n); Q[cbind(seq_len(n - 1L), seq.int(2L, n))] <- -.5; Q[cbind(seq.int(2L, n), seq_len(n - 1L))] <- -.5; rownames(Q) <- colnames(Q) <- rev(labels); K <- solve(Q)
  u <- as.numeric(t(chol(K)) %*% rnorm(n, sd = .45)); names(u) <- rownames(K); species <- rep(labels, each = 40L); x <- rnorm(length(species)); mu <- plogis(-.15 + .35 * x); sigma <- exp(-1 + u[species]); zoi <- plogis(-1.1); coi <- plogis(.1)
  boundary <- rbinom(length(x), 1L, zoi); y <- rbeta(length(x), mu / sigma^2, (1 - mu) / sigma^2); y[boundary == 1L] <- rbinom(sum(boundary), 1L, coi); d <- data.frame(y, x, species)
...
test_that("zero-one-beta admits only the exact zoi random-intercept q1 gate", {
  ...
  zoi_re <- fit$model$random$zoi
  zoi_values <- fit$random_effects$zoi$values
  zoi_contribution <- rowSums(
    matrix(zoi_values[zoi_re$index], nrow = nrow(zoi_re$index)) *
      zoi_re$value
  )
  expect_equal(
    predict(fit, dpar = "zoi", type = "link"),
    as.vector(fit$model$X$zoi %*% coef(fit, "zoi")) + zoi_contribution,
    tolerance = 1e-10
  )
  target <- profile_targets(fit)
...
test_that("zero-one-beta admits only the exact coi random-intercept q1 gate", {
  ...
  expect_named(fit$sdpars$coi, "(1 | id)")
  expect_named(ranef(fit, "coi")$terms, "(1 | id)")
  ...
  expect_equal(
    ...
    rep(unname(coef(fit, "coi")[[1L]]), nrow(newdata)),
    tolerance = 1e-12
  )
  expect_error(
  ...
  target <- subset(profile_targets(fit), parm == "sd:coi:(1 | id)")
  expect_equal(nrow(target), 1L)
  ...
  expect_error(
    drmTMB(
      bf(y ~ x, sigma ~ 1, zoi ~ 1, coi ~ 1 + (1 | p | id)),
      family = zero_one_beta(), data = sim$data
    ),
    "random intercept"
  )
  expect_error(
    drmTMB(
      bf(y ~ x, sigma ~ x, zoi ~ 1, coi ~ 1 + (1 | id)),
      family = zero_one_beta(), data = sim$data
    ),
    "coi q1 gate requires"
  )
  expect_error(
    drmTMB(
      bf(y ~ x, sigma ~ 1, zoi ~ x, coi ~ 1 + (1 | id)),
      family = zero_one_beta(), data = sim$data
    ),
    "coi q1 gate requires"
  )
  expect_error(
    drmTMB(
      bf(y ~ x, sigma ~ 1, zoi ~ 1, coi ~ x + (1 | id)),
      family = zero_one_beta(), data = sim$data
    ),
    "coi q1 gate requires"
  )
  expect_error(
    drmTMB(
      bf(y ~ x, sigma ~ 1, zoi ~ 1, coi ~ 1 + (1 | id) + (1 | id2)),
      family = zero_one_beta(), data = sim$data
    ),
    "random intercept"
  )
  expect_error(
    drmTMB(
      bf(y ~ x + (1 | id), sigma ~ 1, zoi ~ 1, coi ~ 1 + (1 | id)),
      family = zero_one_beta(), data = sim$data
    ),
    "cannot be combined"
  )
  expect_error(
    drmTMB(
      bf(y ~ x, sigma ~ 1 + (1 | id), zoi ~ 1, coi ~ 1 + (1 | id)),
      family = zero_one_beta(), data = sim$data
    ),
    "cannot be combined"
  )
  expect_error(
    drmTMB(
      bf(y ~ x, sigma ~ 1, zoi ~ 1 + (1 | id), coi ~ 1 + (1 | id)),
      family = zero_one_beta(), data = sim$data
    ),
    "cannot be combined"
  )
