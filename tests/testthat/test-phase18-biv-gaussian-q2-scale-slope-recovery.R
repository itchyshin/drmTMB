source_phase18_biv_gaussian_q2_scale_slope_recovery <- function() {
  env <- parent.frame()
  files <- c(
...
test_that("Phase 18 q2 scale slope recovery summary reports bias, MCSE, and coverage", {
  source_phase18_biv_gaussian_q2_scale_slope_recovery()

  result_dir <- tempfile("phase18-biv-gaussian-q2-scale-slope-recovery-")
  ...
  out <- phase18_summarise_biv_gaussian_q2_scale_slope_recovery(
  ...
  expect_identical(out$surface, "biv_gaussian_q2_scale_slope_recovery")
...
test_that("Phase 18 q2 scale slope recovery leaves SD and correlation Wald-unavailable", {
  source_phase18_biv_gaussian_q2_scale_slope_recovery()
  ...
  # The random-effect SD and derived scale-scale correlation rows have no Wald
  # standard error, so their interval endpoints stay NA rather than being
  # reported as interval-ready. This keeps the scale-scale correlation at
  # derived_interval_unavailable, consistent with the q8 endpoint gate.
...
test_that("Phase 18 q2 scale slope recovery grid writer emits coverage tables", {
  source_phase18_biv_gaussian_q2_scale_slope_recovery()

  output_dir <- tempfile("phase18-biv-gaussian-q2-scale-slope-recovery-grid-")
  ...
  expect_identical(out$surface, "biv_gaussian_q2_scale_slope_recovery_grid")
  ...
  expect_error(
    phase18_write_biv_gaussian_q2_scale_slope_recovery_grid_outputs(
      output_dir = output_dir,
      n_rep = 4L,
      master_seed = 20260626L,
      overwrite = FALSE
