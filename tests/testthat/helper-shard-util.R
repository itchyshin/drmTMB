# Pure-logic helpers for deterministic testthat sharding (DRMTMB_TEST_SHARD="k/N").
...
drm_parse_shard_spec <- function(spec) {
  parts <- strsplit(spec, "/", fixed = TRUE)[[1]]
  if (length(parts) != 2L) {
    stop("malformed DRMTMB_TEST_SHARD ", sQuote(spec), '; expected "k/N"', call. = FALSE)
  }
  k <- suppressWarnings(as.integer(parts[1L]))
  n <- suppressWarnings(as.integer(parts[2L]))
  if (anyNA(c(k, n))) {
    stop("malformed DRMTMB_TEST_SHARD ", sQuote(spec), "; k and N must be integers", call. = FALSE)
  }
  if (n < 1L) stop("DRMTMB_TEST_SHARD ", sQuote(spec), ": N must be >= 1", call. = FALSE)
  if (k < 1L || k > n) {
    stop("DRMTMB_TEST_SHARD ", sQuote(spec), ": need 1 <= k <= N", call. = FALSE)
  }
  c(k = k, n = n)
}
...
drm_shard_indices <- function(len, k, n) {
...
drm_shard_filter <- function(dir = "testthat",
                             spec = Sys.getenv("DRMTMB_TEST_SHARD", "")) {
  if (!nzchar(spec)) return(NULL)
  kn <- drm_parse_shard_spec(spec)
  files <- sort(list.files(dir, pattern = "^test-.*\\.[Rr]$"), method = "radix")
  if (length(files) == 0L) {
    stop("DRMTMB_TEST_SHARD set but no test files found in ", sQuote(dir), call. = FALSE)
  }
  idx <- drm_shard_indices(length(files), kn[["k"]], kn[["n"]])
