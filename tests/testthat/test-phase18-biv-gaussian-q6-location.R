source_phase18_biv_gaussian_q6_location <- function() {
  env <- parent.frame()
  files <- c(
...
test_that("Phase 18 bivariate Gaussian q6 location DGP is seeded", {
  source_phase18_biv_gaussian_q6_location()

  conditions <- phase18_biv_gaussian_q6_location_conditions(
    n_id = 12L,
    n_each = 5L
  )
  dat <- phase18_dgp_biv_gaussian_q6_location(
    n_id = 12L,
    n_each = 5L,
    seed = 241L,
    cell_id = "biv_gaussian_q6_location_001",
    replicate = 1L
  )
  again <- phase18_dgp_biv_gaussian_q6_location(
    n_id = 12L,
    n_each = 5L,
    seed = 241L,
    cell_id = "biv_gaussian_q6_location_001",
  ...
  expect_identical(truth$surface, "biv_gaussian_q6_location")
  expect_named(truth$beta_mu1, c("(Intercept)", "x", "z"))
  expect_named(truth$beta_mu2, c("(Intercept)", "x", "z"))
  expect_named(truth$sd_mu, phase18_biv_gaussian_q6_location_sd_names())
  expect_named(truth$cor_mu, phase18_biv_gaussian_q6_location_cor_names())
})

test_that("Phase 18 bivariate Gaussian q6 location smoke runner summarises output", {
  source_phase18_biv_gaussian_q6_location()

  result_dir <- tempfile("phase18-biv-gaussian-q6-location-")
  ...
  out <- phase18_summarise_biv_gaussian_q6_location_smoke(
  ...
  expect_identical(out$surface, "biv_gaussian_q6_location")
  expect_equal(out$run$parallel$backend, "none")
  expect_equal(out$run$parallel$cores, 1L)
  expect_equal(nrow(out$run$summary), 30L)
  ...
  expect_equal(
    ...
    c(
      ...
      "sigma1",
      "sigma2",
      paste0("sd:mu:", phase18_biv_gaussian_q6_location_sd_names()),
      paste0("cor:re_cov:", phase18_biv_gaussian_q6_location_cor_names()),
      "rho12"
    )
  )
  ...
  expect_identical(
    class_by_parameter[[
      "cor:re_cov:cor(mu1:z,mu2:z | p | id)"
    ]],
    "derived_random_correlation"
  )
  ...
  expect_true(file.exists(phase18_result_path(
    result_dir,
    "biv_gaussian_q6_location_001",
    1L
  )))
})

test_that("Phase 18 bivariate Gaussian q6 location grid writer saves artifacts", {
  source_phase18_biv_gaussian_q6_location()

  output_dir <- tempfile("phase18-biv-gaussian-q6-location-grid-")
  ...
  expect_identical(out$surface, "biv_gaussian_q6_location_grid")
  ...
  expect_equal(nrow(replicates), 30L)
  expect_identical(
  ...
  expect_error(
    phase18_write_biv_gaussian_q6_location_grid_outputs(
      output_dir = output_dir,
      conditions = conditions,
      n_rep = 1L,
      master_seed = 20260624L
  ...
  expect_silent(phase18_write_biv_gaussian_q6_location_grid_outputs(
    output_dir = output_dir,
    conditions = conditions,
    n_rep = 1L,
    master_seed = 20260624L,
...
test_that("Phase 18 bivariate Gaussian q6 location helpers reject malformed inputs", {
  source_phase18_biv_gaussian_q6_location()

  expect_error(
    phase18_dgp_biv_gaussian_q6_location(0L, 5L),
    "positive whole number"
  )
  expect_error(
    phase18_dgp_biv_gaussian_q6_location(12L, 5L, residual_rho = 1),
  ...
  expect_error(
    phase18_dgp_biv_gaussian_q6_location_cell(
