...
test_that("N shards partition seq_len(len) exactly", {
  for (len in c(0L, 1L, 7L, 61L, 376L)) {
    for (n in c(1L, 3L, 4L, 7L)) {
      shards <- lapply(seq_len(n), function(k) drm_shard_indices(len, k, n))
...
test_that("drm_parse_shard_spec accepts well-formed specs", {
  expect_equal(drm_parse_shard_spec("1/4"), c(k = 1L, n = 4L))
  expect_equal(drm_parse_shard_spec("4/4"), c(k = 4L, n = 4L))
  expect_equal(drm_parse_shard_spec("1/1"), c(k = 1L, n = 1L))
})

test_that("drm_parse_shard_spec rejects malformed or out-of-range specs", {
  for (bad in c("0/4", "5/4", "a/b", "1/0", "1", "1/2/3", "")) {
    expect_error(drm_parse_shard_spec(bad))
  }
})

test_that("an unset shard spec means no filtering", {
  expect_null(drm_shard_filter(".", ""))
})
...
test_that("the filter selects each file exactly once across shards", {
  ...
  sel <- lapply(seq_len(n), function(k) {
    stripped[grepl(drm_shard_filter(".", paste0(k, "/", n)), stripped)]
  })
