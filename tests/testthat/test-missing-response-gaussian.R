...
test_that("Gaussian REML response masks recover a sigma random-intercept DGP", {
  set.seed(2026081550)
  n_group <- 72L
  n_each <- 20L
  group <- factor(rep(seq_len(n_group), each = n_each))
  ...
  y <- beta[[1L]] + beta[[2L]] * x + rnorm(n_group * n_each, sd = exp(log_sigma + u[group]))
  observed <- runif(n_group * n_each) > 0.25
  dat <- data.frame(y = y, x = x, group = group)
  dat$y[!observed] <- NA_real_
  form <- bf(y ~ x, sigma ~ 1 + (1 | group))
  fit <- drmTMB(form, data = dat, missing = miss_control(response = "include"),
    REML = TRUE, control = drm_control(se = FALSE))
  fit_observed <- drmTMB(form, data = dat[observed, , drop = FALSE], REML = TRUE,
    control = drm_control(se = FALSE))
...
test_that("Gaussian ML response masks recover a sigma random-slope DGP", {
  set.seed(2026081570L)
  n_group <- 72L
  n_each <- 20L
  group <- factor(rep(seq_len(n_group), each = n_each))
  x <- rnorm(n_group * n_each)
  z <- rnorm(n_group * n_each)
  w <- rnorm(n_group * n_each)
  beta <- c(`(Intercept)` = 0.25, x = 0.40)
  beta_sigma <- c(`(Intercept)` = -0.70, z = 0.15)
  sd_sigma <- 0.35
  u <- rnorm(n_group, sd = sd_sigma)
  y <- beta[[1L]] + beta[[2L]] * x + rnorm(
    n_group * n_each,
    sd = exp(beta_sigma[[1L]] + beta_sigma[[2L]] * z + u[group] * w)
  )
  observed <- runif(n_group * n_each) > 0.25
  dat <- data.frame(y = y, x = x, z = z, w = w, group = group)
  dat$y[!observed] <- NA_real_
  form <- bf(y ~ x, sigma ~ z + (0 + w | group))
  fit <- drmTMB(
    form, data = dat, missing = miss_control(response = "include"),
    control = drm_control(se = FALSE)
  )
  ...
  expect_missing_response_sentinel_invariant(fit, sentinels = c(-1e6, 1e6))
  expect_equal(coef(fit, "mu"), beta, tolerance = 0.10)
  expect_equal(coef(fit, "sigma"), beta_sigma, tolerance = 0.15)
  expect_equal(unname(fit$sdpars$sigma), sd_sigma, tolerance = 0.16)
  slope_effects <- fit$random_effects$sigma$terms[["(0 + w | group)"]]
  expect_gt(cor(slope_effects, u), 0.40)
})
