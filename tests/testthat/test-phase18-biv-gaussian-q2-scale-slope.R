source_phase18_biv_gaussian_q2_scale_slope <- function() {
  env <- parent.frame()
  files <- c(
...
test_that("Phase 18 bivariate Gaussian q2 scale slope DGP is seeded", {
  source_phase18_biv_gaussian_q2_scale_slope()

  conditions <- phase18_biv_gaussian_q2_scale_slope_conditions(
    n_id = 12L,
    n_each = 8L
  )
  dat <- phase18_dgp_biv_gaussian_q2_scale_slope(
    n_id = 12L,
    n_each = 8L,
    seed = 241L,
    cell_id = "biv_gaussian_q2_scale_slope_001",
    replicate = 1L
  )
  again <- phase18_dgp_biv_gaussian_q2_scale_slope(
    n_id = 12L,
    n_each = 8L,
    seed = 241L,
    cell_id = "biv_gaussian_q2_scale_slope_001",
  ...
  expect_identical(truth$surface, "biv_gaussian_q2_scale_slope")
  ...
  expect_named(truth$sd_sigma, phase18_biv_gaussian_q2_scale_slope_sd_names())
  expect_named(truth$cor_sigma, phase18_biv_gaussian_q2_scale_slope_cor_name())
  expect_named(truth$residual_rho, "rho12")
})

test_that("Phase 18 bivariate Gaussian q2 scale slope smoke runner summarises output", {
  source_phase18_biv_gaussian_q2_scale_slope()

  result_dir <- tempfile("phase18-biv-gaussian-q2-scale-slope-")
  ...
  out <- phase18_summarise_biv_gaussian_q2_scale_slope_smoke(
  ...
  expect_identical(out$surface, "biv_gaussian_q2_scale_slope")
  ...
  expect_equal(
    ...
    c(
      ...
      "sigma2:(Intercept)",
      "sigma2:x",
      paste0("sd:sigma:", phase18_biv_gaussian_q2_scale_slope_sd_names()),
      paste0("cor:sigma:", phase18_biv_gaussian_q2_scale_slope_cor_name()),
      "rho12"
    )
  )
  ...
  expect_identical(
    class_by_parameter[[
      "cor:sigma:cor(sigma1:x,sigma2:x | p | id)"
    ]],
    "derived_random_correlation"
  )
  ...
  expect_true(file.exists(phase18_result_path(
    result_dir,
    "biv_gaussian_q2_scale_slope_001",
    1L
  )))
})

test_that("Phase 18 bivariate Gaussian q2 scale slope grid writer saves artifacts", {
  source_phase18_biv_gaussian_q2_scale_slope()

  output_dir <- tempfile("phase18-biv-gaussian-q2-scale-slope-grid-")
  ...
  out <- phase18_write_biv_gaussian_q2_scale_slope_grid_outputs(
  ...
  expect_identical(out$surface, "biv_gaussian_q2_scale_slope_grid")
  ...
  expect_error(
    phase18_write_biv_gaussian_q2_scale_slope_grid_outputs(
      output_dir = output_dir,
      conditions = conditions,
      n_rep = 1L,
      master_seed = 20260625L
  ...
  expect_silent(phase18_write_biv_gaussian_q2_scale_slope_grid_outputs(
    output_dir = output_dir,
    conditions = conditions,
    n_rep = 1L,
    master_seed = 20260625L,
...
test_that("Phase 18 bivariate Gaussian q2 scale slope helpers reject malformed inputs", {
  source_phase18_biv_gaussian_q2_scale_slope()

  expect_error(
    phase18_dgp_biv_gaussian_q2_scale_slope(0L, 8L),
    "positive whole number"
  )
  expect_error(
    phase18_dgp_biv_gaussian_q2_scale_slope(12L, 8L, residual_rho = 1),
    "absolute value below 1"
  )
  expect_error(
    phase18_dgp_biv_gaussian_q2_scale_slope(12L, 8L, cor_sigma = 1),
    "absolute value below 1"
  )
  expect_error(
  ...
  expect_error(
    phase18_dgp_biv_gaussian_q2_scale_slope_cell(
