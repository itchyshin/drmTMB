...
test_that("q1 provider scale fixtures stay deterministic and scoped", {
  source_structured_re_bridge_fixtures()

  for (structured_type in c("spatial", "animal", "relmat")) {
    for (endpoint in c("sigma", "mu_sigma")) {
      ...
      expect_equal(native_payload$target$structured_type, structured_type)
      expect_equal(native_payload$target$endpoint, endpoint)
      expect_equal(native_payload$target$estimator, "ML")
      expect_equal(
        native_payload$matrix$matrix_id,
        paste("fixture", "q1", structured_type, sep = "_")
      )
      expect_equal(
