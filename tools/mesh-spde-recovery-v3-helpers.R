...
mesh_v3_design <- function(smoke = FALSE) {
  design <- rbind(
    data.frame(n_site = 128L, replicate = seq_len(50L), seed = 2031281001:2031281050),
    data.frame(n_site = 256L, replicate = seq_len(50L), seed = 2032561001:2032561050)
  )
  if (isTRUE(smoke)) {
    design <- design[design$replicate == 1L, , drop = FALSE]
  }
...
mesh_v3_mc_summary <- function(rows, smoke = FALSE) {
  ...
  summarize_rung <- function(x) {
    ...
    usable <- fit_ok & converged & hessian_ok & finite_positive & objective_ok & gradient_ok
    ...
    data.frame(
      ...
      gradient_ok = sum(gradient_ok), near_zero = sum(near_zero),
