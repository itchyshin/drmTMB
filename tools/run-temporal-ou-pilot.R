...
# temporal OU route. It is not an interval-calibration campaign.
...
out_dir <- Sys.getenv(
  ...
  unset = file.path(
    root,
    "docs/dev-log/simulation-artifacts/2026-09-09-temporal-ou-pilot"
  )
)
...
if (any(file.exists(evidence_files))) {
  stop("Retained OU pilot evidence already exists; do not overwrite it.", call. = FALSE)
}
...
run_one <- function(cell, seed) {
  ...
  if (is.null(result$fit)) {
    selected <- data.frame(
      ...
      elapsed_sec = elapsed_sec, pd_hessian = NA, n_intervals = 0L,
    ...
    attempts <- data.frame(
      ...
      objective = NA_real_, elapsed_sec = elapsed_sec, selected = FALSE,
  ...
  selected <- data.frame(
    ...
    convergence = selected_start$convergence[[1L]], elapsed_sec = elapsed_sec,
    pd_hessian = isTRUE(fit$sdr$pdHess), n_intervals = n_intervals,
    interval_available = available,
    interval_status = if (inherits(intervals, "error")) conditionMessage(intervals) else "available",
    warning = warning, error = NA_character_, stringsAsFactors = FALSE
  )
  attempts <- attempts[, c(
...
summary <- do.call(rbind, lapply(split(results, results$cell), function(x) {
  data.frame(
    ...
    interval_guarded = sum(grepl("not yet qualified", x$interval_status, fixed = TRUE)),
...
provenance <- data.frame(
  ...
  value = c(
    ...
    unname(tools::md5sum(file.path(root, "tools/run-temporal-ou-pilot.R"))),
...
writeLines(c(
  "# Temporal OU timed pilot", "",
  sprintf("The pilot retained %d data sets and %d optimizer starts.", nrow(results), nrow(attempts)),
  sprintf("Total elapsed fit time: %.3f seconds; longest fit: %.3f seconds.", sum(results$elapsed_sec), max(results$elapsed_sec)),
  sprintf("Public Wald interval availability: %d/%d; guarded-unavailable: %d/%d.", sum(results$interval_available), nrow(results), sum(grepl("not yet qualified", results$interval_status, fixed = TRUE)), nrow(results)),
...
if (nrow(results) != 15L || nrow(attempts) != 30L ||
  ...
  stop("Temporal OU pilot completeness checks failed; retained outputs were written for diagnosis.", call. = FALSE)
}
cat("TEMPORAL_OU_PILOT_PASS\n")
