...
# C18 exact q1 leaf: mc-0604 -- one unlabelled structured `zoi` intercept,
...
new_data <- function(seed, n_group = 32L, n_each = 50L, tau = .55) {
  ...
  zoi <- plogis(0 + u[data$species]); coi <- plogis(0)
  boundary <- rbinom(nrow(data), 1, zoi)
  data$y <- ifelse(boundary == 1, rbinom(nrow(data), 1, coi), rbeta(nrow(data), mu / sigma^2, (1 - mu) / sigma^2))
...
run_one <- function(seed, source_sha, runner_sha, tau = .55) {
  out <- data.frame(cell_id = "mc-0604", dpar = "zoi", provider = "animal", seed, source_sha, runner_sha,
    formula = "y ~ x, sigma ~ 1, zoi ~ 1 + animal(1 | species, Ainv = Ainv), coi ~ 1",
    ...
    pdHess = NA, max_gradient = NA_real_, mode_correlation = NA_real_, eta_zoi_min = NA_real_, eta_zoi_max = NA_real_,
    ...
    mu0_hat = NA_real_, mu1_hat = NA_real_, sigma_hat = NA_real_, coi_hat = NA_real_, tau_hat = NA_real_)
  ...
  fit <- tryCatch(withCallingHandlers(
    drmTMB::drmTMB(
      drmTMB::bf(y ~ x, sigma ~ 1, zoi ~ 1 + animal(1 | species, Ainv = Ainv), coi ~ 1),
  ...
  mode <- as.numeric(ranef(fit, "animal_zoi")$terms[["animal(1 | species)"]])
  ...
  out$eta_zoi_min <- min(eta_zoi); out$eta_zoi_max <- max(eta_zoi)
  out$mu0_hat <- fit$coefficients$mu[["(Intercept)"]]; out$mu1_hat <- fit$coefficients$mu[["x"]]
  out$sigma_hat <- exp(fit$coefficients$sigma[["(Intercept)"]]); out$coi_hat <- plogis(fit$coefficients$coi[["(Intercept)"]])
  out$tau_hat <- fit$sdpars$zoi[["animal(1 | species)"]]
  out$boundary_hit <- !all(is.finite(unlist(out[c("mu0_hat", "mu1_hat", "sigma_hat", "coi_hat", "tau_hat")]))) || out$tau_hat <= .05 || out$tau_hat >= 2.5
  out
}
...
dir <- Sys.getenv("DRMTMB_RECOVERY_OUT", unset = file.path(root, "docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-mc-0604-zob-animal-zoi-q1-local-recovery")); dir.create(dir, recursive = TRUE, showWarnings = FALSE)
sha <- trimws(system2("git", c("rev-parse", "HEAD"), stdout = TRUE)); runner_sha <- unname(tools::md5sum(script))
attempts <- do.call(rbind, lapply(2026080201:2026080204, run_one, source_sha = sha, runner_sha = runner_sha)); write_tsv(attempts, file.path(dir, "raw-attempts.tsv"))
...
fixed <- new_data(2026080209L, tau = 0); Ainv <- fixed$Ainv
fixed_fit <- tryCatch(
  drmTMB::drmTMB(
    drmTMB::bf(y ~ x, sigma ~ 1, zoi ~ 1 + animal(1 | species, Ainv = Ainv), coi ~ 1),
...
fixed_record <- if (inherits(fixed_fit, "error")) {
  ...
  data.frame(source_sha = sha, runner_sha = runner_sha, status = "fit_ok", convergence = fixed_fit$opt$convergence,
    pdHess = isTRUE(fixed_fit$sdr$pdHess), tau_hat = unname(fixed_fit$sdpars$zoi[["animal(1 | species)"]]),
    decision = "BOUNDARY_DIAGNOSTIC_ONLY")
}
write_tsv(fixed_record, file.path(dir, "fixed-zoi-animal-boundary-diagnostic.tsv"))
