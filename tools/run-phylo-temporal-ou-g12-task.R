...
actual_commit <- system2('git', c('rev-parse', 'HEAD'), stdout = TRUE, stderr = TRUE)
if (length(actual_commit) != 1L || !identical(actual_commit, source_commit)) stop('Source checkout does not equal --source-commit.', call. = FALSE)
