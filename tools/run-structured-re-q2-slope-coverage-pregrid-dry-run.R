...
seed_manifest <- data.frame(
  ...
  seed = 730000L + seq_len(n_replicates),
  seed_role = "predeclared_q2_slope_pregrid",
...
cell_rows <- lapply(seq_len(nrow(eligible_rule)), function(i) {
  ...
  data.frame(
    pregrid_cell_id = sprintf(
      "q2_slope_pregrid_%s_%s_rep%03d",
...
target_rows <- lapply(seq_len(nrow(rule)), function(i) {
  ...
  data.frame(
    ...
    status = "covered",
    evidence_url = "docs/dev-log/after-task/2026-06-24-q2-slope-coverage-pregrid-dry-run.md",
    claim_boundary = clean_text(paste(
      provider_prefix[[provider]],
      "q2 slope coverage pre-grid dry-run only;",
