...
new_phylo_nbinom2_slope_data <- function(seed, n_level = 8L, n_each = 20L) {
  set.seed(seed)
  ...
  mu <- exp(eta)
  data <- data.frame(
    nb2_phylo = stats::rnbinom(length(site), size = 1 / sigma^2, mu = mu),
    x = x,
    site = site
  )
  list(
...
run_one <- function(seed, replicate_id) {
  sim <- new_phylo_nbinom2_slope_data(seed)
  tree <- sim$tree
  warnings <- character()
  elapsed <- system.time({
...
summarise_results <- function(results, load_status, load_detail) {
  ...
  data.frame(
    micro_shard_id = "count_slope_phylo_nbinom2_q1_mu_one_slope_local_micro_shard",
...
run_log <- data.frame(
  ...
  runner_script = "tools/run-structured-re-count-slope-phylo-nbinom2-local-micro-shard.R",
  ...
  claim_boundary = paste(
    "Local phylo NB2 q1 mu one-slope micro-shard only;",
    ...
    "no REML, no AI-REML, no public support, no structured count sigma,",
