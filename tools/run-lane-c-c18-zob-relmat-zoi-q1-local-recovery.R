#!/usr/bin/env Rscript
#
# C18 (mc-0605): structured `relmat` q1 unlabelled intercept on the `zoi` atom's
...
# ---- DGP: relmat AR1-precision field placed on zoi's linear predictor ------
...
new_data <- function(seed, n_group = 32L, n_each = 50L, tau = .55,
  ...
  zoi_full <- plogis(qlogis(zoi_truth) + field[as.character(species)])
  coi_full <- rep(coi_truth, length(species))
...
run_one <- function(seed, source_sha, runner_sha) {
  ...
  tau_hat <- unname(fit$sdpars$zoi[["relmat(1 | species)"]])
  u_hat <- ranef(fit, "relmat_zoi")$terms[["relmat(1 | species)"]]
...
dir <- Sys.getenv(
  "DRMTMB_RECOVERY_OUT",
  unset = file.path(root, "docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-relmat-zoi-q1-local-recovery")
)
...
fixed_record <- if (inherits(fixed_fit, "error")) {
  ...
  data.frame(
    ...
    tau_hat = unname(fixed_fit$sdpars$zoi[["relmat(1 | species)"]]),
    decision = "BOUNDARY_DIAGNOSTIC_ONLY"
  )
}
write_tsv(fixed_record, file.path(dir, "fixed-zoi-relmat-boundary-diagnostic.tsv"))
