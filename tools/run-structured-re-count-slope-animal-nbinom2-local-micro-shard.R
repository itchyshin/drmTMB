...
animal_covariance <- function(n_level = 8L) {
...
new_animal_nbinom2_slope_data <- function(seed, n_level = 8L, n_each = 20L) {
  set.seed(seed)
  K <- animal_covariance(n_level)
  Q <- solve(K)
  labels <- rownames(K)
  id <- rep(labels, each = n_each)
  ...
  sigma <- 0.55
  eta <- beta_mu[[1L]] + beta_mu[[2L]] * x + intercept[id] + x * slope[id]
  mu <- exp(eta)
  data <- data.frame(
    nb2_animal = stats::rnbinom(length(id), size = 1 / sigma^2, mu = mu),
    x = x,
    id = id
  )
  list(
...
run_one <- function(seed, replicate_id) {
  sim <- new_animal_nbinom2_slope_data(seed)
  Q <- sim$Q
  warnings <- character()
  elapsed <- system.time({
    fit <- withCallingHandlers(
      drmTMB(
        bf(nb2_animal ~ x + animal(1 + x | id, Ainv = Q), sigma ~ 1),
        family = nbinom2(),
        data = sim$data,
  ...
  mu <- coef(fit, "mu")
  sd_mu <- fit$sdpars$mu
  intercept_label <- "animal(1 | id)"
  slope_label <- "animal(0 + x | id)"
  data.frame(
    replicate_id = replicate_id,
    seed = seed,
    ...
    mu_x = unname(mu[["x"]]),
    sigma_estimate = unname(sigma(fit)[[1L]]),
...
summarise_results <- function(results, load_status, load_detail) {
  ...
  data.frame(
    micro_shard_id = "count_slope_animal_nbinom2_q1_mu_one_slope_local_micro_shard",
...
run_log <- data.frame(
  run_id = "count_slope_animal_nbinom2_q1_mu_one_slope_local_micro_shard_run",
  mode = "local_micro_shard",
  runner_script = "tools/run-structured-re-count-slope-animal-nbinom2-local-micro-shard.R",
  ...
  claim_boundary = paste(
    "Local animal A/Ainv NB2 q1 mu one-slope micro-shard only;",
    "sigma is fixed-effect overdispersion only;",
    "no pedigree/Ainv bridge marshalling, no Totoro job submitted,",
    "no DRAC job submitted, no bridge parity, no interval reliability,",
    "no coverage evidence, no q2, no q4, no REML, no AI-REML,",
    "no public support, no structured count sigma,",
    "and no broad bridge support promoted."
  ),
  stringsAsFactors = FALSE
)
