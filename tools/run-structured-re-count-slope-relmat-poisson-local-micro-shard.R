...
relmat_covariance <- function(n_level = 8L) {
...
new_relmat_poisson_slope_data <- function(seed, n_level = 8L, n_each = 20L) {
  set.seed(seed)
  K <- relmat_covariance(n_level)
  Q <- solve(K)
  labels <- rownames(K)
  ...
  data <- data.frame(
    poisson_relmat = stats::rpois(length(id), lambda = exp(eta)),
    x = x,
    id = id
  )
  list(
...
run_one <- function(seed, replicate_id) {
  sim <- new_relmat_poisson_slope_data(seed)
  Q <- sim$Q
  warnings <- character()
  elapsed <- system.time({
    fit <- withCallingHandlers(
      drmTMB(
        bf(poisson_relmat ~ x + relmat(1 + x | id, Q = Q)),
  ...
  mu <- coef(fit, "mu")
  sd_mu <- fit$sdpars$mu
  intercept_label <- "relmat(1 | id)"
  slope_label <- "relmat(0 + x | id)"
  data.frame(
    replicate_id = replicate_id,
    seed = seed,
...
summarise_results <- function(results, load_status, load_detail) {
  ...
  data.frame(
    micro_shard_id = "count_slope_relmat_poisson_q1_mu_one_slope_local_micro_shard",
...
run_log <- data.frame(
  run_id = "count_slope_relmat_poisson_q1_mu_one_slope_local_micro_shard_run",
  mode = "local_micro_shard",
  runner_script = "tools/run-structured-re-count-slope-relmat-poisson-local-micro-shard.R",
  ...
  claim_boundary = paste(
    "Local relmat K/Q Poisson q1 mu one-slope micro-shard only;",
    "no Q bridge marshalling, no Totoro job submitted,",
