...
new_animal_poisson_slope_data <- function(seed, n_level = 8L, n_each = 20L) {
  set.seed(seed)
  ...
  data <- data.frame(
    poisson_animal = stats::rpois(length(id), lambda = exp(eta)),
    x = x,
    id = id
  )
  list(
...
run_one <- function(seed, replicate_id) {
  sim <- new_animal_poisson_slope_data(seed)
  Q <- sim$Q
  warnings <- character()
  elapsed <- system.time({
    fit <- withCallingHandlers(
      drmTMB(
        bf(poisson_animal ~ x + animal(1 + x | id, Ainv = Q)),
...
summarise_results <- function(results, load_status, load_detail) {
  ...
  data.frame(
    micro_shard_id = "count_slope_animal_poisson_q1_mu_one_slope_local_micro_shard",
...
run_log <- data.frame(
  run_id = "count_slope_animal_poisson_q1_mu_one_slope_local_micro_shard_run",
  mode = "local_micro_shard",
  runner_script = "tools/run-structured-re-count-slope-animal-poisson-local-micro-shard.R",
  ...
  claim_boundary = paste(
    "Local animal A/Ainv Poisson q1 mu one-slope micro-shard only;",
