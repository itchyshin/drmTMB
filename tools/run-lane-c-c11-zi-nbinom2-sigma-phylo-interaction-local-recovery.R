...
# Lane C C11 retained local point-recovery receipt. This script tests exactly
# ML ZINB2 sigma ~ phylo_interaction(1 | plant:pollinator), with zi ~ 1.
...
new_data <- function(seed, structured = TRUE, n_plant = 8L, n_pollinator = 8L, n_each = 18L,
                     tau = .60, sigma_intercept = -.20, zi_probability = .20) {
  set.seed(seed)
  ...
  if (structured) {
    pair_cov <- kronecker(drmTMB:::drm_phylo_tip_covariance(pollinator_tree), drmTMB:::drm_phylo_tip_covariance(plant_tree))
    pair_effect <- as.numeric(t(chol(pair_cov)) %*% stats::rnorm(nrow(grid), sd = tau))
  } else {
    pair_cov <- diag(nrow(grid))
    pair_effect <- stats::rnorm(nrow(grid), sd = tau)
  }
  ...
  data$structural_zero <- structural_zero
  support <- aggregate(cbind(structural_zero = as.integer(structural_zero), positive = as.integer(data$count > 0L)) ~ pair,
    data = data, FUN = sum)
  list(data = data, plant_tree = plant_tree, pollinator_tree = pollinator_tree, tau = tau,
    sigma_intercept = sigma_intercept, zi_probability = zi_probability, pair_effect = pair_effect, support = support,
...
accepted_data <- function(seed, structured) {
  for (draw in seq_len(10000L)) {
    sim <- new_data(seed + (draw - 1L) * 100000L, structured = structured)
...
fit_one <- function(sim, structured) {
  if (structured) {
    ...
    formula <- drmTMB::bf(count ~ x, sigma ~ phylo_interaction(
      1 | plant:pollinator, tree1 = plant_tree, tree2 = pollinator_tree
    ), zi ~ 1)
  } else {
    formula <- drmTMB::bf(count ~ x, sigma ~ (1 | pair), zi ~ 1)
  }
...
run_one <- function(source_sha, runner_sha, attempt_id, seed, structured) {
  route <- if (structured) "structured" else "iid_control"
  out <- data.frame(
    fixture_id = "lane_c_c11_zi_nbinom2_sigma_phylo_interaction_local", route, source_sha, runner_sha,
  ...
  if (structured) {
    ...
    fitted <- ranef(fit, "phylo_interaction_sigma")$terms[["phylo_interaction(1 | plant:pollinator)"]]
    out$mode_correlation <- suppressWarnings(stats::cor(
      unname(fitted[names(sim$pair_effect)]), unname(sim$pair_effect)
    ))
  } else {
    out$tau_hat <- fit$sdpars$sigma[["(1 | pair)"]]
    out$mode_correlation <- NA_real_
  }
  finite <- all(is.finite(unlist(out[c("beta0_hat", "beta1_hat", "sigma_intercept_hat", "tau_hat", "zi_logit_hat")])))
  out$boundary_hit <- !finite || out$tau_hat <= .05 || out$tau_hat >= 3 || plogis(out$zi_logit_hat) <= .02 || plogis(out$zi_logit_hat) >= .98
  out
}

summarise_attempts <- function(x, require_mode = FALSE) {
  nm <- c("beta0_hat", "beta1_hat", "sigma_intercept_hat", "tau_hat", "zi_logit_hat")
  valid <- x$fit_status == "fit_ok" & x$convergence == 0L & x$pdHess & x$max_gradient <= .01 & !x$boundary_hit & stats::complete.cases(x[, nm])
  means <- if (any(valid)) colMeans(x[valid, nm, drop = FALSE]) else stats::setNames(rep(NA_real_, length(nm)), nm)
  mode_ok <- !require_mode || all(x$mode_correlation[valid] > .45)
  pass <- length(valid) == 4L && all(valid) && mode_ok &&
  ...
  data.frame(fixture_id = unique(x$fixture_id), route = unique(x$route), planned_attempts = 4L,
    attempted_attempts = nrow(x), fit_ok = sum(x$fit_status == "fit_ok"), nonconverged = sum(x$fit_status == "fit_ok" & x$convergence != 0L),
    pdhess_false = sum(x$fit_status == "fit_ok" & !x$pdHess), boundary_hits = sum(x$fit_status == "fit_ok" & x$boundary_hit),
    mean_beta0_hat = means[["beta0_hat"]], mean_beta1_hat = means[["beta1_hat"]], mean_sigma_intercept_hat = means[["sigma_intercept_hat"]],
    mean_tau_hat = means[["tau_hat"]], mean_zi_logit_hat = means[["zi_logit_hat"]],
...
artifact_dir <- file.path(repo_root, "docs", "dev-log", "implementation-recovery", paste0("2026-07-30-lane-c-c11-zi-nbinom2-sigma-phylo-interaction-local-", run_id))
...
for (route in c("structured", "iid_control")) {
  structured <- identical(route, "structured")
  rows <- vector("list", length(seeds)); path <- file.path(artifact_dir, paste0(route, "-raw-attempts.tsv"))
  for (i in seq_along(seeds)) { rows[[i]] <- run_one(source_sha, runner_sha, i, seeds[[i]], structured); write_tsv(do.call(rbind, rows[seq_len(i)]), path) }
  rows <- do.call(rbind, rows); write_tsv(rows, path)
  write_tsv(summarise_attempts(rows, require_mode = structured), file.path(artifact_dir, paste0(route, "-summary.tsv")))
}
