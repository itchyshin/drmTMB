...
print_help <- function() {
  cat(
    paste(
      "Usage: Rscript tools/run-gaussian-mu-slope-tranche74-spatial-host-smoke.R [options]",
      "",
      "Tranche 74 fail-closed runner for the q1 mu one-slope spatial n=5 smoke.",
...
t74_source_snapshot_path <- "/home/snakagaw/codex/drmTMB-q1mu-slope-tranche73-clean-source-56add7f0-20260702T123451Z"
t74_run_root_path <- "/home/snakagaw/drmtmb-qseries/q1-mu-slope-spatial-tranche73-clean-source-20260702T123451Z"
default_output_dir <- file.path(
  t74_run_root_path,
  "tranche74-spatial-host-smoke-artifacts"
)
...
host_label <- clean_ascii(arg_value("host-label", "totoro_q1mu_slope_spatial_t74_t73_clean_source_n5"))
source_snapshot_path <- clean_ascii(arg_value(
  "source-snapshot-path",
  t74_source_snapshot_path
))
run_root_path <- clean_ascii(arg_value("run-root-path", t74_run_root_path))
...
if (!identical(provider, "spatial")) {
  stop("Tranche 74 is spatial-only; use --provider=spatial.", call. = FALSE)
}
if (!target %in% c("both", "mu_intercept", "mu_x")) {
  stop("`--target` must be both, mu_intercept, or mu_x.", call. = FALSE)
}
if (!identical(n_rep, 5L)) {
  stop("Tranche 74 is fixed at --n-rep=5.", call. = FALSE)
}
if (!identical(seeds, expected_seeds)) {
  stop(
    "Tranche 74 seed manifest must be 861001,861002,861003,861004,861005.",
    call. = FALSE
  )
}
if (isTRUE(write_dashboard)) {
  stop(
    "Tranche 74 is artifact-only; use --write-dashboard=false.",
    call. = FALSE
  )
}
if (!identical(source_snapshot_path, t74_source_snapshot_path)) {
  stop(
    "Tranche 74 must use the exact T73 source snapshot path: ",
    t74_source_snapshot_path,
    call. = FALSE
  )
}
if (!identical(run_root_path, t74_run_root_path)) {
  stop(
    "Tranche 74 must use the exact T73 run root path: ",
    t74_run_root_path,
    call. = FALSE
  )
}
...
raw_path <- file.path(output_dir, "structured-re-gaussian-mu-slope-tranche74-spatial-host-smoke-results.tsv")
run_log_path <- file.path(output_dir, "structured-re-gaussian-mu-slope-tranche74-spatial-host-smoke-run-log.tsv")
summary_path <- if (identical(summary_path_arg, "NA")) {
  file.path(output_dir, "structured-re-gaussian-mu-slope-tranche74-spatial-host-smoke-summary.tsv")
} else {
  summary_path_arg
}
run_id <- paste0("tranche74_spatial_mu_slope_", host_label, "_n5")

claim_boundary <- paste(
  "Tranche 74 spatial-only q1 mu one-slope n=5 host-smoke runner contract;",
...
manifest <- do.call(
  ...
  lapply(targets, function(target_id) {
    data.frame(
      manifest_id = paste0("tranche74_spatial_", target_id, "_seed_", seeds),
      provider = "spatial",
      target = target_id,
...
approval <- Sys.getenv("DRMTMB_Q1MU_SLOPE_T74_EXECUTION_APPROVED", unset = "")
if (!identical(approval, "rose_fisher_gauss_noether_grace")) {
  stop(
    "Refusing Tranche 74 execution: set ",
    "DRMTMB_Q1MU_SLOPE_T74_EXECUTION_APPROVED=rose_fisher_gauss_noether_grace ",
...
load_current_source <- function(path) {
  if (!requireNamespace("devtools", quietly = TRUE)) {
    stop("The Tranche 74 runner requires devtools for load_all().", call. = FALSE)
  }
...
summaries <- do.call(
  ...
  lapply(names(target_parameters), function(target_id) {
    ...
    data.frame(
      result_id = paste0("tranche74_spatial_mu_slope_", target_id),
      cell_id = "qseries_spatial_q1_mu_one_slope",
      provider = "spatial",
      target = target_id,
      endpoint_member = target_members[[target_id]],
      ...
      promotion_decision = "do_not_promote",
      support_cell_decision = "unchanged_point_fit_planned_planned",
      rose_audit = "rose_no_status_claim_from_t74_artifact",
      ...
      claim_boundary = claim_boundary,
      next_gate = paste(
        "Review Tranche 74 retained attempt artifacts for pdHess and finite",
...
message("[tranche74] wrote ", raw_path)
message("[tranche74] wrote ", summary_path)
message("[tranche74] wrote ", run_log_path)
message("[tranche74] coverage_decision=coverage_not_authorized")
message("[tranche74] promotion_decision=do_not_promote")
