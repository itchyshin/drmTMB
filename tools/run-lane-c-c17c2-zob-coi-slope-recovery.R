...
simulate_one <- function(M, seed, beta_coi_x = 0.40, sd_coi = 0.45) {
  set.seed(seed)
  n_each <- 50L
  ...
  coi <- stats::plogis(
    0.10 + beta_coi_x * x + b[as.character(id)] * x
  )
  ...
  boundary_x_sd <- vapply(group_levels, function(group) {
    values <- x[boundary == 1L & id == group]
  ...
  list(
    ...
    min_boundary_x_sd = if (all(is.na(boundary_x_sd))) {
      NA_real_
    } else {
      min(boundary_x_sd, na.rm = TRUE)
    },
    boundary_x_sd_gate = all(!is.na(boundary_x_sd) & boundary_x_sd >= 0.5)
  )
}
...
fit_one <- function(M, seed, source_sha, runner_sha256) {
  ...
  empty <- function(status, error) data.frame(
    ...
    beta_coi1_error = NA_real_, log_sigma_error = NA_real_,
    sd_coi_relative_error = NA_real_,
    min_group_zero = sim$min_group_zero,
    min_group_one = sim$min_group_one,
    min_group_interior = sim$min_group_interior,
    min_boundary_x_sd = sim$min_boundary_x_sd,
    support_gate = sim$min_group_zero >= 2L &&
      sim$min_group_one >= 2L && sim$min_group_interior >= 10L,
    boundary_x_sd_gate = sim$boundary_x_sd_gate,
    boundary_hit = NA,
    glmer_beta_coi0 = NA_real_, glmer_beta_coi1 = NA_real_,
    glmer_sd_coi = NA_real_,
    glmer_beta_coi0_difference = NA_real_,
    glmer_beta_coi1_difference = NA_real_,
    glmer_sd_coi_difference = NA_real_,
    error = error
  )
  ...
  sd_coi_hat <- unname(fit$sdpars$coi[["(0 + x | id)"]])
  modes <- ranef(fit, "coi")$terms[["(0 + x | id)"]]
  beta_mu <- coef(fit, "mu")
  ...
  glmer_beta <- lme4::fixef(glmer_fit)
  glmer_sd <- attr(lme4::VarCorr(glmer_fit)$id, "stddev")
  glmer_sd_coi <- unname(glmer_sd[["x"]])
  ...
  data.frame(
    ...
    beta_coi1_error = abs(unname(beta_coi[["x"]]) - 0.40),
    ...
    glmer_beta_coi0 = unname(glmer_beta[["(Intercept)"]]),
    glmer_beta_coi1 = unname(glmer_beta[["x"]]),
    glmer_sd_coi = glmer_sd_coi,
    glmer_beta_coi0_difference = abs(
      unname(beta_coi[[1L]]) - unname(glmer_beta[["(Intercept)"]])
    ),
    glmer_beta_coi1_difference = abs(
      unname(beta_coi[["x"]]) - unname(glmer_beta[["x"]])
    ),
...
run_id <- Sys.getenv(
  "C17_RUN_ID",
  "2026-08-02-lane-c-c17c2-zob-coi-slope-recovery-run-1"
)
...
source_files <- c(
  "R/drmTMB.R", "R/family.R", "R/methods.R", "R/profile.R",
  "src/drmTMB.cpp", "tests/testthat/test-zero-one-beta.R",
  file.path("tools", basename(script))
)
...
if (!requireNamespace("lme4", quietly = TRUE)) {
  stop("C17-C2 requires lme4 for the boundary-only comparator")
}
...
exact_command <- paste(
  ...
  "tools/run-lane-c-c17c2-zob-coi-slope-recovery.R"
)
...
prospective_seeds <- 2026081781:2026081784
grid <- expand.grid(
...
for (i in seq_len(nrow(grid))) {
  ...
  attempts <- rbind(attempts, attempt)
  write_tsv(attempts, raw_path)
  writeLines(
    paste(utc_now(), "FIT", grid$M[[i]], grid$seed[[i]], attempt$status),
    heartbeat
  )
  flush(heartbeat)
}
...
summary_rows <- do.call(rbind, lapply(split(attempts, attempts$M), function(x) {
  ...
  data.frame(
    ...
    boundary_x_sd_pass = sum(x$boundary_x_sd_gate),
    ...
    mean_beta_coi1_error = mean(x$beta_coi1_error),
    ...
    max_glmer_difference = max(
      x$glmer_beta_coi0_difference,
      x$glmer_beta_coi1_difference,
      x$glmer_sd_coi_difference,
      na.rm = TRUE
    )
  )
}))
...
if (smoke_only) {
  ...
  claim_pass <- with(
    ...
    diagnostics_pass == 4L &&
      ...
      mean_beta_coi1_error <= 0.20 && mean_log_sigma_error <= 0.15 &&
      mean_sd_coi_relative_error <= 0.40 && max_glmer_difference <= 1e-3
  )
