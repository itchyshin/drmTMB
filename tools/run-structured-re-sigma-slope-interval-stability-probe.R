...
next_gate <- function(stability_status) {
  ...
  "Diagnose persistent boundary/profile failures before coverage-grid design or public interval wording."
}
...
variants <- list(
  strong = list(
    n_each = 22L,
    seed_offset = 0L,
    sds = c(sigma_intercept = 0.60, sigma_x = 0.45)
  ),
  stronger_sigma = list(
    n_each = 24L,
    seed_offset = 100L,
    sds = c(sigma_intercept = 0.85, sigma_x = 0.65)
  )
)
variant_seeds <- c(phylo = 331L, spatial = 332L, animal = 333L, relmat = 334L)
...
for (variant in names(variants)) {
  ...
  for (provider in providers) {
    sim <- make_provider_data(
      provider,
      seed = variant_seeds[[provider]] + variant_spec$seed_offset,
      n_each = variant_spec$n_each,
      sds = variant_spec$sds
    )
    ...
    for (endpoint_member in endpoint_members) {
      ...
      probe_id <- paste0(
        "sigma_slope_interval_stability_",
        variant,
        "_",
        provider,
        "_",
      ...
      status_rows[[length(status_rows) + 1L]] <- data.frame(
        ...
        n_each = variant_spec$n_each,
        intended_sd_sigma_intercept = variant_spec$sds[["sigma_intercept"]],
        intended_sd_sigma_x = variant_spec$sds[["sigma_x"]],
        ...
        status = "covered",
        evidence_url = "docs/dev-log/after-task/2026-06-24-sigma-slope-interval-stability-probe.md",
...
for (object_name in c("method_out", "status_out")) {
