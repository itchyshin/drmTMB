...
# Current-source Arc 1 profile receipt for mc-0269, the exact Gaussian REML
# independent location random-slope SD target.
...
seed <- as.integer(arg_value("seed", "2026080251"))
groups <- as.integer(arg_value("groups", "64"))
each <- as.integer(arg_value("each", "12"))
if (anyNA(c(seed, groups, each)) || groups < 24L || each < 4L) {
  stop("seed must be finite, groups >= 24, and each >= 4.", call. = FALSE)
}
...
out <- arg_value(
  ...
  file.path(root, "docs/dev-log/interval-feasibility/results", source_sha,
            "arc1-gaussian-reml-slope-profile-feasibility")
)
...
truth_sd <- .35
u_slope <- stats::rnorm(groups, sd = truth_sd)
dat <- data.frame(
  y = .20 + .50 * x + u_slope[id] * x + stats::rnorm(length(id), sd = .70),
  x = x,
  id = id
)
...
fit <- drmTMB::drmTMB(
  drmTMB::bf(y ~ x + (0 + x | id)),
  family = stats::gaussian(), data = dat, REML = TRUE
)
...
if (!inherits(prof, "error")) {
  ...
  interval <- data.frame(
    ...
    cell_id = "mc-0269", target_id = "mc-0269::sd:mu:(0 + x | id)",
...
clean <- identical(conf_status, "profile") && all(is.finite(c(lower, estimate, upper))) &&
  ...
  identical(fit$opt$convergence, 0L) && isTRUE(fit$sdr$pdHess)
receipt <- data.frame(
  cell_id = "mc-0269", target_id = "mc-0269::sd:mu:(0 + x | id)",
  cohort_id = "arc1-gaussian-reml-slope-profile-feasibility",
  family = "gaussian", provider = "ordinary_re",
  dgp_id = "arc1_gaussian_reml_independent_mu_slope",
  dgp_version = sprintf("g%d_each%d_seed%d_current_source_v1", groups, each, seed),
  formula = "bf(y ~ x + (0 + x | id)); gaussian(identity); REML=TRUE",
  true_parameter_scale = "0.35 on the location random-slope SD scale",
  profile_parameter = target_name, seed = seed,
  execution_information_rung = sprintf("g%d_each%d", groups, each),
  binding_source = "tools/run-arc1-gaussian-reml-slope-profile-feasibility.R",
  source_sha = source_sha, runner_sha256 = runner_sha256,
  estimator = "REML", target_type = "direct",
...
print(receipt[c("cell_id", "target_id", "estimator", "conf_status", "lower", "upper", "convergence", "pdHess", "profile_boundary", "promotion_eligible")])
