...
print_help <- function() {
  cat(
    paste(
      "Usage: Rscript tools/run-gaussian-mu-slope-tranche85-spatial-drac-host-smoke.R [options]",
      "",
      "Tranche 85 fail-closed runner for the q1 mu one-slope spatial n=5 smoke.",
      ...
      "exact T83 DRAC source path, exact T83 DRAC run root, fixed seed manifest,",
...
normalize_path_allow_missing <- function(path) {
  path <- normalizePath(path, winslash = "/", mustWork = FALSE)
  if (file.exists(path)) {
  ...
  repeat {
    ...
    if (file.exists(candidate) || identical(parent, candidate)) {
      break
    }
    missing_parts <- c(basename(candidate), missing_parts)
    candidate <- parent
  }
  if (file.exists(candidate)) {
    normalized_candidate <- normalizePath(candidate, winslash = "/", mustWork = TRUE)
    return(do.call(file.path, as.list(c(normalized_candidate, missing_parts))))
  }
  path
}

is_descendant_path <- function(path, root) {
  path <- sub("/+$", "", path)
  root <- sub("/+$", "", root)
...
t85_source_snapshot_path <- "/project/def-snakagaw/snakagaw/drmtmb-qseries/20260702-q1-mu-slope-spatial-tranche80-drac-source-56add7f0/source"
t85_run_root_path <- "/project/def-snakagaw/snakagaw/drmtmb-qseries/20260702-q1-mu-slope-spatial-tranche80-drac-source-56add7f0"
default_output_dir <- file.path(
  t85_run_root_path,
  "results/tranche85-spatial-drac-path-gate"
)
...
host_label <- clean_ascii(arg_value("host-label", "drac_rorqual_q1mu_slope_spatial_t80_t77_runner_n5"))
source_snapshot_path <- clean_ascii(arg_value(
  "source-snapshot-path",
  t85_source_snapshot_path
))
run_root_path <- clean_ascii(arg_value("run-root-path", t85_run_root_path))
...
load_source <- arg_bool("load-source", TRUE)
...
if (!identical(provider, "spatial")) {
  stop("Tranche 85 is spatial-only; use --provider=spatial.", call. = FALSE)
}
if (!target %in% c("both", "mu_intercept", "mu_x")) {
  stop("`--target` must be both, mu_intercept, or mu_x.", call. = FALSE)
}
if (!identical(n_rep, 5L)) {
  stop("Tranche 85 is fixed at --n-rep=5.", call. = FALSE)
}
if (!identical(seeds, expected_seeds)) {
  stop(
    "Tranche 85 seed manifest must be 861001,861002,861003,861004,861005.",
    call. = FALSE
  )
}
if (isTRUE(write_dashboard)) {
  stop(
    "Tranche 85 is artifact-only; use --write-dashboard=false.",
    call. = FALSE
  )
}
if (!identical(source_snapshot_path, t85_source_snapshot_path)) {
  stop(
    "Tranche 85 must use the exact T83 DRAC source path: ",
    t85_source_snapshot_path,
    call. = FALSE
  )
}
if (!identical(run_root_path, t85_run_root_path)) {
  stop(
    "Tranche 85 must use the exact T83 DRAC run root path: ",
    t85_run_root_path,
    call. = FALSE
  )
}
...
raw_path <- file.path(output_dir, "structured-re-gaussian-mu-slope-tranche85-spatial-host-smoke-results.tsv")
run_log_path <- file.path(output_dir, "structured-re-gaussian-mu-slope-tranche85-spatial-host-smoke-run-log.tsv")
summary_path <- if (identical(summary_path_arg, "NA")) {
  file.path(output_dir, "structured-re-gaussian-mu-slope-tranche85-spatial-host-smoke-summary.tsv")
} else {
  summary_path_arg
}
run_id <- paste0("tranche85_spatial_mu_slope_", host_label, "_n5")

claim_boundary <- paste(
  "Tranche 85 spatial-only q1 mu one-slope n=5 host-smoke runner contract;",
  "execute mode is fail-closed behind",
  "DRMTMB_Q1MU_SLOPE_T77_EXECUTION_APPROVED=rose_fisher_gauss_noether_grace;",
  "source code must load from the exact T83 DRAC source path;",
  "load-source=false is an explicit installed-package route that uses library(drmTMB) after T125 review;",
  "inst/sim/R/sim_runner.R is sourced before dependent spatial DGP/run files;",
  "artifacts must write under the exact T83 DRAC run root;",
  "missing output paths are normalized through their nearest existing parent to tolerate DRAC /project-to-/lustre09 aliases;",
...
manifest <- do.call(
  ...
  lapply(targets, function(target_id) {
    data.frame(
      manifest_id = paste0("tranche85_spatial_", target_id, "_seed_", seeds),
      provider = "spatial",
      target = target_id,
...
approval <- Sys.getenv("DRMTMB_Q1MU_SLOPE_T77_EXECUTION_APPROVED", unset = "")
if (!identical(approval, "rose_fisher_gauss_noether_grace")) {
  stop(
    "Refusing Tranche 85 execution: set ",
    "DRMTMB_Q1MU_SLOPE_T77_EXECUTION_APPROVED=rose_fisher_gauss_noether_grace ",
    "after Rose/Fisher/Gauss/Noether/Grace approval and checkpoint.",
    call. = FALSE
  )
}
...
output_dir_norm <- normalize_path_allow_missing(output_dir)
if (!is_descendant_path(output_dir_norm, run_root)) {
  stop("`--output-dir` must be under the exact T83 DRAC run root.", call. = FALSE)
}
...
load_drmTMB_package <- function(path, load_source) {
  if (isTRUE(load_source)) {
    if (!requireNamespace("devtools", quietly = TRUE)) {
      stop("The Tranche 85 runner requires devtools for load_all().", call. = FALSE)
    }
    devtools::load_all(path, quiet = TRUE)
    return("devtools_load_all_exact_t83_drac_source_path")
  }
  suppressPackageStartupMessages(library(drmTMB))
  version <- tryCatch(
    as.character(utils::packageVersion("drmTMB")),
    error = function(e) "unknown_version"
  )
  paste0("library_drmTMB_installed_package_", version)
}
...
load_status <- tryCatch(
  load_drmTMB_package(source_root, load_source),
  error = function(e) {
    if (isTRUE(load_source)) {
      paste("devtools_load_all_failed:", conditionMessage(e))
    } else {
      paste("library_drmTMB_installed_package_failed:", conditionMessage(e))
    }
  }
)
load_ok <- if (isTRUE(load_source)) {
  identical(load_status, "devtools_load_all_exact_t83_drac_source_path")
} else {
  startsWith(load_status, "library_drmTMB_installed_package_")
}
if (!isTRUE(load_ok)) {
  run_log <- data.frame(
    run_id = run_id,
  ...
  if (isTRUE(load_source)) {
    stop("drmTMB could not be loaded from the exact T83 DRAC source path.", call. = FALSE)
  }
  stop("installed drmTMB could not be loaded with library(drmTMB).", call. = FALSE)
}
...
summaries <- do.call(
  ...
  lapply(names(target_parameters), function(target_id) {
    ...
    data.frame(
      result_id = paste0("tranche85_spatial_mu_slope_", target_id),
      cell_id = "qseries_spatial_q1_mu_one_slope",
      provider = "spatial",
      target = target_id,
      endpoint_member = target_members[[target_id]],
      ...
      promotion_decision = "do_not_promote",
      support_cell_decision = "unchanged_point_fit_planned_planned",
      rose_audit = "rose_no_status_claim_from_t85_artifact",
      fisher_review = "fisher_n5_smoke_not_coverage",
      noether_review = "noether_spatial_q1_mu_direct_sd_identity_preserved",
      grace_review = "grace_exact_t73_source_and_run_root_required",
      claim_boundary = claim_boundary,
      next_gate = paste(
        "Review Tranche 85 retained attempt artifacts for pdHess and finite",
        "direct-SD intervals before any admission wording; no coverage design",
        "or status edit without Rose/Fisher/Gauss/Noether/Grace review."
      ),
      stringsAsFactors = FALSE
    )
  })
...
message("[tranche85] wrote ", raw_path)
message("[tranche85] wrote ", summary_path)
message("[tranche85] wrote ", run_log_path)
message("[tranche85] coverage_decision=coverage_not_authorized")
message("[tranche85] promotion_decision=do_not_promote")
