...
new_relmat_nbinom2_slope_data <- function(seed, n_level = 8L, n_each = 20L) {
  set.seed(seed)
  ...
  mu <- exp(eta)
  data <- data.frame(
    nb2_relmat = stats::rnbinom(length(id), size = 1 / sigma^2, mu = mu),
    x = x,
    id = id
  )
  list(
...
run_one <- function(seed, replicate_id) {
  sim <- new_relmat_nbinom2_slope_data(seed)
  Q <- sim$Q
  warnings <- character()
  elapsed <- system.time({
    fit <- withCallingHandlers(
      drmTMB(
        bf(nb2_relmat ~ x + relmat(1 + x | id, Q = Q), sigma ~ 1),
        family = nbinom2(),
        data = sim$data,
...
summarise_results <- function(results, load_status, load_detail) {
  ...
  data.frame(
    micro_shard_id = "count_slope_relmat_nbinom2_q1_mu_one_slope_local_micro_shard",
...
run_log <- data.frame(
  run_id = "count_slope_relmat_nbinom2_q1_mu_one_slope_local_micro_shard_run",
  mode = "local_micro_shard",
  runner_script = "tools/run-structured-re-count-slope-relmat-nbinom2-local-micro-shard.R",
  ...
  claim_boundary = paste(
    "Local relmat K/Q NB2 q1 mu one-slope micro-shard only;",
    ...
    "no coverage evidence, no coverage-evaluable denominator, no q2,",
    "no q4, no REML, no AI-REML, no public support, no structured count sigma,",
