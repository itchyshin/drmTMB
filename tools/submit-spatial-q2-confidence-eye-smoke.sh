...
run_root="${CE_RUN_BASE}/spatial-q2-confidence-eye-${packet_sha}"
...
export CE_RUN_ROOT="${run_root}"
export DRMTMB_LIB="${run_root}/lib"
...
array_job=$(sbatch --parsable \
  ...
  --array="1-60%60" \
  --output="${run_root}/logs/smoke-%A_%a.out" \
  --error="${run_root}/logs/smoke-%A_%a.err" \
  ...
  "${packet}/tools/slurm/spatial-q2-confidence-eye.sbatch")
