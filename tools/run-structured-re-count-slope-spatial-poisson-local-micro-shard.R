...
new_spatial_poisson_slope_data <- function(seed, n_level = 8L, n_each = 20L) {
  set.seed(seed)
...
run_one <- function(seed, replicate_id) {
  sim <- new_spatial_poisson_slope_data(seed)
  coords <- sim$coords
  warnings <- character()
  elapsed <- system.time({
...
summarise_results <- function(results, load_status, load_detail) {
  ...
  data.frame(
    micro_shard_id = "count_slope_spatial_poisson_q1_mu_one_slope_local_micro_shard",
...
run_log <- data.frame(
  ...
  runner_script = "tools/run-structured-re-count-slope-spatial-poisson-local-micro-shard.R",
  ...
  claim_boundary = paste(
    "Local fixed-covariance spatial Poisson q1 mu one-slope micro-shard only;",
    "no range-estimating spatial support, no Totoro job submitted,",
    "no DRAC job submitted, no bridge parity,",
