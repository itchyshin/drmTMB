...
# Two shapes of written refusal count, and both must name a line of R/ source.
#
#   1. a family-tag refusal, the original case: the bridge will not marshal the
#      family at all, cited at R/julia-bridge.R:NNN.
#   2. an accessor-level FENCE, added 2026-09-05 (leaf `uncited-accessors`): the
#      route exists but a named accessor refuses on a `drmTMB_julia` object
#      because the bridge object lacks what that accessor needs, cited at any
#      R/*.R:NNN. Before this, a written boundary of that shape could only read
#      UNCITED -- the classifier had no way to express it, so a capability with
#      a real, cited, tested fence was counted as if nothing had been written.
#
# A row that merely ASSERTS "no bridge route" with no file:line behind it still
# reads UNCITED; that is the whole point of the axis and is unchanged.
sb_is_refused <- function(bridge_route) {
  grepl("REFUSED at drm_julia_family_tag\\(\\)", bridge_route, perl = TRUE) |
    grepl("FENCED at R/[A-Za-z0-9_.-]+\\.R:[0-9]+", bridge_route, perl = TRUE)
}
