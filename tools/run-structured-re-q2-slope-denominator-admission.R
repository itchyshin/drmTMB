...
rows <- lapply(seq_len(nrow(status)), function(i) {
  ...
  boundary <- paste0(
    ...
    " q2 slope denominator-admission diagnostic only;",
    provider_boundary[[provider]],
    " no interval reliability, interval coverage, coverage acceptance, q4/q8, REML, AI-REML, or broad bridge support promoted."
  )
  data.frame(
    ...
    status = "covered",
    evidence_url = "docs/dev-log/after-task/2026-06-24-q2-slope-denominator-admission.md",
