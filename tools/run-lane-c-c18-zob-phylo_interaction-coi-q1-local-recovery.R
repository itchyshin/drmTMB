#!/usr/bin/env Rscript
#
# C18 point recovery: structured `phylo_interaction` provider on the `coi`
# atom of zero_one_beta(), q1 (one unlabelled intercept), cell mc-0617.
#
...
#
# `coi` is informed only by boundary (y in {0,1}) observations, so this
# script additionally requires, per the C18 gate, that every group have
# BOTH a 0 and a 1 among its boundary observations (n_separated_groups ==
# 0). A group whose boundary observations all fall on one side is
# completely separated: it contributes nothing to coi, yet the GMRF prior
# supplies curvature so the fit converges cleanly and looks fine.
#
# Structured routing for `coi ~ phylo_interaction(...)` does not exist in
...
simulate_one <- function(seed, tau = .55) {
  ...
  mu <- plogis(-.15 + .35 * d$x)
  sigma <- exp(-1)
  zoi <- .5                   # fixed, unstructured
  coi <- plogis(u[gid])       # qlogis(.5) == 0: structured term carries the whole coi linear predictor

  boundary <- rbinom(nrow(d), 1L, zoi)
  y <- rbeta(nrow(d), mu / sigma^2, (1 - mu) / sigma^2)
  y[boundary == 1L] <- rbinom(sum(boundary), 1L, coi[boundary == 1L])
  d$y <- y
...
formula_str <- "y ~ x, sigma ~ 1, zoi ~ 1, coi ~ phylo_interaction(1 | plant:pollinator, tree1 = plant_tree, tree2 = pollinator_tree)"
...
fit_one <- function(seed, sha, runner) {
  ...
  f <- tryCatch(
    drmTMB::drmTMB(
      drmTMB::bf(
        y ~ x, sigma ~ 1, zoi ~ 1,
        coi ~ drmTMB::phylo_interaction(1 | plant:pollinator, tree1 = plant_tree, tree2 = pollinator_tree)
      ),
  ...
  base <- data.frame(
    cell_id = "mc-0617", dpar = "coi", seed, source_sha = sha, runner_sha = runner,
  ...
  tau_hat <- unname(f$sdpars$coi[["phylo_interaction(1 | plant:pollinator)"]])
  mode <- ranef(f, "phylo_interaction_coi")$terms[["phylo_interaction(1 | plant:pollinator)"]]
  cbind(
    base, status = "fit_ok",
...
out <- Sys.getenv(
  "DRMTMB_RECOVERY_OUT",
  unset = file.path(root, "docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-phylo_interaction-coi-q1-local-run-1")
)
dir.create(out, recursive = TRUE, showWarnings = FALSE)
sha <- trimws(system2("git", c("rev-parse", "HEAD"), stdout = TRUE))
runner <- unname(tools::md5sum(script))

attempts <- do.call(rbind, lapply(2026080711:2026080714, fit_one, sha = sha, runner = runner))
...
write_tsv(
  data.frame(
    cell_id = "mc-0617", planned_attempts = 4L, attempted_attempts = nrow(attempts),
...
# coi. Diagnostic fixture only; it is not part of the pass/fail gate above.
...
fixed_record <- if (inherits(fixed_fit, "error")) {
  data.frame(cell_id = "mc-0617", source_sha = sha, runner_sha = runner, status = "fit_error", decision = "BOUNDARY_DIAGNOSTIC_ONLY", error = conditionMessage(fixed_fit))
} else {
  data.frame(
    cell_id = "mc-0617", source_sha = sha, runner_sha = runner, status = "fit_ok",
    convergence = fixed_fit$opt$convergence, pdHess = isTRUE(fixed_fit$sdr$pdHess),
    tau_hat = unname(fixed_fit$sdpars$coi[["phylo_interaction(1 | plant:pollinator)"]]),
    decision = "BOUNDARY_DIAGNOSTIC_ONLY", error = ""
  )
}
write_tsv(fixed_record, file.path(out, "fixed-coi-phylo_interaction-boundary-diagnostic.tsv"))
