...
spatial_coords <- function(n_level = 8L) {
...
new_spatial_nbinom2_slope_data <- function(seed, n_level = 8L, n_each = 20L) {
  set.seed(seed)
  coords <- spatial_coords(n_level)
  ...
  mu <- exp(eta)
  data <- data.frame(
    nb2_spatial = stats::rnbinom(length(site), size = 1 / sigma^2, mu = mu),
    x = x,
    site = site
  )
  list(
...
run_one <- function(seed, replicate_id) {
  sim <- new_spatial_nbinom2_slope_data(seed)
  coords <- sim$coords
  warnings <- character()
  elapsed <- system.time({
  ...
  mu <- coef(fit, "mu")
  sd_mu <- fit$sdpars$mu
  intercept_label <- "spatial(1 | site)"
  slope_label <- "spatial(0 + x | site)"
  data.frame(
    replicate_id = replicate_id,
    seed = seed,
...
summarise_results <- function(results, load_status, load_detail) {
  ...
  data.frame(
    micro_shard_id = "count_slope_spatial_nbinom2_q1_mu_one_slope_local_micro_shard",
...
run_log <- data.frame(
  run_id = "count_slope_spatial_nbinom2_q1_mu_one_slope_local_micro_shard_run",
  mode = "local_micro_shard",
  runner_script = "tools/run-structured-re-count-slope-spatial-nbinom2-local-micro-shard.R",
  ...
  claim_boundary = paste(
    "Local fixed-covariance spatial NB2 q1 mu one-slope micro-shard only;",
    "sigma is fixed-effect overdispersion only;",
    "no range-estimating spatial support, no Totoro job submitted,",
