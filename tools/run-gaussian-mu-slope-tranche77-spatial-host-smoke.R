...
print_help <- function() {
  cat(
    paste(
      "Usage: Rscript tools/run-gaussian-mu-slope-tranche77-spatial-host-smoke.R [options]",
      "",
      "Tranche 77 fail-closed runner for the q1 mu one-slope spatial n=5 smoke.",
      ...
      "exact T73 source snapshot, exact T73 run root, fixed seed manifest,",
...
t77_source_snapshot_path <- "/home/snakagaw/codex/drmTMB-q1mu-slope-tranche73-clean-source-56add7f0-20260702T123451Z"
t77_run_root_path <- "/home/snakagaw/drmtmb-qseries/q1-mu-slope-spatial-tranche73-clean-source-20260702T123451Z"
default_output_dir <- file.path(
  t77_run_root_path,
  "tranche77-spatial-host-smoke-artifacts"
)
...
host_label <- clean_ascii(arg_value("host-label", "totoro_q1mu_slope_spatial_t77_t73_clean_source_n5"))
source_snapshot_path <- clean_ascii(arg_value(
  "source-snapshot-path",
  t77_source_snapshot_path
))
run_root_path <- clean_ascii(arg_value("run-root-path", t77_run_root_path))
...
if (!identical(provider, "spatial")) {
  stop("Tranche 77 is spatial-only; use --provider=spatial.", call. = FALSE)
}
if (!target %in% c("both", "mu_intercept", "mu_x")) {
  stop("`--target` must be both, mu_intercept, or mu_x.", call. = FALSE)
}
if (!identical(n_rep, 5L)) {
  stop("Tranche 77 is fixed at --n-rep=5.", call. = FALSE)
}
if (!identical(seeds, expected_seeds)) {
  stop(
    "Tranche 77 seed manifest must be 861001,861002,861003,861004,861005.",
    call. = FALSE
  )
}
if (isTRUE(write_dashboard)) {
  stop(
    "Tranche 77 is artifact-only; use --write-dashboard=false.",
    call. = FALSE
  )
}
if (!identical(source_snapshot_path, t77_source_snapshot_path)) {
  stop(
...
if (!identical(run_root_path, t77_run_root_path)) {
  stop(
    "Tranche 77 must use the exact T73 run root path: ",
    t77_run_root_path,
    call. = FALSE
  )
}
...
raw_path <- file.path(output_dir, "structured-re-gaussian-mu-slope-tranche77-spatial-host-smoke-results.tsv")
run_log_path <- file.path(output_dir, "structured-re-gaussian-mu-slope-tranche77-spatial-host-smoke-run-log.tsv")
summary_path <- if (identical(summary_path_arg, "NA")) {
  file.path(output_dir, "structured-re-gaussian-mu-slope-tranche77-spatial-host-smoke-summary.tsv")
} else {
  summary_path_arg
}
run_id <- paste0("tranche77_spatial_mu_slope_", host_label, "_n5")

claim_boundary <- paste(
  "Tranche 77 spatial-only q1 mu one-slope n=5 host-smoke runner contract;",
  "execute mode is fail-closed behind",
  "DRMTMB_Q1MU_SLOPE_T77_EXECUTION_APPROVED=rose_fisher_gauss_noether_grace;",
  "source code must load from the exact T73 Totoro snapshot;",
  "inst/sim/R/sim_runner.R is sourced before dependent spatial DGP/run files;",
  "artifacts must write under the exact T73 qseries run root;",
...
manifest <- do.call(
  ...
  lapply(targets, function(target_id) {
    data.frame(
      manifest_id = paste0("tranche77_spatial_", target_id, "_seed_", seeds),
      provider = "spatial",
      target = target_id,
...
if (!identical(approval, "rose_fisher_gauss_noether_grace")) {
  stop(
    "Refusing Tranche 77 execution: set ",
    "DRMTMB_Q1MU_SLOPE_T77_EXECUTION_APPROVED=rose_fisher_gauss_noether_grace ",
...
if (!startsWith(output_dir_norm, paste0(run_root, "/"))) {
  stop("`--output-dir` must be under the exact T73 run root.", call. = FALSE)
}
...
load_current_source <- function(path) {
  if (!requireNamespace("devtools", quietly = TRUE)) {
    stop("The Tranche 77 runner requires devtools for load_all().", call. = FALSE)
  }
...
load_status <- tryCatch(
  {
    ...
    "devtools_load_all_exact_t73_source_snapshot"
  },
  error = function(e) {
    paste("devtools_load_all_failed:", conditionMessage(e))
  }
)
if (!identical(load_status, "devtools_load_all_exact_t73_source_snapshot")) {
  run_log <- data.frame(
    run_id = run_id,
    mode = mode,
    host_label = host_label,
  ...
  stop("drmTMB could not be loaded from the exact T73 source snapshot.", call. = FALSE)
}
...
summaries <- do.call(
  ...
  lapply(names(target_parameters), function(target_id) {
    ...
    data.frame(
      result_id = paste0("tranche77_spatial_mu_slope_", target_id),
      cell_id = "qseries_spatial_q1_mu_one_slope",
      provider = "spatial",
      target = target_id,
      endpoint_member = target_members[[target_id]],
      ...
      promotion_decision = "do_not_promote",
      support_cell_decision = "unchanged_point_fit_planned_planned",
      rose_audit = "rose_no_status_claim_from_t77_artifact",
      ...
      claim_boundary = claim_boundary,
      next_gate = paste(
        "Review Tranche 77 retained attempt artifacts for pdHess and finite",
...
message("[tranche77] wrote ", raw_path)
message("[tranche77] wrote ", summary_path)
message("[tranche77] wrote ", run_log_path)
message("[tranche77] coverage_decision=coverage_not_authorized")
message("[tranche77] promotion_decision=do_not_promote")
