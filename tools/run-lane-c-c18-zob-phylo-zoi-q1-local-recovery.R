...
# C18 structured zero-one-beta atom recovery: mc-0603, provider `phylo`, q1
# (one unlabelled structured intercept on `zoi`'s linear predictor).
#
...
# feasibility campaign. `coi` stays fixed (unstructured) here; the phylo field
# rides on `zoi`'s linear predictor around the zoi = 0.50 baseline.
#
...
simulate_one <- function(seed, tau = .55) {
  ...
  zoi <- plogis(qlogis(.50) + b[species]); coi <- .50
...
fit_one <- function(seed, sha, md5) {
  ...
  tree <- sim$tree
  fit <- tryCatch(drmTMB::drmTMB(drmTMB::bf(y ~ x, sigma ~ 1, zoi ~ drmTMB::phylo(1 | species, tree = tree), coi ~ 1), family = drmTMB::zero_one_beta(), data = sim$data, control = drmTMB::drm_control(se = TRUE, optimizer = list(eval.max = 2000L, iter.max = 2000L))), error = identity)
  if (inherits(fit, "error")) return(data.frame(cell_id = "mc-0603", dpar = "zoi", seed, source_sha = sha, runner_md5 = md5, status = "fit_error", error = conditionMessage(fit)))
  tau_hat <- unname(fit$sdpars$zoi[["phylo(1 | species)"]]); u_hat <- ranef(fit, "phylo_zoi")$terms[["phylo(1 | species)"]]
  data.frame(cell_id = "mc-0603", dpar = "zoi", seed, source_sha = sha, runner_md5 = md5, status = "fit_ok", convergence = fit$opt$convergence, pdHess = isTRUE(fit$sdr$pdHess), max_gradient = max(abs(fit$obj$gr(fit$opt$par)), na.rm = TRUE), tau_truth = .55, tau_hat, mode_correlation = cor(u_hat[names(sim$b)], sim$b), min_group_zero = sim$min_group_zero, min_group_one = sim$min_group_one, min_group_interior = sim$min_group_interior, n_separated_groups = sim$n_separated_groups, min_boundary_per_group = sim$min_boundary_per_group, clamp_active = any(abs(fit$obj$report()$log_sigma) > 11.99), boundary_hit = !is.finite(tau_hat) || tau_hat <= .05 || tau_hat >= 2.5)
}
...
out <- Sys.getenv("DRMTMB_RECOVERY_OUT", unset = file.path(root, "docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-phylo-zoi-q1-local-recovery")); dir.create(out, recursive = TRUE, showWarnings = FALSE)
sha <- trimws(system2("git", c("rev-parse", "HEAD"), stdout = TRUE)); md5 <- unname(tools::md5sum(script)); attempts <- do.call(rbind, lapply(2026080201:2026080204, fit_one, sha = sha, md5 = md5)); write_tsv(attempts, file.path(out, "raw-attempts.tsv"))
...
fixed <- simulate_one(2026080299L, tau = 0); tree <- fixed$tree; fixed_fit <- tryCatch(drmTMB::drmTMB(drmTMB::bf(y ~ x, sigma ~ 1, zoi ~ drmTMB::phylo(1 | species, tree = tree), coi ~ 1), family = drmTMB::zero_one_beta(), data = fixed$data, control = drmTMB::drm_control(se = TRUE, optimizer = list(eval.max = 2000L, iter.max = 2000L))), error = identity)
fixed_record <- if (inherits(fixed_fit, "error")) data.frame(source_sha = sha, runner_md5 = md5, status = "fit_error", decision = "BOUNDARY_DIAGNOSTIC_ONLY") else data.frame(source_sha = sha, runner_md5 = md5, status = "fit_ok", convergence = fixed_fit$opt$convergence, pdHess = isTRUE(fixed_fit$sdr$pdHess), tau_hat = unname(fixed_fit$sdpars$zoi[["phylo(1 | species)"]]), decision = "BOUNDARY_DIAGNOSTIC_ONLY")
write_tsv(fixed_record, file.path(out, "fixed-zoi-phylo-boundary-diagnostic.tsv"))
