...
claim_boundary <- function(provider, denominator_status) {
  ...
  clean_text(paste(
    provider_clause,
    "q2 slope denominator-extension diagnostic only;",
    "status =",
    denominator_status,
    "with",
    blocked_clause,
    "no interval reliability, coverage acceptance, q4/q8, REML, AI-REML,",
    "or broad bridge support promoted."
  ))
}

next_gate <- function(denominator_status) {
  if (identical(denominator_status, "extension_candidate")) {
    return(
      "Accumulate replicated denominator counts and MCSE before coverage wording."
    )
  }
  "Keep target out of coverage denominators until profile failures are diagnosed."
}

variants <- list(
  extension_seed_a = list(
    n_each = 30L,
    sd_mu1_x = 1.05,
    sd_mu2_x = 0.90,
    cor_mu1_mu2_x = 0.20,
...
provider_seeds <- c(phylo = 501L, spatial = 502L, animal = 503L, relmat = 504L)
...
for (variant_name in names(variants)) {
  ...
  for (provider in providers) {
    sim <- make_provider_data(
      provider,
      seed = provider_seeds[[provider]] + variant$seed_offset,
      variant = variant
    )
    ...
    for (endpoint_member in endpoint_members) {
      ...
      extension_status <- classify_status(rows)
      admission_row <- admission[
        admission$structured_type == provider &
          admission$endpoint_member == endpoint_member,
        ,
        drop = FALSE
      ]
      admission_status <- admission_row$denominator_admission[[1L]]
      denominator_status <- if (
        identical(admission_status, "diagnostic_denominator_candidate") &&
          identical(extension_status, "wald_profile_finite") &&
        ...
        "not_admitted_from_smoke"
      } else {
        "not_admitted_extension_failure"
      }
      extension_id <- paste0(
      ...
      status_rows[[length(status_rows) + 1L]] <- data.frame(
        ...
        denominator_extension_status = denominator_status,
        failure_class = classify_failure(rows),
        coverage_status = "not_evaluated",
        interval_claim_status = "diagnostic_only",
        status = "covered",
        evidence_url = "docs/dev-log/after-task/2026-06-24-q2-slope-denominator-extension.md",
        claim_boundary = claim_boundary(provider, denominator_status),
        next_gate = next_gate(denominator_status),
        stringsAsFactors = FALSE
      )
    }
  }
...
cat("wrote ", artifact_path, " with ", nrow(method_out), " rows\n", sep = "")
cat("wrote ", status_path, " with ", nrow(status_out), " rows\n", sep = "")
