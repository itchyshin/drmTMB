...
# Animal correlation can be run only through the explicit diagnostic holdout:
#   --holdout=animal_cor
#
...
parse_args <- function(args) {
  ...
  for (arg in args) {
    ...
    } else if (startsWith(arg, "--holdout=")) {
      out$holdout <- sub("^--holdout=", "", arg)
...
make_q2_slope_data <- function(provider, seed, n_each = 20L) {
  ...
  actual_n_groups <- length(labels)
  endpoint <- rep(labels, each = n_each)
  x <- rep(seq(-1.25, 1.25, length.out = n_each), times = actual_n_groups)

  eta1 <- TRUTH$mu1_intercept +
    TRUTH$mu1_x_fixef * x +
    effects[endpoint, "mu1_x"] * x
  eta2 <- TRUTH$mu2_intercept +
    TRUTH$mu2_x_fixef * x +
...
truth_for <- function(target) {
  if (identical(target, "mu1:x")) {
    return(TRUTH$sd_mu1_x)
  }
  if (identical(target, "mu2:x")) {
    return(TRUTH$sd_mu2_x)
  }
  if (identical(target, "mu1:x+mu2:x")) {
    return(TRUTH$cor_mu1_mu2_x)
  }
...
make_summary <- function(
  ...
  n_wald_cov <- sum(
    !is.na(wald_fin$wald_contains) & wald_fin$wald_contains,
    na.rm = TRUE
  )
  ...
  n_prof_cov <- sum(
    !is.na(prof_fin$profile_contains) & prof_fin$profile_contains,
    na.rm = TRUE
  )
  ...
  n_boot_cov <- sum(
    !is.na(boot_fin$bootstrap_contains) & boot_fin$bootstrap_contains,
    na.rm = TRUE
  )
  ...
  data.frame(
    ...
    claim_boundary = paste(
      provider,
      "q2-slope coverage grid shard only;",
...
message("[grid] Explicit diagnostic holdouts: --holdout=animal_cor")

if (!is.na(args$holdout) && !is.na(args$shard)) {
  stop("Use either --shard=N or --holdout=animal_cor, not both.", call. = FALSE)
}
if (!is.na(args$holdout)) {
  if (!identical(args$holdout, "animal_cor")) {
    stop(
      sprintf(
        "Invalid --holdout=%s. The only explicit holdout is animal_cor.",
        args$holdout
      ),
      call. = FALSE
    )
  }
  shard_row <- data.frame(
    shard = 11L,
    provider = "animal",
    target = "mu1:x+mu2:x",
    target_kind = "direct_correlation",
    stringsAsFactors = FALSE
  )
} else {
  # Validate shard arg
...
out_dir <- if (!is.na(args$out_dir)) {
  ...
  file.path(
    ...
    sprintf("q2-slope-coverage-grid-shard%02d", shard_row$shard)
  )
}
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

rep_file_stem <- sprintf("%02d-%s-%s", shard_row$shard, provider, tok)
