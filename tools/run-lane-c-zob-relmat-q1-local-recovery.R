...
run_one <- function(seed, source_sha, runner_sha) {
  ...
  out$boundary_hit <- !all(is.finite(estimate)) || any(abs(estimate[c("beta0_hat", "beta1_hat")]) >= 5) ||
    out$sigma_hat <= .05 || out$sigma_hat >= 2.5 || out$zoi_hat <= .01 || out$zoi_hat >= .99 ||
    out$coi_hat <= .01 || out$coi_hat >= .99 || out$tau_hat <= .05 || out$tau_hat >= 2.5
  out
}
