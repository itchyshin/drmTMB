#!/usr/bin/env Rscript
#
# Small q4 animal bounded-correlation diagnostic.
#
# This runner compares a zero-correlation positive control, the current
# all-free staged fit, and bounded theta = cap * tanh(eta) optimizer-layer
# continuations for the hard animal q4 all-four one-slope seeds. It is
# diagnostic evidence only: interval coverage, inference_ready, supported, q8,
# REML, AI-REML, and broad bridge support are out of scope.
...
if (any(args %in% c("--help", "-h"))) {
  cat(
    paste(
      "Usage: Rscript tools/run-structured-re-q4-animal-bounded-correlation-diagnostic.R [options]",
      "",
      "Options:",
      ...
      "  --caps=a,b                Theta caps for bounded continuations (default: 0.50,0.80,0.95).",
...
cap_arg <- strsplit(arg_value("caps", "0.50,0.80,0.95"), ",", fixed = TRUE)[[
  1L
]]
theta_caps <- as.numeric(trimws(cap_arg[nzchar(trimws(cap_arg))]))
if (
  !length(theta_caps) || any(!is.finite(theta_caps)) || any(theta_caps <= 0)
) {
  stop(
    "`--caps` must be a comma-separated list of positive finite values.",
    call. = FALSE
  )
}
theta_caps <- sort(unique(theta_caps))
...
run_log_path <- file.path(
  artifact_dir,
  "structured-re-q4-animal-bounded-correlation-run-log.tsv"
)
...
bounded_inner_par <- function(par, theta_index, theta_cap) {
  out <- par
  if (length(theta_index)) {
    out[theta_index] <- theta_cap * tanh(par[theta_index])
  }
  out
}

bounded_outer_gradient <- function(obj, par, theta_index, theta_cap) {
  inner <- bounded_inner_par(par, theta_index, theta_cap)
  grad <- obj$gr(inner)
  if (length(theta_index)) {
    grad[theta_index] <- grad[theta_index] *
      theta_cap *
      (1 - tanh(par[theta_index])^2)
  }
  grad
}

fit_bounded_start_map_object <- function(obj, theta_cap) {
  theta_index <- theta_indices(obj$par)
  if (!length(theta_index)) {
    return(list(
      fit_error = simpleError(
        "No live theta_phylo parameters for bounded fit."
      ),
      warnings = character(),
      outer_gradient = NA_real_,
      theta_cap = theta_cap,
      theta_eta_max = NA_real_,
      cap_saturation = NA
    ))
  }
  warnings <- character()
  ...
  outer_par <- opt$par
  inner_par <- bounded_inner_par(outer_par, theta_index, theta_cap)
  outer_gradient <- tryCatch(
    bounded_outer_gradient(obj, outer_par, theta_index, theta_cap),
  ...
  list(
    ...
    sdr = sdr,
    attempts = data.frame(
      optimizer_preset = "bounded_theta_nlminb",
      convergence = opt$convergence,
      objective = opt$objective,
      stringsAsFactors = FALSE
    ),
    selected = list(optimizer_preset = "bounded_theta_nlminb"),
    warnings = warnings,
    ...
    theta_cap = theta_cap,
    theta_eta_max = if (length(theta_index)) {
      max(abs(outer_par[theta_index]), na.rm = TRUE)
    } else {
      NA_real_
    },
    cap_saturation = if (length(theta_inner)) {
      max(abs(theta_inner), na.rm = TRUE) >= 0.95 * theta_cap
    } else {
      NA
    }
  )
...
fit_geometry <- function(fit) {
  if (!is.null(fit$fit_error)) {
    return(list(
      ...
      theta_cap = fit$theta_cap %||% NA_real_,
      theta_eta_max = fit$theta_eta_max %||% NA_real_,
      cap_saturation = fit$cap_saturation %||% NA,
...
strategy_status <- function(strategy, geom) {
  ...
  if (isTRUE(strategy$bounded_theta) && isTRUE(geom$cap_saturation)) {
    return("bounded_cap_saturated")
  }
  if (
    isTRUE(geom$pdHess) &&
...
blocker_component <- function(strategy, geom, status) {
  ...
  if (status == "bounded_theta_pass_smoke") {
    return("bounded_theta_geometry_promising")
  }
  if (status == "bounded_cap_saturated") {
    return("bounded_theta_cap_saturated")
  }
...
bounded_strategies <- stats::setNames(
  lapply(theta_caps, function(theta_cap) {
    list(
      description = paste0(
        "optimizer-layer bounded continuation with theta = ",
        format(theta_cap, nsmall = 2L),
        " * tanh(eta), staged from the zero-correlation fit"
      ),
      log_sd_start = "staged",
      theta_start = "zero",
  ...
  paste0("bounded_theta_", format(theta_caps, nsmall = 2L))
)
...
claim_boundary_text <- paste(
  "Animal q4 all-four one-slope bounded-correlation diagnostic only:",
  "strategies compare a zero-correlation control, the current all-free staged",
  "fit, and optimizer-layer theta = cap * tanh(eta) continuations;",
...
next_gate_text <- paste(
  "If bounded theta caps rescue all three hard seeds without cap saturation,",
  "run a 16-replicate diagnostic admission rerun before any DRAC coverage-grid",
  "design; if low caps pass but high caps fail, keep this as bounded/penalized",
  "design evidence only; Fisher/Rose must approve denominator policy before any",
  "status edit."
)
...
for (replicate_index in replicate_indices) {
  ...
  message(
    "Running animal q4 bounded-correlation replicate ",
    replicate_index,
    " (",
    variant,
    ", seed ",
  ...
  for (strategy_name in names(strategies)) {
    ...
    fit <- if (isTRUE(strategy$bounded_theta)) {
      fit_bounded_start_map_object(built$obj, strategy$theta_cap)
    } else {
    ...
    rows[[length(rows) + 1L]] <- data.frame(
      diagnostic_id = paste0(
        "q4_animal_bounded_correlation_",
        replicate_index,
        "_",
        strategy_name
      ),
      ...
      theta_parameterization = if (isTRUE(strategy$bounded_theta)) {
        "optimizer_layer_cap_tanh"
      } else {
        "current_tmb_theta"
      },
      theta_cap = format_num(strategy$theta_cap %||% NA_real_, digits = 8L),
      ...
      theta_eta_max_abs = format_num(geom$theta_eta_max, digits = 8L),
      cap_saturation = if (isTRUE(strategy$bounded_theta)) {
        as.character(isTRUE(geom$cap_saturation))
      } else {
        "not_applicable"
      },
      ...
      evidence_url = "docs/dev-log/after-task/2026-06-29-q-series-q4-animal-bounded-correlation-diagnostic.md",
...
message("Wrote bounded-correlation artifact: ", artifact_path)
if (write_dashboard) {
