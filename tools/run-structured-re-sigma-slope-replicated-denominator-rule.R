...
denominator_action <- function(admission_status, finite_count, pdhess_count) {
  if (
...
claim_boundary <- function(provider) {
  clean_text(paste(
    ...
    "sigma-only one-slope replicated-denominator rule only;",
...
next_gate <- function(action) {
  if (identical(action, "eligible_for_pregrid_with_retention")) {
    return(paste(
      "Run a predeclared sigma-only one-slope coverage pre-grid with all fit,",
      "profile, nonfinite-interval, and bootstrap-refit outcomes retained in",
      "the denominator; require MCSE <= 0.01 before coverage wording."
    ))
  }
  paste(
...
rows <- lapply(seq_len(nrow(admission)), function(i) {
  ...
  key <- stability$structured_type == provider &
    stability$endpoint_member == endpoint_member
  stability_rows <- stability[key, , drop = FALSE]
  stability_variant_count <- nrow(stability_rows)
  stability_finite_count <- sum(
    stability_rows$wald_status == "finite" &
      stability_rows$profile_status == "finite"
  )
  stability_pdhess_count <- sum(as.integer(stability_rows$n_pdhess) == 1L)
  action <- denominator_action(
    row$denominator_admission[[1L]],
    stability_finite_count,
    stability_pdhess_count
  )
  ...
  data.frame(
    ...
    stability_variant_count = stability_variant_count,
    stability_wald_profile_finite_count = stability_finite_count,
    stability_pdhess_true_count = stability_pdhess_count,
    ...
    status = "covered",
    evidence_url = "docs/dev-log/after-task/2026-06-24-sigma-slope-replicated-denominator-rule.md",
