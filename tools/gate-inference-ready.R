...
# miss-ratio > 2 check. This script replaces the allowlist with a computation.
#
...
#   attempt_status / <chan>_status.
# and, per (member, channel), emits the gate metrics + PASS/FAIL.
#
...
#       interval is NOT dropped -- it is scored as a miss on the side its point
#       estimate lies (here: scored as a miss, side unknown -> counted `high`
#       when est > truth else `low` if the estimate column is present, otherwise
#       counted as an unsided miss that still fails coverage).
#   P1  n_miss >= MIN_MISS (default 40): a power floor. Below it -> interval_feasible.
#   G1  |coverage - 0.95| <= 2 * MCSE_coverage      (two-sided; over-coverage fails)
...
#
# Exit code 0 iff at least one (member, channel) is evaluable; the PASS/FAIL is
# in the emitted table (and the machine-readable last line `GATE_RESULT=...`).
# This is deliberately a REPORTING tool: promotion is a human decision on top of it.
...
gate_one <- function(lo, hi, est, truth, label) {
  ...
  # covered among FINITE; non-finite among fitted are scored as misses.
  covered <- finite & lo <= truth & hi >= truth
  coverage <- sum(covered) / max(n_fit, 1L)     # UNCENSORED: denominator = fitted, not finite
  mcse <- sqrt(coverage * (1 - coverage) / max(n_fit, 1L))
  # miss sides: finite misses by bound; non-finite scored by est if available
...
for (chan in members) {
  ...
  est_col <- if ("estimate_sd" %in% names(d)) "estimate_sd" else NA
  est <- if (!is.na(est_col)) d[[est_col]] else rep(NA_real_, nrow(d))
  rows[[chan]] <- gate_one(d[[lo_col]], d[[hi_col]], est, truth, chan)
}
if (!length(rows)) stop("no member/channel columns found for: ", paste(members, collapse = ", "))
