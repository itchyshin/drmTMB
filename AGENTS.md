...
> [`docs/dev-log/handover/AGENTS-handover-log-archive.md`](docs/dev-log/handover/AGENTS-handover-log-archive.md).**
> Keep this section to the **current** handover only — append the new one here and roll the previous
> entry into that archive. This file is loaded in full by every agent session; it is instruction, not log.
