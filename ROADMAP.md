...
- Closure boundary: Phase 6c now includes the ordinary grouped q > 2 Gaussian
  ...
  own q2 smoke/recovery lane. Univariate correlated residual-scale slope
  blocks and p8/q8 endpoints remain planned. The structured
...
| 185 | Bivariate slope route | Superseded by Slice 83, the q4/q6 location smoke routes, the q2 scale-slope route, and the same-response q2 location-scale slope route: matching slope-only `mu1`/`mu2` blocks such as `(0 + x | p | id)`, matching q=4/q=6 location blocks such as `(1 + x | p | id)` and `(1 + x + z | p | id)`, matching q=2 `sigma1`/`sigma2` scale-slope blocks, and matching same-response `mu1`/`sigma1` or `mu2`/`sigma2` slope-only blocks are now fitted, while all-four p8/q8 endpoint slope blocks remain closed. |
...
- Slice 189 is done: the double-hierarchical endpoint map now reflects the
  ...
  `mu`/`sigma` slope route, and the still-closed p8/q8 location-scale and
  spatial q=4 endpoints.
