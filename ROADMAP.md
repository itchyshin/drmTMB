...
| 1816 | Random-slope workflow plan | Done locally: `phase18_random_slope_workflow_plan()` filters the structured workflow registry to admitted random-slope rows, labels dispatch state as `ready_existing_task`, `source_test_audit`, or `needs_wrapper_target`, and keeps blocked, design-only, and diagnostic rows out of the plan. The current plan has nine rows: eight map to existing Actions tasks, while the bivariate Gaussian slope-only row remains the explicit wrapper target. |
