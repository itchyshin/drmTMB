...
- The first spatial fitted paths are now `spatial(1 | site, coords = coords)`
  ...
  `zi` and NB2 fixed-`zi` spatial `mu` local-fit gates remain planned.
...
outside the exact Poisson spatial `zi` and NB2 fixed-`zi` spatial `mu`
...
- Core ordinary grouped status: the random-intercept, one-slope, and q > 2
  ...
  Multiple structured slopes, larger structured slope covariance/correlation, residual-scale
...
- Non-Gaussian `mu` slope gate: #441 is closed at the admission-decision level
  ...
  the exact Poisson spatial `zi`, NB2 fixed-`zi` spatial `mu`, and
  truncated-NB2 relmat `hu` local-fit gates, ordinal mixed models, and
...
- Harden and extend Student-t, lognormal, Gamma, beta, Poisson, and
  ...
  slice. A fixed-effect skew-normal guard grid is banked as diagnostic-hold
  evidence; formal operating-characteristic grids and skew-t remain later
  evidence gates.
...
- Status: `poisson(link = "log")` is implemented as a fixed-effect baseline
  ...
  location models with ordered cutpoints and fixed latent logistic scale. A
  later Q-Series v1.0 gate adds one local fit-only/extractor row for
  `mu ~ phylo(1 | id, tree = tree)`.
- Ordinal random effects remain planned as a separate non-Gaussian mixed-model
  lane outside that exact phylogenetic `mu` intercept gate. Ordinary grouped
  intercepts, ordinal random slopes, and ordinary grouped multi-slope
  covariance belong to the Phase 4/6c boundary, not to the ordinal MVP itself.
...
- Add skew-t only after Student-t, the first skew-normal slice, Gaussian
  ...
  keeps fitted tail-floor exposure and fixed-gradient warnings visible, but it
  is not a formal 500- or 1000-replicate operating-characteristic result.
...
| Phylogenetic structured effects | Univariate Gaussian `mu` and `sigma` intercepts, matching univariate `mu`/`sigma` correlation, one-slope `mu`, the first sigma-only and matched `mu+sigma` one-slope native point-fit/extractor cells, bivariate q=2 intercept and slope-only location covariance, the exact shared-label all-four one-slope point-fit/extractor cell with deterministic same-target fixture evidence, direct-SD, q=2 correlation-regression, and q=4 location-scale paths are fitted. | Direct phylogenetic SDs and q=2 correlations have profile targets; full q=4 correlations are derived-only, while block-diagonal q=4 fallback correlations are direct targets that still need fit-specific profile diagnostics; the sigma-only, matched one-slope, slope-only q=2 `mu1:x`/`mu2:x`, and exact all-four one-slope rows have native point-fit/extractor evidence plus deterministic same-target fixture evidence where documented. | Multiple phylogenetic slopes, block-diagonal all-four one-slope covariance, broad bridge/inference beyond deterministic same-target fixtures, slope correlations, direct-SD formulas combined with structured `sigma`, structured `rho12`, non-Gaussian phylogenetic effects, intervals, coverage, and REML/AI-REML for this q8-shaped cell remain planned. |
| Coordinate spatial structured effects | Fitted for univariate Gaussian `mu` and `sigma` intercepts, matching univariate `mu`/`sigma` correlation, one numeric `mu` slope and the first fixed-covariance sigma-only and matched `mu+sigma` one-slope native point-fit/extractor cells with independent coordinate fields, constant and slope-only bivariate Gaussian `mu1`/`mu2` q=2 location covariance, constant q=4 location-scale covariance, the exact shared-label fixed-covariance all-four one-slope q8-shaped cell, ordinary Poisson/NB2 q=1 `mu` intercept and unlabelled one-slope terms, and the row-specific zero-inflated NB2 fixed-`zi` spatial `mu` local-fit gate. | `sdpars$mu`, `sdpars$sigma`, marker-specific `ranef()` blocks, `profile_targets()`, `check_drm()`, `corpairs(level = "spatial")`, `summary()$covariance`, a `mu` slope-field profile interval, deterministic fixed-covariance same-target fixtures, the q=2 dense covariance comparator, q=2 slope-only extractor tests, q=4 extractor/diagnostic tests, exact all-four one-slope extractor tests, exact all-four one-slope fixture parity, and count structured tests are covered. The zero-inflated NB2 fixed-`zi` spatial `mu` row is local fit-only/extractor evidence only. | Mesh/SPDE, range-estimating spatial support, multiple slopes, block-diagonal or broader intercept-plus-slope labelled structured covariance, broad bridge/inference beyond deterministic same-target fixtures, intervals, coverage, REML/AI-REML for this q8-shaped cell, slope correlations, spatial direct-SD, spatial `corpair()`, pure, multiple, or labelled count spatial slopes, and zero-inflated spatial effects outside the exact Poisson spatial `zi` and NB2 fixed-`zi` spatial `mu` local-fit gates remain planned. |
| Non-Gaussian families | Fixed-effect likelihoods are fitted; plain binomial fits 0/1 and `cbind(successes, failures)` logit models with ordinary `mu` random intercepts and independent slopes as first slices; ordinary Poisson and NB2 `mu` random intercepts plus independent numeric slopes are fitted for non-zero-inflated count models; q=1 structured `mu` intercept terms from `phylo()`, `phylo_interaction()`, `spatial()`, `animal()`, or `relmat()` and unlabelled one-slope terms from `phylo()`, `spatial()`, `animal()`, or `relmat()` are fitted for ordinary Poisson/NB2; a simultaneous two-provider NB2 `mu` route, `spatial(1 | site, coords = coords) + relmat(1 | id, Q = Q)`, builds and surfaces both structured fields on a crossed `site x id` design with recovery-only evidence (both fixed-covariance variance components recover with a positive-definite Hessian; intervals and coverage unsupported); ordinary Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial `mu` random intercepts have Phase 18 artifact lanes and independent numeric `mu` slopes have focused source tests; ordinary NB2, lognormal, and Gamma have first log-`sigma` random-intercept gates; and five Q-Series v1.0 rows have local fit-only/extractor evidence: Student-t `nu ~ phylo(1 | id, tree = tree)`, cumulative-logit ordinal `mu ~ phylo(1 | id, tree = tree)`, truncated-NB2 hurdle `hu ~ relmat(1 | id, Q = Q)`, zero-inflated Poisson `zi ~ spatial(1 | id, coords = coords)`, and zero-inflated NB2 fixed-`zi` `mu ~ spatial(1 | id, coords = coords)`. | Binomial fixed-effect coefficients use the Wald path and focused `stats::glm()` parity tests; ordinary binomial random-effect SDs have direct profile targets, with only the exact `mc-0061` slope domain inference-ready with caveats; Poisson, NB2, Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial `mu` random-effect SDs appear in `sdpars$mu`, random effects, and direct `profile_targets()` rows; Student-t, zero-truncated NB2, bounded-response, and positive-continuous artifact lanes record fixed-effect Wald rows and direct-SD profile rows for ordinary `(1 | id)` in `mu`; `tests/testthat/test-nongaussian-mu-random-slopes.R` records CRAN-safe slope recovery, prediction, extractor, profile-target, and diagnostic checks; NB2, lognormal, and Gamma `sigma` random-intercept SDs appear in `sdpars$sigma`, `random_effects$sigma`, and direct `log_sd_sigma` profile-target rows; family-specific fixed-effect summaries, structured count diagnostics, and intervals exist where already implemented; the Student-t `nu`, cumulative-logit ordinal `mu`, truncated-NB2 hurdle `hu`, zero-inflated Poisson `zi`, and zero-inflated NB2 fixed-`zi` spatial `mu` rows are local fit-only/extractor rows with intervals unsupported and coverage planned. | Correlated or labelled binomial slopes, binomial structured effects, zero-inflation beyond the exact Poisson spatial `zi` and NB2 fixed-`zi` spatial `mu` local-fit gates, hurdle random effects beyond the exact truncated-NB2 relmat `hu` local-fit gate, ordinal random effects beyond the exact phylogenetic `mu` local-fit gate, pure, multiple, or labelled structured count slopes, simultaneous structured count types beyond the crossed two-provider NB2 `mu` recovery route, labelled count covariance, cross-parameter covariance, NB2 `sigma` slopes or structured effects, correlated Student-t/zero-truncated NB2/positive-continuous/bounded-response random slopes, non-Gaussian `sigma` random effects outside the ordinary NB2, lognormal, and Gamma intercept gates, Student-t `nu` random effects beyond the exact phylo local-fit gate, exact 0/1 boundary mass, other non-Gaussian scale random effects, and other shape or inflation random effects still need separate implementation evidence before broad simulation. |

| Slice | Lane | Target Before Phase 18 |
| --- | --- | --- |
| 190 | Non-Gaussian `mu` random effects | Done and extended: first candidates were ordinary `mu` random intercepts for Poisson and NB2-style count likelihoods. Ordinary Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial `mu` random intercepts now have focused source-test slices and Phase 18 artifact lanes, and independent numeric `mu` slopes have a CRAN-safe focused recovery slice. Ordinal, zero-inflation, hurdle, shape, correlated slopes, and structured non-Gaussian paths retain explicit unsupported messages except for the later row-specific local fit-only Q-Series gates for Student-t `nu ~ phylo(1 | id, tree = tree)`, truncated-NB2 hurdle `hu ~ relmat(1 | id, Q = Q)`, zero-inflated Poisson `zi ~ spatial(1 | id, coords = coords)`, and zero-inflated NB2 fixed-`zi` `mu ~ spatial(1 | id, coords = coords)`. |
...
| 195 | Zero-inflation, hurdle, and one-inflation random effects | Done and extended: `zi`, `hu`, `zoi`, and `coi` random-effect requests receive component-specific boundaries, with one later row-specific local fit-only gate for zero-inflated Poisson `zi ~ spatial(1 | id, coords = coords)`. Fixed-effect zero-inflation and hurdle paths remain implemented, and fixed-effect `zoi`/`coi` are now fitted only in `zero_one_beta()`; other count-side random effects in zero-inflated or hurdle routes and covariance among `mu`, `sigma`, shape, inflation, hurdle, or one-inflation random effects remain future work until likelihood, interval, and recovery evidence exists. |
| 196 | Ordinal mixed models | Done and extended: cumulative-logit `mu` random-effect bar terms have an ordinal-specific boundary, and the exact Q-Series row `mu ~ phylo(1 | id, tree = tree)` now has local fit-only/extractor evidence. Ordinary grouped intercepts such as `(1 | id)`, ordinal random slopes, scale/discrimination formulas, known covariance, other phylo/spatial ordinal effects, and `ordinal::clmm` comparator recovery stay planned. |
| 197 | Structured non-Gaussian random effects | Done and extended: phylogenetic, spatial, animal, and `relmat()` structured markers have a structured non-Gaussian boundary. First Poisson/NB2 q=1 structured `mu` slices have source-test, smoke, or formal-admission evidence where documented, and three later Q-Series rows add local fit-only/extractor evidence for Student-t `nu ~ phylo(1 | id, tree = tree)`, cumulative-logit ordinal `mu ~ phylo(1 | id, tree = tree)`, and zero-inflated Poisson `zi ~ spatial(1 | id, coords = coords)`. Broad structured count, bounded, ordinal, shape, inflation, and hurdle paths stay deferred until ordinary family-specific random effects and their intervals are stable. Gaussian structured slope evidence now includes one-slope `mu`, sigma-only, matched `mu+sigma`, bivariate slope-only q=2 `mu1`/`mu2`, and exact shared-label all-four one-slope native point-fit/extractor cells with deterministic same-target fixtures only where documented for phylo, fixed-covariance spatial, A-matrix animal, and K/Q relmat, while sparse large-pedigree or large-matrix claims, multiple structured slopes, broader labelled structured covariance, broad bridge/inference beyond deterministic same-target fixtures, intervals, coverage, REML, AI-REML, and slope correlations remain planned. |
...
| 272 | Random slopes | Structured random slopes | Superseded by Slices 39-82 and the q-series sigma/matched-slope/q2-slope tranche: one-slope univariate Gaussian `phylo()`, fixed-covariance `spatial()`, A-matrix `animal()`, and K/Q `relmat()` `mu`, sigma-only, matched `mu+sigma`, and slope-only bivariate `mu1`/`mu2` q=2 native point-fit/extractor paths are now fitted with endpoint metadata, focused source tests, and deterministic same-target fixtures where documented; the exact shared-label phylo, fixed-covariance spatial, A-matrix animal, and K/Q relmat all-four one-slope cells have native point-fit/extractor evidence plus deterministic same-target fixture evidence. Multiple structured slopes, broader intercept-plus-slope labelled structured covariance, broad bridge/inference beyond deterministic same-target fixtures, slope correlations, structured `rho12`, intervals, coverage, REML/AI-REML, and non-Gaussian structured effects remain planned. |
...
- `docs/design/44-structured-slope-parity-gate.md` records the current
  ...
  non-Gaussian structured effects remain in the failure ledger.
