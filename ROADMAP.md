...
- The R-side labelled covariance block registry now records the implemented
  ...
  double-hierarchical endpoint. The first ordinary q=8 random-slope endpoint
  block is now visible through ordinary extractors, `profile_targets()`,
  `check_drm()`, and Phase 18 smoke/recovery artifacts, but it remains a
  diagnostic surface rather than coverage or power evidence. Dormant q=3
  scaffolds remain invisible to ordinary extractor/profile output. The
  corresponding constant phylogenetic q=4 state
  ...
  location block has an opt-in smoke artifact route; q=8 now has diagnostic
  grid presets and a five-row stress audit confirming `hold_diagnostic`; staged
  starts and interval evidence remain needed before promotion.
...
- Closure boundary: Phase 6c now includes the ordinary grouped q > 2 Gaussian
  ...
  rates of 0.263 and 0.158, two leading-minor optimization errors, and no
  usable Wald intervals. The 2026-06-08 five-row stress audit converged only
  2/5 fits under `se = FALSE`, and the follow-up `se = TRUE` Hessian probe made
  those two formerly converged hard rows nonconverged with ill-conditioned q8
  correlation matrices. Q4 SD-staged and theta-staged starts are now
  source-tested diagnostic options, but the 2026-06-09 sample-size ladder is
  the important q8 result: larger replication improves Hessian and conditioning
  behaviour, while low and baseline rows remain fragile. The paired high-sample
  800/1600 optimizer-budget pilot did not change convergence on the high row.
  Direct q8 endpoint SD profiles can be computed on selected rows, but
  derived-correlation bootstrap intervals remain unpromoted after the first
  two-refit pilot. Q8 coverage, q8 power, optimizer/start and sample-size
  promotion, derived-correlation interval design, and richer q8 variants remain
  planned. Univariate correlated residual-scale slope blocks
...
| Bivariate ordinary covariance | Fitted for matching labelled random intercepts in `mu1`/`mu2`, `sigma1`/`sigma2`, one or more same-response `mu`/`sigma` intercept or slope-only blocks, all-four q=4 intercept blocks, matching slope-only `mu1`/`mu2` and `sigma1`/`sigma2` blocks, matching q=4 and q=6 `mu1`/`mu2` location blocks with smoke artifact routing, and the first q8 all-endpoint ordinary Gaussian block with diagnostic smoke/recovery routing. | Constant q=2 correlation targets, the slope-slope `mu1`/`mu2` target, the scale-slope `sigma1`/`sigma2` target, and same-response q2 `mu`/`sigma` targets are profile-ready; q2 same-response, q2 scale-slope, and q8 rows now have smoke/recovery grid writers; q > 2 location-block SDs and q8 endpoint SDs are direct targets, while q > 2 and q8 correlations are derived-only with explicit unavailable interval status. | Q8 coverage, q8 power, and formal q > 2 simulation recovery remain closed; the 2026-06-06 same-response q2 formal audit is diagnostic only because convergence/positive-Hessian rates were 0.856 and 0.884 and fixed-effect Wald coverage was 0.796-0.850; the 2026-06-07 q8 local audit is also diagnostic only because 38/40 manifests completed, convergence rates were 0.263 and 0.158, two fits had leading-minor optimization errors, and no Wald intervals were usable. The 2026-06-09 q8 usability pilot adds q4-to-q8 SD and theta starts plus a sample-size ladder: high replication improved Hessian and conditioning behaviour, but the paired 800/1600 optimizer-budget pilot did not change high-row convergence and derived-correlation intervals still need follow-up. |
...
| Non-Gaussian families | Fixed-effect likelihoods are fitted; ordinary Poisson and NB2 `mu` random intercepts plus independent numeric slopes are fitted for non-zero-inflated count models; one q=1 structured `mu` intercept from `phylo()`, `spatial()`, `animal()`, or `relmat()` is fitted for ordinary Poisson/NB2; ordinary Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial `mu` random intercepts have Phase 18 artifact lanes and independent numeric `mu` slopes have focused source tests; fixed-effect `skew_normal()` has a univariate `mu`/`sigma`/`nu` first slice; ordinary NB2 has the first log-`sigma` random-intercept gate. | Poisson, NB2, Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial `mu` random-effect SDs appear in `sdpars$mu`, random effects, and direct `profile_targets()` rows; Student-t, zero-truncated NB2, bounded-response, and positive-continuous artifact lanes record fixed-effect Wald rows and direct-SD profile rows for ordinary `(1 | id)` in `mu`; skew-normal source tests cover density, normal limit, weighted objective, CDF-tail-floor behaviour, fixed-effect `nu` intervals, deterministic recovery, Gaussian-limit false-positive behaviour, methods, simulation, and malformed neighbours; `tests/testthat/test-nongaussian-mu-random-slopes.R` records CRAN-safe slope recovery, prediction, extractor, profile-target, and diagnostic checks; NB2 `sigma` random-intercept SDs appear in `sdpars$sigma`, `random_effects$sigma`, and direct `log_sd_sigma` profile-target rows; family-specific fixed-effect summaries, structured count diagnostics, and intervals exist where already implemented. | Zero-inflation, hurdle, ordinal, structured slopes, simultaneous structured count types, labelled count covariance, cross-parameter covariance, NB2 `sigma` slopes or structured effects, correlated Student-t/zero-truncated NB2/positive-continuous/bounded-response random slopes, non-Gaussian `sigma` random effects, Student-t or skew-normal `nu` random effects, exact 0/1 boundary mass, other non-Gaussian scale random effects, skew-t, and shape random effects still need separate implementation evidence before broad simulation. |
...
| 1519-1538 | Skew-normal source map | Done locally and superseded by the fixed-effect first slice: `docs/design/123-phase-18-skew-normal-source-map-slices-1519-1538.md` now records comparator sources, local boundaries, and first implementation tests for the implemented univariate fixed-effect `skew_normal()` route. |
...
| 1669-1718 | Next Team B skew-normal decision gate | Done locally for the fixed-effect first slice: the moment parameterization is accepted, `fitted()`, `sigma()`, and `predict(dpar = "nu")` use the public `mu`/`sigma`/`nu` contract, and source tests cover density, normal limit, sign convention, weighted objective, fixed-effect `nu` intervals, deterministic recovery, Gaussian-limit false-positive behaviour, methods, simulation, and malformed neighbours. Smoke and symmetric false-positive artifact lanes plus a formal recovery design gate now exist; a simple 2026-06-08 `se = TRUE` Hessian pilot converged 8/8 fixed-effect fits with `pdHess = TRUE`, but symmetric and predictor-varying cells still need false-positive and recovery work. Running and auditing the formal recovery grid, external comparators, examples, and runtime grids remain future evidence work. |
...
| 1669-1672 | Skew-normal parameterization decision | Done locally and now implemented: `docs/design/127-phase-18-skew-normal-parameterization-decision-slices-1669-1672.md` chooses the moment contract for the first fitted lane, with public `mu = E[y]`, public `sigma = SD[y]`, `nu` as slant/shape, and internal transform to native `xi`, `omega`, and `alpha`; the constructor and TMB branch now use that contract. |
| 1673-1702 | Skew-normal first-test contract | Done locally and partly implemented: `docs/design/128-phase-18-skew-normal-test-contract-slices-1673-1702.md` records the density normalization, Gaussian normal-limit, sign-orientation, false-positive, and admission gates; density, normal-limit, objective, method, simulation, deterministic recovery, Gaussian-limit false-positive, and malformed-neighbour checks now exist as source tests, and a symmetric false-positive artifact lane now exists; running a formal recovery grid and external comparator programme remains future work. |
| 1629-1630 and 1687-1688 | Tweedie semantic and skew-normal boundary tests | Done locally and superseded for skew-normal: `docs/design/129-phase-18-semantic-boundary-tests-slices-1629-1630-1687-1688.md` records that Tweedie zero-regime comparator cells reassert `fitted()` as unconditional `mu` and response-scale `nu` in `(1, 2)`; skew-normal boundary tests now assert that only the fixed-effect first slice is exposed. |
...
| 1689-1702 | Skew-normal implementation gate | Done locally for the first slice: `docs/design/132-phase-18-skew-normal-implementation-gate-slices-1689-1702.md` now records the admitted fixed-effect boundary, the constructor/TMB/method/documentation source path, and the fixed-effect `nu` interval-status source test; deterministic recovery, Gaussian-limit false-positive source tests, smoke artifacts, false-positive artifacts, and the formal recovery design gate now exist, while running and auditing formal recovery, external comparators, examples, and runtime grids remain future evidence work. |
| 1703 | Skew-normal density contract fixture | Done locally and connected to the first slice: `tests/testthat/helper-skew-normal-density.R`, `tests/testthat/test-skew-normal-density-contract.R`, and `tests/testthat/test-skew-normal-location-scale.R` check the public-moment to native-density transform, integration to one, the `nu = 0` Gaussian limit, the third-moment sign orientation, objective equality, deterministic recovery, Gaussian-limit false-positive behaviour, methods, simulation, and malformed-neighbour boundaries for the implemented fixed-effect route. |
...
| 1840 | Bivariate q8 endpoint diagnostic artifact gate | Done locally and updated by the artifact-lane slice: the structured workflow registry now has `ready_grid` `bivariate_gaussian_q8_endpoint` and `bivariate_gaussian_q8_endpoint_recovery` rows with opt-in Actions tasks; `phase18_biv_gaussian_q8_endpoint_precode_gate()` names the eight all-endpoint labels and records the 28 implied correlations; and `phase18_biv_gaussian_q8_endpoint_diagnostic_conditions()` plus the diagnostic grid writer provide replication, endpoint-SD ratio, residual `rho12`, and latent-correlation preset cells for follow-up diagnostics. The 2026-06-08 five-row stress audit completed 5/5 manifests and converged 2/5 fits under `se = FALSE`, so q8 coverage and power remain held until deliberately sized evidence is accepted. |
| 1841-1845 | Skew-normal and q8 diagnostic evidence pilots | Done locally as diagnostic evidence: the fixed-effect `skew_normal()` smoke lane, symmetric false-positive lane, and formal-recovery design gate now have artifact routes; the 2026-06-08 skew-normal pilot ran 3 slant cells x 3 replicates with 9/9 convergence under `se = FALSE`, so it computed no Hessian evidence and showed weak `nu` recovery, while the symmetric false-positive cell converged with `pdHess = FALSE` and fitted `|nu| = 0.981` at a 0.5 threshold. These runs keep skew-normal at first-slice/pilot status, not formal-recovery-ready status. |
| 1846-1848 | Q8 Hessian rescue and skew-normal Hessian pilot | Done locally as evidence-gate documentation: `docs/design/163-phase-18-q8-hessian-start-rescue.md` records that the q8 `se = TRUE` probe made both formerly converged stress rows nonconverged and originally required an explicit staged-start hook; `docs/design/164-phase-18-skew-normal-hessian-comparator-pilot.md` records that simple fixed-effect skew-normal fits can produce positive-Hessian evidence, with 8/8 converged `pdHess = TRUE` fits, while symmetric false positives, `nu ~ w` under-recovery, and unavailable local `sn`/`gamlss` comparators remain open. Slice 1853 now implements the private q4-to-q8 staged-start mapper and first paired pilot, but broader hard-row staged-start/Hessian auditing remains open. |
| 1849-1850 | Q8 start-hook preflight and skew-normal comparator scale map | Done locally as pre-implementation evidence: `docs/design/165-phase-18-q8-start-hook-preflight.md` fixes the internal q8 hook point after `add_covariance_probe_parameter(spec)` and before `TMB::MakeADFun()`, keeps public start-like control names reserved, and limits staged starts to validated named `spec$start` overrides; `docs/design/166-phase-18-skew-normal-comparator-scale-map.md` and source tests map public skew-normal `mu`, `sigma`, and `nu` to native Azzalini `xi`, `omega`, and `alpha`, separate `sn`/`RTMBdist` native density checks from `brms`/`glmmTMB` moment-scale fitted-model checks, and mark `gamlss.dist::SN2` as a different two-piece family rather than a same-density comparator. |
| 1851-1852 | Q8 private start override and skew-normal `glmmTMB` comparator smoke | Done locally as cleanup evidence: `drm_apply_start_override()` is now a private no-op hook in the ordinary `drmTMB()` path at the planned pre-`MakeADFun()` point, with source tests for no-op preservation, unknown/duplicate/wrong-length/non-finite overrides, mapped-slot preservation, and provenance/applied-count metadata; `docs/dev-log/simulation-artifacts/2026-06-08-skew-normal-glmmtmb-comparator-smoke/` records one simple `sigma ~ 1`, `nu ~ 1` comparator smoke where `glmmTMB::skewnormal()` matches `drmTMB` only when started with nonzero `psi`, while the default shape start stays at the symmetric boundary. Q8 still needs paired hard-row staged-start diagnostics; skew-normal still needs formal comparator and false-positive/recovery grids. |
| 1853 | Q8 q4-to-q8 staged-start mapper | Done locally as start-rescue evidence: `drm_fit_spec()` now gives internal tooling the same MakeADFun/optimizer tail as `drmTMB()`, and `drm_qgt2_staged_start_override()` maps shared fixed effects by column name plus q>2 endpoint SDs by covariance-member key. The follow-up theta slice adds optional pair-key `theta_re_cov` copying, so the ordinary default remains neutral but `q4_theta_staged` is available for diagnostics. The pilot artifact at `docs/dev-log/simulation-artifacts/2026-06-08-q8-staged-start-pilot/` shows cold q8 convergence code 1 versus q4-staged q8 convergence code 0 on `q8_diag_001` with seed `20260641`; q8 coverage, power, and interval promotion remain closed until deliberately sized paired audits pass. |
| 1854 | Q8 sample-size usability, optimizer-budget, and inference boundary | Done locally as diagnostic evidence: `docs/dev-log/simulation-artifacts/2026-06-09-q8-usability-pilot/` compares cold, q4 SD-staged, and q4 theta-staged starts on five hard rows plus a 24 x 6, 48 x 10, and 96 x 12 sample-size ladder. High replication improved q8 Hessian and conditioning behaviour, including `pdHess = TRUE` for cold and SD-staged `se = TRUE` fits, but those fits still returned optimizer code 1. `docs/dev-log/simulation-artifacts/2026-06-09-q8-optimizer-budget-pilot/` reran the high row with 800 and 1600 evaluations/iterations; doubling the budget did not change convergence, `pdHess`, or the printed q8 correlation diagnostics. `docs/dev-log/simulation-artifacts/2026-06-09-q8-usability-inference-pilot/` records one direct endpoint-SD profile interval and no derived-correlation bootstrap interval rows from two requested refits. Q8 status is sample-size dependent, not binary; coverage and power remain closed. |
