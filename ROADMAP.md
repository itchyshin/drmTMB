...
| 3 | Lognormal, Gamma, Student-t, and bounded-response `mu` | Lognormal, Gamma, Student-t, beta, and beta-binomial ordinary `mu` random intercepts are fitted as narrow first slices. Beta/beta-binomial and lognormal/Gamma ordinary `mu` random intercepts now also have Phase 18 artifact lanes; Student-t remains source-tested only. Student-t, positive-continuous, bounded-response random slopes, and richer mixed-model surfaces remain later because scale, tail, strict-boundary, denominator, and weak-SD diagnostics need their own grids. |
...
- Slices 1279-1378 add the core-family completion map, fixed-effect
  ...
  lognormal/Gamma. These lanes add DGP, summariser, smoke, repeatable
  grid-output, first-wave summary, manual Actions-dispatch, and focused-test
  evidence for already fitted one-response families; they do not open random
  slopes, structured, mixed-response, skew-normal, Tweedie, or generalized
  Gamma likelihoods.
