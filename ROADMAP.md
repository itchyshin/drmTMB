...
| Non-Gaussian families | Fixed-effect likelihoods are fitted; ordinary Poisson and NB2 `mu` random intercepts plus independent numeric slopes are fitted for non-zero-inflated count models; ordinary zero-truncated NB2, lognormal, and Gamma have first `mu` random-intercept source-test slices; ordinary NB2 has the first log-`sigma` random-intercept gate. | Poisson, NB2, zero-truncated NB2, lognormal, and Gamma `mu` random-effect SDs appear in `sdpars$mu`, random effects, and direct `profile_targets()` rows; NB2 `sigma` random-intercept SDs appear in `sdpars$sigma`, `random_effects$sigma`, and direct `log_sd_sigma` profile-target rows; family-specific fixed-effect summaries and intervals exist where already implemented. | Zero-inflation, hurdle, ordinal, structured, cross-parameter covariance, NB2 `sigma` slopes or structured effects, positive-continuous random slopes or `sigma` random effects, other non-Gaussian scale random effects, and shape random effects still need separate implementation evidence before broad simulation. |

| Slice | Lane | Target Before Phase 18 |
| --- | --- | --- |
| 190 | Non-Gaussian `mu` random effects | Done and extended: first candidates were ordinary `mu` random intercepts for Poisson and NB2-style count likelihoods. Ordinary zero-truncated NB2, lognormal, and Gamma `mu` random intercepts now have focused source-test slices, while Student-t, beta, beta-binomial, ordinal, zero-inflation, hurdle, shape, and structured non-Gaussian paths retain explicit unsupported messages. |
...
| 2 | NB2 and zero-truncated NB2 `mu` | NB2 ordinary `mu` random intercepts, independent numeric slopes, and ordinary log-`sigma` random intercepts are now fitted for non-zero-inflated models. Zero-truncated NB2 ordinary `mu` random intercepts are fitted as a narrow source-test slice. Zero-inflated NB2 random effects, zero-truncated random slopes, NB2 `sigma` slopes, joint `mu`/`sigma`, and structured `sigma` effects remain planned. |
| 3 | Lognormal, Gamma, and Student-t `mu` | Lognormal and Gamma ordinary `mu` random intercepts are fitted as narrow source-test slices. Student-t and richer positive-continuous mixed-model surfaces remain later because scale/tail interaction and boundary diagnostics need their own grids. |
...
| 307 | Inflation and hurdle random-effect decision gate | Done locally as a no-fit decision: `zi`, `hu`, future `zoi`, and future `coi` stay fixed-effect-only or planned for now; random effects wait for clear use cases, recovery, diagnostics, prediction semantics, and interval-status rules. |
...
| 581-590 | NB2 q1 formal shard execution | In progress: dispatch the 16 `nbinom2_phylo_q1_formal` shards with `n_reps = 500`, `cores = 10`, `backend = "multicore"`, and `profile_parameters = "log_sd_phylo"`; promotion remains blocked until all artifacts are downloaded, merged, and audited together. |
...
| 1319-1328 | Count first-wave closure | Done locally as Slice C: `docs/design/113-phase-18-count-first-wave-closure-slices-1319-1328.md` inventories the paired Poisson/NB2 `mu`, NB2 log-`sigma`, Poisson q1 phylo, and NB2 q1 phylo evidence lanes; keeps NB2 q1 at `hold_smoke_only` until the 500-replicate shards are audited; and records Slice D choices without adding COM-Poisson, Tweedie, zero-one beta, skew-normal, or new random-effect syntax. |
...
| 307 | Documentation | Inflation and hurdle random-effect gate | Done locally as a no-fit decision: `zi`, `hu`, future `zoi`, and future `coi` stay fixed-effect-only or planned until use cases and validation justify a new latent layer. |
