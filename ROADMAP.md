...
| 3 | Lognormal, Gamma, and Student-t `mu` | Lognormal, Gamma, and Student-t ordinary `mu` random intercepts are fitted as narrow source-test slices. Student-t random slopes plus richer positive-continuous mixed-model surfaces remain later because scale/tail interaction and boundary diagnostics need their own grids. |
