...
| 1329-1338 | Zero-one bounded-response design gate | Done locally as Slice D3: `docs/design/114-phase-18-zero-one-bounded-response-design-gate-slice-d3.md` records fixed-effect zero-one beta as the next bounded-response likelihood candidate, separates it from strict `beta()` and denominator-aware `beta_binomial()`, and keeps `zoi`/`coi` formulas, zero-one random effects, bounded-response random slopes, structured bounded responses, Tweedie, skew-normal, COM-Poisson, and generalized Poisson out of the fitted surface. |
