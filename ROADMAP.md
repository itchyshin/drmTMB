...
- Closure boundary: Phase 6c now includes the ordinary grouped q > 2 Gaussian
  ...
  smoke/recovery diagnostic artifacts, while q8 coverage, q8 power, and richer
  q8 variants remain planned. Univariate correlated residual-scale slope blocks
...
| 1840 | Bivariate q8 endpoint diagnostic artifact gate | Done locally and updated by the artifact-lane slice: the structured workflow registry now has `ready_grid` `bivariate_gaussian_q8_endpoint` and `bivariate_gaussian_q8_endpoint_recovery` rows with opt-in Actions tasks, and `phase18_biv_gaussian_q8_endpoint_precode_gate()` names the eight all-endpoint labels, records the 28 implied correlations, and keeps q8 coverage and power held until deliberately sized evidence is accepted. |
