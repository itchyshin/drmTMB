...
The native-REML admission for the matching labelled bivariate spatial q2
location-intercept cell is narrower than the broader ML row. It requires
complete response pairs, unit weights, intercept-only `sigma1`, `sigma2`, and
`rho12`, no known `meta_V()` covariance, and no additional ordinary random
