...
- **Overdispersed, zero-heavy, truncated, or hurdle counts.** Use
  ...
  non-Gaussian random-effect slices. Ordinary Poisson also has the first q=1
  phylogenetic `mu` intercept slice,
  `bf(count ~ x + phylo(1 | species, tree = tree))`, when the structured
  effect belongs on the log-mean scale. Correlated count slope blocks,
  zero-inflation random effects, NB2 `sigma` random effects, and non-Gaussian
...
- **Known sampling variance or covariance.** Use Gaussian meta-analysis with
  `meta_V(V = V)`; `meta_known_V(V = V)` remains a compatibility alias. Read
  [Mean effects and residual heterogeneity](https://itchyshin.github.io/drmTMB/articles/meta-analysis.html).
...
| --- | --- | --- | --- |
| Fixed-effect one-response families | Stable for Gaussian, Student-t, lognormal, Gamma, beta, beta-binomial, Poisson, NB2, truncated NB2, hurdle NB2, zero-inflated Poisson, zero-inflated NB2, and cumulative-logit ordinal location; ordinary Poisson and NB2 `mu` random intercepts and independent numeric slopes are the first non-Gaussian random-effect slices, and ordinary Poisson has a q=1 phylogenetic `mu` intercept first slice | Wald fixed-effect intervals by default; explicit direct profile targets are listed by `profile_targets()`; Poisson and NB2 ordinary random-effect SDs are direct `log_sd_mu` profile targets; the first Poisson phylogenetic SD is a direct `log_sd_phylo` profile target | Random effects are otherwise mostly Gaussian-only; non-Gaussian `sigma` and shape random effects, correlated count slopes, zero-inflated count random effects, NB2/spatial/animal/`relmat()` structured count effects, ordinal scale, and richer bounded-response families including zero-one inflation remain planned |
...
and the first ordinary Poisson q=1 phylogenetic `mu` intercept are fitted, while
most other non-Gaussian random-effect and structured-dependence combinations
remain planned after fixed-effect likelihoods, diagnostics, and simulations are
stable.
