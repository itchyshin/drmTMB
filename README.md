...
- **Continuous proportions with structural exact 0 or 1 values.** Use
  ...
  `zoi ~ x + (0 + x | id)`, or the exact `coi ~ 1 + (1 | id)` random
  intercept; none is profile-ready. The `coi` intercept has population-level
  point-recovery evidence at `M = 64` with 50 observations per group, while
  groups with few observed zeroes or ones can have weak conditional modes.
  Inspect atom counts before interpreting individual modes. `coi` slopes,
