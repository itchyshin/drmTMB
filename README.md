...
| Missing data | Bounded `miss_control()` preview: complete-case dropping remains the default; `response = "include"` is G3 recovery-verified for 14 routes, including beta-binomial whole-row masks and ordered-factor cumulative logit; one `mi()` missing predictor at a time is modelled for the implemented predictor-family catalogue in Gaussian responses, and Poisson/binomial/NB2/beta responses accept one binary `mi()` predictor | The 14 response-mask routes have direct encoded-sentinel, observed-row parity, row/extractor, malformed-input, and fixed-seed 25% MCAR recovery tests. Beta-binomial includes ordinary random-intercept recovery; cumulative logit is fixed-effect ordered-factor only. Missing-predictor routes retain their focused likelihood tests and `imputed()` summaries | This is not a general missing-data framework. The other four fitted response routes remain scheduled through MR-T5–MR-T6. Multiple missing predictors, response plus `mi()`, broader structured-route masking claims, EM/profile engines, REML for explicit missing-data routes, simulation-based imputation summaries, response imputation, measurement-error models, and pigauto interoperability remain planned |
