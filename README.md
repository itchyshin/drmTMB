...
| Known sampling covariance | Stable for Gaussian `meta_V(V = V)` with diagonal, dense, and row-paired bivariate known covariance when diagnostics are clean; deprecated `meta_known_V(V = V)` remains supported only as a compatibility alias; `meta_V(V = vi)` plus predictor-dependent `sigma` is currently diagnostic for inference because of issue #417 | `check_drm()` reports dense full `V` as a note with dimension, density, size, rank, and conditioning; fixed effects and response-scale residual summaries use the usual interval routes when the Hessian is positive-definite | Dense covariance is small-to-moderate unless sparse or block-sparse evidence is added; full dense known `V` with non-unit likelihood weights is rejected; issue #417 reports `pdHess = FALSE` for `meta_V(V = vi)` plus `sigma ~ moderator`, so Hessian-based SEs and Wald intervals from that combination should be treated as diagnostic until resolved |
