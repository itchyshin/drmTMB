...
- **Overdispersed, zero-heavy, truncated, or hurdle counts.** Use
  ...
  phylogenetic `mu` intercept slices, such as
  `bf(count ~ x + phylo(1 | species, tree = tree), sigma ~ z)` for NB2, when
  the structured effect belongs on the log-mean scale. Ordinary NB2 also fits
  the first grouped overdispersion slice, `bf(count ~ x, sigma ~ z + (1 | id))`.
  Correlated count slope blocks, zero-inflation random effects, NB2 `sigma`
  slopes or structured `sigma` effects, and non-Gaussian
  spatial/animal/`relmat()` routes remain planned.
  Read
  [Choosing response families](https://itchyshin.github.io/drmTMB/articles/distribution-families.html).
...
| --- | --- | --- | --- |
| One-response families | Stable for Gaussian, Student-t, lognormal, Gamma, beta, zero-one beta, beta-binomial, Poisson, NB2, truncated NB2, hurdle NB2, zero-inflated Poisson, zero-inflated NB2, and cumulative-logit ordinal location; ordinary Poisson and NB2 `mu` random intercepts and independent numeric slopes are the first count random-effect slices; ordinary Student-t, lognormal, Gamma, beta, and beta-binomial `mu` random intercepts now have Phase 18 artifact lanes, while ordinary zero-truncated NB2 `mu` random intercepts remain source-tested; ordinary NB2 now has the first log-`sigma` random-intercept slice; and ordinary Poisson/NB2 now have q=1 phylogenetic `mu` intercept first slices | Wald fixed-effect intervals by default; explicit direct profile targets are listed by `profile_targets()`; ordinary Poisson, NB2, Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial `mu` random-effect SDs are direct `log_sd_mu` profile targets; the bounded-response, positive-continuous, and Student-t artifact lanes record fixed-effect Wald rows and direct-SD profile rows for ordinary `(1 | id)` in `mu`; NB2 ordinary `sigma` random-intercept SDs are direct `log_sd_sigma` targets; Poisson/NB2 phylogenetic SDs are direct `log_sd_phylo` profile targets | Random effects are otherwise mostly Gaussian-only; non-Gaussian `sigma` random effects outside the ordinary NB2 intercept gate, bounded-response, positive-continuous, and Student-t random slopes, Student-t `nu` random effects, correlated count slopes, zero-inflated count random effects, phylogenetic count slopes, spatial/animal/`relmat()` count effects, ordinal scale, zero-one beta random effects, and bivariate bounded-response families remain planned |
...
intercepts are fitted. The beta/beta-binomial and lognormal/Gamma ordinary
`mu` random intercepts and the Student-t ordinary `mu` random intercept now
have small Phase 18 artifact lanes, not broad bounded-response,
positive-continuous, or Student-t random-effect claims.
