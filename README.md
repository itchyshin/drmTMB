...
- **Continuous response, changing mean or family-specific variation.** Use
  ...
  independent numeric slopes. Their exact independent-slope ledger cells are
  `inference_ready_with_caveats` for true slope SD 0.50 and M>=16; this is not a
  `supported` or all-design claim. Predictor-dependent Tweedie
...
| Bivariate Gaussian residual `rho12` | Stable for fixed-effect `mu1`, `mu2`, `sigma1`, `sigma2`, and predictor-dependent residual `rho12` | `rho12()` extracts response-scale residual correlations. A profile interval is an admitted reporting claim only for constant `rho12 ~ 1`; a regression `rho12 ~ x` gives a point curve without a released interval claim | Residual `rho12` is not a group-level, phylogenetic, or spatial correlation |
...
and independent numeric-slope evidence: Student-t, skew-normal, lognormal, Gamma, Tweedie,
beta, zero-one beta, beta-binomial, binomial, Poisson, NB2, zero-truncated NB2,
and cumulative logit. Exact ledger cells for binomial, cumulative-logit,
skew-normal, Tweedie, and zero-one-beta slopes have stronger
`inference_ready_with_caveats` profile-coverage evidence under their recorded
designs. Beyond that universal ordinary gate, the mixed surface is deliberately
small: ordinary
...
`M={16,32,64}`), and the Gamma ordinary log-`sigma` intercept domain (true
SD 0.40, `n_each=12`, and `M >= 32`; `M=16` is borderline), have
coverage-backed `inference_ready_with_caveats` evidence. Gamma `sigma` slopes,
labels, combined `mu`/`sigma` effects, REML, and `supported` remain outside that
claim. Most other
