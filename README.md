...
| Animal and lower-level relatedness structured effects | Gaussian `mu` intercepts are fitted for `animal(1 | id, pedigree = ...)`, `animal(1 | id, A/Ainv = ...)`, and `relmat(1 | id, K/Q = ...)`; matching labelled `mu1`/`mu2` terms now fit the first q=2 bivariate location covariance | `sdpars$mu`, `corpars$animal` / `corpars$relmat`, `ranef("animal_mu")`, `ranef("relmat_mu")`, `corpairs()`, `summary()$covariance`, direct `profile_targets()`, and `check_drm()` expose the fitted structured fields | Large-pedigree sparse precision construction, structured slopes, `sigma` relatedness models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and generic direct-SD grammar remain planned |
