...
| Planned or reserved | Public grammar or design notes may exist, but `drmTMB()` should reject it or treat it as design-only until likelihood, tests, docs, and after-task evidence land. |
...
| Missing data | Bounded `miss_control()` preview: complete-case dropping remains the default; `response = "include"` is G3 recovery-verified for all fitted response routes, including beta-binomial whole-row masks, ordered-factor cumulative logit, non-hurdle truncated NB2, and the fixed-effect count mixtures; one `mi()` missing predictor at a time is modelled for the implemented predictor-family catalogue in Gaussian responses, while the exact two-continuous-predictor `impute_joint(cbind(x1, x2) ~ z)` route is available for Gaussian and as a Poisson proof route | Every response-mask route has direct sentinel, observed-row parity, row/extractor, malformed-input, and fixed-seed 25% MCAR recovery evidence. The joint Gaussian route has an independent observed-data likelihood oracle and a fixed-seed recovery smoke; the Poisson route has independent focused integration tests. Mixtures additionally separate missing zeros from missing positive counts. Beta-binomial and truncated NB2 include ordinary random-intercept recovery; cumulative logit and the count mixtures are fixed-effect masking slices | This is not a general missing-data framework. The joint route has exactly two bare continuous predictors with fixed-effect imputation terms only; response plus `mi()`, additional or mixed-type missing predictors, broader random/structured-route masking claims, EM/profile engines, REML for explicit missing-data routes, simulation-based imputation summaries, response imputation, measurement-error models, and pigauto interoperability remain planned |
...
`relmat()` cell additionally requires the same label, group ordering, and
supplied covariance matrix `K` in both formulas. It has
`point_fit_recovery` evidence only; supplied precision `Q`, `animal()`, slopes,
scale-side blocks, q4+, intervals, and coverage remain unsupported.
