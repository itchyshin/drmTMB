...
drm_julia_bridge_default_dpar_labels <- function(labels, formula, family_type) {
  ...
  # Matched by the `biv_` PREFIX, not by an explicit family list. A bivariate
  # family's DRM.jl blocks are sigma1/sigma2/rho12, never a scalar `sigma`, so
  # the univariate branch below is not merely unhelpful for one -- it is
  # actively wrong: it would send a `sigma` label DRM.jl has no block for, and
  # the echo aborts `coef_labels supplies names for unknown dpar "sigma"; the
  # model has dpars: mu1, mu2, sigma1, sigma2, rho12`. Measured live at pin
  # 430ef64cc, 2026-09-05, with biv_lognormal()'s registry row in place and
  # this line still matching the literal "biv_gaussian": BOTH the short form
  # bf(mu1 = y1 ~ x, mu2 = y2 ~ x) AND the full form that spells out
  # sigma1/sigma2/rho12 aborted, because the defaulter adds `sigma`
  # regardless of what the user wrote. The
  # prefix is the same convention `drm_julia_fe_only_fence_families()` already
  # uses for the bivariate exemption, so a later bivariate admission needs no
  # edit here. A bivariate family with an EXTRA dpar (biv_student's `nu`) still
  # needs its own default added by its own row.
