...
,
    ...
    next_action = c(
      ...
      "Keep the include==drop equality tests green on both sides (DRM.jl default suite + tests/testthat/test-julia-missing.R). Non-Gaussian observed-data likelihoods stay gated until audited; missing predictors stay predictor='fail'. G3 ATTEMPTED 2026-09-05 (leaf A8) and BLOCKED by two new findings on this route: the julia fit's own opt$convergence flag is FALSE on this missing-response fixture (despite a profile CI that tracks engine=\"tmb\" to 5e-6/7e-6), and engine=\"julia\" bootstrap fails ALL 99 replicates outright. Investigate both (docs/dev-log/evidence/julia-r-parity/p2-g3/g3-qualification-receipt.md) before attempting G3 again here. Tracked as DRM.jl #646. RE-RUN G3 HERE: the two findings that blocked it were measured at the dead pin 430ef64cc and NEITHER reproduces at DRM.jl aee371cc (is_converged TRUE / opt$convergence 0; bootstrap 25/25 successful, 0 failed). Non-Gaussian observed-response masks stay gated (inst/extdata/julia-gates.tsv:6, stated reason 'not audited for non-Gaussian masks' -- an audit, not a decision); missing predictors stay predictor='fail' by D-181 #1 / D-209 sec.3. Receipt banked upstream in DRM.jl PR #734.",
