...
drm_julia_capability_comparison <- function() {
  data.frame(
    capability_id = c(
      ...
      # A4-INTEGRATION (2026-09-05): comparison rows for the two A4 admissions
      # still open at the time this leaf was built (#1174 cumulative_logit,
      # #1176 skew_normal) -- their own OWNS carved out this data.frame.
      # beta_binomial's fe_beta_binomial row arrives with #1172 (already
      # patched there). A4-INTEGRATION follow-up (2026-09-05): tweedie,
      # zero_one_beta, and truncated_nbinom2 (#1169/#1171/#1173) merged
      # with NO comparison row of their own -- their fe_tweedie,
      # fe_zero_one_beta, and fe_truncated_nbinom2 rows are added below.
      "fe_cumulative_logit",
      "fe_skew_normal",
      "fe_tweedie",
      "fe_zero_one_beta",
    ...
    r_bridge_status = c(
      ...
      "experimental",
      "experimental",
      # A4-INTEGRATION follow-up (2026-09-05): comparison rows for the
      # three A4 admissions that landed with NO row of their own -- each
      # PR (#1169 tweedie, #1171 zero_one_beta, #1173 truncated_nbinom2)
      # abandoned this data.frame's row gate. Same wave-1 partial evidence
      # bar as fe_cumulative_logit/fe_skew_normal above, but r_bridge_status
      # is held at experimental for the identical reason those two rows
      # were: R_BRIDGE_STATUSES (tools/validate-mission-control.py) does not
      # yet accept "partial" on THIS branch or on clean origin/main, and
      # setting it would add three brand-new "invalid r_bridge_status"
      # validator errors that do not exist on main today. Promote all five
      # A4 rows to partial together once #1172 lands the vocabulary fix.
      "experimental",
      "experimental",
      "experimental"
    ),
    ...
    claim_status = c(
      ...
      "partial",
      "experimental",
      # A4-INTEGRATION: same wave-1 partial bar as the other A4 rows -- point+SE
      # receipt on one committed fixture, no interval evidence.
      "partial",
      "partial",
      "partial",
      "partial",
...
,
      ...
      "Fix predict() for this family (drm_julia_predict_fixed_eta() needs the SAME intercept-dropping projection as the payload's mu design) before promoting beyond partial; keep the fe_cumulative_logit receipt re-runnable.",
      ...
      "Keep the fe_tweedie receipt re-runnable (DRM.jl docs/dev-log/evidence/parity-fixtures.tsv capability_id fe_tweedie, parity-se.tsv cell_id se_tweedie_locscale_power). Promote r_bridge_status to partial together with fe_cumulative_logit, fe_skew_normal, fe_zero_one_beta, and fe_truncated_nbinom2 once #1172 lands the R_BRIDGE_STATUSES vocabulary fix, and re-run the validator. Do not promote claim_status beyond partial without a bridge-side inference (G3) receipt.",
      "Keep the fe_zero_one_beta receipt re-runnable (DRM.jl docs/dev-log/evidence/parity-fixtures.tsv capability_id zero_one_beta, parity-se.tsv cell_id se_zero_one_beta). Promote r_bridge_status to partial together with the other four A4 rows once #1172 lands the R_BRIDGE_STATUSES vocabulary fix. Do not promote claim_status beyond partial without a bridge-side inference (G3) receipt.",
      "Keep the fe_truncated_nbinom2 receipt re-runnable (DRM.jl docs/dev-log/evidence/parity-fixtures.tsv capability_id truncated_nbinom2, parity-se.tsv cell_id se_truncated_nbinom2). Promote r_bridge_status to partial together with the other four A4 rows once #1172 lands the R_BRIDGE_STATUSES vocabulary fix; resolve the truncated_nbinom2/hu spelling asymmetry (also named on the hurdle_nbinom2 row) before any promotion beyond partial claim_status."
    ),
    issue = c(
      rep("drmTMB#544", 10),
      "drmTMB#499",
