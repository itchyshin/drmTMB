...
drm_julia_capability_comparison <- function() {
  data.frame(
    ...
    r_bridge_status = c(
      ...
      # completed. biv_gaussian_residual (idx2) was found to have NO
      # profile/bootstrap-ready target on this route at all (a structural
      # bridge gap, not fixed by this leaf); that gap was closed by leaf A8b
      # (DRM.jl PR #647) -- see idx2's own comment and claim_boundary/
      # next_action below. gaussian_response_mask (idx4) stays partial:
      ...
      "supported",
      # biv_gaussian_residual PROMOTED partial -> supported 2026-09-05 (leaf
      # A8b). design/192 defines `partial` on this axis as "bridge-side
      # inference (G3) still unqualified"; G3 is now qualified for THIS route,
      # so that definition no longer describes it. Receipt:
