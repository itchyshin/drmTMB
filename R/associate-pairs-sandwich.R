...
drm_pair_staged_eta_sandwich <- function(
  ...
  if (!inherits(association_fit, "drm_pair_association") ||
    ...
    cli::cli_abort("The staged sandwich is defined only for Bernoulli x ordinary-NB2 association fits.")
  }
  ...
  if (!setequal(types, c("binomial", "nbinom2"))) {
    cli::cli_abort("The staged sandwich needs the original Bernoulli and ordinary-NB2 margin fits.")
  }
  ...
  if (length(beta_b) != ncol(x_b) || length(beta_n) != ncol(x_n) ||
      length(gamma_n) != ncol(z_n) || length(alpha) != ncol(x_a)) {
  ...
  if (!isTRUE(all.equal(p, components$binary_p, tolerance = 1e-8)) ||
  ...
  marginal <- drm_pair_sandwich_margin_blocks(
    binary_y = components$binary_y, nbinom2_y = components$nbinom2_y,
    p = p, mu = mu, sigma = sigma, x_b = x_b, x_n = x_n, z_n = z_n
  )
  association <- drm_pair_sandwich_association_blocks(
    binary_y = components$binary_y, nbinom2_y = components$nbinom2_y,
    lambda_b = lambda_b, xi_n = xi_n, tau_n = tau_n, a = a,
    x_b = x_b, x_n = x_n, z_n = z_n, x_a = x_a, control = control
  )
  if (!identical(association$status, "ok")) return(association)

  p_b <- ncol(x_b); p_n <- ncol(x_n); p_s <- ncol(z_n); p_a <- ncol(x_a)
  ...
  scores <- cbind(marginal$score_b, marginal$score_n, marginal$score_s,
    association$score_a)
  colnames(scores) <- c(colnames(x_b), paste0("nbinom2_mu:", colnames(x_n)),
    paste0("nbinom2_sigma:", colnames(z_n)), paste0("association:", colnames(x_a)))
  ...
  if (any(!is.finite(bread)) || any(!is.finite(meat)) ||
  ...
  if (inherits(inverse_bread, "error")) return(drm_pair_sandwich_unavailable("bread_solve_failure"))
  ...
  list(
    status = "ok", covariance = covariance, alpha_covariance = alpha_covariance,
    alpha_se = sqrt(diag(alpha_covariance)), eta = 0.999999 * tanh(a),
    eta_se = sqrt(eta_variance), scores = scores, bread = bread, meat = meat,
...
drm_pair_sandwich_control <- function() {
  ...
  list(step = 1e-2, derivative_relative_tolerance = 1e-3,
    derivative_absolute_tolerance = 1e-6, rcond_min = 1e-10,
...
drm_pair_sandwich_margin_blocks <- function(binary_y, nbinom2_y, p, mu, sigma,
                                             x_b, x_n, z_n) {
  r <- sigma^(-2)
  score_b_scalar <- binary_y - p
  score_n_scalar <- r * (nbinom2_y - mu) / (r + mu)
  d <- digamma(nbinom2_y + r) - digamma(r) + log(r) + 1 - log(r + mu) -
    (r + nbinom2_y) / (r + mu)
  score_s_scalar <- -2 * r * d
  d_dr <- trigamma(nbinom2_y + r) - trigamma(r) + 1 / r - 1 / (r + mu) +
    (nbinom2_y - mu) / (r + mu)^2
...
drm_pair_sandwich_association_blocks <- function(binary_y, nbinom2_y,
                                                  lambda_b, xi_n, tau_n, a,
                                                  x_b, x_n, z_n, x_a, control) {
  n <- length(a)
  p_b <- ncol(x_b); p_n <- ncol(x_n); p_s <- ncol(z_n); p_a <- ncol(x_a)
  score_a <- matrix(NA_real_, n, p_a)
  bread_ab <- matrix(0, p_a, p_b); bread_an <- matrix(0, p_a, p_n)
  bread_as <- matrix(0, p_a, p_s); bread_aa <- matrix(0, p_a, p_a)
  diagnostics <- vector("list", n)
  for (i in seq_len(n)) {
    fn <- function(q) drm_pair_sandwich_row_logprob(
      binary_y[[i]], nbinom2_y[[i]], q[[2L]], q[[3L]], q[[4L]], q[[1L]]
    )
    ...
    if (!identical(derivative$status, "ok")) {
      return(drm_pair_sandwich_unavailable(paste0("association_", derivative$reason)))
    }
    gradient <- derivative$gradient; hessian <- derivative$hessian
  ...
  list(status = "ok", score_a = score_a, bread_ab = bread_ab, bread_an = bread_an,
    bread_as = bread_as, bread_aa = bread_aa, derivative_diagnostics = diagnostics)
}

drm_pair_sandwich_row_logprob <- function(binary_y, nbinom2_y, lambda_b, xi_n,
                                          tau_n, a) {
  result <- drm_pair_bernoulli_nbinom2_rectangle_probability(
    binary_y = binary_y, binary_p = stats::plogis(lambda_b), nbinom2_y = nbinom2_y,
    nbinom2_mu = exp(xi_n), nbinom2_sigma = exp(tau_n),
    eta = 0.999999 * tanh(a)
  )
  if (!identical(result$status, "ok")) return(NA_real_)
...
drm_pair_sandwich_stable_derivatives <- function(fn, q, control) {
  ...
  difference <- max(abs(c(first$gradient - second$gradient,
    first$hessian - second$hessian)))
  scale <- max(1, abs(c(first$gradient, first$hessian, second$gradient, second$hessian)))
  if (difference > control$derivative_absolute_tolerance +
      control$derivative_relative_tolerance * scale) {
    return(list(status = "unavailable", reason = "step_unstable"))
  }
  list(status = "ok", gradient = second$gradient, hessian = second$hessian,
...
drm_pair_sandwich_derivatives <- function(fn, q, h) {
  ...
  for (j in seq_len(p)) {
    e <- rep(0, p); e[[j]] <- h
    gradient[[j]] <- (-fn(q + 2 * e) + 8 * fn(q + e) - 8 * fn(q - e) + fn(q - 2 * e)) / (12 * h)
  }
...
drm_pair_sandwich_gradient <- function(fn, q, h) {
  ...
  for (j in seq_len(p)) {
    e <- rep(0, p); e[[j]] <- h
    out[[j]] <- (-fn(q + 2 * e) + 8 * fn(q + e) - 8 * fn(q - e) + fn(q - 2 * e)) / (12 * h)
  }
  out
}
