...
No platform result is implied by these receipts. R-release, R-devel, and R-oldrelease remain
independently pending until each Ligges message, result URL, `00check.log`, raw test output,
upload timestamp, candidate SHA-256, and size are archived. `platform-clean` is not claimed.
