# After Task: Arc 6 installed-package test-path repair

## 1. Goal

Repair the two Arc 6 tests that failed only under `R CMD check`, without
...
--no-build-vignettes .` succeeds. Local `R CMD check` reaches installation but
cannot complete because this machine cannot resolve CRAN and therefore lacks
the optional `palmerpenguins` Suggests package; main-branch Linux CI is the
final installed-package check.
...
package context that caused the regression.
...
the installed-package test layout used by `R CMD check`. The regression made
that missing check-context boundary visible.
...
This repair covers only the two direct-lognormal Arc 6 source-path tests. It
does NOT cover or alter the evidence boundaries for direct `rho12`, Student-t, staged
