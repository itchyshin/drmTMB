...
Execute the exact animal A/Ainv NB2 local diagnostic micro-shard for the
ordinary count one-slope recovery lane: `animal(1 + x | id, Ainv = Q)` in
`mu`, `nbinom2()` family, q1, one independent structured slope, and fixed-effect
overdispersion `sigma`. Keep the evidence local, diagnostic, and separate from
pedigree/Ainv bridge marshalling, recovery, coverage, interval, bridge, REML,
AI-REML, public support, Totoro, and DRAC claims.

## 2. Implemented

- Added a source-controlled runner,
  `tools/run-structured-re-count-slope-animal-nbinom2-local-micro-shard.R`,
  ...
  fixed-seed NB2 one-slope fits with fixed-effect `sigma`, and writes
  replicate, summary, and run-log TSV artifacts.
- Added
  `docs/dev-log/dashboard/structured-re-count-slope-animal-nbinom2-local-micro-shard.tsv`
  as the one-row mission-control sidecar for this exact local diagnostic
  execution.
- Added generated artifacts under
  `docs/dev-log/simulation-artifacts/2026-06-26-count-slope-animal-nbinom2-local-micro-shard/`.
...
- I kept NB2 `sigma` as fixed-effect overdispersion only. This row does not
  add structured count `sigma` support.
- I kept this as the animal NB2 sibling of the local animal Poisson smoke row
...
- `docs/dev-log/after-task/2026-06-26-count-slope-animal-nbinom2-local-micro-shard.md`
- `docs/dev-log/check-log.md`
- `docs/dev-log/dashboard/README.md`
- `docs/dev-log/dashboard/structured-re-count-slope-animal-nbinom2-local-micro-shard.tsv`
- `docs/dev-log/simulation-artifacts/2026-06-26-count-slope-animal-nbinom2-local-micro-shard/structured-re-count-slope-animal-nbinom2-local-micro-shard-replicates.tsv`
- `docs/dev-log/simulation-artifacts/2026-06-26-count-slope-animal-nbinom2-local-micro-shard/structured-re-count-slope-animal-nbinom2-local-micro-shard-run-log.tsv`
- `docs/dev-log/simulation-artifacts/2026-06-26-count-slope-animal-nbinom2-local-micro-shard/structured-re-count-slope-animal-nbinom2-local-micro-shard-summary.tsv`
...
- `Rscript --vanilla tools/run-structured-re-count-slope-animal-nbinom2-local-micro-shard.R --n_rep=4 --attempt-temp-install` passed and wrote the three artifact TSVs.
- `air format tools/run-structured-re-count-slope-animal-nbinom2-local-micro-shard.R tests/testthat/test-structured-re-conversion-contracts.R` passed.
- `python3 -m py_compile tools/validate-mission-control.py` passed.
- `python3 tools/validate-mission-control.py` passed and reported one count-slope animal NB2 local micro-shard row.
- `git diff --check` passed.
- `Rscript --no-environ --no-init-file -e "source('/Users/z3437171/shinichi-brain/tools/check-after-task.R'); main_check_after_task('docs/dev-log/after-task/2026-06-26-count-slope-animal-nbinom2-local-micro-shard.md')"` passed.
...
- The runner uses the same temporary source-install path as the Poisson, phylo,
  and spatial count micro-shards, so it exercises the current source rather
  than an unrelated installed package.
...
- The R dashboard contract test has a separate animal NB2 test, so the NB2 row
  cannot be inferred from the animal Poisson or spatial NB2 rows.

## 7a. Issue Ledger

- Fixed: the animal NB2 support cell now has one local execution smoke row
...
- Checked the existing animal NB2 q-series, runner, dispatch, and shard-pack
  rows before adding the local sidecar.
- Verified that the new sidecar links back to the existing animal NB2
...
- Two smoke seeds estimate one structured SD near the lower boundary. That is
...
NB2 siblings need their own evidence even when the random-effect formula is
identical to the Poisson row. The `sigma` column makes the count family
different enough that the support cell should own its own runner, artifacts,
