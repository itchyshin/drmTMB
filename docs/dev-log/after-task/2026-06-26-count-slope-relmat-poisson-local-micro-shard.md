...
Execute the exact relmat K/Q Poisson local diagnostic micro-shard for the
ordinary count one-slope recovery lane: `relmat(1 + x | id, Q = Q)` in `mu`,
`poisson()` family, q1, and one independent structured slope. Keep the evidence
local, diagnostic, and separate from Q bridge marshalling, recovery, coverage,
interval, bridge, REML, AI-REML, public support, Totoro, and DRAC claims.
...
- Added a source-controlled runner,
  ...
  fixed-seed Poisson one-slope fits through `Q = Q`, and writes replicate,
  summary, and run-log TSV artifacts.
- Added
  `docs/dev-log/dashboard/structured-re-count-slope-relmat-poisson-local-micro-shard.tsv`
  as the one-row mission-control sidecar for this exact local diagnostic
  execution.
- Added generated artifacts under
  `docs/dev-log/simulation-artifacts/2026-06-26-count-slope-relmat-poisson-local-micro-shard/`.
...
- I kept this as the relmat Poisson sibling of the local animal Poisson smoke
...
- `docs/dev-log/after-task/2026-06-26-count-slope-relmat-poisson-local-micro-shard.md`
- `docs/dev-log/check-log.md`
- `docs/dev-log/dashboard/README.md`
- `docs/dev-log/dashboard/structured-re-count-slope-relmat-poisson-local-micro-shard.tsv`
- `docs/dev-log/simulation-artifacts/2026-06-26-count-slope-relmat-poisson-local-micro-shard/structured-re-count-slope-relmat-poisson-local-micro-shard-replicates.tsv`
- `docs/dev-log/simulation-artifacts/2026-06-26-count-slope-relmat-poisson-local-micro-shard/structured-re-count-slope-relmat-poisson-local-micro-shard-run-log.tsv`
- `docs/dev-log/simulation-artifacts/2026-06-26-count-slope-relmat-poisson-local-micro-shard/structured-re-count-slope-relmat-poisson-local-micro-shard-summary.tsv`
...
- `Rscript --vanilla tools/run-structured-re-count-slope-relmat-poisson-local-micro-shard.R --n_rep=4 --attempt-temp-install` passed and wrote the three artifact TSVs.
- `air format tools/run-structured-re-count-slope-relmat-poisson-local-micro-shard.R tests/testthat/test-structured-re-conversion-contracts.R` passed.
- `python3 -m py_compile tools/validate-mission-control.py` passed.
- `python3 tools/validate-mission-control.py` passed and reported one count-slope relmat Poisson local micro-shard row.
- `git diff --check` passed.
- `Rscript --no-environ --no-init-file -e "source('/Users/z3437171/shinichi-brain/tools/check-after-task.R'); main_check_after_task('docs/dev-log/after-task/2026-06-26-count-slope-relmat-poisson-local-micro-shard.md')"` passed.
...
- The runner uses the same temporary source-install path as the phylo, spatial,
  and animal count micro-shards, so it exercises the current source rather than
  an unrelated installed package.
...
- The R dashboard contract test has a separate relmat Poisson test, so the
  relmat row cannot be inferred from animal, spatial, or phylo smoke rows.

## 7a. Issue Ledger

- Fixed: the relmat Poisson support cell now has one local execution smoke row
...
- Checked the existing relmat Poisson q-series, runner, dispatch, and
  shard-pack rows before adding the local sidecar.
- Verified that the new sidecar links back to the existing relmat Poisson
...
Relmat K/Q rows need exact route labels at two levels. The broad contract can
say K/Q because future recovery may test covariance and precision routes, but a
local smoke row should name the formula route actually fit before any
neighboring cell or bridge route is trusted.
