# C18 `mc-0605` completion review — relatedness-matrix zero-one-beta `zoi` q1

## Decision

**GO — point-fit recovery only.** `mc-0605` is the exact ordinary-ML
`zero_one_beta()` route with one unlabelled q1 `relmat(1 | ..., ...)`
...
None of the three defects touched `mc-0605`'s recovery evidence — the four
...
- Four planned source-pinned recovery attempts (seeds `2026080521-2026080524`) are
  retained in
  `docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-relmat-zoi-q1-local-recovery/raw-attempts.tsv`; all
...
- `docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-relmat-zoi-q1-local-recovery/summary.tsv` records
  4/4 passing attempts, mean relative tau error **0.228**, decision
  `PASS_POINT_RECOVERY_LOCAL`.
- Minimum conditional-mode correlation against the simulated latent field
  across the four seeds was **0.72**.
...
| --- | --- | --- |
| Noether | GO | Parameter map, zoi endpoint routing, relmat structured-atom oracle, F1/F2/F3 repair scope. |
...
All three reviews apply only to `mc-0605` at `point_fit_recovery` grade.
