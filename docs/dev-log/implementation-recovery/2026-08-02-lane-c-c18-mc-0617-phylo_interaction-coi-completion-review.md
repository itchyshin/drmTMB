# C18 `mc-0617` completion review — phylogenetic-interaction zero-one-beta `coi` q1

## Decision

**GO — point-fit recovery only.** `mc-0617` is the exact ordinary-ML
`zero_one_beta()` route with one unlabelled q1 `phylo_interaction(1 | ..., ...)`
intercept on `coi`'s linear predictor. This is a single-cell ledger
...
None of the three defects touched `mc-0617`'s recovery evidence — the four
...
- Four planned source-pinned recovery attempts (seeds `2026080711-2026080714`) are
  retained in
  `docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-phylo_interaction-coi-q1-local-run-1/raw-attempts.tsv`; all
...
- `docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-phylo_interaction-coi-q1-local-run-1/summary.tsv` records
  4/4 passing attempts, mean relative tau error **0.204**, decision
  `PASS_POINT_RECOVERY_LOCAL`.
- Minimum conditional-mode correlation against the simulated latent field
  across the four seeds was **0.75**.
...
| --- | --- | --- |
| Noether | GO | Parameter map, coi endpoint routing, phylo_interaction structured-atom oracle, F1/F2/F3 repair scope. |
...
All three reviews apply only to `mc-0617` at `point_fit_recovery` grade.
