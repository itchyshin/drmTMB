# C2 relmat Poisson q2 local receipt

This retained receipt covers exactly
`count ~ x + relmat(1 + x | p | site, Q = Q)` with ordinary `poisson()` and a
named supplied precision matrix. It has three planned seeds
...
coverage, alternate representation, or broader grammar claim.
