# C18 `mc-0603` completion review — phylogenetic zero-one-beta `zoi` q1

## Decision

**GO — point-fit recovery only.** `mc-0603` is the exact ordinary-ML
...
None of the three defects touched `mc-0603`'s recovery evidence — the four
...
- Four planned source-pinned recovery attempts (seeds `2026080201-2026080204`) are
  retained in
  `docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-phylo-zoi-q1-local-recovery/raw-attempts.tsv`; all
...
- `docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-phylo-zoi-q1-local-recovery/summary.tsv` records
  4/4 passing attempts, mean relative tau error **0.108**, decision
  `PASS_POINT_RECOVERY_LOCAL`.
- Minimum conditional-mode correlation against the simulated latent field
  across the four seeds was **0.86**.
...
| --- | --- | --- |
| Noether | GO | Parameter map, zoi endpoint routing, phylo structured-atom oracle, F1/F2/F3 repair scope. |
...
All three reviews apply only to `mc-0603` at `point_fit_recovery` grade.
