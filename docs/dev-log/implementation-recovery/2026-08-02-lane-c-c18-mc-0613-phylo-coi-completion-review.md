# C18 `mc-0613` completion review — phylogenetic zero-one-beta `coi` q1

## Decision

**GO — point-fit recovery only.** `mc-0613` is the exact ordinary-ML
`zero_one_beta()` route with one unlabelled q1 `phylo(1 | ..., ...)`
...
None of the three defects touched `mc-0613`'s recovery evidence — the four
...
- Four planned source-pinned recovery attempts (seeds `2026080211-2026080214`) are
  retained in
  `docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-phylo-coi-q1-local-recovery/raw-attempts.tsv`; all
...
- `docs/dev-log/implementation-recovery/2026-08-02-lane-c-c18-zob-phylo-coi-q1-local-recovery/summary.tsv` records
  4/4 passing attempts, mean relative tau error **0.191**, decision
  `PASS_POINT_RECOVERY_LOCAL`.
- Minimum conditional-mode correlation against the simulated latent field
  across the four seeds was **0.66**.
...
| --- | --- | --- |
| Noether | GO | Parameter map, coi endpoint routing, phylo structured-atom oracle, F1/F2/F3 repair scope. |
...
All three reviews apply only to `mc-0613` at `point_fit_recovery` grade.
