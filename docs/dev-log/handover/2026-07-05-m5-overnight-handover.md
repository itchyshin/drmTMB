...
## Merge status (row 105) — MERGED, with a CI hiccup caught + fixed
- **PR #733 merged row 105 to main → 103/104.** But its ubuntu R CMD check **failed**:
  6 `test-phylo-utils.R` direct-DLL unit tests build TMB data by hand and were missing
  the new `has_phylo_mu2` field (`getParameterOrder` error). My local targeted tests
  all go through the R assembly (which adds the field), so they passed; the full
  R CMD check exercises the direct-DLL tests, which don't. I merged before catching it
  — a masked CI exit code (`gh pr checks … | tail` returned tail's 0, not gh's failure).
- **Fixed** on branch `drmtmb/row105-ci-fix` (follow-up PR): added the inert
  `has_phylo_mu2` data + `u_phylo2`/`log_sd_phylo2` params to the two shared
  test-phylo-utils helpers, mirroring the existing `has_phylo_mu=0L` scaffold.
  test-phylo-utils.R → **186/0** locally; the row-105 engine + admission are untouched.
- Verify main is green once the fix PR lands: `git fetch origin main &&
  python3 tools/qseries_v1_claim_guard.py` (103/104) + the fix PR's CI is green.
- **Lessons (applied):** (1) for engine / C++-template changes run the FULL suite (or
  R CMD check) locally before merging — targeted tests miss direct-DLL data builders;
  (2) capture `gh pr checks` exit via `${PIPESTATUS[0]}`, never `tail`'s.
