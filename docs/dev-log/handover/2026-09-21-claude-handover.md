...
| GLLVModels.jl | PR #428 already owns the landing, quickstart, model, and diagnostics routes; the two remaining reader edits are an internal model paragraph and a rename-history block in Diagnostics.  Separately, advisory R/Julia parity CI is red across NB2, truncated-NB2, and Student-t mechanisms. | Finish #428 rather than opening another documentation lane: explain predictor-informed latent patterns in ordinary words and move diagnostic rename history to the changelog.  Do not make a speculative parity repair; first create build-independent, family-specific health invariants against an authoritative frozen build. | The rendered extensionless routes match `origin/main@6275df5`, not #428 yet.  The parity finding is not evidence that documentation work regressed the engine. |
