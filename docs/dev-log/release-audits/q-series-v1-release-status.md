...
76 row-level roles: 8 exact Gaussian
`inference_ready` anchors, 48 additional Gaussian
basic-working rows, and 20 basic-distribution recovery
rows. The remaining 28 rows stay in post-v1.0 validation or design.
...
| --- | ---: | ---: | --- |
| Practical v1.0 row surface | 76/104 | 73.1% | Exact Gaussian anchors plus additional Gaussian basic-working rows and basic-distribution recovery rows. |
| Gaussian v1.0 core | 56/67 | 83.6% | Gaussian rows inside the exact-anchor or basic-working v1.0 surface. |
| Basic-distribution recovery | 20/37 | 54.1% | Non-Gaussian rows with point-fit recovery evidence only. |
...
| Post-v1.0 validation/design | 28/104 | 26.9% | Rows deliberately left outside the v1.0 practical surface. |
...
| `basic_distribution_recovery` | 20 | Basic-distribution recovery rows for v1.0. | Recovery-only evidence is not interval evidence, coverage evidence, `inference_ready`, `supported`, REML, AI-REML, or broad structured-covariance support. |
...
| `basic_distribution_post_v1_design` | 17 | Non-Gaussian rows outside the v1.0 basic-distribution surface. | Leave for post-v1.0 family-specific implementation, rejection, or limitation design. |
