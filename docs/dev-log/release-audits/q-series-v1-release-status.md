...
103 row-level roles: 8 exact Gaussian
`inference_ready` anchors, 59 additional Gaussian
basic-working rows, and 36 basic-distribution recovery
rows. The remaining 1 rows stay in post-v1.0 validation or design.
...
| --- | ---: | ---: | --- |
| Practical v1.0 row surface | 103/104 | 99.0% | Exact Gaussian anchors plus additional Gaussian basic-working rows and basic-distribution recovery rows. |
| Gaussian v1.0 core | 67/67 | 100.0% | Gaussian rows inside the exact-anchor or basic-working v1.0 surface. |
| Basic-distribution recovery | 36/37 | 97.3% | Non-Gaussian rows with point-fit recovery evidence only. |
...
| Post-v1.0 validation/design | 1/104 | 1.0% | Rows deliberately left outside the v1.0 practical surface. |
...
| `basic_distribution_recovery` | 36 | Basic-distribution recovery rows for v1.0. | Recovery-only evidence is not interval evidence, coverage evidence, `inference_ready`, `supported`, REML, AI-REML, or broad structured-covariance support. |
...
| `basic_distribution_post_v1_design` | 1 | Non-Gaussian rows outside the v1.0 basic-distribution surface. | Leave for post-v1.0 family-specific implementation, rejection, or limitation design. |
