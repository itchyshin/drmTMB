...
| **platform-clean (Cursor)** | Repair on `main` (#941). Draft **#945** adjudicates fixed-tarball win-builder: R-release + R-devel **1 NOTE** (CondExp ERROR cleared). Ledger still **`tarball-clean`** pending owner authorize. Worktree `~/local-scratch/worktrees/drmTMB-07-platform`. | PR #945; `platform/winbuilder-emails.md`; `platform/PLATFORM-NOT-READY.md` | Do not write `platform-clean` / upload / DESCRIPTION 0.7.0 without owner word. |
