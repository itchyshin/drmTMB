...
| --- | --- | --- | --- |
| **0.7 exact-candidate closeout (Codex)** | Final immutable bytes pass exact-byte local and three-arm win-builder checks; exact-source 3-OS CI and three sanitizer arms also pass. Executable ledger proves `platform-clean`; Grace and Pat are READY, with Rose re-auditing this build-excluded landing. | [`release-audits/2026-08-19-070-cran-release-ledger-1d6445db.json`](release-audits/2026-08-19-070-cran-release-ledger-1d6445db.json) and [`release/0.7.0-cran-gate/candidate-6170fbeee/`](release/0.7.0-cran-gate/candidate-6170fbeee/) | Only evidence/governance closeout remains. No candidate-byte edit, `submit_cran()`, submission, #1033, or `_julia_skip2_artifacts/` action is authorized. `5153ae7e…` and `6b45164b…` are predecessor evidence only. |
