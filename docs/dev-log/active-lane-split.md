...
| --- | --- | --- | --- |
| C18 — structured zero-one-beta atom effects | **Planning only.** The proposed programme first splits the ten collapsed structured `zoi`/`coi` ledger rows, then conditionally implements evidence-backed q1 provider cells. | Planning branch `claude/c18-structured-atoms-plan`; PR #895 carries the [`handover/2026-08-02-claude-handover.md`](handover/2026-08-02-claude-handover.md) snapshot | Do not mutate overlapping package/TMB/ledger files until PR #893 lands or closes, unless Shinichi gives fresh overlap authorization. |
