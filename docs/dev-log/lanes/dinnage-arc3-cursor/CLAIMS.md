...
Updated: 2026-09-16 ~10:13 MDT (**B2 #1375 @ `19991695c` CONFLICTING — rebase in flight**; Rd fix on tip; prior D-263 ACCEPT stale; **no merge**).

- **`main` tip:** `f2d20fe65` (coord); tip-identity last **green** @ `e169d7a76` (run `35111843393`). Docs-only coord pushes OK; no receipt PR unless `R/` on `main` moves.
...
- **Wave B2 — PR [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `19991695c` (`cursor/dinnage-arc3-b2-surfaces-20260916`). **CONFLICTING** with `main` — **rebase in flight**. Local **47/47** reported. **Rd `@param` fix pushed** on tip. **D-263:** prior **ACCEPT stale**; **awaiting fresh D-263 + settled green CI** (**not MERGE-READY**). **P0:** C17 inert re-cert on earlier tip (`mc-0568` / C17+C14 @ `c493fc0bb`). **No merge** until rebase completes. **P2:** confint Rd cross-link, audit note, check-log (fixer). Prefer **Composer** / Grace watchers; agents **do not merge #1375**. **Branch push OK.** Active lease: `gauss:dinnage-arc3-B2` (release when idle).
- **A4 record:** DONE (#1351).
