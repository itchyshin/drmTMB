...
Updated: 2026-09-16 ~10:52 MDT (**B2 #1375 @ `1424f1971`** — **D-263 ACCEPT**; MERGE-READY pending green CI).
...
- **Wave B2 — PR [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `1424f1971` (`cursor/dinnage-arc3-b2-surfaces-20260916`). Tip release-shard fixes: **REML IC**, **A-2 explicit `missing=` gating**, **census TSV**. **D-263: ACCEPT** @ `1424f1971` (Shinichi). **P2 (optional):** NEWS / `?miss_control` wording. **MERGE-READY pending settled green CI** on tip; merge when green (keep-going). Tip-identity receipt **LAST** on branch. Prefer **Composer** / Grace watchers. Active lease: `gauss:dinnage-arc3-B2` (release when idle).
- **A4 record:** DONE (#1351).
