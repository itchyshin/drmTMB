...
Updated: 2026-09-16 ~10:10 MDT (**B2 #1375 — CI red Rd fix in flight**; prior D-263 ACCEPT @ `c493fc0bb`; no new ACCEPT).

- **`main` tip:** `ba2e8d31` (coord); tip-identity last **green** @ `e169d7a76` (run `35111843393`). Docs-only coord pushes OK; no receipt PR unless `R/` on `main` moves.
...
- **Wave B2 — PR [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `c493fc0bb` (`cursor/dinnage-arc3-b2-surfaces-20260916`). Local **47/47** reported. **D-263:** independent **ACCEPT** posted (PR review comment; GitHub **Approve** blocked — same-author). **P0:** C17 inert re-cert **pushed** (`mc-0568` / C17+C14 on `R/drmTMB.R` + `R/methods.R`). **Awaiting** settled green CI only (**not MERGE-READY** until CI green). **P2:** confint Rd cross-link, audit note, check-log (fixer). Prefer **Composer** / Grace watchers; agents **do not merge #1375** until CI settles. Active lease: `gauss:dinnage-arc3-B2` (release when idle).
- **A4 record:** DONE (#1351).
