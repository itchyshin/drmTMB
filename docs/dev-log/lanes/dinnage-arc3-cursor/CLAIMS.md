...
Updated: 2026-09-16 ~10:40 MDT (**B2 #1375 @ `296a1f763`** — **CI RED** shard 3/4 `test-package-skeleton.R` / `drm_formula`; fixer in flight; **D-263 ACCEPT** @ tip stands until tip moves; **no merge**).

- **`main` tip:** `6c3890d33` (coord); tip-identity last **green** @ `e169d7a76` (run `35111843393`). Docs-only coord pushes OK; no receipt PR unless `R/` on `main` moves.
...
- **Wave B2 — PR [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `296a1f763` (`cursor/dinnage-arc3-b2-surfaces-20260916`). GitHub **MERGEABLE** (rebase complete; do not treat **CONFLICTING** @ `19991695c` as live). Local **47/47** reported. **D-263:** independent **ACCEPT** @ `296a1f763` **stands until tip moves** (GitHub **Approve** blocked — same-author). **CI RED:** run [`35120853817`](https://github.com/itchyshin/drmTMB/actions/runs/35120853817) — release **shard 3/4** failed `tests/testthat/test-package-skeleton.R` (`drm_formula`); **fixer in flight** (Composer). **No merge** / **not MERGE-READY** until CI settles green; tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. Active lease: `gauss:dinnage-arc3-B2` (release when idle).
- **A4 record:** DONE (#1351).
