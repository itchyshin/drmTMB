...
Updated: 2026-09-16 ~10:16 MDT (**B2 #1375 @ `19991695c` CONFLICTING — rebase in flight**; fresh D-263 ACCEPT on tip; **no merge** until mergeable + post-rebase CI).
...
- **Wave B2 — PR [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `19991695c` (`cursor/dinnage-arc3-b2-surfaces-20260916`). GitHub **CONFLICTING** with `main` — **rebase in flight**. Local **47/47** reported. **Rd `@param` fix** on tip. **D-263:** fresh **ACCEPT** @ `19991695c` (**stale if tip moves** after rebase). **P0:** C17 inert re-cert on earlier tip (`mc-0568` / C17+C14 @ `c493fc0bb`). **Not MERGE-READY** — **no merge** until GitHub **mergeable**, settled green CI on post-rebase tip (+ **re-ACCEPT** if head moves). **P2:** confint Rd cross-link, audit note, check-log (fixer). Prefer **Composer** / Grace watchers; agents **do not merge #1375**. **Branch push OK.** Active lease: `gauss:dinnage-arc3-B2` (release when idle).
- **A4 record:** DONE (#1351).
