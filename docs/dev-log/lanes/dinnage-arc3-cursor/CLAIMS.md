...
Updated: 2026-09-16 ~09:15 MDT (**B2 #1375 — awaiting fresh D-263 + CI** @ `c493fc0bb`).
...
- **Wave B2 — PR [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `c493fc0bb` (`cursor/dinnage-arc3-b2-surfaces-20260916`). Local **47/47** reported. **D-263:** prior **REQUEST CHANGES** (not ACCEPT; **not MERGE-READY**). **P0:** C17 inert re-cert **pushed** (`mc-0568` / C17+C14 on `R/drmTMB.R` + `R/methods.R`). **Awaiting** settled green CI + fresh **D-263 ACCEPT**. **P2:** confint Rd cross-link, audit note, check-log (fixer). Merge gate unchanged; prefer **Composer** / Grace watchers; agents **do not merge #1375**. Active lease: `gauss:dinnage-arc3-B2` (release when idle).
- **A4 record:** DONE (#1351).
