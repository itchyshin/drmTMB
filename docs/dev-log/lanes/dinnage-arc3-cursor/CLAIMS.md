...
Updated: 2026-09-16 ~10:19 MDT (**B2 #1375 @ `296a1f763`** — CI run `35120853817`; **D-263 ACCEPT** @ `296a1f763`).

- **`main` tip:** `e8d1aadaf` (coord); tip-identity last **green** @ `e169d7a76` (run `35111843393`). Docs-only coord pushes OK; no receipt PR unless `R/` on `main` moves.
...
- **Wave B2 — PR [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `296a1f763` (`cursor/dinnage-arc3-b2-surfaces-20260916`). Local **47/47** reported. **D-263:** independent **ACCEPT** @ `296a1f763` (PR review comment; tip delta `_pkgdown.yml` only — restores B2 reference-index guard after rebase; GitHub **Approve** blocked — same-author). **P0:** C17 inert re-cert on branch (`mc-0568` / C17+C14; unchanged since `c493fc0bb`). **CI:** run `35120853817` — **await settled green**, then merge (Shinichi keep-going). **Not MERGE-READY** until CI green. Prefer **Composer** / Grace watchers. Active lease: `gauss:dinnage-arc3-B2` (release when idle).
- **A4 record:** DONE (#1351).
