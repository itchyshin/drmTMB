...
Updated: **2026-09-16 ~11:01 MDT** · **`main` @ `c259a1336`**

## Current scope

**Receipt gate CLEAR** on `main` (tip-identity green @ `e169d7a76`, run `35111843393`; coord-only moves since — no receipt PR). **Coord/docs push OK.** **Wave A: CLOSED.** **B1 [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **B2 [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `14618e6e4` — local **47/47** reported; **inert C17 recert pushed** @ `14618e6e4`. **D-263:** prior **ACCEPT @ `1424f1971` stale**; **awaiting fresh D-263 + settled green CI** (**not MERGE-READY**). **P2:** confint Rd cross-link, audit note, check-log (fixer). Tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. **DRM.jl twin map (Jason):** [`drm-jl-followups.md`](drm-jl-followups.md).
...
| #1375 | B2 surfaces | OPEN (C17 recert; stale D-263; await CI) | `14618e6e4` |

## Wave B

**Awaiting fresh D-263 + CI.** [#1375](https://github.com/itchyshin/drmTMB/pull/1375) @ `14618e6e4` on `cursor/dinnage-arc3-b2-surfaces-20260916`. **Inert C17 recert pushed** @ `14618e6e4`; prior **D-263 ACCEPT @ `1424f1971` stale**. **Not MERGE-READY.** Merge only after fresh D-263 and settled green CI + tip-identity receipt **LAST** on branch. P2: docs/audit/check-log (fixer). See [`wave-b-brief.md`](wave-b-brief.md) § B2.
