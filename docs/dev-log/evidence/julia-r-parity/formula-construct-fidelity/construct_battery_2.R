...
OUT <- Sys.getenv("BATTERY_OUT", "/private/tmp/claude-503/-Users-z3437171-Dropbox-Github-Local-drmTMB/7db7461b-e1ee-4ad0-a526-010c1c2e26a6/scratchpad/j467/battery-002.tsv")
...
gmixchar <- rep(c("a", "B", "c"), length.out = n)       # bare character, mixed case
gifac <- factor(gi)                                     # factor whose labels are numbers
gone <- factor(rep("only", n))
gcase <- factor(rep(c("A", "a", "b"), length.out = n))  # locale vs codepoint collation
`__bridge_scale_1` <- z                                 # user column named like a bridge atom
dat <- data.frame(y, x, z, gi, g3, g2, gchar, flag, gmix, g3rev, gempty,
                  gord, gsum, gnaf, gmixchar, gifac, gone, gcase,
                  `__bridge_scale_1` = `__bridge_scale_1`,
                  check.names = FALSE, stringsAsFactors = FALSE)

cases <- list(
  char_mixed_case    = "gmixchar",
  factor_num_levels  = "gifac",
  factor_one_level   = "gone",
  factor_case_pair   = "gcase",
  empty_lvl_inter    = "x * gempty",
  I_identity         = "I(x)",
  I_paren            = "I((x))",
  I_logcall          = "x + I(log(x + 2))",
  I_sum              = "I(x^2 + z)",
  poly_3             = "poly(x, 3)",
  poly_two_cols      = "poly(x, 2) + poly(z, 2)",
  scale_both_parts   = "scale(x) + scale(z)",
  cross3_power2      = "(x + z + g2)^2",
  cross_power3       = "(x + z + g2)^3",
  no_intercept_minus = "x - 1",
  no_intercept_zero  = "0 + x",
  interaction_only   = "x:z",
  minus_expanded     = "x + z + x:z - z",
  minus_from_power   = "(x + z)^2 - x:z",
  minus_star_inter   = "x * z - x:z",
  log_times_factor   = "log(x + 2) * g2",
  factor_minus       = "x + g3 - x",
  bridge_name_clash  = "scale(x) + `__bridge_scale_1`",
  poly_minus         = "poly(x, 2) + z - z",
  I_in_interaction   = "I(x^2):g2"
)
sigma_cases <- list(
  sigma_empty_lvl    = list(mu = "x", sigma = "gempty"),
  sigma_char_mixed   = list(mu = "x", sigma = "gmixchar"),
  sigma_scale_shared = list(mu = "scale(x)", sigma = "scale(x)"),
  sigma_cross        = list(mu = "x", sigma = "(x + z)^2")
)
