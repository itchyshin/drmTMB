...
The frozen four-fixture receipt run has been generated and classified. It
supports exact point-target agreement in the bounded fixtures, but it does not
support coupled-NB2 profile-endpoint parity: several direct fixed-effect
endpoints differ materially and the `L22` interval has no finite endpoint in
either engine. G4 is therefore adverse/classified, not passed. The next gate is
a cross-objective endpoint oracle that retains profile NLL, likelihood-ratio
