...
| Animal models | First fitted known-matrix slices | `animal(1 | id, A = A)` and `animal(1 | id, Ainv = Ainv)` now fit a univariate Gaussian `mu` intercept, and matching labelled bivariate `mu1`/`mu2` terms fit the first q=2 location covariance, with diagnostics, profile targets, extractor rows, and recovery tests | Add small runnable examples and keep the next parity steps scoped: pedigree-to-Ainv construction, slopes, `sigma`, q=4, predictor-dependent `corpair()`, and direct-SD grammar. |
| `relmat()` models | First fitted known-matrix slices | `relmat(1 | id, K = K)` and `relmat(1 | id, Q = Q)` now fit the same univariate Gaussian `mu` intercept route, and matching labelled bivariate `mu1`/`mu2` terms fit the first q=2 location covariance for lower-level user-supplied relatedness or precision matrices | Keep `relmat()` distinct from `meta_V(V = V)`; add examples and diagnostics before expanding to slopes, `sigma`, q=4, predictor-dependent `corpair()`, or direct-SD grammar. |
