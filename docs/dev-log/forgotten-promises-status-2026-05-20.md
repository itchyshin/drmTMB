...
| Spatial models | Partial, fitted univariate coordinate `mu` path, parity target clarified | README and docs show `spatial(1 | site, coords = coords)` and one numeric `spatial(1 + x | site, coords = coords)` slope; structural-dependence docs now treat spatial as the phylo sibling, not an afterthought; the next parity gate is coordinate-spatial q=2 bivariate `mu1`/`mu2` covariance with `corpairs(level = "spatial")` | Mesh/SPDE, multiple slopes, slope correlations, spatial `sigma`, spatial q=4 location-scale, spatial direct-SD, and spatial `corpair()` regression remain planned behind the q=2 gate. |
