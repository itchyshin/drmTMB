...
| Animal models | Planned marker only, parity target clarified | `animal()` has reference docs and planned syntax; README/ROADMAP say no fitted animal likelihood exists; the structural-dependence article now shows planned q=2 and q=4 examples as phylo-parity targets | Implement only after pedigree or `A`/`Ainv` validation, sparse precision design, diagnostics, profile targets, and recovery tests. |
| `relmat()` models | Planned marker only, parity target clarified | `relmat()` has reference docs and lower-level known-relatedness design wording; structural-dependence docs now keep it as step 5 after animal, phylo, spatial, and phylo+spatial | Decide one low-level route and matrix contract before any fit; avoid confusing it with `meta_V(V = V)`. |
