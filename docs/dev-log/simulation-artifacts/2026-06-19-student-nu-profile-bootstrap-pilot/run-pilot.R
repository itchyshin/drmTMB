...
script_path <- if (length(file_arg)) {
  ...
  normalizePath(
    "docs/dev-log/simulation-artifacts/2026-06-19-student-nu-profile-bootstrap-pilot/run-pilot.R",
    mustWork = TRUE
  )
}
...
run_summary <- data.frame(
  surface = "student_nu_profile_bootstrap_pilot",
  interpretation_label = "diagnostic_interval_pilot",
  master_seed = master_seed,
...
utils::write.csv(
  conditions,
  file.path(tables_dir, "student-nu-profile-bootstrap-pilot-conditions.csv"),
  row.names = FALSE
)
utils::write.csv(
  fit_rows,
  file.path(tables_dir, "student-nu-profile-bootstrap-pilot-fit-status.csv"),
  row.names = FALSE
)
utils::write.csv(
  fit_summary,
  file.path(tables_dir, "student-nu-profile-bootstrap-pilot-fit-summary.csv"),
  row.names = FALSE
)
utils::write.csv(
  nu_profile_intervals,
  file.path(tables_dir, "student-nu-profile-pilot-intervals.csv"),
  row.names = FALSE
)
utils::write.csv(
  profile_summary,
  file.path(tables_dir, "student-nu-profile-pilot-summary.csv"),
  row.names = FALSE
)
utils::write.csv(
  nu_bootstrap_intervals,
  file.path(tables_dir, "student-nu-bootstrap-pilot-intervals.csv"),
  row.names = FALSE
)
utils::write.csv(
  bootstrap_summary,
  file.path(tables_dir, "student-nu-bootstrap-pilot-summary.csv"),
  row.names = FALSE
)
utils::write.csv(
  ...
  file.path(
    tables_dir,
    "student-nu-profile-bootstrap-pilot-diagnostics.csv"
  ),
  row.names = FALSE
)
utils::write.csv(
  out$summary$interval_failures,
  file.path(tables_dir, "student-nu-profile-bootstrap-pilot-failures.csv"),
  row.names = FALSE
)
utils::write.csv(
  run_summary,
  file.path(artifact_dir, "student-nu-profile-bootstrap-pilot-run-summary.csv"),
  row.names = FALSE
)
...
message("Wrote Student-t profile/bootstrap pilot to ", artifact_dir)
