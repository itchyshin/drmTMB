#!/usr/bin/env Rscript
# mc-0717 Totoro smoke: one job (toy / reject / one of 27 fits).
...
extract_drmtmb_corr <- function(fit) {
  ...
  if (length(sd0) != 1L || length(sd1) != 1L || length(rho_re) != 1L) {
    stop("extractor names drifted from Design 257", call. = FALSE)
  }
  list(
    sd0 = as.numeric(sd0),
    sd1 = as.numeric(sd1),
...
fit_one <- function(dat, method) {
  if (identical(method, "drmtmb_corr")) {
    fit <- drmTMB::drmTMB(
      drmTMB::bf(cbind(success, failure) ~ x + (1 + x | id)),
  ...
  if (identical(method, "drmtmb_iid")) {
    fit <- drmTMB::drmTMB(
      drmTMB::bf(cbind(success, failure) ~ x + (1 | id) + (0 + x | id)),
      family = stats::binomial(),
      data = dat
    )
    conv <- as.integer(fit$opt$convergence)
  ...
  if (identical(method, "glmmtmb_corr")) {
    ...
    fit <- glmmTMB::glmmTMB(
      cbind(success, failure) ~ x + (1 + x | id),
      family = stats::binomial(),
      data = dat
    )
...
run_fit <- function(opt) {
  ...
  sim <- tryCatch(
    mspl_q2_data(n_group = 56L, n_each = opt$n_each, seed = opt$seed),
    error = function(e) e
  )
...
run_toy <- function(opt) {
  # Plumbing check only. Seed 871000 is not in the 27-fit denominator.
  toy_seed <- 871000L
  toy_n <- 4L
  sim <- mspl_q2_data(n_group = 12L, n_each = toy_n, seed = toy_seed)
  fit <- drmTMB::drmTMB(
    drmTMB::bf(cbind(success, failure) ~ x + (1 + x | id)),
    family = stats::binomial(),
    data = sim$data
...
run_reject <- function(opt) {
  sim <- mspl_q2_data(n_group = 10L, n_each = 8L, seed = 871999L)
  q2 <- drmTMB::bf(cbind(success, failure) ~ x + (1 + x | id))
  probes <- list(
    reml = function() {
      drmTMB::drmTMB(q2, family = stats::binomial(), data = sim$data, REML = TRUE)
    },
    missing_response = function() {
      dat <- sim$data
      dat$success[[1L]] <- NA_integer_
      drmTMB::drmTMB(
        q2,
        family = stats::binomial(),
        data = dat,
    ...
    labelled = function() {
      drmTMB::drmTMB(
        drmTMB::bf(cbind(success, failure) ~ x + (1 + x | p | id)),
        family = stats::binomial(),
        data = sim$data
      )
    },
    mixed = function() {
      drmTMB::drmTMB(
        drmTMB::bf(cbind(success, failure) ~ x + (1 | id) + (1 + x | id)),
        family = stats::binomial(),
        data = sim$data
      )
    },
  ...
  if (!all(tab$stayed_red)) {
    stop("REJECTION MATRIX FAIL: a neighbour fitted: ", paste(tab$neighbour[!tab$stayed_red], collapse = ", "), call. = FALSE)
  }
