...
n_rep <- as.integer(value_arg("n-rep", "2"))
...
if (is.na(n_rep) || n_rep < 2L) {
  stop(
    "This calibrated-grid delta mini-grid requires at least two replicates.",
    call. = FALSE
  )
}
...
empty_target_rows <- function(
  ...
  data.frame(
    ...
    theta_parameter_count = 0L,
    theta_covariance_status = "unavailable",
    corpairs_estimate = NA_real_,
    report_estimate = NA_real_,
    max_abs_report_corpairs_delta = NA_real_,
    gradient_l2 = NA_real_,
    finite_difference_step_min = NA_real_,
    finite_difference_step_max = NA_real_,
    delta_se = NA_real_,
    lower = NA_real_,
    upper = NA_real_,
    interval_method = "wald_delta_finite_difference",
    interval_status = "delta_unavailable",
    interval_source = "finite_difference_delta",
    boundary_clamped = FALSE,
    coverage_indicator = "not_evaluable_fit_error",
    coverage_mcse = "not_computed",
    failure_rate_mcse = "not_computed",
    mcse_status = "pending_summary",
    claim_boundary = claim_boundary,
    stringsAsFactors = FALSE
  )
}

make_single_result <- function(seed, sd_scale, replicate_id, claim_boundary) {
  sim <- make_q4_data(seed = seed, sd_scale = sd_scale)
  dat <- sim$data
  tree <- sim$tree
  axis_pairs <- c(
    "mu1_mu2",
  ...
  if (length(theta_opt_pos) != 6L || length(theta_full_pos) != 6L) {
    return(
      empty_target_rows(
        ...
        elapsed = elapsed,
        warning_context = collapse_context(warnings_seen),
        failure_reason = "theta_phylo_parameter_count_not_six",
        claim_boundary = claim_boundary
      )
    )
  }
  ...
  finite_interval <- is.finite(delta_se) &
    is.finite(lower) &
    is.finite(upper) &
    lower < upper &
    delta_se > 0
  accepted_interval <- finite_interval & isTRUE(fit$sdr$pdHess)
  max_report_delta <- max(abs(report_estimate - pairs$estimate))
  ...
  data.frame(
    ...
    max_abs_report_corpairs_delta = max_report_delta,
    gradient_l2 = sqrt(rowSums(fd$gradient^2)),
    finite_difference_step_min = min(fd$step),
    finite_difference_step_max = max(fd$step),
    delta_se = delta_se,
    lower = ifelse(accepted_interval, lower, NA_real_),
    upper = ifelse(accepted_interval, upper, NA_real_),
    interval_method = "wald_delta_finite_difference",
    ...
    interval_source = "finite_difference_delta",
    boundary_clamped = accepted_interval & ((lower == -1) | (upper == 1)),
    coverage_indicator = ifelse(
      accepted_interval,
      ifelse(
        lower <= sim$true_cor & sim$true_cor <= upper,
        "delta_diagnostic_covers_true",
        "delta_diagnostic_misses_true"
      ),
      "not_evaluable_delta_unavailable"
    ),
    coverage_mcse = "not_computed",
    failure_rate_mcse = "not_computed",
    mcse_status = "pending_summary",
...
add_mini_grid_summaries <- function(results) {
  results$coverage_success <- results$coverage_indicator ==
    "delta_diagnostic_covers_true"
  results$failure_flag <- results$fit_status != "fit_ok" |
    is.na(results$pdHess) |
    results$pdHess != TRUE |
    results$interval_status != "finite_delta_diagnostic"

  for (target in unique(results$target_name)) {
    idx <- results$target_name == target
    denominator <- sum(idx)
    coverage_rate <- mean(results$coverage_success[idx])
    failure_rate <- mean(results$failure_flag[idx])
    results$coverage_mcse[idx] <- sprintf(
      "%.6f",
      sqrt(coverage_rate * (1 - coverage_rate) / denominator)
    )
    results$failure_rate_mcse[idx] <- sprintf(
      "%.6f",
      sqrt(failure_rate * (1 - failure_rate) / denominator)
    )
  }

  results$coverage_success <- NULL
  results$failure_flag <- NULL
  results$mcse_status <- "computed_mini_grid_diagnostic_not_calibrated"
  results
}

make_result_rows <- function() {
  claim_boundary <- paste(
    "Q4 derived-correlation calibrated-grid delta mini-grid diagnostic only;",
    "no q4 interval reliability, interval coverage, q4 REML, AI-REML, or broad",
    "bridge support is promoted."
  )
  ...
  for (i in seq_len(nrow(grid))) {
    seed <- seed_start + grid$replicate_index[[i]] - 1L
    scale_tag <- gsub("[.]", "", sprintf("%0.2f", grid$sd_scale[[i]]))
    replicate_id <- sprintf(
      "delta_grid_mini_sd%s_%03d",
      scale_tag,
      grid$replicate_index[[i]]
    )
    rows[[i]] <- make_single_result(
      seed = seed,
      sd_scale = grid$sd_scale[[i]],
      replicate_id = replicate_id,
      claim_boundary = claim_boundary
    )
  }
  add_mini_grid_summaries(do.call(rbind, rows))
}
