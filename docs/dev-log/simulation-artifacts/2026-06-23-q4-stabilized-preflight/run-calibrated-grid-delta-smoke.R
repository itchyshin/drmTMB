...
if (is.na(n_rep) || n_rep != 1L) {
  stop(
    "This calibrated-grid delta smoke only permits one replicate; use --n-rep=1.",
    call. = FALSE
  )
}
...
output_dir <- value_arg("output-dir", artifact_dir)
output_file <- value_arg(
  "output-file",
...
make_result_rows <- function() {
  ...
  claim_boundary <- paste(
    "Q4 derived-correlation calibrated-grid delta smoke only; no q4 interval",
  ...
  data.frame(
    ...
    delta_se = delta_se,
    lower = ifelse(finite_interval, lower, NA_real_),
    upper = ifelse(finite_interval, upper, NA_real_),
    interval_method = "wald_delta_finite_difference",
    ...
    interval_source = "finite_difference_delta",
    boundary_clamped = finite_interval & ((lower == -1) | (upper == 1)),
    coverage_indicator = "not_evaluated",
    coverage_mcse = "not_computed_single_replicate",
    failure_rate_mcse = "not_computed_single_replicate",
    mcse_status = "insufficient_replicates",
