# F2 — MSPL finiteness in the rare-prevalence regime, logit. Runner.
...
grid <- expand.grid(eta_d = c(-6, -8), G = c(30L, 100L), n_per = c(10L, 50L),
                    stringsAsFactors = FALSE)
grid$q <- "q1"
grid <- grid[order(-grid$eta_d, grid$G, grid$n_per), ]
...
for (i in seq_len(nrow(grid))) {
  g <- grid[i, ]
  for (r in RFROM:RTO) {
    seed <- 20260811 + 100000 * g$cell + r          # prereg sec 3, frozen
