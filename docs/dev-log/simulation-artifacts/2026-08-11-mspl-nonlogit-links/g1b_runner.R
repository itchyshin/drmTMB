# G1b — completion and finiteness measured SEPARATELY. Runner.
#
# Identical to g1_runner.R except: the seed prefix is 20260812 (a fresh stream,
# PREREGISTRATION-G1b.md section 4.1) and each row carries an explicit
# `completed` flag so finiteness can be scored on the completed subset without
# dropping any row.
#
...
fit_one <- function(sim, q, est, lk) {
  tryCatch({
    ...
    list(
      ...
      # Endpoint C: the call returned AND fit$mspl is populated.
      completed     = !is.null(m),
      err = NA_character_)
...
for (i in seq_len(nrow(grid))) {
  ...
  lk <- link_of(g$cond)
  for (r in RFROM:RTO) {
    seed <- 20260812 + 100000 * g$cell + r          # G1b prereg 4.1, frozen
