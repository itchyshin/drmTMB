...
download_matrix <- download.packages(pkgs, destdir = tarball_dir, repos = repo, type = "source")
downloaded <- as.data.frame(download_matrix, stringsAsFactors = FALSE)
names(downloaded) <- c("Package", "dest")
utils::write.table(
  downloaded,
