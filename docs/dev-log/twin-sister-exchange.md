...
| 2026-07-25 | `gllvmTMB` PR #785, merged at `fe8388d` | A single-core, rank-1 synthetic mixed-family two-tier ladder showed that five BFGS trajectories can dominate wall time while an exact-model warm refit is cheap. The audit found no measured native-code defect and retained all restart/convergence safeguards. | **Explicit defer:** do not create a drmTMB C++ optimisation arc from this result. If a drmTMB workflow becomes costly, first freeze its own representative timing receipt (restart elapsed time, evaluations, dimensions, cores, compatible warm start) and profile its own dominant path. | No code copied. This is workflow/measurement discipline only; it is not drmTMB speed, correctness, coverage, or likelihood evidence. No sibling comment required. |
