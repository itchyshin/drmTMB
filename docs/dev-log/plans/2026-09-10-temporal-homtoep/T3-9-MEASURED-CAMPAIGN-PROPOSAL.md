...
- **Storage:** write raw task results and a manifest under the durable DRAC
  project allocation, never `/scratch`; mirror completed immutable artifacts to
  Totoro. Do not store campaign outputs in the package working tree until the
  reverify step selects the complete retained set.
