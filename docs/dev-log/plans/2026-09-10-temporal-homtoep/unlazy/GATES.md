...
- [x] T3-12: Package build/check and independent Noether plus Pat reviews close P2 with a complete after-task report.
  CHECK: Rscript --vanilla tools/temporal-homtoep-gates.R T3-12 --reverify
  EXPECT: TEMPORAL_HOMTOEP_T3_12_PASS
  EVIDENCE: Noether and Pat independently cleared the repaired likelihood/interface on 2026-09-10; their original reviews caught and the repair resolved the stale profile-refusal test, profile/Wald diagnostic ambiguity, and internal-ID error label. The final default `R CMD build .` plus `R CMD check --no-manual` completed installation, documentation, examples, full tests, and vignette rebuilding. It retained two package-wide source-layout warnings because generated `inst/doc` outputs are excluded from the source tar; this is recorded, not treated as warning-free verification. The no-campaign reverify emitted `TEMPORAL_HOMTOEP_T3_12_PASS`. Receipts: `docs/dev-log/evidence/temporal-homtoep/2026-09-10-p2-closeout.md` and `docs/dev-log/after-task/2026-09-10-temporal-homtoep-p2-closeout.md`.
