...
| Choose Your Model | `bivariate-coscale` | Changing residual coupling with rho12 | 2 | Important residual-correlation tutorial; rendered figures lack detected source `fig.alt`. | Add or verify alt text and check `rho12` versus group-level correlation language. |
...
| Inference, Diagnostics, and Figures | `convergence` | Improving convergence | 0 | Diagnostic page; should not make Hessian warnings sound like total failure. | Align with `check_drm()` status labels and remedies. |
