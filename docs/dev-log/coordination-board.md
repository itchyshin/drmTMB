...
## Active Lane Split
- **2026-09-16 ~10:05 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `017ab34da`** (coord-only; tip-identity **green** @ `e169d7a76`, run `35111843393`). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `09d61406f`. **CI RED** @ `09d61406f`: ubuntu release shards 1–4 fail — undocumented Rd `@param` on `drm_validate_complete_predictors` (`formula`, `data`). **Gauss:** Rd/`man/` fix **in flight** (`gauss:dinnage-arc3-B2`). **D-263:** re-ACCEPT on `09d61406f` valid until tip moves; **re-ACCEPT again** after Gauss push (GitHub **Approve** blocked — same-author). **P0:** C17 inert re-cert unchanged on current tip (`mc-0568` / C17+C14 @ `c493fc0bb`). **No merge** until settled green CI. Tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. **Coord/docs push OK.** **Wave C: HOLD.**
