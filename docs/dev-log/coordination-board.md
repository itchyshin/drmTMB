...
## Active Lane Split
- **2026-09-16 ~09:35 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `ba2e8d31`** (tip-identity **green** @ `e169d7a76`, run `35111843393`; later coord-only pushes — no receipt PR). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `09d61406f` (synchronize after D-263 ACCEPT @ `c493fc0bb`). **New tip:** `09d6140` pkgdown reference-index guard (`_pkgdown.yml` only; **no** `R/` / C17 delta). **D-263 ACCEPT stale** — need fresh independent re-ACCEPT on `09d61406f` before merge. **P0:** C17 inert re-cert remains on `c493fc0bb` (`mc-0568` / C17+C14). **CI** in flight @ `09d61406f` (run `35116393006`). **Not MERGE-READY** — no merge until settled green CI + fresh D-263 ACCEPT; tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. **Coord/docs push OK.** **Wave C: HOLD.**
