...
## Active Lane Split
- **2026-09-16 ~09:38 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `f4fb5b44`** (coord-only; tip-identity **green** @ `e169d7a76`, run `35111843393`). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `09d61406f`. **Tip delta:** pkgdown reference-index guard (`_pkgdown.yml` only; **no** `R/` / C17 change vs `c493fc0bb`). **D-263:** fresh independent **re-ACCEPT** on tip (PR review comment; GitHub **Approve** blocked — same-author). **P0:** C17 inert re-cert unchanged (`mc-0568` / C17+C14 on `c493fc0bb`). **CI** in flight @ `09d61406f` (run `35116393006`). **Not MERGE-READY** until settled green CI; **merge after green** (Shinichi keep-going). Tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. **Coord/docs push OK.** **Wave C: HOLD.**
