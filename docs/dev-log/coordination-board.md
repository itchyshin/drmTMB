...
## Active Lane Split
- **2026-09-16 ~10:45 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `2d80d22de`** (coord; tip-identity **green** @ `e169d7a76`, run `35111843393`; coord-only pushes — no receipt PR). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `05c28c1bb`; **`R/bf.R` `call_names` fix** (restore names on `drm_formula$calls` after resolve; clears skeleton `drm_formula` regression). Local **47/47** reported. **D-263: ACCEPT** @ `05c28c1bb` (Shinichi; bf.R fix). **MERGE-READY pending settled green CI** — **keep-going:** merge #1375 when CI green (run [`35123724324`](https://github.com/itchyshin/drmTMB/actions/runs/35123724324) @ `05c28c1bb` **in flight**); tip-identity receipt **LAST** on `R/` before merge. **P0:** C17 inert re-cert on earlier tip (`mc-0568` / C17+C14 @ `c493fc0bb`). Prefer **Composer** / `pr_merge_when_green.sh`. **Coord/docs push OK.** **Wave C: HOLD.**
