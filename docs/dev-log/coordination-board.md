...
## Active Lane Split
- **2026-09-16 ~12:00 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `7e1d187dc`** (shipped B2 @ `4806b4839` / [#1375](https://github.com/itchyshin/drmTMB/pull/1375); PR tip `af9fc76ff`). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) MERGED** @ `4806b4839`. **`main` R-CMD-check RED** — B2 A-2 missing-data predictor-fail gate is too broad; treat **`main` as not green** until fixed. **Hotfix — [#1379](https://github.com/itchyshin/drmTMB/pull/1379) OPEN** (`hotfix/a2-missing-predictor-gate`): narrow the gate; **merge first.** **Tip-identity receipt — [#1377](https://github.com/itchyshin/drmTMB/pull/1377) OPEN** (receipt-only regen post-B2; replaces closed [#1376](https://github.com/itchyshin/drmTMB/pull/1376)): **merge after #1379** when R-CMD-check green, then clear `receipt-staleness`. Prefer **Composer** for lane slices. **Wave C: HOLD** ([`wave-b-brief.md`](lanes/dinnage-arc3-cursor/LOOP/wave-b-brief.md) § Wave C).
