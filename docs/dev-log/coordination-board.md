...
## Active Lane Split
- **2026-09-16 ~10:52 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `9c4f021fc`** (coord; tip-identity **green** @ `e169d7a76`, run `35111843393`; coord-only pushes — no receipt PR). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `1424f1971` — release-shard fixes (**REML IC**, **A-2 explicit `missing=` gating**, **census TSV**). **D-263: ACCEPT** @ `1424f1971` (Shinichi; A-2 gating + REML IC + census OK). **P2 (optional):** NEWS / `?miss_control` wording polish. **MERGE-READY pending settled green CI** on tip (`35124451699` dispatch in flight); then merge (keep-going). Tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. **Coord/docs push OK.** **Wave C: HOLD.**
