...
## Active Lane Split
- **2026-09-16 ~09:15 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `ba2e8d31`** (tip-identity **green** @ `e169d7a76`, run `35111843393`; later coord-only pushes — no receipt PR). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `c493fc0bb`; local **47/47**; **D-263: REQUEST CHANGES** (prior review — not ACCEPT). **P0:** C17 inert re-cert **pushed** (`mc-0568` / C17+C14 on `R/drmTMB.R` + `R/methods.R`). **Awaiting** settled green CI + fresh **D-263 ACCEPT**. **P2:** confint Rd cross-link, audit note, check-log (fixer). **Not MERGE-READY** — no merge until both gates pass; tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. **Coord/docs push OK.** **Wave C: HOLD.**
