...
## Active Lane Split
- **2026-09-16 ~10:12 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `f2d20fe65`** (coord-only; tip-identity **green** @ `e169d7a76`, run `35111843393`). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `19991695c`. **Rd `@param` fix pushed** (was CI RED @ `09d61406f`: undocumented `@param` on `drm_validate_complete_predictors`). **D-263:** prior **ACCEPT stale** (incl. @ `c493fc0bb` / `09d61406f`); **awaiting fresh D-263 + settled green CI**. **P0:** C17 inert re-cert on earlier tip (`mc-0568` / C17+C14 @ `c493fc0bb`). **Not MERGE-READY** — no merge until fresh D-263 and CI green. Tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. **Coord/docs push OK.** **Wave C: HOLD.**
