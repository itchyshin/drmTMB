...
## Active Lane Split
- **2026-09-16 ~11:10 MDT — Dinnage arc3 (worktree `local-scratch/lanes/drmTMB-dinnage-arc3-cursor`; [`CLAIMS.md`](lanes/dinnage-arc3-cursor/CLAIMS.md), [`LOOP/checkpoint.md`](lanes/dinnage-arc3-cursor/LOOP/checkpoint.md)).** **`main` @ `11f4a65f2`** (coord-only; tip-identity **green** @ `e169d7a76`, run `35111843393`). **Wave B1 — [#1373](https://github.com/itchyshin/drmTMB/pull/1373) MERGED** @ `f822a34da`. **Wave B2 — [#1375](https://github.com/itchyshin/drmTMB/pull/1375) OPEN** @ `78df3d4d6`. **Inert C17 recert** @ `14618e6e4` (`mc-0568` / C17+C14); **check-log merge tip** @ `78df3d4d6` (post-C17). **D-263: re-ACCEPT posted @ `78df3d4d6`** (Composer + reviewer lane; review + comments). **MERGE-READY** — CI [`35125446950`](https://github.com/itchyshin/drmTMB/actions/runs/35125446950) **green** (settled). **`tools/pr_merge_when_green.sh`** merge-commit poller armed (not squash). Tip-identity receipt **LAST** on `R/` before merge. Prefer **Composer** / Grace watchers. **Coord/docs push OK.** **Wave C: HOLD.**
