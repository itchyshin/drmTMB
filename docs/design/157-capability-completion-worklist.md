...
slant diagnostic. The support-floor diagnostic shows source-level beta shape
floors activating only in high-scale source cells; six small fitted beta,
zero-one beta, and missing-predictor cells converged with `pdHess = TRUE`;
the four fitted response-route cells that expose shape vectors stayed above
the floor; and six malformed boundary cells errored visibly. The residual
...
The Student-t Wald calibration diagnostic adds 100-replicate-per-cell interval
depth for the fixed-effect finite-variance shape route: 200 requested
Student-t fits had minimum convergence rate 0.91, minimum `pdHess` rate 0.89,
maximum `student_nu` warning rate 0.23, maximum `student_nu` error rate 0.11,
