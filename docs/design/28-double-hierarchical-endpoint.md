...
| Bivariate random-slope covariance blocks and structured spatial q=4 blocks | First slice for ordinary matching slope-only `mu1`/`mu2`; broader blocks planned | matching slope-only `mu1`/`mu2` is fitted; random-slope q=6/q=8 endpoint blocks, spatial q=4, and spatial slope correlations remain closed |
