...
| Random slopes | Implemented for ordinary Gaussian `mu` multi-slope blocks, independent Gaussian `sigma` one-slope terms, ordinary Poisson/NB2 `mu` independent numeric slopes, and coordinate spatial, phylogenetic, animal-model, and `relmat()` Gaussian `mu` one-slope fields | Tested with focused recovery, smoke, direct profile, weak-boundary, diagnostic checks, and parser-boundary checks for unsupported multiple structured slopes | Correlated non-Gaussian slopes, bivariate random slopes, multiple structured slopes, structured slope correlations, and spatial slope correlations remain planned or unsupported | Admit fitted ordinary, count-`mu`, and one-slope Gaussian structured cases |
