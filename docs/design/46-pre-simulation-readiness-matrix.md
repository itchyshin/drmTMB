...
| Non-Gaussian models | Implemented for fixed-effect count, bounded, positive-continuous, and ordinal families, ordinary non-zero-inflated Poisson and NB2 `mu` random intercepts and independent numeric slopes, ordinary zero-truncated NB2 `mu` random intercepts, ordinary lognormal/Gamma `mu` random intercepts, ordinary NB2 log-`sigma` random intercepts, and ordinary Poisson/NB2 q=1 phylogenetic `mu` intercepts | Tested for the fixed-effect paths, Phase 18 fixed-effect proportion, positive-continuous, and ordinal DGP/smoke/grid artifact lanes, Poisson/NB2 `mu` random-effect smoke surfaces, zero-truncated NB2 `mu` random-intercept focused recovery, lognormal/Gamma `mu` random-intercept focused recovery, NB2 log-`sigma` focused recovery, Wald rows, direct `log_sd_sigma` profile-target rows, a small NB2 log-`sigma` smoke-grid artifact writer, weak-SD diagnostics, Poisson phylogenetic q=1 smoke/formal infrastructure, NB2 phylogenetic q=1 smoke/boundary tests, a 288-cell NB2 q1 formal-condition sentinel, and a 24-cell x 5-replicate representative NB2 q1 audit | Non-Gaussian `sigma` random effects outside the ordinary NB2 intercept gate, zero-truncated and positive-continuous random slopes, hurdle random effects, structured slopes, spatial/animal/`relmat()` structured non-Gaussian effects, mixed-response bivariate families, and most non-Gaussian random-effect covariance remain planned or unsupported | Admit ordinary Poisson/NB2 `mu` random-effect smoke grids, the zero-truncated NB2 and positive-continuous `mu` random-intercept source-test lanes, the separate NB2 log-`sigma` random-intercept smoke lane, and the fixed-effect proportion, positive-continuous, and ordinal artifact lanes; keep Poisson/NB2 phylogenetic q=1 in separate smoke/formal-admission lanes until 500-replicate recovery evidence exists |
