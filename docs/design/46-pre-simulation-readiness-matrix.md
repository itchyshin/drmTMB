...
| `animal()` models | Known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `A` or `Ainv` | Tested with known-matrix recovery, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$animal`, `ranef("animal_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, `check_drm()` diagnostics, runnable examples, and an explicit Phase 18 ADEMP sheet | Pedigree-to-Ainv construction, structured slopes, `sigma` animal models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-design implementation; broad grids wait for a DGP helper, runner, and summariser |
| `relmat()` models | Known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `K` or `Q` | Tested with covariance/precision equivalence, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$relmat`, `ranef("relmat_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, `check_drm()` diagnostics, runnable examples, and an explicit Phase 18 ADEMP sheet | Structured slopes, `sigma` relatedness models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-design implementation; broad grids wait for a DGP helper, runner, and summariser |
