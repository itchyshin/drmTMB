...
| NB2 phylogenetic q=1 `mu` | Fitted for ordinary non-zero-inflated `phylo(1 | species, tree = tree)` on the log-mean scale with fixed-effect `sigma` overdispersion | Focused likelihood, extractor, direct `log_sd_phylo` profile-target, `ranef("phylo_mu")`, `check_drm()`, deterministic recovery smoke, malformed-neighbour boundary tests, overdispersion-aware DGP, smoke runner, CSV grid writer, optional direct profile artifacts, formal-grid spec/read-back QA, grouped-comparator rows, manual Actions dispatch, a 288-cell one-replicate formal-condition sentinel, and a 24-cell x 5-replicate representative audit | Ready for smoke/formal-admission runs only; local Slices 541-555 artifacts pass QA but return `hold_smoke_only`, and formal recovery artifacts still need the 500-replicate grid before broad Phase 18 operating-characteristic evidence |
