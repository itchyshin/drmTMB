...
| Coordinate spatial structured effects | Univariate Gaussian `mu` intercepts, one numeric coordinate-spatial slope, and constant bivariate Gaussian `mu1`/`mu2` q=2 covariance are fitted | Direct SD/correlation targets, `ranef("spatial_mu")`, `corpairs(level = "spatial")`, `summary()$covariance`, one-slope smoke evidence, q=2 dense-comparator evidence, and diagnostics exist | Mesh/SPDE, multiple slopes, spatial `sigma`, spatial q=4, direct-SD, and spatial `corpair()` regression remain planned | Admit univariate coordinate-spatial `mu` intercept and one-slope grids; keep bivariate q=2 spatial out of broad grids until an ADEMP row admits it explicitly |
