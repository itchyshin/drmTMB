...
| Structured non-Gaussian dependence | First Poisson q=1 slice only | Ordinary Poisson `phylo(1 | species, tree = tree)` in `mu` has focused extractor, profile-target, diagnostic, malformed-neighbour evidence, the ADEMP gate in `docs/design/70-phase-18-poisson-structured-q1-ademp.md`, and the runner contract in `docs/design/72-poisson-phylo-q1-runner-contract.md`; neighbouring structured count effects are still planned | Poisson q=1 is ready only for a focused smoke/recovery runner; broad non-Gaussian structured grids are not ready |
