...
| `animal()` models | Known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `A` or `Ainv` | Tested with known-matrix recovery, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$animal`, `ranef("animal_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, and `check_drm()` diagnostics | Pedigree-to-Ainv construction, structured slopes, `sigma` animal models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Admit only the known-matrix Gaussian `mu` intercept smoke cell until the bivariate q=2 animal lane has an explicit ADEMP row |
| `relmat()` models | Known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `K` or `Q` | Tested with covariance/precision equivalence, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$relmat`, `ranef("relmat_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, and `check_drm()` diagnostics | Structured slopes, `sigma` relatedness models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Admit only the known-matrix Gaussian `mu` intercept smoke cell until the bivariate q=2 `relmat()` lane has an explicit ADEMP row |
