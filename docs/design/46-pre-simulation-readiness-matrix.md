...
| Non-Gaussian models | Implemented for fixed-effect count, bounded, positive-continuous, and ordinal families, ordinary non-zero-inflated Poisson and NB2 `mu` random intercepts and independent numeric slopes, ordinary NB2 log-`sigma` random intercepts, and ordinary Poisson/NB2 q=1 phylogenetic `mu` intercepts | Tested for the fixed-effect paths, Poisson/NB2 `mu` random-effect smoke surfaces, NB2 log-`sigma` focused recovery, Wald rows, direct `log_sd_sigma` profile-target rows, a small NB2 log-`sigma` smoke-grid artifact writer, weak-SD diagnostics, Poisson phylogenetic q=1 smoke/formal infrastructure, and an NB2 phylogenetic q=1 smoke/boundary test | Non-Gaussian `sigma` random effects outside the ordinary NB2 intercept gate, structured slopes, spatial/animal/`relmat()` structured non-Gaussian effects, mixed-response bivariate families, and most non-Gaussian random-effect covariance remain planned or unsupported | Admit ordinary Poisson/NB2 `mu` random-effect smoke grids and the separate NB2 log-`sigma` random-intercept smoke lane; keep Poisson/NB2 phylogenetic q=1 in separate smoke/formal-admission lanes until recovery evidence exists |
...
| NB2 phylogenetic q=1 `mu` | Fitted for ordinary non-zero-inflated `phylo(1 | species, tree = tree)` on the log-mean scale with fixed-effect `sigma` overdispersion | Focused likelihood, extractor, direct `log_sd_phylo` profile-target, `ranef("phylo_mu")`, `check_drm()`, deterministic recovery smoke, and malformed-neighbour boundary tests | Ready for smoke/formal-admission planning only; overdispersion-aware recovery grids are still needed before broad Phase 18 operating-characteristic evidence |
...
| Structured non-Gaussian dependence | First Poisson/NB2 q=1 slices only | Ordinary Poisson and ordinary NB2 `phylo(1 | species, tree = tree)` in `mu` have focused extractor, profile-target, diagnostic, and malformed-neighbour evidence. The Poisson route also has optional profile-interval artifacts, the ADEMP gate in `docs/design/70-phase-18-poisson-structured-q1-ademp.md`, the runner contract in `docs/design/72-poisson-phylo-q1-runner-contract.md`, an opt-in smoke runner, a formal-grid spec/QA wrapper, and a manual Actions task; neighbouring structured count effects are still planned | Poisson/NB2 q=1 are ready only for focused smoke and formal-admission lanes; broad non-Gaussian structured grids are not ready |
