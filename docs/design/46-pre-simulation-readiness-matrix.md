...
| Phylogenetic models | Implemented for Gaussian `mu` intercept effects, one numeric `mu` slope, selected bivariate `mu1`/`mu2` phylogenetic location correlations, constant q=4 location-scale blocks, and direct `sd_phylo*()` paths | Tested with profile targets, direct-SD surfaces, bivariate phylogenetic covariance rows, one-slope recovery, diagnostics, examples, multiple-slope rejection boundaries, and the Ayumi q2 hard-case stress ledger | Multiple phylogenetic `mu` slopes, phylogenetic slope correlations, richer q=4 predictor-dependent `corpair()` regressions, standalone structured `sigma`-`sigma` covariance, and phylogenetic non-Gaussian effects remain planned or unsupported | Admit small controlled intercept, documented direct-SD subsets, and focused one-slope smoke cells; keep Ayumi-style row-level q2 stress cases in the failure/diagnostic ledger until start, profile, and Hessian evidence improves |
