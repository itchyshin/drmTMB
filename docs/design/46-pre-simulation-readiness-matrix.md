...
| Structured non-Gaussian dependence | First Poisson q=1 slice only | Ordinary Poisson `phylo(1 | species, tree = tree)` in `mu` has focused extractor, profile-target, diagnostic, and malformed-neighbour evidence; neighbouring structured count effects are still planned | Poisson q=1 is ready only for smoke/ADEMP planning; broad non-Gaussian structured grids are not ready |
