...
| Poisson phylogenetic q=1 `mu` | Fitted for ordinary non-zero-inflated `phylo(1 | species, tree = tree)` on the log-mean scale | Focused likelihood, extractor, direct `log_sd_phylo` profile-target, `ranef("phylo_mu")`, `check_drm()`, malformed-neighbour boundary tests, ADEMP sheet, DGP, fit summariser, smoke runner, aggregate summaries, manifests, failure ledgers, Wald fixed-effect intervals, Wald coverage, and profile-target status rows | Ready for opt-in smoke runs only; not yet broad Phase 18 operating-characteristic evidence |
...
| Structured non-Gaussian dependence | First Poisson q=1 slice only | Ordinary Poisson `phylo(1 | species, tree = tree)` in `mu` has focused extractor, profile-target, diagnostic, malformed-neighbour evidence, the ADEMP gate in `docs/design/70-phase-18-poisson-structured-q1-ademp.md`, the runner contract in `docs/design/72-poisson-phylo-q1-runner-contract.md`, and an opt-in smoke runner; neighbouring structured count effects are still planned | Poisson q=1 is ready only for focused smoke runs; broad non-Gaussian structured grids are not ready |
