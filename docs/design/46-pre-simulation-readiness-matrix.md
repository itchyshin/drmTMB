...
| Coordinate spatial structured effects | Univariate Gaussian `mu` intercepts, one numeric coordinate-spatial slope, and constant bivariate Gaussian `mu1`/`mu2` q=2 covariance are fitted | Direct SD/correlation targets, `ranef("spatial_mu")`, `corpairs(level = "spatial")`, `summary()$covariance`, one-slope smoke evidence, q=2 dense-comparator evidence, diagnostics, the spatial q=2 ADEMP sheet, q=2 DGP/smoke-runner evidence, fixed-effect Wald CSVs, and profile-status/evidence CSVs exist | Mesh/SPDE, multiple slopes, spatial `sigma`, spatial q=4, direct-SD, and spatial `corpair()` regression remain planned | Admit univariate coordinate-spatial `mu` intercept, one-slope, and constant q=2 bivariate location-covariance grid artifacts; broad q=2 reports still need larger replicate runs and interpretation |
...
| Spatial models | Implemented for coordinate spatial Gaussian `mu` intercepts, one numeric slope with independent spatial fields, and constant bivariate `mu1`/`mu2` q=2 spatial covariance | Tested with a coordinate-spatial one-slope smoke surface, direct SD targets, `ranef("spatial_mu")`, `corpairs(level = "spatial")`, q=2 profile-target labels, q=2 recovery and dense-comparator evidence, diagnostics, reader-facing boundary text, the spatial q=2 ADEMP sheet, q=2 DGP/smoke-runner evidence, fixed-effect Wald CSVs, and profile-status/evidence CSVs | Mesh/SPDE models, multiple spatial slopes, spatial slope correlations, spatial `sigma`, bivariate spatial q=4, spatial direct-SD, and spatial `corpair()` regressions remain planned | Admit univariate Gaussian coordinate-spatial `mu` intercept and one-slope grids plus the constant q=2 bivariate spatial location-covariance grid artifacts; broad q=2 reports still need larger replicate runs and interpretation |
