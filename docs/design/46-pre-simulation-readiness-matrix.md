...
| `animal()` models | Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `pedigree`, `A`, or `Ainv` | Tested with pedigree-to-additive-matrix equivalence, known-matrix recovery, dense-likelihood comparison, pedigree and matrix validation errors, `sdpars$mu`, `corpars$animal`, `ranef("animal_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, `check_drm()` diagnostics, runnable examples, an explicit Phase 18 ADEMP sheet, q=2 DGP/smoke/grid-writer artifacts, an animal-only pedigree q=2 smoke cell, fixed-effect Wald CSVs, profile-status/evidence CSVs, and an opt-in profile smoke | Sparse large-pedigree precision construction, structured slopes, `sigma` animal models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix and small dense-pedigree Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-grid artifacts; fixed-effect Wald coverage is present, while structured SD/correlation and residual `rho12` coverage require explicit profile requests |
| `relmat()` models | Known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `K` or `Q` | Tested with covariance/precision equivalence, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$relmat`, `ranef("relmat_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, `check_drm()` diagnostics, runnable examples, an explicit Phase 18 ADEMP sheet, q=2 DGP/smoke/grid-writer artifacts, fixed-effect Wald CSVs, profile-status/evidence CSVs, and an opt-in profile smoke | Structured slopes, `sigma` relatedness models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-grid artifacts; fixed-effect Wald coverage is present, while structured SD/correlation and residual `rho12` coverage require explicit profile requests |
