...
| --- | --- | --- | --- | --- |
| Fixed-effect one-response families | Family registry entries and likelihood rows in `docs/design/02-family-registry.md` and `docs/design/03-likelihoods.md` | Family-specific tests listed in `docs/design/34-validation-debt-register.md`; fixed-effect Wald rows for count, proportion, ordinal, and continuous-shape paths where recently hardened; Slices 1289-1298 add private fixed-effect `beta()`/`beta_binomial()` artifact evidence and the follow-up wires that proportion lane into first-wave summary plus manual Actions dispatch; Slices 1299-1308 add private fixed-effect `lognormal()`/`Gamma(link = "log")` artifact evidence and the follow-up wires that positive-continuous lane into first-wave summary plus manual Actions dispatch | Distribution-family tutorial and stable-core matrix separate fitted fixed-effect families from random-effect extensions | Admit named fixed-effect families; do not borrow this evidence for non-Gaussian random effects |
...
| Non-Gaussian models | Implemented for fixed-effect count, bounded, positive-continuous, and ordinal families, ordinary non-zero-inflated Poisson and NB2 `mu` random intercepts and independent numeric slopes, ordinary NB2 log-`sigma` random intercepts, and ordinary Poisson/NB2 q=1 phylogenetic `mu` intercepts | Tested for the fixed-effect paths, private fixed-effect proportion and positive-continuous artifact lanes, Poisson/NB2 `mu` random-effect smoke surfaces, NB2 log-`sigma` focused recovery, Wald rows, direct `log_sd_sigma` profile-target rows, a small NB2 log-`sigma` smoke-grid artifact writer, weak-SD diagnostics, Poisson phylogenetic q=1 smoke/formal infrastructure, NB2 phylogenetic q=1 smoke/boundary tests, a 288-cell NB2 q1 formal-condition sentinel, and a 24-cell x 5-replicate representative NB2 q1 audit | Non-Gaussian `sigma` random effects outside the ordinary NB2 intercept gate, structured slopes, spatial/animal/`relmat()` structured non-Gaussian effects, mixed-response bivariate families, and most non-Gaussian random-effect covariance remain planned or unsupported | Admit ordinary Poisson/NB2 `mu` random-effect smoke grids and the separate NB2 log-`sigma` random-intercept smoke lane; keep Poisson/NB2 phylogenetic q=1 in separate smoke/formal-admission lanes until 500-replicate recovery evidence exists |
