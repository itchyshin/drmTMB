...
| `animal()` models | Known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `A` or `Ainv` | Tested with known-matrix recovery, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$animal`, `ranef("animal_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, `check_drm()` diagnostics, runnable examples, an explicit Phase 18 ADEMP sheet, first q=2 DGP/smoke/grid-writer artifacts, and an interval-status plan | Pedigree-to-Ainv construction, structured slopes, `sigma` animal models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-grid artifacts; interval coverage waits for opt-in profile evidence and interval-status artifacts |
| `relmat()` models | Known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `K` or `Q` | Tested with covariance/precision equivalence, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$relmat`, `ranef("relmat_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, `check_drm()` diagnostics, runnable examples, an explicit Phase 18 ADEMP sheet, first q=2 DGP/smoke/grid-writer artifacts, and an interval-status plan | Structured slopes, `sigma` relatedness models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-grid artifacts; interval coverage waits for opt-in profile evidence and interval-status artifacts |
