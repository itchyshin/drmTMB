...
| Coordinate spatial Gaussian `mu` | Fitted for intercept, one numeric slope, and constant bivariate `mu1`/`mu2` q=2 location covariance | Spatial one-slope smoke surface, direct SD targets, spatial `corpairs()` profile target, q=2 dense covariance comparator, diagnostics, `docs/design/56-phase-18-spatial-q2-ademp.md`, and `phase18_run_spatial_q2_smoke()` | Ready for focused univariate Gaussian `mu` grids and a constant q=2 bivariate spatial location-covariance smoke grid; broad q=2 reports still need CSV artifacts and interval-status tables |
