...
| Coordinate spatial structured effects | Univariate Gaussian `mu` intercepts and one numeric coordinate-spatial slope are fitted | Direct SD targets, `ranef("spatial_mu")`, one-slope smoke evidence, and diagnostics exist | Docs now put coordinate-spatial q=2 bivariate `mu1`/`mu2` covariance as the next parity gate; mesh/SPDE, multiple slopes, spatial `sigma`, spatial q=4, direct-SD, and spatial `corpair()` remain planned | Admit univariate coordinate-spatial `mu` intercept and one-slope grids only |
...
| Spatial models | Implemented for coordinate spatial Gaussian `mu` intercepts and one numeric slope with independent spatial fields | Tested with a coordinate-spatial one-slope smoke surface, direct SD targets, `ranef("spatial_mu")`, diagnostics, and reader-facing boundary text | The next parity gate is coordinate-spatial q=2 bivariate `mu1`/`mu2` covariance with `corpairs(level = "spatial")`, profile-target labels, and recovery evidence; mesh/SPDE models, multiple spatial slopes, spatial slope correlations, spatial `sigma`, bivariate spatial q=4, spatial direct-SD, and spatial `corpair()` regressions remain planned | Admit only univariate Gaussian coordinate-spatial `mu` intercept and one-slope grids |
...
| Coordinate spatial Gaussian `mu` | Fitted for intercept and one numeric slope | Spatial one-slope smoke surface, direct SD targets, and diagnostics | Ready for focused univariate Gaussian `mu` grids; bivariate spatial q=2 location covariance is the next implementation gate, not a Phase 18 admitted surface yet |
