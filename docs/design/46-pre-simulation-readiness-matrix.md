...
| Non-Gaussian models | Implemented for fixed-effect count, bounded, positive-continuous, and ordinal families, ordinary non-zero-inflated Poisson and NB2 `mu` random intercepts and independent numeric slopes, and the first ordinary Poisson q=1 phylogenetic `mu` intercept | Tested for the fixed-effect paths and for Poisson/NB2 `mu` random-effect smoke surfaces, Wald rows, profile SD rows, weak-SD diagnostics, and a Poisson phylogenetic q=1 smoke/boundary test | Non-Gaussian `sigma` random effects, NB2/spatial/animal/`relmat()` structured non-Gaussian effects, mixed-response bivariate families, and most non-Gaussian random-effect covariance remain planned or unsupported | Admit only the ordinary Poisson and NB2 `mu` random-effect smoke grids for non-Gaussian mixed models; keep Poisson phylogenetic q=1 in a separate smoke/ADEMP-planning lane until recovery evidence exists |
...
| Phylogenetic models | Implemented for Gaussian `mu` and `sigma` intercept effects, matching univariate `mu`/`sigma` structured correlations, one numeric `mu` slope, selected bivariate `mu1`/`mu2` phylogenetic location correlations, constant q=4 location-scale blocks, direct `sd_phylo*()` paths, and ordinary Poisson q=1 phylogenetic `mu` intercepts | Tested with profile targets, direct-SD surfaces, bivariate phylogenetic covariance rows, univariate `sigma` smoke tests, one-slope recovery, diagnostics, examples, multiple-slope rejection boundaries, the Ayumi q2 hard-case stress ledger, and Poisson q=1 smoke, boundary, grid-writer, optional direct profile-interval, formal-grid QA, and Actions dry-run tests | Multiple phylogenetic `mu` slopes, residual-scale structured slopes, Poisson phylogenetic slopes, NB2 or zero-inflated phylogenetic effects, phylogenetic slope correlations, richer q=4 predictor-dependent `corpair()` regressions, direct-SD formulas combined with structured `sigma`, and broad phylogenetic non-Gaussian effects remain planned or unsupported | Admit small controlled Gaussian intercept and one-slope smoke cells; keep structured `sigma` intercepts as smoke-tested first slices until recovery grids land; keep Poisson q=1 in a separate smoke/formal-admission lane until formal recovery evidence exists; keep Ayumi-style row-level q2 stress cases in the failure/diagnostic ledger until start, profile, and Hessian evidence improves |
...
| Structured non-Gaussian dependence | First Poisson q=1 slice only | Ordinary Poisson `phylo(1 | species, tree = tree)` in `mu` has focused extractor, profile-target, optional profile-interval, diagnostic, malformed-neighbour evidence, the ADEMP gate in `docs/design/70-phase-18-poisson-structured-q1-ademp.md`, the runner contract in `docs/design/72-poisson-phylo-q1-runner-contract.md`, an opt-in smoke runner, a formal-grid spec/QA wrapper, and a manual Actions task; neighbouring structured count effects are still planned | Poisson q=1 is ready only for focused smoke and formal-admission runs; broad non-Gaussian structured grids are not ready |
