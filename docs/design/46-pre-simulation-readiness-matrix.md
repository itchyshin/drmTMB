...
| Non-Gaussian models | Implemented for fixed-effect count, bounded, positive-continuous, and ordinal families, ordinary non-zero-inflated Poisson and NB2 `mu` random intercepts and independent numeric slopes, ordinary NB2 log-`sigma` random intercepts, and ordinary Poisson/NB2 q=1 phylogenetic `mu` intercepts | Tested for the fixed-effect paths, Poisson/NB2 `mu` random-effect smoke surfaces, NB2 log-`sigma` focused recovery, Wald rows, profile SD rows, weak-SD diagnostics, Poisson phylogenetic q=1 smoke/formal infrastructure, and an NB2 phylogenetic q=1 smoke/boundary test | Non-Gaussian `sigma` random effects outside the ordinary NB2 intercept gate, structured slopes, spatial/animal/`relmat()` structured non-Gaussian effects, mixed-response bivariate families, and most non-Gaussian random-effect covariance remain planned or unsupported | Admit only the ordinary Poisson and NB2 `mu` random-effect smoke grids for broad non-Gaussian mixed models; keep NB2 log-`sigma` and Poisson/NB2 phylogenetic q=1 in separate smoke/formal-admission lanes until recovery evidence exists |
