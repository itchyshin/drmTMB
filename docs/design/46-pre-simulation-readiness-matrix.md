...
| `animal()` and `relmat()` | Fitted for animal pedigree or known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance; one-slope markers remain parser/planned only | Pedigree-to-additive-matrix equivalence, known-matrix recovery, bivariate dense-likelihood comparison, validation-error tests, extractor/profile/diagnostic rows, runnable examples, an explicit q=2 ADEMP sheet, q=2 smoke-grid artifacts including the animal-only pedigree spelling, fixed-effect Wald interval CSVs, profile-status/evidence CSVs, an opt-in profile smoke, ASReml efficiency note, structured-slope parity gate, and reader-facing boundaries | Ready for known-matrix and small dense-pedigree Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-grid artifacts; fixed-effect Wald coverage is present, while structured SD/correlation and residual `rho12` coverage require explicit profile requests |
