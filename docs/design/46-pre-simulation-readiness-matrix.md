...
| Coordinate spatial structured effects | Univariate Gaussian `mu` intercepts, one numeric coordinate-spatial slope, constant bivariate Gaussian `mu1`/`mu2` q=2 covariance, and constant q=4 location-scale covariance are fitted | Direct SD/correlation targets, `ranef("spatial_mu")`, `corpairs(level = "spatial")`, `summary()$covariance`, one-slope smoke evidence, q=2 dense-comparator evidence, q=4 extractor/diagnostic tests, diagnostics, the spatial q=2 ADEMP sheet, q=2 DGP/smoke-runner evidence, fixed-effect Wald CSVs, and profile-status/evidence CSVs exist | Mesh/SPDE, multiple slopes, standalone spatial `sigma`, direct-SD, spatial `corpair()` regression, and non-Gaussian spatial effects remain planned | Admit univariate coordinate-spatial `mu` intercept, one-slope, and constant q=2 bivariate location-covariance grids; treat constant q=4 location-scale as fitted smoke/artifact evidence until q=4 recovery and convergence evidence exist |
...
| Phylogenetic models | Implemented for Gaussian `mu` intercept effects, one numeric `mu` slope, selected bivariate `mu1`/`mu2` phylogenetic location correlations, constant q=4 location-scale blocks, direct `sd_phylo*()` paths, and ordinary Poisson q=1 phylogenetic `mu` intercepts | Tested with profile targets, direct-SD surfaces, bivariate phylogenetic covariance rows, one-slope recovery, diagnostics, examples, multiple-slope rejection boundaries, the Ayumi q2 hard-case stress ledger, and a Poisson q=1 smoke/boundary test | Multiple phylogenetic `mu` slopes, Poisson phylogenetic slopes, NB2 or zero-inflated phylogenetic effects, phylogenetic slope correlations, richer q=4 predictor-dependent `corpair()` regressions, standalone structured `sigma`-`sigma` covariance, and broad phylogenetic non-Gaussian effects remain planned or unsupported | Admit small controlled Gaussian intercept, documented direct-SD subsets, and focused one-slope smoke cells; keep Poisson q=1 in a separate smoke/ADEMP-planning lane until recovery evidence exists; keep Ayumi-style row-level q2 stress cases in the failure/diagnostic ledger until start, profile, and Hessian evidence improves |
| Spatial models | Implemented for coordinate spatial Gaussian `mu` intercepts, one numeric slope with independent spatial fields, constant bivariate `mu1`/`mu2` q=2 spatial covariance, and constant q=4 location-scale covariance | Tested with a coordinate-spatial one-slope smoke surface, direct SD targets, `ranef("spatial_mu")`, `corpairs(level = "spatial")`, q=2 and q=4 profile-target labels, q=2 recovery and dense-comparator evidence, q=4 extractor/diagnostic tests, diagnostics, reader-facing boundary text, the spatial q=2 ADEMP sheet, q=2 DGP/smoke-runner evidence, fixed-effect Wald CSVs, and profile-status/evidence CSVs | Mesh/SPDE models, multiple spatial slopes, spatial slope correlations, standalone spatial `sigma`, spatial direct-SD, spatial `corpair()` regressions, and non-Gaussian spatial effects remain planned | Admit univariate Gaussian coordinate-spatial `mu` intercept and one-slope grids plus constant q=2 bivariate spatial location-covariance; keep q=4 as smoke/artifact evidence until q=4 recovery, convergence, and interval-status artifacts land |
| `animal()` models | Gaussian `mu` intercepts and one numeric `mu` slope, matching bivariate `mu1`/`mu2` q=2 location covariance, and constant all-four q=4 location-scale covariance are fitted through `pedigree`, `A`, or `Ainv` | Tested with pedigree-to-additive-matrix equivalence, known-matrix recovery, one-slope recovery, dense-likelihood comparison, pedigree and matrix validation errors, `sdpars$mu`, `corpars$animal`, `ranef("animal_mu")`, `corpairs()`, `summary()$covariance`, direct q=2 and derived q=4 profile-target status, `check_drm()` diagnostics, runnable examples, an explicit Phase 18 ADEMP sheet, q=2 DGP/smoke/grid-writer artifacts, an animal-only pedigree q=2 smoke cell, fixed-effect Wald CSVs, profile-status/evidence CSVs, opt-in profile smoke, q=4 DGP/smoke/grid-writer artifacts, derived-unavailable q=4 interval-status rows, and focused q=4 smoke tests | Sparse large-pedigree precision construction, multiple structured slopes, slope correlations, standalone `sigma` animal models, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix and small dense-pedigree Gaussian `mu` intercept/slope smoke cells, q=2 bivariate smoke-grid artifacts, and focused q=4 smoke artifacts; fixed-effect Wald coverage is present for q=2, while q=4 defaults to point-estimate smoke with derived structured-correlation intervals unavailable |
| `relmat()` models | Known-matrix Gaussian `mu` intercepts and one numeric `mu` slope, matching bivariate `mu1`/`mu2` q=2 location covariance, and constant all-four q=4 location-scale covariance are fitted through `K` or `Q` | Tested with covariance/precision equivalence, one-slope recovery, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$relmat`, `ranef("relmat_mu")`, `corpairs()`, `summary()$covariance`, direct q=2 and derived q=4 profile-target status, `check_drm()` diagnostics, runnable examples, an explicit Phase 18 ADEMP sheet, q=2 DGP/smoke/grid-writer artifacts, fixed-effect Wald CSVs, profile-status/evidence CSVs, opt-in profile smoke, q=4 DGP/smoke/grid-writer artifacts, derived-unavailable q=4 interval-status rows, and focused q=4 smoke tests | Multiple structured slopes, slope correlations, standalone `sigma` relatedness models, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix Gaussian `mu` intercept/slope smoke cells, q=2 bivariate smoke-grid artifacts, and focused q=4 smoke artifacts; fixed-effect Wald coverage is present for q=2, while q=4 defaults to point-estimate smoke with derived structured-correlation intervals unavailable |
