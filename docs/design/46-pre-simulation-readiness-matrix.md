...
| `animal()` models | Known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance are fitted through `A` or `Ainv` | Tested with known-matrix recovery, dense-likelihood comparison, matrix validation errors, `sdpars$mu`, `corpars$animal`, `ranef("animal_mu")`, `corpairs()`, `summary()$covariance`, direct profile targets, `check_drm()` diagnostics, runnable examples, an explicit Phase 18 ADEMP sheet, q=2 DGP/smoke/grid-writer artifacts, fixed-effect Wald CSVs, profile-status/evidence CSVs, and an opt-in profile smoke | Pedigree-to-Ainv construction, structured slopes, `sigma` animal models, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain planned | Ready for known-matrix Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-grid artifacts; fixed-effect Wald coverage is present, while structured SD/correlation and residual `rho12` coverage require explicit profile requests |
...
| `animal()` and `relmat()` | Fitted for known-matrix Gaussian `mu` intercepts and matching bivariate `mu1`/`mu2` q=2 location covariance; one-slope markers remain parser/planned only | Known-matrix recovery, bivariate dense-likelihood comparison, validation-error tests, extractor/profile/diagnostic rows, runnable examples, an explicit q=2 ADEMP sheet, q=2 smoke-grid artifacts, fixed-effect Wald interval CSVs, profile-status/evidence CSVs, an opt-in profile smoke, ASReml efficiency note, structured-slope parity gate, and reader-facing boundaries | Ready for known-matrix Gaussian `mu` intercept smoke cells and q=2 bivariate smoke-grid artifacts; fixed-effect Wald coverage is present, while structured SD/correlation and residual `rho12` coverage require explicit profile requests |
