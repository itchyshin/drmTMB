...
| --- | --- | --- | --- | --- |
| Fixed-effect one-response families | Family registry entries and likelihood rows in `docs/design/02-family-registry.md` and `docs/design/03-likelihoods.md` | Family-specific tests listed in `docs/design/34-validation-debt-register.md`; fixed-effect Wald rows for count, proportion, ordinal, and continuous-shape paths where recently hardened; Slices 1289-1298 add private fixed-effect `beta()`/`beta_binomial()` artifact evidence | Distribution-family tutorial and stable-core matrix separate fitted fixed-effect families from random-effect extensions | Admit named fixed-effect families; do not borrow this evidence for non-Gaussian random effects |
