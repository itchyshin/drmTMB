...
| Bivariate Gaussian and residual `rho12` | Ready for matching slope-only `mu1`/`mu2`; q4 and q6 location blocks have smoke artifact routing, while p8/q8 stays closed | Ready for selected `mu1`/`mu2` and location-scale subsets; hard real-data q2/q4 cases remain diagnostic | Ready for constant q=2 location and constant q=4 location-scale smoke/artifact subsets | Ready for q=2 smoke artifacts and q=4 point-estimate smoke artifacts | Ready for q=2 smoke artifacts and q=4 point-estimate smoke artifacts | Ready for residual `rho12`, selected group/structured q=2 `corpairs()` rows, slope-only `mu1`/`mu2`, and q4/q6 location smoke; the q2 scale-intercept block now has a recovery lane reporting bias, MCSE, and fixed-effect Wald coverage, while broader coverage and power still need deliberately sized pilots | q > 2 rows are visible point estimates with explicit interval limits unless direct targets exist | Use `correlation_block_status` to artifact the residual, q=2, and q=4 status boundary without promoting q=4 intervals |
