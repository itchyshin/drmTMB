...
The first-wave summary runner now stages the proportion fixed-effect grid beside
the Gaussian location-scale, `meta_V(V = V)`, paired Poisson/NB2 `mu` random
effect, Gaussian random-slope, and coordinate-spatial `mu` slope surfaces. The
manual GitHub Actions runner also accepts `task = "proportion_fixed_effect"`.

This does not change the fitted likelihood surface. It gives the already-fitted
fixed-effect beta and beta-binomial routes the same artifact discipline that
the first-wave count route already had.
