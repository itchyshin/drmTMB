# 224 — AGHQ + Cox–Reid-style observed-information profile: symbolic ↔ implementation alignment

**Status:** implemented design record. O3 is implemented in `R/aghq-coxreid.R` and exercised by
`tests/testthat/test-aghq-coxreid.R`; the historical S1 plan and review material below is retained as
provenance. This was slice **S1** of the approved arc
...
**One-line purpose.** Record two random-effect integration levers for non-Gaussian families: the
binomial joint-Laplace restricted likelihood (O2), and the package-specific nested AGHQ +
Cox–Reid-style observed-information profile (O3). O3 is not asserted to be generic non-Gaussian
REML: the formal Cox–Reid construction requires an orthogonal interest/nuisance parameterization,
which this record does not establish. The implementation supports the documented point-estimation
and interval workflow; it does not change any coverage tier or authorize a new campaign.
...
Outer (O3 profile): ℓ_R(σ) = min_{β,θ} [ −log L_AGHQ(β,θ,σ) + ½ log|I_{(β,θ)}(β,θ,σ)| ]   (adjust fixed effects)
...
`(β, θ)`. This is a Cox–Reid-style observed-information adjustment, not a claim that this
parameterization satisfies the orthogonality condition of Cox & Reid (1987). It is precisely what
`scratchpad/cumlogit_reml_scoping.R`
...
node placement, (b) sums the conditional leaf over nodes, (c) applies the O3 observed-information adjustment over
...
**4.5 AGHQ symbols are implemented externally, not in the compiled TMB DLL.**
`R/aghq-coxreid.R` implements GH nodes/weights (`drm_o3_gh()`), the per-cluster conditional leaf
(`drm_o3_cluster_ll()`), adaptive modes and curvatures (`drm_o3_mode()`), and the marginal integral
(`drm_o3_marginal_ll()`). Its private `nodes` argument belongs to `drm_o3_fit()`, not to the
public `drmTMB()` API. The core TMB likelihood still accumulates its joint NLL; O3 intentionally
performs the nested integration outside the DLL.
...
| `½log|S|` | joint-Laplace determinant contribution | emergent from joint Laplace `−½log|H_joint|` | = glmmTMB REML; differs from the marginal-profile construction (§4.1) |
...
| O3 inner | AGHQ marginal over `u` | `R/aghq-coxreid.R:drm_o3_marginal_ll()` | rolled ref `cumlogit_reml_scoping.R:98-111` |
| O3 outer | observed-information adjustment over `(β,θ)` | `R/aghq-coxreid.R:drm_o3_cr_negll()` via `optimHess` | scale-matched to `θ₀+log-gap`; Cox–Reid-style only |
...
### 5.4 AGHQ integrator — implemented externally (both families)

| Symbol | Meaning | Code mapping | Note |
|---|---|---|---|
| `nodes` | node count | private `drm_o3_fit()` argument | deliberately not a `drmTMB()` argument |
| `û_m` | per-cluster Laplace mode | `drm_o3_mode()$uhat` | adaptive recentre point |
| `z_k,w_k` | GH nodes/weights | `drm_o3_gh()` | Golub–Welsch construction in R |
| adaptive sum | `√2σ̂ Σ w_k e^{z_k²}φ(node)f(node)` | `drm_o3_marginal_ll()` | `nodes=1` gives the adaptive Laplace identity (§4.4) |
| node comparison | private `nodes` control | `drm_o3_fit()` | a numerical diagnostic, not a public capability claim |

---

## 6. Historical build plan (completed portions retained for provenance)
...
3. **S4b + S5 + S6 (the O3 core):** the external **nested** estimator (§2) was implemented in
   `R/aghq-coxreid.R`: AGHQ inner plus O3 observed-information adjustment, with the adaptive
   Jacobian (§4.4). Its deterministic checks remain distinct from a coverage campaign.
4. **S6b (interval):** `drm_o3_profile_ci()` profiles `ℓ_R` on `log σ`; the boundary convention and
   evidence limits remain those in §4.7. No coverage strength is borrowed from the point-bias ladder.
...
**AGHQ integrator — IMPLEMENTED + deterministically checked (2026-07-18), `R/aghq-coxreid.R`.** Adaptive-GH marginal
...
**Ordinal O3 (nested) — IMPLEMENTED + deterministically checked (2026-07-18), `R/aghq-coxreid.R`.** In
...
Cox, D. R., & Reid, N. (1987). Parameter orthogonality and approximate conditional inference.
*Journal of the Royal Statistical Society: Series B*, 49(1), 1–39.
<https://doi.org/10.1111/j.2517-6161.1987.tb01422.x>

Jiang, J. (1996). REML estimation: asymptotic behavior and related topics. *The Annals of Statistics*,
24(1), 255–286. <https://doi.org/10.1214/aos/1033066209>

Liu, Q., & Pierce, D. A. (1994). A note on Gauss–Hermite quadrature. *Biometrika*, 81(3), 624–629.
<https://doi.org/10.1093/biomet/81.3.624>
