...
| `biv_lognormal()` | `mu1`, `mu2` identity on `log(y)`; `sigma1`, `sigma2` log; `rho12` guarded atanh | `rho12` is within-row residual correlation on the log-response scale | Fixed-effect `mu1`/`mu2` formulas with intercept-only `sigma1`, `sigma2`, and `rho12`; complete, finite, strictly positive pairs, with implicit likelihood weights of one and no `weights` argument. Sigma/rho predictors, all random or structured effects, incomplete pairs, offsets, `meta_V`, `mi()`, REML, Julia, intervals, coverage, and capability promotion are deferred. | Exact transformed-scale likelihood oracle, product-margin, swap, response-scale fitted, simulation, and error-contract tests in `tests/testthat/test-biv-lognormal.R`; no recovery or inference claim. |
