...
| `lognormal()` | `mu` identity on `log(y)`; `sigma` log | none | Fixed effects plus separate ordinary gates for `mu` random intercepts/independent numeric slopes or one `sigma` random intercept. The exact sigma-intercept ledger domain is inference-ready with caveats. Arc 3a also fits one unlabelled q1 `phylo()` or `relmat()` intercept using `K` or `Q` in `mu`; its recovery certification is pending. Combining `mu` and `sigma` random effects, structured slopes or labels, `sigma` structure, other providers, and bivariate or mixed lognormal models remain unsupported or planned. | `tests/testthat/test-lognormal-location-scale.R`; `tests/testthat/test-nongaussian-mu-random-slopes.R`; `tests/testthat/test-arc2c-sigma-random-intercept.R`; `tests/testthat/test-positive-continuous-structured-mu.R`; random-effect recovery and Arc 4a coverage evidence. |
| `Gamma(link = "log")` | `mu` log; `sigma` log | no public `nu`; internal shape is `1 / sigma^2` | Fixed effects plus separate ordinary gates for `mu` random intercepts/independent numeric slopes or one `sigma` random intercept at recovery grade. The pre-existing unlabelled `relmat()` intercept or one-slope `mu` route remains recovery grade; Arc 3a also fits one unlabelled q1 `phylo()` intercept in `mu`, with recovery certification pending. Combining `mu` and `sigma` random effects, new-route slopes, `sigma` structure, labels, other structured providers, and bivariate or mixed Gamma models remain unsupported or planned. | `tests/testthat/test-gamma-location-scale.R`; `tests/testthat/test-nongaussian-mu-random-slopes.R`; `tests/testthat/test-arc2c-sigma-random-intercept.R`; `tests/testthat/test-positive-continuous-structured-mu.R`; random-effect recovery tests. |
...
one unlabelled q1 `phylo()` or `relmat(K/Q)` intercept in `mu` fits, with
...
intercept are exact fitted structured exceptions; Arc 3a recovery
