...
`zoi ~ x + (0 + x | id)` q1 slope, and `coi ~ 1 + (1 | id)` q1 intercept;
all three are point-fit-only. Other `zi`, `hu`, `zoi`, and `coi` random effects remain outside the fitted surface and receive
...
`zoi ~ x + (0 + x | id)`, and `coi ~ 1 + (1 | id)` q1 routes are
point-fit-only. The `coi` intercept has population-level recovery at `M = 64`
with 50 observations per group; sparse observed atoms can weaken individual
conditional modes. Correlated or labelled `mu` slopes, `coi` slopes, other `zoi` random effects,
