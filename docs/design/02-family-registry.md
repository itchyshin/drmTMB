...
| `bernoulli()` / `binomial()` | planned `mu` logit | none; no modelled `sigma` in the first dispersionless route | Planned in `drmTMB#569`. The first slice should decide accepted response encodings, then fit fixed-effect logit-`mu` models only. Ordinary random intercepts, independent slopes, structured effects, bivariate/mixed-response models, and R-Julia bridge claims wait for separate evidence. | Planned. Do not infer support from `beta_binomial()` or from the missing-predictor `impute_model(..., family = binomial())` route. |
