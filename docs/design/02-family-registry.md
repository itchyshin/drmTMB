...
| `zero_one_beta()` | `mu` logit; `sigma` log; `zoi` logit; `coi` logit | no public `nu`; interior precision is `phi = 1 / sigma^2`; `zoi`/`coi` describe exact-boundary mass | Fixed effects plus ordinary unlabelled `mu` random intercepts and independent numeric slopes for continuous `[0, 1]` responses with exact structural zeroes or ones. Exact `mc-0575` is inference-ready with caveats for true SD 0.50 and M>=16; a strictly-interior-generator rerun is required before claiming exactly 15% observed boundary mass. Correlated or labelled `mu` slopes, `sigma`/`zoi`/`coi` random effects, structured effects, known covariance, denominator syntax, and bivariate or mixed bounded-response models remain planned. | Mixture/recovery/boundary tests plus Arc 4c 1,200-attempt-per-M profile coverage and executed-DGP endpoint diagnostics. |
