...
| Gaussian residual-scale random effects | Historical Phase 6c state: the location-scale and scale articles named residual-scale random intercepts and independent `sigma ~ z + (0 + w | id)` slopes on `log(sigma)`. Current 0.6.0 also fits unlabelled ordinary correlated intercept-slope and multi-slope blocks. | Labelled residual-scale slope covariance and cross-formula `mu`-`sigma` slope covariance remain planned. |
...
| Ordinary bivariate slope rows | The bivariate coscale article now includes the first location slope-slope covariance slice, the first same-response q2 `mu`/`sigma` slope slice, and the first q2 `sigma1`/`sigma2` scale-slope slice with purpose, equations, syntax, output interpretation, diagnostics, and direct-target guidance. The matching q4 and q6 location blocks now have smoke artifact lanes, and q8 now has a fitted all-endpoint route with diagnostic smoke/recovery artifacts and a 2026-06-07 diagnostic hold audit. | A fuller simulated plasticity-syndrome worked example remains future work; formal recovery, coverage, and power at broad grid scale plus q8 coverage/power evidence remain planned. |
| Selected non-Gaussian ordinary `mu` slopes | The model map and implementation map point readers to ordinary recovery-grade non-Gaussian `mu` random intercepts and independent numeric slopes for every fitted univariate family, plus the experimental complete-data unlabelled binomial q2 point-fit route and exact row-specific structured gates. | Correlated non-Gaussian slopes outside the exact binomial q2 route, labelled covariance, structured non-Gaussian slopes beyond the exact recorded cells, and non-Gaussian `sigma`, shape, inflation, hurdle, or ordinal random effects outside their exact gates remain planned or blocked. |
| Structured Gaussian one-slope routes | Historical Phase 6c state: the model map named the first Gaussian one-slope `mu` route for `phylo()`, coordinate `spatial()`, `animal()`, and `relmat()`. Current 0.6.0 also fits q1 `sigma` one-slope paths for all four providers; phylo, A-matrix animal, and K/Q relmat are inference-ready with caveats, while spatial remains point-fit/extractor only. | Multiple or labelled structured slopes, structured slope correlations, spatial sigma-slope intervals, structured `rho12`, sparse large-pedigree speed claims, mesh/SPDE slopes, and non-Gaussian structured slopes beyond exact fitted cells remain planned. |
