...
| Ordinary bivariate slope-only rows | The bivariate coscale article now includes the first slope-slope covariance slice with purpose, equations, syntax, output interpretation, diagnostics, and direct-target guidance. | A fuller simulated plasticity-syndrome worked example remains future work; intercept-plus-slope q4 and p8/q8 endpoint covariance remain planned. |
