...
| Ordinary bivariate slope rows | The bivariate coscale article now includes the first slope-slope covariance slice with purpose, equations, syntax, output interpretation, diagnostics, and direct-target guidance. The matching q4 location block now has a smoke artifact lane, and the matching q6 location block is source-tested. | A fuller simulated plasticity-syndrome worked example remains future work; q6 artifact routing, residual-scale slope, same-response location-scale slope covariance, and p8/q8 endpoint covariance remain planned. |
