...
| Selected non-Gaussian ordinary `mu` slopes | The model map and implementation map point readers to ordinary recovery-grade non-Gaussian `mu` random intercepts and independent numeric slopes for every fitted univariate family, plus exact row-specific structured gates. | Correlated non-Gaussian slopes, labelled covariance, structured non-Gaussian slopes beyond the exact recorded cells, and non-Gaussian `sigma`, shape, inflation, hurdle, or ordinal random effects outside their exact gates remain planned or blocked. |
