...
| Intervals and targets | `confint()` exposes fitted fixed-effect `mu` rows as Wald intervals; explicit `confint(..., parm = "ordinal:cutpoint:<label>", method = "profile")` profiles one ordered latent-logistic cutpoint, while raw `theta_ord` rows remain internal diagnostics. | Cutpoint profiles are pointwise implementation evidence only: their calibration, ordinal-scale intervals, transformed category-probability intervals, and broad random-effect interval promotion remain later work. |
