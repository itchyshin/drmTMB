...
| `y ~ x1 + (1 | id)`, `family = poisson(link = "log")` | Implemented first slice | Ordinary Poisson `mu` random intercept; the group effect enters the log-mean predictor. Poisson random slopes, labelled covariance blocks, zero-inflated Poisson random effects, and cross-parameter covariance remain planned. |
...
| `zi`, `hu`, `zoi`, `coi` | Fixed effects first; random effects later only for high-value use cases. A Poisson `mu` random intercept can be fitted only without a `zi` formula in the first non-Gaussian slice. For percentage or proportion data, `zoi` and `coi` are planned for zero-one-inflated bounded-response families before random effects or covariance among these parameters are opened. |
