...
| `y ~ x1 + (1 + x1 | id)`, `family = poisson(link = "log")` | Implemented ML-Laplace point-fit recovery | One complete-data ordinary Poisson log-mean model estimates an unlabelled intercept-slope covariance block. The recovered estimands are `sd0`, `sd1`, and group-level `rho_re` (never residual `rho12`) under the design-17 map `ρ = 0.999999 tanh(η)`. Ledger cell `mc-0718` is `point_fit_recovery` for this exact route; it is not the independent-slope cell `mc-0431` and not Wave 1 binomial `mc-0717`. NB2, REML, missing-response, labels, mixed `(1 \| g) + (1 + x \| g)`, structured ordinary correlated, intervals, coverage, and `supported` remain unsupported. |
