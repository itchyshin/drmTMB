...
| `y ~ x1`, `sigma ~ x1`, `nu ~ x2`, `family = skew_normal()` | Implemented first slice | Fixed-effect univariate skew-normal location-scale-shape model. Public `mu` is `E[y]`, public `sigma` is `SD[y]`, and `nu` is residual slant. Random effects, structured effects, known sampling covariance, bivariate skew-normal models, `rho12`, and latent `skew(id)` syntax remain planned. |
...
| `nu`; future `tau` | Fixed effects first. Student-t and skew-normal `nu` fixed-effect formulas are implemented inside their fitted family routes. Random-effect bar terms in `nu` fail with a shape-specific boundary; future `nu`/`tau` random effects and ID-level skewness such as `skew(id) ~ x` need separate likelihood recovery and simulations before fitting. `tau` is reserved for a possible second shape parameter and is not current syntax. |
