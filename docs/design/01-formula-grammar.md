...
| `count ~ x`, `sigma ~ phylo_interaction(1 \| plant:pollinator, tree1 = plant_tree, tree2 = pollinator_tree)`, `zi ~ 1`, `family = nbinom2()` | Exact code-admission gate | Complete-response ML q1 ZINB2 route only: the structured pair field enters log-`sigma`, while `mu` and `zi` remain fixed-effect. Its direct SD target is visible but not profile-ready. Point-fit recovery remains blocked pending the required IID ZINB pair-RE control; no ledger or interval claim follows. |
