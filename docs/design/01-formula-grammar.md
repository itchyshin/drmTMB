...
| `cbind(successes, failures) ~ x1 + (1 + x1 | id)`, `family = stats::binomial(link = "logit")` | Experimental q=2 point-fit slice | One complete-data native-TMB binomial model estimates an unlabelled ordinary intercept-slope covariance block. ML and experimental `estimator = "mspl"` point fitting, extraction, point prediction, and simulation are locally tested. MSPL admits only one such block with a full-rank dense fixed design and positive integer frequency weights; `logLik()`, AIC/BIC, likelihood-ratio comparisons, Wald SEs, profiles, and confidence intervals are deliberately unavailable. REML, missing-response integration, labels, multiple grouping factors or slopes, structured effects, and interval or coverage claims remain unsupported. |
...
| `bf(y ~ z + mi(x1) + mi(x2), sigma ~ 1)` with `impute = impute_joint(cbind(x1, x2) ~ z)` and `missing = miss_control(predictor = "model")` | Implemented joint continuous missing-predictor slice | Fits two correlated continuous Gaussian predictor models with a shared fixed-effect right-hand side and estimates their residual correlation. For a Gaussian response, the TMB/Laplace calculation equals the direct observed-data likelihood. The Poisson proof route uses the same grammar with continuous latent TMB/Laplace integration. Exactly two bare additive `mi()` terms, fixed-effect imputation terms, complete responses, and no REML, response mask, zero inflation, or response random/structured terms in the Poisson route are admitted. Interactions, a third predictor, non-Gaussian predictor models, grouped/structured joint imputation, and other non-Gaussian response families remain outside this route. |
...
| `count ~ z + mi(treatment)` with `family = poisson()`, `impute = list(treatment = impute_model(treatment ~ z, family = binomial()))`, and `missing = miss_control(predictor = "model")` | Implemented first non-Gaussian response missing-predictor slice | Fits one fixed-effect Bernoulli/logit predictor model for a missing binary `mi()` predictor inside an ordinary Poisson response mean model, summing exactly over the two binary states with the Poisson response likelihood. The separate two-continuous-predictor `impute_joint()` proof route is recorded above. Zero-inflated, random-effect, or structured Poisson response models with `mi()`, other non-binary Poisson predictor models, and missing Poisson responses remain planned. |
...
| `imputed(fit)` for a fitted MD3a/MD3b/MD4/MD6a/MD6b/MD6c/MD7a/MD7b/MD7c/MD7d/MD7e/MD7f/MD8a/MD8b/MD8c/MD9a/MD9b `mi(x)` model | Implemented missing-predictor summary slice | Reports conditional modes for fitted Gaussian missing predictor values, including each member of an MD9b joint continuous pair; conditional probabilities for fitted binary missing predictor values; conditional expected scores for fitted ordered missing predictor values; conditional modal category scores for fitted unordered predictor values; conditional means for fitted strict proportion, boundary proportion, denominator-aware beta-binomial, lognormal, Gamma, and Tweedie predictor values; and conditional expected counts for fitted Poisson, NB2, and zero-truncated NB2 missing predictor values. Scalar Gaussian routes include likelihood-based conditional standard errors when `sdreport()` is available; joint continuous, finite-state, beta/proportion, boundary-proportion, beta-binomial, count, lognormal, Gamma, and Tweedie routes currently report `NA` standard errors. This is not response imputation, simulation-based multiple imputation, posterior summaries, credible intervals, EM engines, REML for missing-data routes, or measurement-error modelling. |
