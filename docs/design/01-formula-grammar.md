...
| `y ~ treatment + temporal(1 \| id, time = occasion, structure = "ar1")` or `y ~ treatment + temporal(1 \| id, time = elapsed, structure = "ou")`, `sigma ~ 1`, `family = gaussian()` | Implemented temporal Gaussian point-fit slice | One univariate Gaussian `mu` temporal intercept field with an AR1 or OU covariance. AR1 requires finite integer occasions and permits signed persistence; OU accepts finite numeric elapsed time and estimates positive exponential decay. Genuine gaps are retained, input rows may be shuffled, and duplicate `id`--time keys are rejected before response omission. Either field may be paired with exactly one ordinary `(1 \| id)` intercept using the same ID. The combined model needs multiple series and at least three distinct positive lags; AR1 also needs an odd lag. Only fixed `mu` effects, offsets, and constant `sigma` are admitted; any supplied likelihood weights must all equal one. Temporal slopes, a second grouping factor, structured effects, `sd()` formulae, `meta_V()`, REML, non-Gaussian families, forecast or `newdata` prediction, and variance-parameter intervals remain outside this slice. OU Wald intervals are deferred pending the inherited AR1 calibration repair. |
