...
| `animal(1 + x | id, pedigree = ped)` | Implemented first one-slope slice | Univariate Gaussian `mu` path with independent animal-model intercept and slope fields; A-matrix `animal(1 + x | id, A = A)` also has first sigma-only and matched `mu+sigma` native point-fit/extractor cells. Multiple animal slopes, broader intercept-plus-slope labelled structured covariance, bridge/inference for matched slope cells, and slope correlations remain planned. Covariance labels in univariate `mu` or `sigma` remain rejected; use matching `animal(0 + x | p | id, A/Ainv = ...)` in bivariate `mu1`/`mu2` for the first slope-only q2 cell, or the exact all-four shared-label `animal(1 + x | p | id, A = A)` cell when all four endpoints share the same label. |
...
| labelled `relmat(0 + x | p | id, K = K)` or `relmat(0 + x | p | id, Q = Q)` in bivariate `mu1` and `mu2` | Implemented first q=2 slope-only slice | Matching labelled lower-level relatedness slope-only terms estimate two location-slope SDs and one `cor(mu1:x,mu2:x | p | id)` correlation from the same known latent matrix. This is separate from the exact all-four shared-label `relmat(1 + x | p | id, K/Q = ...)` cell; partial or location-only intercept-plus-slope labelled structured covariance remains planned. |
...
| `relmat(1 + x | id, K = K)` or `relmat(1 + x | id, Q = Q)` | Implemented first one-slope slice | Univariate Gaussian `mu`, sigma-only, and matched `mu+sigma` native point-fit/extractor paths with independent relatedness intercept and slope fields; multiple `relmat()` slopes, broader intercept-plus-slope labelled structured covariance, bridge/inference for matched slope cells, and slope correlations remain planned. Covariance labels in univariate `mu` or `sigma` remain rejected; use matching `relmat(0 + x | p | id, K/Q = ...)` in bivariate `mu1`/`mu2` for the first slope-only q2 cell, or the exact all-four shared-label `relmat(1 + x | p | id, K/Q = ...)` cell when all four endpoints share the same label. |
...
| `y01 ~ x1`, `family = stats::binomial(link = "logit")` | Implemented fixed-effect slice | Plain Bernoulli response model for 0/1 event data. It fits fixed-effect `mu` only, with `mu` as event probability. Non-logit links, factor-response ordering, proportions plus `weights`, `weights = trials`, `successes / trials`, `sigma`, `nu`, `zi`, `zoi`, `coi`, random effects, structured effects, bivariate responses, mixed responses, and `engine = "julia"` remain unsupported until separate evidence exists. |
| `cbind(successes, failures) ~ x1`, `family = stats::binomial(link = "logit")` | Implemented fixed-effect slice | Plain binomial response model for counted events out of known trials, with `trials = successes + failures` and no extra-binomial variation. This is the ordinary binomial likelihood, not `beta_binomial()`, not a continuous-proportion beta model, and not the internal binary missing-predictor imputation route. |
...
| `phylo(0 + x | p | species, tree = tree)` in bivariate `mu1` and `mu2` | Implemented first q=2 slope-only slice | Matching labelled phylogenetic slope-only terms estimate two location-slope SDs and one `cor(mu1:x,mu2:x | p | species)` correlation. The separate shared-label all-four `phylo(1 + x | p | species, tree = tree)` cell has native point-fit/extractor evidence only; bridge parity, intervals, coverage, and broader q8 variants remain planned. |
| `phylo(1 + x1 | species, tree = tree)` | Implemented first one-slope slice | Univariate Gaussian `mu`, sigma-only, and matched `mu+sigma` native point-fit/extractor paths with independent phylogenetic intercept and slope fields; the exact shared-label all-four bivariate `phylo(1 + x | p | species, tree = tree)` block is a separate native point-fit/extractor cell. Multiple phylogenetic slopes, block-diagonal labelled slope covariance, bridge/inference for matched slope cells, and slope correlations remain planned. Covariance labels in univariate `mu` or `sigma` remain rejected; use matching `phylo(0 + x | p | species, tree = tree)` in bivariate `mu1`/`mu2` for the first slope-only q2 cell. |
...
| `spatial(0 + x | p | site, coords = coords)` in bivariate `mu1` and `mu2` | Implemented first q=2 slope-only slice | Matching labelled fixed-covariance spatial slope-only terms estimate two location-slope SDs and one `cor(mu1:x,mu2:x | p | site)` correlation. The separate shared-label all-four `spatial(1 + x | p | site, coords = coords)` cell has native point-fit/extractor evidence only; bridge parity, intervals, coverage, range-estimating spatial support, and broader q8 variants remain planned. |
...
currently exact-Gaussian and mean-side-only for phylogenetic structured effects;
it rejects scale-side, matched `mu`/`sigma`, q2, and q4 phylogenetic REML layouts
until a separate restricted-likelihood design is validated. Labelled structured
...
| `spatial(1 | p | site, coords = coords)` / `spatial(0 + x | p | site, coords = coords)` in bivariate `mu1`/`mu2` | Implemented first bivariate q=2 Gaussian location-covariance slices when matching labelled intercept-only or slope-only terms appear in `mu1` and `mu2`. |
