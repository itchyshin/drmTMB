...
| `y ~ x + phylo(1 \| species, tree = tree, model = "ou")`, `sigma ~ 1`, `family = gaussian()` | Implemented local-fit slice | A stationary evolutionary OU random intercept with tip covariance \(s_{phylo}^2\exp(-\alpha d_{ij})\), where \(d_{ij}\) is patristic tree distance and \(\alpha>0\) is estimated. Brownian motion remains the default when `model` is omitted or `"bm"`. This initial route is univariate Gaussian ML with a single location intercept field, fixed mean effects and offsets, a constant residual `sigma`, and at least three observed species. It rejects slopes, residual-scale phylogenetic OU, ordinary or other structured effects, temporal terms, REML, non-Gaussian families, `newdata`, and forecasts. `decay_phylo`, conditional modes, fitted values, residuals and seeded simulation are available. The retained local panel exposed weak intercept--tree-field separation in several draws, so it supplies no point-recovery, interval, or coverage claim; Wald, profile and bootstrap decay intervals remain unavailable. |
