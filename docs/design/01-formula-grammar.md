...
| `missing = miss_control(response = "drop")` | Implemented | Default top-level missing-data policy. It drops rows with a missing response before likelihood construction; for bivariate Gaussian fits, a row with either response missing is dropped as a complete pair. |
| `missing = miss_control(response = "include")` with a current univariate fitted response family | Implemented | Retains supported missing univariate responses when predictors, grouping variables, structured inputs, weights, and known sampling variances are complete. A missing response contributes no direct response likelihood, `fit$missing_data` stores original-row accounting, `nobs()` counts likelihood-contributing rows, and response residuals are `NA` for masked responses. Route-specific family boundaries remain documented in the missing-data vignette. |
