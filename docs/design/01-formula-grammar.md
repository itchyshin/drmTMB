...
| `y ~ x1 + (1 | id) + (0 + x1 | id)`, `sigma ~ x2`, `family = nbinom2()` | Implemented first slice | Ordinary NB2 `mu` random intercepts and independent numeric slopes; group effects enter the log-mean predictor while `sigma` remains fixed-effect overdispersion. The independent-slope ledger cell is `mc-0402`. Labelled covariance blocks and joint `mu`/`sigma` random effects remain planned except for the separate one-pair complete-data labelled intercept route below; zero-inflated NB2 random effects outside the exact diagnostic-only fixed-`zi` spatial structured-`mu` local-fit gate remain planned. |
...
| `y ~ x1`, `sigma ~ x2 + (1 | id)`, `family = nbinom2()` | Implemented first slice | Ordinary NB2 grouped overdispersion random intercepts; group effects enter the log-`sigma` predictor while `mu` has fixed effects only. Exact q1 structured `sigma` intercept-plus-one-slope routes for `phylo()`, `spatial()`, `animal()`, and `relmat()` are also fitted at recovery grade. Ordinary NB2 `sigma` slopes, labelled covariance blocks, and joint `mu`/`sigma` random effects remain planned except for the separate one-pair complete-data labelled intercept route below; zero-inflated/truncated/hurdle scale random effects, richer structured sigma blocks, and structured-sigma intervals/coverage remain planned. |
| `y ~ x1 + (1 | p | id)`, `sigma ~ x2 + (1 | p | id)`, `family = nbinom2()` | Implemented, parity-only | Fits exactly one matching labelled ordinary random-intercept pair through the non-centred mean--scale transform `rho = 0.999999 * tanh(eta_cor_mu_sigma)`. The data must be complete and non-zero-inflated; slopes, additional or unmatched blocks, structured effects, missing-data integration, intervals, coverage, and general NB2 covariance claims remain outside this route. |
