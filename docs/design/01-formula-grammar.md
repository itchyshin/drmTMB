...
| `y ~ treatment + temporal(1 \| id, time = occasion, structure = "homtoep")`, `sigma ~ 1`, `family = gaussian()` | Native point-fit validation slice | A direct homogeneous Toeplitz temporal process with a common complete equally spaced integer schedule of 3--12 occasions. It estimates a process SD and a positive-definite free correlation for every discrete lag through partial-autocorrelation coordinates; `cor_lag1`, ..., `cor_lag(K-1)` are the reported correlations. Input rows may be shuffled but raw duplicate ID--occasion keys fail before response omission. This first provider rejects ordinary random intercepts, irregular or unequal retained schedules (use OU for irregular time), REML, slopes, non-Gaussian families, `newdata`, forecasts, intervals, and calibration claims. Dense likelihood, score, Hessian, and conditional-mode identities are retained; recovery and reader-facing acceptance remain gated. |
