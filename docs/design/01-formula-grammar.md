...
| `y ~ treatment + phylo(1 \| species, tree = tree) + temporal(1 \| species, time = elapsed, structure = "ou")`, `sigma ~ 1`, `family = gaussian()` | Implemented paired point-fit development slice | A phylogenetically correlated stable `mu` intercept plus independent within-species OU deviations. The two terms must share the same species ID; the tree tips must exactly match the observed species. It is Gaussian ML only, has constant `sigma`, finite numeric times, unique raw species--time keys, at least three species, at least two distinct times per species, and at least three positive distinct lags. It rejects ordinary intercepts, slopes, another structured term, REML, non-unit weights, non-Gaussian families, forecasts, and `newdata`. Dense likelihood, method, and fixed-mean profile identities pass, but retained 24-fixture point recovery missed the predeclared mean fixed-effect error threshold. It is therefore not an inference-qualified interval workflow: Wald, profile, bootstrap, variance, decay, forecast, and `newdata` inference are unavailable pending repaired recovery and calibration evidence. This additive covariance is not the future separable phylogeny-by-OU field. |
