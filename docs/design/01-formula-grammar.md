...
| `spatial(1 | site, mesh = mesh)`, `sigma ~ 1`, `family = gaussian()` | Implemented contract slice; local-fit only | A univariate Gaussian location intercept uses a fixed-kappa SPDE GMRF field projected as `A_st %*% omega`. Build metric input with `spatial_coords(data, longitude, latitude, crs_out)` and `make_mesh(coords, kappa)`. This is distinct from, and does not alter, the dense `coords =` covariance route. `kappa` is fixed configuration (not a range estimate); `sdpars$mu` reports the GMRF field scale, while projected marginal variation is location-dependent. Slopes, labels, sigma/shape effects, non-Gaussian or bivariate mesh fields, mesh-plus-coords, REML, missing-data routes, intervals, coverage, anisotropy, barriers, replicate fields, and time remain rejected or unclaimed. |
...
| `spatial(1 | site, coords = coords)` | Implemented first structured spatial random intercept for univariate Gaussian `mu`, univariate Gaussian `sigma`, and matching `mu`/`sigma` location-scale blocks; coordinates define a fixed coordinate covariance foundation. Separately, `spatial(1 | site, mesh = mesh)` admits a fixed-kappa univariate Gaussian ML `mu` intercept at local-fit level; broader mesh/SPDE fitting remains planned. |
