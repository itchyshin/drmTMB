...
| `sd_phylo(species) ~ x_species` and bivariate `sd_phylo1()` / `sd_phylo2()` | Planned | Family B structured direct-SD targets now have a univariate tip-scaling design contract, but fitting and recovery tests are still pending. |
