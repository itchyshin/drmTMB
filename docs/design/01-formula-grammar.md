...
`animal()`, `relmat()`, `phylo_interaction()`, `meta_V()`, `sd_phylo()`,
`temporal()`, and `corpair()`. This is a marker-specific rule; arbitrary namespaced R functions
...
| `associate_pairs(fit_1, fit_2, kernel = latent_normal(), association = ~ 1)` | Implemented beta slices; interval-feasible | A post-fit, margin-first object for exactly matched complete rows and fixed-effect ML margins. Reviewed pairs are Gaussian × literal-Bernoulli, Gaussian × ordinary-NB2, literal-Bernoulli × literal-Bernoulli, literal-Bernoulli × ordinary-NB2, and ordinary-NB2 × ordinary-NB2, in either input order. Stage 1 is frozen; stage 2 estimates latent-normal `eta`; public `vcov()` and `confint()` report alpha-scale Godambe-Wald uncertainty when diagnostics pass. The literal-Bernoulli × ordinary-NB2 beta route additionally accepts an intercept-bearing fixed-effect association formula, giving row-specific `eta_i`; its alpha coefficient intervals are interval-feasible. Constant models expose transformed intervals through `confint(type = "eta")`, while `predict(type = "eta", se.fit = TRUE, interval = "confidence")` supplies pointwise row-specific eta uncertainty. The intercept-only Bernoulli × ordinary-NB2 route is inference-ready with caveats from retained coverage evidence. It is neither a `drmTMB()` formula nor mixed-family `rho12`. Broader families, random/structured association effects, partial pairs, offsets, weights, `mi()`, `meta_V()`, REML, simultaneous eta bands, and profiles are outside these slices. |
| `y ~ x1`, `sigma ~ x1` | Implemented | Univariate Gaussian location-scale model. |
| `y ~ treatment + temporal(1 \| id, time = occasion, structure = "ar1")` or `y ~ treatment + temporal(1 \| id, time = elapsed, structure = "ou")`, `sigma ~ 1`, `family = gaussian()` | Implemented temporal Gaussian profile slice | One univariate Gaussian `mu` temporal intercept field with an AR1 or OU covariance. AR1 requires finite integer occasions and permits signed persistence; OU accepts finite numeric elapsed time and estimates positive exponential decay. Genuine gaps are retained, input rows may be shuffled, and duplicate `id`--time keys are rejected before response omission. Either field may be paired with exactly one ordinary `(1 \| id)` intercept using the same ID. The combined model needs multiple series and at least three distinct positive lags; AR1 also needs an odd lag. Only fixed `mu` effects, offsets, and constant `sigma` are admitted; any supplied likelihood weights must all equal one. `confint(..., parm = "mu:<coefficient>", method = "profile")` profiles those mean effects and warns when the fitted Hessian is irregular. Deterministic dense-oracle agreement is retained. A retained 3,000-fit OU campaign qualified fixed-`mu` profiles in the exact U1--U3 cells; this is not a general temporal coverage claim, and AR1 profile coverage remains uncalibrated. Temporal slopes, a second grouping factor, structured effects, `sd()` formulae, `meta_V()`, REML, non-Gaussian families, forecast or `newdata` prediction, and variance-parameter, bootstrap, or persistence/decay intervals remain outside this slice. OU Wald intervals are deferred pending the inherited AR1 calibration repair. |
| `y ~ treatment + temporal(1 \| id, time = occasion, structure = "homtoep")`, `sigma ~ 1`, `family = gaussian()` | Implemented marginal-covariance profile slice | A homogeneous Toeplitz **marginal covariance** for a common complete equally spaced integer schedule of 3--12 occasions. `sigma` is the total within-series SD and `cor_lag1`, ..., `cor_lag(K-1)` are positive-definite free lag correlations through partial-autocorrelation coordinates. There is no temporal latent mode, process SD, or separately estimated iid residual SD: those quantities are non-identifiable with one row per ID--occasion when every lag is free. Input rows may be shuffled but raw duplicate ID--occasion keys fail before response omission. This first provider rejects ordinary random intercepts, irregular or unequal retained schedules (use OU for irregular time), REML, slopes, non-Gaussian families, `newdata`, and forecasts. `confint(..., parm = "mu:<coefficient>", method = "profile")` is available for mean regression coefficients only. A retained 4,000-fit campaign qualified those profiles in the three predeclared 80-series, six-occasion cells with AR1-shaped, non-exponential, and negative first-lag correlations; this does not establish coverage for other panels. Wald, scale, and lag-correlation intervals remain unavailable. Dense likelihood, score, Hessian, residual whitening, correlated-residual simulation, and recovery identities are retained. |
| `y ~ treatment + temporal(1 \| id, time = occasion, structure = "hetar1")`, `sigma ~ 1`, `family = gaussian()` | Implemented interval-feasibility slice | A latent Gaussian D-R-D covariance with one temporal-process SD for each of 3--12 common complete equally spaced integer occasions and one signed, gap-aware AR1 persistence parameter. The schedule must contain an odd lag so both signs of persistence are identifiable. For a series, \(V=D R(\phi)D+\sigma^2I\); real integer gaps are retained, so equal occasion SDs exactly reduce to AR1. `sigma` is an independent constant residual SD, not a time-varying scale. It rejects an ordinary intercept, irregular or incomplete schedules, aliases, REML, non-Gaussian families, `newdata`, forecasts and temporal scale terms. Point estimates, labelled temporal SDs, fitted values, residuals and simulation have deterministic D-R-D dense-oracle evidence. When the full observed Hessian is positive definite, `vcov()` and Wald intervals are available for fixed `mu` effects only. Profile, process-SD, persistence and residual-SD intervals remain unavailable. The retained fixtures establish fit-level interval feasibility only, not coverage or standard-error calibration. |
| `y ~ x + phylo(1 \| species, tree = tree, model = "ou")`, `sigma ~ z`, `sd(species, level = "phylogenetic") ~ w`, `family = gaussian()` | Implemented local-fit/oracle slice | A stationary evolutionary OU location intercept with tip covariance \(\gamma_i\gamma_j\exp(-\alpha_\mu d_{ij})\), where \(d_{ij}\) is patristic tree distance, \(\alpha_\mu>0\), and the optional direct-SD amplitude is \(\gamma=\exp(W\delta)\). Brownian motion remains the default when `model` is omitted or `"bm"`. This initial route is univariate Gaussian with one location intercept field, fixed mean effects and offsets, a fixed-effect residual scale \(\sigma_i=\exp(H_i\kappa)\), and at least three observed species; native ML and REML fit. `sigma ~ z` changes independent residual variation, not the OU process, so it does not introduce `alpha_sigma`. The fitted location rate is reported as `decay_phylo` (compatibility name for `alpha_mu`). It rejects slopes, a phylogenetic residual-scale OU field, ordinary or other structured effects, temporal terms, non-Gaussian families, `newdata`, and forecasts. `decay_phylo`, conditional modes, fitted values, residuals and seeded simulation are available. The retained local panel exposed weak intercept--tree-field separation in several draws, so this is point-fit/oracle evidence only: it supplies no point-recovery, interval, or coverage claim; Wald, profile and bootstrap decay intervals remain unavailable. |
| `y ~ x + phylo(1 \| species, tree = tree, model = "ou")`, `sigma ~ z + phylo(1 \| species, tree = tree, model = "ou")`, `family = gaussian()` | Implemented Laplace-approximated-ML local-fit/oracle slice | The two matching unlabelled intercept terms define **independent** stationary OU fields: \(u\sim OU(\alpha_\mu,s_\mu)\) enters location and \(v\sim OU(\alpha_\sigma,s_\sigma)\) enters residual log-scale. They must share the same grouping variable, tree, observed-tip order, and augmented-tree layout. `decay_phylo` is `alpha_mu`; `decay_phylo:sigma` is `alpha_sigma`. Fixed `sigma` predictors remain deterministic residual-scale regression. This first joint surface is univariate Gaussian ML only, complete-data, unit-weight, intercept-only, and has no phylogenetic mu--sigma correlation: `corpars$phylo` and `corpairs()` do not expose one. It supports conditional prediction and same-row seeded marginal simulation, but not `newdata` or forecasting. It rejects REML, slopes, labels/coupled covariance blocks, `corpair()`, direct phylogenetic-SD formulas, known sampling covariance, ordinary or other structured random effects, weights, missingness, temporal terms, bivariate/non-Gaussian families, and bridge routing. The retained fixed-seed preflight exposed weak identification and is negative engineering evidence; a predeclared replicated campaign is required before any point-recovery, interval, coverage, or model-selection claim. One-row-per-species empirical fits are feasibility/boundary examples only. Wald, profile, and bootstrap intervals for either decay remain deferred, as do cross-field OU correlation and every broader OU parity claim. |
| `y ~ treatment + phylo(1 \| species, tree = tree) + temporal(1 \| species, time = elapsed, structure = "ou")`, `sigma ~ 1`, `family = gaussian()` | Implemented paired interval-feasibility development slice | A phylogenetically correlated stable `mu` intercept plus independent within-species OU deviations. The two terms must share the same species ID; the tree tips must exactly match the observed species. It is Gaussian ML only, has constant `sigma`, finite numeric times, unique raw species--time keys, at least three species, at least two distinct times per species, and at least three positive distinct lags. It rejects ordinary intercepts, slopes, another structured term, REML, non-unit weights, non-Gaussian families, forecasts, and `newdata`. Dense likelihood, method, and fixed-mean profile identities pass, but retained 24-fixture point recovery missed the predeclared mean fixed-effect error threshold and the 3,500-fit campaign found intercept-profile undercoverage in three primary cells. Fixed-mean profile endpoints are interval-feasibility diagnostics, not inference-ready results; Wald, bootstrap, variance, decay, forecast, and `newdata` inference remain unavailable. This additive covariance is not the future separable phylogeny-by-OU field. |
...
| `y ~ x1 + (1 | id) + (0 + x1 | id)`, `sigma ~ x1`, `family = lognormal()` | Implemented first slice | Ordinary lognormal `mu` random intercepts and independent numeric slopes enter the log-response location. Arc 3a separately admits one unlabelled q1 `phylo()` or `relmat()` intercept using `K` or `Q` in `mu`. Labelled slopes, other structured routes, known covariance, and broader bivariate lognormal extensions remain planned. |
| `y ~ x1 + (1 + x1 | id)`, `sigma ~ 1`, `family = lognormal()` | Implemented ML-Laplace point-fit recovery | One complete-data ordinary lognormal log-location model estimates an unlabelled intercept-slope covariance block. The recovered estimands are `sd0`, `sd1`, and group-level `rho_re` (never residual `rho12`) under the design-17 map `ρ = 0.999999 tanh(η)`, the same report symbols as Poisson (`eta_cor_mu`, `rho_mu_re`; no `logsech_mu_re`). Ledger cell `mc-0720` is `point_fit_recovery` for this exact route; it is not the independent-slope cell `mc-0380` and not Wave 2.5 NB2 `mc-0719`. The slope predictor must vary within at least one group; a predictor that is constant within every group is rejected before the fit because `sd1` and `rho_re` are unidentified. Gamma neighbour, REML, missing-response, labels, mixed `(1 \| g) + (1 + x \| g)`, structured ordinary correlated, intervals, coverage, and `supported` remain unsupported. |
...
| `y ~ x1 + (1 | id) + (0 + x1 | id)`, `sigma ~ x2`, `zoi ~ x3`, `coi ~ x4`, `family = zero_one_beta()` | Implemented first slice | Zero-one beta model for continuous proportions on `[0, 1]` with exact structural boundary mass and ordinary `mu` random intercepts or independent numeric slopes. The exact independent-slope ledger cell is inference-ready with caveats for true SD 0.50 and M>=16; rare machine-exact endpoints from nominally interior beta draws require a strictly-interior rerun before claiming exactly 15% observed boundary mass. `mu` and `sigma` describe the interior beta component, `zoi` is the exact-boundary probability, `coi` is the probability of an exact one conditional on the boundary, and `fitted()` includes boundary mass. Exact unlabelled q1 `sigma ~ 1 + (1 | id)`, `zoi ~ 1 + (1 | id)`, separately evidenced matching-predictor sigma or zoi slopes, and exact `sigma ~ phylo(1 | species, tree = tree)` or `sigma ~ animal(1 | species, Ainv = Ainv)` are point-fit-only, with visible direct targets that are not profile-ready; they do not supply interval or coverage evidence. The exact coi slope `coi ~ x + (0 + x | id)` is code-admitted but has a retained blocked local recovery fixture, so it remains outside point-fit recovery and the ledger. Other coi random effects, joint distributional-parameter effects, correlated or labelled covariance, structured effects, denominators, and bivariate or mixed bounded-response models remain planned. |
| `y ~ x1`, `family = poisson(link = "log")` | Implemented | Fixed-effect univariate Poisson mean model for non-negative integer counts. |
| `y ~ x1 + (1 | id) + (0 + x1 | id)`, `family = poisson(link = "log")` | Implemented first slice | Ordinary Poisson `mu` random intercepts and independent numeric slopes; the group effects enter the log-mean predictor. One exact q1 `zi ~ spatial()` intercept is diagnostic-only. Labelled ordinary covariance blocks, zero-inflated Poisson random effects beyond that exact gate, and cross-parameter covariance remain planned. |
| `y ~ x1 + (1 + x1 | id)`, `family = poisson(link = "log")` | Implemented ML-Laplace point-fit recovery | One complete-data ordinary Poisson log-mean model estimates an unlabelled intercept-slope covariance block. The recovered estimands are `sd0`, `sd1`, and group-level `rho_re` (never residual `rho12`) under the design-17 map `ρ = 0.999999 tanh(η)`. Ledger cell `mc-0718` is `point_fit_recovery` for this exact route; it is not the independent-slope cell `mc-0431` and not Wave 1 binomial `mc-0717`. REML, missing-response, labels, mixed `(1 \| g) + (1 + x \| g)`, structured ordinary correlated, intervals, coverage, and `supported` remain unsupported. |
...
| `y ~ x1 + (1 | id) + (0 + x1 | id)`, `sigma ~ x2`, `family = nbinom2()` | Implemented first slice | Ordinary NB2 `mu` random intercepts and independent numeric slopes; group effects enter the log-mean predictor while `sigma` remains fixed-effect overdispersion. The independent-slope ledger cell is `mc-0402`. Labelled covariance blocks, zero-inflated NB2 random effects outside the exact diagnostic-only fixed-`zi` spatial structured-`mu` local-fit gate, and joint `mu`/`sigma` random effects remain planned. |
| `y ~ x1 + (1 + x1 | id)`, `sigma ~ 1`, `family = nbinom2()` | Implemented ML-Laplace point-fit recovery | One complete-data ordinary NB2 log-mean model estimates an unlabelled intercept-slope covariance block. The recovered estimands are `sd0`, `sd1`, and group-level `rho_re` (never residual `rho12`) under the design-17 map `ρ = 0.999999 tanh(η)`, the same report symbols as Poisson (`eta_cor_mu`, `rho_mu_re`; no `logsech_mu_re`). Ledger cell `mc-0719` is `point_fit_recovery` for this exact route; it is not the independent-slope cell `mc-0402` and not Wave 2 Poisson `mc-0718`. The slope predictor must vary within at least one group; a predictor that is constant within every group is rejected before the fit because `sd1` and `rho_re` are unidentified. Zero-inflated or truncated NB2, REML, missing-response, labels, mixed `(1 \| g) + (1 + x \| g)`, structured ordinary correlated, intervals, coverage, and `supported` remain unsupported. |
...
| `count ~ x`, `sigma ~ 1 + (1 \| group)`, `zi ~ 1`, `family = nbinom2()` | Exact point-fit-only control | Complete-response ML ZINB2 IID sigma-intercept control: fixed-effect `mu` and intercept-only `zi`; the group field enters log-`sigma`. Its direct SD target is visible but not profile-ready. Sigma predictors/slopes/labels, `mu`/`zi` random effects, missing responses, REML, intervals, and coverage remain outside this gate. |
| `count ~ x`, `sigma ~ phylo_interaction(1 \| plant:pollinator, tree1 = plant_tree, tree2 = pollinator_tree)`, `zi ~ 1`, `family = nbinom2()` | Point-fit recovery only | Complete-response ML q1 ZINB2 route only: the structured pair field enters log-`sigma`, while `mu` and `zi` remain fixed-effect. Its direct SD target is visible but not profile-ready. A retained IID ZINB sigma-intercept control and source-bound integrated structured recovery support point-fit recovery only; no profile, interval, coverage, or inference claim follows. |
...
| `count ~ x + provider(1 + x | p | group, input = object)`, `family = poisson(link = "log")` | Exact labelled q2 point-fit slice | Ordinary non-zero-inflated Poisson admits exactly one labelled intercept-plus-one-numeric-slope `mu` block for `phylo(tree = tree)`, `spatial(coords = coords)`, `animal(Ainv = Ainv)`, or `relmat(Q = Q)`. It estimates two latent SDs and their intercept--slope correlation, but its three direct targets are visible and not profile-ready; retained local recovery evidence is point-fit-only. Pure structured slopes, multiple structured slopes, q4+, zero-inflated structure, simultaneous providers, ordinary count random effects, alternate provider inputs, intervals, and coverage remain outside this slice. |
| `count ~ x + phylo(1 + x | species, tree = tree)` or `count ~ x + relmat(1 + x | id, Q = Q)`, `sigma ~ z`, `family = nbinom2()` | Implemented first one-slope slice | Ordinary non-zero-inflated NB2 q=1 structured `mu` intercepts and unlabelled intercept-plus-one-slope terms on the log-mean scale for one of `phylo()`, `spatial()`, `animal()`, or `relmat()`, while `sigma` remains fixed-effect overdispersion unless using a separate fitted sigma route. The separate exact labelled NB2 q2 exception is `phylo(1 + x | p | species, tree = tree)` only, at point-fit-only status. Exact q1 structured `sigma` intercept-plus-one-slope routes for those four providers are recovery grade. One exact crossed `mu ~ spatial(1 | site, coords = coords) + relmat(1 | id, Q = Q)` route is recovery-only. Pure or multiple structured `mu` slopes, other labelled q2/q4 count blocks, zero-inflated NB2 structure outside the exact local-fit gate, richer or labelled structured sigma, structured-sigma intervals/coverage, simultaneous structured types beyond that exact crossed gate, and joint `mu`/`sigma` random effects remain planned. |
...
| `cbind(successes, failures) ~ x1 + (1 + x1 | id)`, `family = stats::binomial(link = "logit")` | Implemented ML-Laplace point-fit recovery | One complete-data native-TMB binomial logit model estimates an unlabelled ordinary intercept-slope covariance block. The recovered estimands are `sd0`, `sd1`, and group-level `rho_re` (never residual `rho12`) under the log-sech Cholesky. Ledger cell `mc-0717` is `point_fit_recovery` for this exact route; it is not the independent-slope cell `mc-0061`. The slope predictor must vary within at least one group; a predictor that is constant within every group is rejected before the fit because `sd1` and `rho_re` are unidentified. Experimental `estimator = "mspl"` point fitting remains a separate, narrower path. REML, missing-response integration, labels, mixed `(1 \| g) + (1 + x \| g)`, multiple grouping factors or slopes, structured effects, intervals, coverage, and `supported` remain unsupported. |
...
| `spatial(1 | site, mesh = mesh)`, `sigma ~ 1`, `family = gaussian()` | Implemented contract slice; exact fixed-domain `n = 128, 256` designs at `point_fit_recovery` | A univariate Gaussian location intercept uses a fixed-kappa SPDE GMRF field projected as `A_st %*% omega`. Build metric input with `spatial_coords(data, longitude, latitude, crs_out)` and `make_mesh(coords, kappa)`. This is distinct from, and does not alter, the dense `coords =` covariance route. `kappa` is fixed configuration (not a range estimate); `sdpars$mu` reports the GMRF field scale, while projected marginal variation is location-dependent. The retained `n = 64` recovery rung failed, so the tested designs do not establish a universal sample-size floor. Slopes, labels, sigma/shape effects, non-Gaussian or bivariate mesh fields, mesh-plus-coords, REML, missing-data routes, intervals, coverage, anisotropy, barriers, replicate fields, and time remain rejected or unclaimed. |
...
| `spatial(1 | site, coords = coords)` | Implemented first structured spatial random intercept for univariate Gaussian `mu`, univariate Gaussian `sigma`, and matching `mu`/`sigma` location-scale blocks; coordinates define a fixed coordinate covariance foundation. Separately, `spatial(1 | site, mesh = mesh)` admits a fixed-kappa univariate Gaussian ML `mu` intercept at `point_fit_recovery` for its exact tested `n = 128, 256` fixed-domain designs; broader mesh/SPDE fitting and interval inference remain planned. |
