...
| `y ~ treatment + occasion + temporal(1 \\| id, time = occasion, structure = "ar1")`, `sigma ~ 1`, `family = gaussian()` | Implemented first temporal slice | One univariate Gaussian `mu` temporal intercept field with a stationary AR1 covariance. `time` must be a finite integer observation column; genuine gaps are retained and duplicate `id`--time keys are rejected before response omission. The field may be paired with exactly one ordinary `(1 \\| id)` intercept using the same ID. The combined model needs multiple series, at least three distinct positive lags including an odd lag; AR1-only needs at least two including an odd lag. Only fixed `mu` effects, offsets, and constant `sigma` are admitted; any supplied likelihood weights must all equal one. Temporal slopes, a second grouping factor, structured effects, `sd()` formulae, `meta_V()`, REML, non-Gaussian families, forecast or `newdata` prediction, and variance-parameter intervals remain outside this slice. |
