...
| Phylogenetic effects | Univariate `mu` intercepts, bivariate `mu1`/`mu2` intercept correlation, all-four q=4 phylogenetic intercept block, `sd_phylo*()` direct-SD routes, and q=2 predictor-dependent `corpair(..., level = "phylogenetic", from = "mu1", to = "mu2")`. | Phylogenetic slopes, animal-model aliases, `relmat()` fitting, q=4 predictor-dependent phylogenetic correlations, and non-Gaussian phylogenetic random effects. |
