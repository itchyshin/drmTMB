...
| Coordinate spatial effects | Univariate Gaussian `mu` intercept and one independent `mu` slope through `spatial(1 + x | site, coords = coords)`. | Spatial slope correlations, spatial scale, bivariate spatial q=4 blocks, spatial direct-SD models, and spatial `corpair()` regressions. |
