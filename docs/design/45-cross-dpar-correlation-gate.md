...
| Ordinary Gaussian `mu` | Univariate Gaussian random intercepts, independent slopes, correlated q=2 blocks, and q > 2 location blocks such as `(1 + x1 + x2 | id)`. q=3 has recovery-smoke coverage; larger q is advanced. | Predictor-modelled slope correlations and bivariate random slopes. |
...
| Bivariate Gaussian intercept blocks | Matching labelled `mu1`/`mu2`, `sigma1`/`sigma2`, same-response `mu`/`sigma`, and all-four `mu1`/`mu2`/`sigma1`/`sigma2` random-intercept blocks. | Bivariate slope blocks, q=6 or q=8 location-scale slope endpoints, mixed-distribution bivariate likelihoods, and q=4 direct correlation profiles. |
| Phylogenetic effects | Univariate `mu` intercepts, bivariate `mu1`/`mu2` intercept correlation, all-four q=4 phylogenetic intercept block, `sd_phylo*()` direct-SD routes, and q=2 predictor-dependent `corpair(..., level = "phylogenetic", from = "mu1", to = "mu2")`. | Phylogenetic slopes, q=4 predictor-dependent phylogenetic correlations, and non-Gaussian phylogenetic random effects. Known-matrix animal and `relmat()` Gaussian `mu` intercepts plus constant bivariate q=2 location covariance are fitted first slices, but animal/`relmat()` slopes, scale models, q=4 blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain outside Wave A. |
| Coordinate spatial effects | Univariate Gaussian `mu` intercept, one independent `mu` slope through `spatial(1 + x | site, coords = coords)`, and constant bivariate `mu1`/`mu2` q=2 covariance through matching `spatial(1 | p | site, coords = coords)` terms. | Spatial slope correlations, spatial scale, bivariate spatial q=4 blocks, spatial direct-SD models, and spatial `corpair()` regressions. |
