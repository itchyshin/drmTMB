...
| Bivariate Gaussian covariance blocks | Matching labelled `mu1`/`mu2`, `sigma1`/`sigma2`, same-response `mu`/`sigma`, all-four `mu1`/`mu2`/`sigma1`/`sigma2` random-intercept blocks, matching slope-only `mu1`/`mu2` blocks, and smoke-artifact-routed matching q=4/q=6 `mu1`/`mu2` location blocks. | Residual-scale slope blocks, p8/q8 location-scale slope endpoints, same-response location-scale slope covariance, mixed-distribution bivariate likelihoods, and q > 2 direct correlation profiles. |
