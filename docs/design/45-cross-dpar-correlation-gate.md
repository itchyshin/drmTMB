...
| Coordinate spatial effects | Univariate Gaussian `mu` intercept, one independent `mu` slope through `spatial(1 + x | site, coords = coords)`, constant bivariate `mu1`/`mu2` q=2 covariance, and constant q=4 location-scale covariance through matching `spatial(1 | p | site, coords = coords)` terms. | Spatial slope correlations, standalone spatial scale routes, spatial direct-SD models, spatial `corpair()` regressions, and non-Gaussian spatial effects. |
