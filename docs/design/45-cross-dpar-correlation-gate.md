...
| Ordinary Gaussian `mu` | Univariate Gaussian random intercepts, independent slopes, correlated q=2 blocks, q > 2 location blocks such as `(1 + x1 + x2 | id)`, matching slope-only bivariate `mu1`/`mu2` blocks such as `(0 + x | p | id)`, same-response matching slope-only `mu`/`sigma` blocks, q4/q6 bivariate location blocks with smoke artifact routing such as `(1 + x | p | id)` and `(1 + x + z | p | id)`, and the first source-tested q8 all-endpoint block with matching `(1 + x | p | id)` terms in all four bivariate endpoint formulas. q=3 has recovery-smoke coverage; larger q is advanced, while q4/q6 bivariate location and q8 have smoke/source lanes only. | Predictor-modelled slope correlations, q8 recovery/coverage/power evidence, and broader all-endpoint coefficient sets. |
| Gaussian `sigma` | Random intercepts and independent numeric slopes in the residual-scale formula, multiple matched univariate `mu`/`sigma` random-intercept covariance blocks, the matching bivariate `sigma1`/`sigma2` q2 slope-only block, one same-response matching slope-only bivariate `mu`/`sigma` block, and the scale endpoints in the first ordinary q8 source-tested block. | Correlated univariate residual-scale slope blocks, q8 artifact evidence, and q8 variants outside the matching one-slope ordinary Gaussian route. |
| Bivariate Gaussian covariance blocks | Matching labelled `mu1`/`mu2`, `sigma1`/`sigma2`, same-response `mu`/`sigma`, all-four `mu1`/`mu2`/`sigma1`/`sigma2` random-intercept blocks, matching slope-only `mu1`/`mu2`, `sigma1`/`sigma2`, and same-response `mu`/`sigma` blocks, smoke-artifact-routed matching q=4/q=6 `mu1`/`mu2` location blocks, and the first source-tested q8 all-endpoint block. | q8 recovery/coverage/power evidence, mixed-distribution bivariate likelihoods, and q > 2 direct correlation profiles. |
...
before they enter broad simulations. The ordinary q8 route has source tests but
does not yet have artifact or recovery evidence.
