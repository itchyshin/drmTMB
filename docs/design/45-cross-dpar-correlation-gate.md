...
| Phylogenetic effects | Univariate `mu` intercepts, bivariate `mu1`/`mu2` intercept correlation, all-four q=4 phylogenetic intercept block, `sd_phylo*()` direct-SD routes, and q=2 predictor-dependent `corpair(..., level = "phylogenetic", from = "mu1", to = "mu2")`. | Phylogenetic slopes, q=4 predictor-dependent phylogenetic correlations, and non-Gaussian phylogenetic random effects. Known-matrix animal and `relmat()` Gaussian `mu` intercepts are fitted first slices, but animal/`relmat()` slopes, scale models, bivariate covariance, and `corpair()` parity remain outside Wave A. |
