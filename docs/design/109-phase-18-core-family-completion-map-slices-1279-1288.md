...
| --- | --- | --- | --- |
| Ordinary counts | Fixed-effect Poisson, NB2, zero-inflated Poisson/NB2, zero-truncated NB2, and hurdle NB2 are fitted where documented. Ordinary non-zero-inflated Poisson/NB2 `mu` random intercepts and independent numeric `mu` slopes are fitted. Ordinary zero-truncated NB2 `mu` random intercepts are fitted as a positive-count first slice. Ordinary NB2 log-`sigma` random intercepts are fitted. Ordinary Poisson/NB2 q=1 phylogenetic `mu` intercepts are fitted as narrow first slices. | Paired Poisson/NB2 `mu` random-effect smoke and grid artifacts exist. Zero-truncated NB2 `mu` random-intercept focused recovery exists in the source test lane. NB2 log-`sigma` random-intercept smoke/grid artifacts exist. Poisson/NB2 q=1 phylogenetic lanes have smoke/formal infrastructure, with NB2 held at `hold_smoke_only` until the 500-replicate gate runs and is audited. | Close the first-wave count story as "ordinary count mixed models, zero-truncated NB2 random intercepts, and q=1 phylogenetic smoke/formal-admission routes." Do not add zero-inflation random effects, hurdle random effects, zero-truncated random slopes, count spatial/animal/`relmat()` routes, or structured count slopes until the first-wave claim is split into reviewable PRs. |
| Proportions and bounded responses | Fixed-effect `beta()` and `beta_binomial()` are fitted for strict continuous proportions and successes out of known trials. | At this planning point, the fixed-effect proportion ADEMP sheet existed but the DGP, summariser, smoke runner, and repeatable grid writer had not yet joined the first-wave artifact path. Slices 1289-1298 later added that artifact lane. | Keep the first artifact lane fixed-effect only. Keep `zoi`, `coi`, exact 0/1 boundary mass, bounded-response random effects, and mixed-response bounded models in the failure ledger. |
...
| Positive continuous responses | Fixed-effect lognormal location-scale and Gamma mean-CV likelihoods are fitted. | Slices 1299-1308 add the positive-continuous ADEMP/artifact lane with DGP, summariser, smoke runner, repeatable grid writer, first-wave runner inclusion, and manual Actions dispatch for `lognormal()` and `Gamma(link = "log")`. | Keep random effects, known sampling covariance, phylogenetic terms, structured effects, generalized Gamma, Tweedie, and bivariate positive-continuous models out. |
...
1. Finish the count first-wave story without adding more count syntax.
...
5. Only then decide whether the next shape implementation is skew-normal or
   whether Student-t formal grids are enough for the first public shape story.
