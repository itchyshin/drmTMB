...
6. Treat the zero-one bounded-response design gate as the chosen D3
   documentation lane: fixed-effect zero-one beta can be designed next, but
   `zoi`/`coi` formulas, zero-one random effects, bounded-response random
   slopes, structured bounded responses, and mixed bounded-response models stay
   planned.
