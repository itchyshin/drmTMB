...
| Proportions and bounded responses | Fixed-effect `beta()` and `beta_binomial()` are fitted, the fixed-effect ADEMP sheet exists, and Slices 1289-1298 restore the private DGP, summariser, smoke runner, summary helper, repeatable grid writer, and focused test lane. | Keep the lane fixed-effect only. Add first-wave summary and manual Actions integration separately before any bounded-response random effects, `zoi`, `coi`, exact 0/1 boundary mass, known-covariance bounded responses, or mixed-response bounded models. |
...
2. Add first-wave summary and manual Actions integration for the fixed-effect
   proportion artifact lane.
3. Add the fixed-effect positive-continuous artifact lane.
4. Add the fixed-effect ordinal artifact lane.
5. Only then choose between skew-normal implementation and deeper formal grids
