...
| Positive continuous responses | Fixed-effect lognormal location-scale and Gamma mean-CV likelihoods are fitted, and Slices 1299-1308 restore the private DGP, summariser, smoke runner, summary helper, repeatable grid writer, and focused test lane. | Add first-wave summary and manual Actions integration separately before Tweedie, generalized Gamma, positive-response random effects, known-covariance positive responses, structured effects, or mixed-response positive-continuous models. |
...
1. Keep NB2 q1 formal recovery on the sharded Actions path, not a local
   singleton run.
2. Add first-wave summary and manual Actions integration for the fixed-effect
   positive-continuous artifact lane.
