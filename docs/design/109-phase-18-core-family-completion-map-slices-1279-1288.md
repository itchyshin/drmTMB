...
| Proportions and bounded responses | Fixed-effect `beta()` and `beta_binomial()` are fitted, and the fixed-effect ADEMP sheet exists. The clean reconciliation branch does not yet carry the later private DGP, summariser, smoke runner, or repeatable grid writer lane. | Add a fixed-effect-only artifact lane before any bounded-response random effects, `zoi`, `coi`, exact 0/1 boundary mass, known-covariance bounded responses, or mixed-response bounded models. |
...
1. Keep NB2 q1 formal recovery on the sharded Actions path, not a local
   singleton run.
2. Add the fixed-effect proportion artifact lane.
