...
2. p8/q8 endpoint covariance has a first ordinary Gaussian q8 smoke/recovery
...
| q8 all-endpoint block | all four dpars with intercept and slope endpoints | fitted first ordinary Gaussian slice with diagnostic smoke/recovery artifacts; the 2026-06-07 local two-cell recovery audit is `hold_diagnostic`; the 2026-06-09 usability pilot shows sample-size-dependent Hessian and conditioning improvement at high replication; q8 correlations are derived-unavailable until a validated interval method succeeds |
