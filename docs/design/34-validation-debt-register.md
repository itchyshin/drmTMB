...
| Zero inflation, hurdle, zero-one inflation, and one inflation | Fixed-effect `zi` and `hu` paths exist for selected count families; one diagnostic-only Poisson q=1 `zi ~ spatial()` intercept and one diagnostic-only truncated-NB2 q=1 `hu ~ relmat(K/Q)` intercept are fitted. Fixed-effect bounded-response `zoi`/`coi` paths exist in `zero_one_beta()`; its exact ordinary `zoi` random-intercept/same-raw-symbol slope and `coi` random-intercept routes are point-fit-only. | Count-side random effects can mimic structural zeros; hurdle, inflation, and exact-boundary components can be weakly separated from mean, dispersion, sampling zeros, and sampling ones. The `coi` intercept has a sparse-atom conditional-mode warning. | Keep the exact q=1 `zi`/`hu` count gates diagnostic-only and the exact ordinary atom gates point-fit-only; exclude other random effects in `zi`, `hu`, `zoi`, and `coi`, plus joint/correlated atom effects, until separately evidenced. |
