...
| `ordinary_biv_corpairs` | Ordinary bivariate covariance and `corpairs()` | partial | moderate | Matching slope-only `mu1`/`mu2`, matching q2 same-response `mu`/`sigma` slope, matching q2 `sigma1`/`sigma2` scale-slope, and q=4/q=6 `mu1`/`mu2` location covariance are fitted, with smoke artifact routing for q4/q6 location blocks. The same-response q2 500-replicate audit and robust-refit follow-up are diagnostic, not power evidence: 130 weak fits were not rescued, while two clean fits showed endpoint-profile feasibility. Keep p8/q8 endpoint and predictor-dependent slope-correlation routes blocked until recovery evidence and interval policy are explicit. |
