...
| `nbinom2_phylo_q1_mu` | Ordinary NB2 q=1 phylogenetic `mu` intercept | partial | high until the 500-replicate formal recovery grid is run and audited | The ordinary non-zero-inflated NB2 route fits `phylo(1 | species, tree = tree)` in `mu` with fixed-effect `sigma`, direct `log_sd_phylo` profile-target exposure, `ranef("phylo_mu")`, focused smoke tests, an overdispersion-aware DGP/grid writer, optional profile artifacts, a formal-grid QA wrapper, a manual Actions task, and an ordinary grouped species-intercept comparator row. Slices 541-555 add a local 288-cell one-replicate sentinel and a 24-cell x 5-replicate representative audit; those artifacts pass read-back QA but keep the promotion decision at `hold_smoke_only` because the formal recovery gate remains unmet. Slices 561-575 cancelled a singleton 500-replicate Actions dispatch after manifest timings implied about 27-31 optimistic 10-worker hours, then added sharded formal-grid dispatch and shard metadata so partial artifacts cannot become coverage claims. Profile failures at true `sd_phylo = 0` and low-count fixed-`sigma` instability remain validation-debt signals. Keep NB2 phylogenetic slopes, NB2 `sigma` phylogeny, zero-inflated NB2 phylogeny, spatial, animal, and `relmat()` structured count routes blocked until separate recovery and diagnostic evidence lands. |
...
- Check-log evidence: family-specific check-log entries are summarized in
  ...
  The clean-branch follow-up also adds first-wave summary inclusion and the
  opt-in manual `proportion_fixed_effect` Actions task.
...
| NB2 and zero-truncated NB2 ordinary random effects | NB2 is fitted for non-zero-inflated `mu` random intercepts, independent numeric `mu` slopes, and ordinary log-`sigma` random intercepts; NB2 now has a smoke runner, fixed-effect Wald intervals, direct SD profile intervals for `mu`, a weak-SD boundary test, focused `sigma` intercept recovery/extractor tests, and a separate NB2 log-`sigma` smoke-grid artifact writer with direct `log_sd_sigma` profile-target rows; zero-truncated NB2 remains planned. | Confounding among overdispersion, group-level heterogeneity, zero truncation, count-side mean effects, and grouped overdispersion heterogeneity. | Admit NB2 only as a focused first slice until larger grids vary overdispersion, group count, repeats, true `mu` SD, true `sigma` SD, and mean count; exclude zero-truncated NB2 until fitted. |
