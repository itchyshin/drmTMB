...
| `phylo_structured_effects` | Phylogenetic structured effects | partial | moderate | Gaussian `mu`/`sigma` intercepts, one numeric `mu` slope, the first sigma-only residual-scale one-slope cell, the matched `mu+sigma` one-slope native point-fit/extractor cell, matching univariate `mu`/`sigma` correlation, and bivariate/q4 slices are fitted; ordinary Poisson/NB2 q=1 rows are tracked separately; keep multiple slopes, labelled structured slope covariance, bridge/inference evidence for the matched slope cell, direct-SD formulas combined with structured `sigma`, structured `rho12`, and broader non-Gaussian phylogenetic effects in the debt ledger. The q=4 scale-side endpoints (phylogenetic `sigma1`/`sigma2` SDs and the scale-scale and mean-scale correlations) are **weakly identified at about one observation per tip**, not non-identified: report the mean-side model with fixed-effect `sigma ~ predictors` as the supported one-record-per-species analysis, and gate any scale-side phylogenetic estimate behind within-species replication or the planned `estimator = "penalized"` prior path (see `docs/dev-log/known-limitations.md` and de Villemereuil & Nakagawa 2014; Nakagawa et al. 2025). |
| `poisson_phylo_q1_mu` | Ordinary Poisson q=1 phylogenetic `mu` intercept and one-slope terms | partial | moderate to high until recovery grids exist | The first phylogenetic count route is fitted for `phylo(1 | species, tree = tree)` and the unlabelled `phylo(1 + x | species, tree = tree)` one-slope term in ordinary Poisson `mu`; the opt-in smoke runner, repeatable CSV artifact writer, optional direct `log_sd_phylo` profile-interval artifacts, formal-grid spec, read-back QA, promotion decision, and manual Actions task now exist for the intercept route, but formal recovery grids remain the next evidence gate. The same source path now also admits q=1 `spatial()`, `animal()`, and `relmat()` count `mu` intercept and unlabelled one-slope terms with focused tests. Keep pure, multiple, or labelled count structured slopes, simultaneous structured types, zero-inflation, and cross-parameter covariance blocked until separate evidence lands. |
| `nbinom2_phylo_q1_mu` | Ordinary NB2 q=1 structured `mu` intercept and one-slope terms | partial | high after the 500-replicate phylogenetic shard audit | The ordinary non-zero-inflated NB2 route fits one q=1 `phylo()`, `spatial()`, `animal()`, or `relmat()` intercept or unlabelled intercept-plus-one-slope term in `mu` with fixed-effect `sigma`, direct `log_sd_phylo` profile-target exposure, marker-specific `ranef()` blocks, focused tests, and structured diagnostics. The phylogenetic intercept lane also has an overdispersion-aware DGP/grid writer, optional profile artifacts, a formal-grid QA wrapper, a manual Actions task, and an ordinary grouped species-intercept comparator row. Slices 541-555 add a local 288-cell one-replicate sentinel and a 24-cell x 5-replicate representative audit; those artifacts pass read-back QA but keep the promotion decision at `hold_smoke_only` because the formal recovery gate remains unmet. Slices 561-575 cancelled a singleton 500-replicate Actions dispatch after manifest timings implied about 27-31 optimistic 10-worker hours, then added sharded formal-grid dispatch and shard metadata so partial artifacts cannot become coverage claims. The later 16-shard, 500-replicate formal artifact set has all 288 condition cells and 144,000 `ok` manifest rows, but the merged audit keeps the route on hold because direct `log_sd_phylo` profile intervals fail frequently at the true-zero boundary and low-count, low-overdispersion cells retain fixed-`sigma` instability. Keep pure, multiple, or labelled NB2 structured slopes, structured `sigma`, zero-inflated structure, simultaneous structured types, and labelled count covariance blocked until separate recovery and diagnostic evidence lands. |
| `spatial_mu_coord` | Coordinate spatial univariate Gaussian `mu`/`sigma` plus count q=1 `mu` | partial | moderate | Gaussian location and residual-scale intercepts plus one numeric `mu` slope, the first fixed-covariance sigma-only residual-scale one-slope cell, and the matched fixed-covariance `mu+sigma` one-slope native point-fit/extractor cell are fitted with direct SD targets and smoke/source evidence; ordinary Poisson/NB2 q=1 spatial `mu` intercept and unlabelled one-slope terms now reuse the structured count gate. Keep mesh/SPDE, multiple slopes, labelled structured slope covariance, bridge/inference evidence for the matched slope cell, direct-SD surfaces, pure, multiple, or labelled count spatial slopes, and zero-inflated spatial effects in debt. |
...
| `animal_mu_relatedness` | Animal-model Gaussian `mu`/`sigma` intercept and one-slope `mu` effects | partial | moderate to high for sparse large pedigrees | Dense pedigree and known `A`/`Ainv` first slices are fitted, and the A-matrix sigma-only and matched `mu+sigma` one-slope native point-fit/extractor cells are source-tested; keep sparse large-pedigree construction, multiple slopes, labelled structured slope covariance, bridge/inference evidence for the matched slope cell, direct-SD grammar, and non-Gaussian animal effects in debt. |
...
| `relmat_mu_relatedness` | `relmat()` Gaussian `mu`/`sigma` intercept and one-slope `mu` effects | partial | moderate for matrix conditioning | Known `K`/`Q` intercept and one-slope first slices are fitted, and the K/Q sigma-only and matched `mu+sigma` one-slope native point-fit/extractor cells are source-tested; keep multiple slopes, labelled structured slope covariance, bridge/inference evidence for the matched slope cell, direct-SD grammar, and non-Gaussian `relmat()` effects in debt. |
...
- Debt: formal recovery grids, zero-inflated structured boundaries, labels,
  simultaneous structured count types, and pure, multiple, or labelled
...
| Structured non-Gaussian dependence | Ordinary Poisson and ordinary NB2 now fit one q=1 structured `mu` intercept through `phylo()`, `phylo_interaction()`, `spatial()`, `animal()`, or `relmat()`, and one unlabelled intercept-plus-one-slope term through `phylo()`, `spatial()`, `animal()`, or `relmat()`; pure, multiple, or labelled structured count slopes, simultaneous structured types, zero-inflation, scale, and cross-parameter covariance paths remain blocked. | Known dependence matrices, Laplace random effects, sparse group support, overdispersion, and non-Gaussian links can create boundary and runtime failures before biology is interpretable. | Treat the Poisson and NB2 q=1 structured rows as source-test, smoke, or formal-admission evidence only. The phylogenetic intercept routes carry the longest smoke/formal audit trail; spatial, animal, and `relmat()` have source-level recovery tests and count-structured q1 artifacts, but all one-slope cells still need their own operating-characteristic evidence before any broad simulation parity claim. |
