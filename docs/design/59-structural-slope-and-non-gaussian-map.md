...
| Selected ordinary non-Gaussian `mu` random intercepts | Yes, first slice | Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial fit ordinary unlabelled `mu` random intercepts such as `(1 | id)` | Random slopes, labelled covariance, `sigma` or shape random effects, and structured dependence |
