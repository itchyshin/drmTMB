...
| Phylogenetic Gaussian effects | Yes | `phylo(1 | species, tree = tree)` fits univariate Gaussian `mu` and/or `sigma` intercepts; `phylo(1 + x | species, tree = tree)` fits independent phylogenetic intercept and slope fields for univariate Gaussian `mu`; matching bivariate `mu1`/`mu2`, selected q=4 location-scale, `sd(..., level = "phylogenetic")` direct-SD surfaces, and q=2 phylogenetic `corpair()` routes are also fitted. Deprecated `sd_phylo*()` spellings remain compatibility aliases | Multiple phylogenetic slopes, residual-scale structured slopes, phylogenetic slope correlations, bivariate phylogenetic slopes, and phylogenetic non-Gaussian effects |
