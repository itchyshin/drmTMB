...
| `log(sigma)` clamp | `src/drmTMB.cpp` guard; `drm_control(logsigma_clamp = ..., logsigma_clamp_margin = ...)` | `c(-12, 12)` with margin 3: identity in `[-12, 12]`, saturating to `[-15, 15]`; `NULL` disables | Numerical guard so a runaway Gaussian scale predictor cannot overflow `log(sigma)`; bit-identical inside the band. Widen or disable for legitimately huge unstandardized scales. The first fixed-effect sensitivity pilot shows negligible impact when inactive and material impact when the default band binds; fits now warn when the clamp is active at the optimum (docs 170, 176). |
| `se` | `drm_control()` | `TRUE` | `FALSE` skips `sdreport()` (no Wald SE/`vcov`/Wald CI); point estimates, prediction, simulation, profiles remain. |
| `missing = miss_control(...)` | `drmTMB()` | drop responses, fail on missing predictors | Missing-data policy; `response = "include"` masks supported observations for every current univariate fitted response route and bivariate Gaussian partial-response rows; `predictor = "model"` remains the separate explicit `mi()` surface. |
