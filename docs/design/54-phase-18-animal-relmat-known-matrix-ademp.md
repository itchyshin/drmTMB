...
| 7. Code availability | Runnable user examples now live in the structural-dependence article, and the first q=2 DGP helper, summariser, smoke runner, and CSV grid writer live under `inst/sim/`; the interval-status plan is in `docs/design/55-phase-18-animal-relmat-q2-interval-status.md`, while interval artifact code remains to be added. |
