...
3. **Ordinary Gaussian slope pilots.** Run Gaussian `mu` q=3/q=4 and
...
4. **Count `mu` slope pilot.** Run ordinary Poisson and NB2 independent slopes
   together only when the report keeps family-specific rows. Keep
   zero-inflated, hurdle, and labelled covariance requests out.
5. **Source-tested non-Gaussian smoke artifacts.** Give Student-t, lognormal,
...
6. **Structured Gaussian one-slope wrapper.** Run `phylo()`, `spatial()`,
