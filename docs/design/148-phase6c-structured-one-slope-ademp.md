...
`spatial()` is the artifact-ahead route because it already has the manual
`spatial_mu_slope` Phase 18 task. `phylo()`, `animal()`, and `relmat()` are
fitted/source-tested and registry wrapper targets, but still need standalone
artifact writers before they can be treated as Actions-ready one-slope lanes.
...
known-matrix one-slope routes need separate wrapper artifacts.
...
| Artifact maturity | route-specific route status | `spatial_mu_slope` task status or wrapper-target status for `phylo()`, `animal()`, and `relmat()` |
...
| Route artifact readiness | `spatial_mu_slope` task status versus wrapper-target status for `phylo()`, `animal()`, and `relmat()` |
...
| 7. Code availability | To be recorded when the missing wrapper DGPs, runners, or grid writers are added under `inst/sim/`. |
