...
| Structural dependencies | partial | planned | partial | partial | planned | planned | partial | planned | partial | planned | Animal, phylo, relmat, spatial, kernel, and SPDE rows each need provenance, PSD/name-alignment tests, recovery, bridge parity, docs, and visuals. Sub-type milestone (Curie+Fisher verified; aggregate point cell HELD partial -- 1 of 6 sub-types): native Gaussian relmat (user-supplied known K) random-intercept POINT recovery is clean (500 reps x n_id in {40, 80}; 0 errors; pdHess 1.000; fixed effects unbiased; sd_relmat -3.0% at n_id=40 shrinking to -1.0% at n_id=80, the expected ML small-sample bias; fixed-effect Wald coverage 0.936-0.960). The cell stays partial until additional sub-types (animal/phylo/spatial/kernel/SPDE) have comparable recovery (see docs/dev-log/simulation-artifacts/2026-06-20-relmat-structured-recovery/). |
