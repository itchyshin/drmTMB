...
6. Continue the broader numerical-guard programme beyond the fixed-effect
   ...
   `sigma1`/`sigma2` clamp-sensitivity diagnostic, the first Student-t Wald
   calibration diagnostic, and the ADEMP design in
   `docs/design/176-numerical-guard-simulation-audit.md`: larger bivariate
   scale-route grids, larger skew-normal guard grids, Student-t
   profile/bootstrap calibration, additional random-effect and structured
