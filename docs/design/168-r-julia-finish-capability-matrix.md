...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Dashboard and issue ledger | covered | partial | covered | unsupported | unsupported | unsupported | covered | covered | unsupported | partial | Post-#634 mission-control text now treats native R/TMB Big 4 evidence as banked diagnostic evidence and names direct DRM.jl, then Julia-via-R registry/parity, as the next companion gates. Keep `status.json`, `sweep.json`, check-log, and after-task notes synchronized with issues. |
...
5. Open follow-on bridge issues before relaxing any R-side Julia gate. The
   ...
   The post-#634 work order is direct DRM.jl evidence first from a clean
   DRM.jl worktree, then Julia-via-R registry/parity evidence in `drmTMB`;
   the user's saved DRM.jl checkout may contain unrelated Ayumi files and
   should not be reused as the implementation surface.
