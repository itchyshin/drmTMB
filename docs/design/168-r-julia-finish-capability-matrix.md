...
| Random slopes | partial | planned | covered | partial | partial | planned | partial | covered | partial | planned | A 500-replicate native R/TMB Gaussian correlated random-slope recovery (bf(y ~ x + (1 + x | id), sigma ~ 1); n_group in {40, 80}; 0 errors; pdHess 1.000; Curie+Fisher verified) promotes point estimates to covered, scoped to POINT recovery: fixed effects near-unbiased (rel bias <= 1%) at both group counts, and the random intercept/slope SDs consistent with the expected ML small-sample downward bias (sd_slope -6.7% at n_group=40 shrinking to -1.1% at n_group=80). A Florence-approved recovery figure (docs/dev-log/figure-audits/2026-06-20-random-slope-recovery/) of this verified artifact promotes the visual cell to covered (dots + MC error bars + zero line; POINT recovery + RE-SD consistency, not coverage). The random-effect correlation rho was not validated and RE-SD interval calibration is not claimed; Wald stays partial (n_group=40 b1 coverage 0.922 < 0.93). A 500-replicate-per-cell profile-likelihood calibration on the same DGP (`confint(method="profile")` for the two fixed-effect mu coefficients; n_group {40,80}; 0 fit errors, 0 profile CI failures, pdHess 1.000; Fisher verified; `docs/dev-log/simulation-artifacts/2026-06-20-random-slope-profile-calibration/`) moves the **profile** cell to partial, scoped to the two fixed-effect mu coefficients only. Profile is parity-or-better than Wald at every cell (slightly wider, the better small-sample interval): all four profile cells sit at/above the 0.93 floor and are nominal by n_group=80 (0.948-0.966), where the n_group=40 slope Wald is 0.922. Held at partial rather than covered because the n_group=40 profile cells (slope 0.932, intercept 0.938) are within ~1 Monte-Carlo SE of the floor, and RE-SD interval calibration, rho, bootstrap, and the Julia bridge are not addressed. Independent-slope-only, non-Gaussian, structured/phylogenetic slopes, bootstrap, and the Julia bridge remain planned. |
...
| Structural: relmat (known relatedness) | partial | planned | covered | partial | planned | planned | planned | planned | partial | planned | Native Gaussian known-K (user-supplied) random-intercept POINT recovery covered (500 reps x n_id {40,80}; 0 errors; pdHess 1.000; sd_relmat -3.0% -> -1.0%, the expected ML small-sample bias; fixed-effect Wald 0.936-0.960; Curie+Fisher). RE-SD interval calibration (profile/bootstrap), R-Julia bridge parity, docs, and visuals planned. (Per-sub-type row minted 2026-06-20, owner-approved, replacing the Structural dependencies aggregate.) See docs/dev-log/simulation-artifacts/2026-06-20-relmat-structured-recovery/. |
| Structural: animal (pedigree) | partial | planned | covered | partial | planned | planned | planned | planned | partial | planned | Native Gaussian pedigree numerator-relationship-matrix random-intercept POINT recovery covered (500 reps x n_id {40,80}; 0 errors; pdHess 1.000; sd_animal -3.1% -> -1.3%; fixed-effect Wald 0.924-0.960; Curie+Fisher). RE-SD interval calibration, bridge parity, docs, and visuals planned. See docs/dev-log/simulation-artifacts/2026-06-20-animal-pedigree-recovery/. |
| Structural: spatial (coordinate kernel) | partial | planned | covered | partial | planned | planned | planned | planned | partial | planned | Native Gaussian fixed exponential coordinate kernel (range = median pairwise distance) random-intercept POINT recovery covered (500 reps x n_site {20,40}; 0 errors; pdHess 1.000; sd_spatial -10.9% -> -2.8%). Intercept Wald under-nominal (0.888-0.910; spatial-mean confounding) -> Wald partial. Estimated-range Matern kernel (design 179), RE-SD intervals, bridge parity, docs, and visuals planned. See docs/dev-log/simulation-artifacts/2026-06-20-spatial-coords-recovery/. |
| Structural: phylo (tree + interaction) | partial | planned | partial | partial | planned | planned | partial | planned | partial | planned | Single-tree phylo SD is weakly identified (sd_phylo -32% at 60 species -> -4.8% at 240; intercept Wald 0.728 -> 0.916) -> POINT held partial as an honest identifiability diagnostic. The coevolutionary phylo_interaction term recovers better (sd_coev -6.4% -> -1.6%; design 178 Stage 0). Adequate-N recovery, the additive multi-component fit (design 178 Stage 1 engine extension), profile/bootstrap, R-Julia bridge (Route A pending DRM.jl), docs, and visuals planned. See docs/dev-log/simulation-artifacts/2026-06-20-{phylo-sd,coevolution-phylo-interaction}-recovery/. |
| Structural: kernel (estimated NNGP/Matern) | planned | planned | planned | planned | planned | planned | planned | planned | planned | planned | Estimated-kernel (NNGP / Matern with estimated range and smoothness) covariance is net-new -- the scalable replacement for fixed relmat/spatial matrices. Plan: design 179 (in-process Julia) + DRM.jl#270 + GLLVM.jl#62 (Matern-SPDE FEM core). Not implemented. |
| Structural: SPDE (mesh) | planned | planned | planned | planned | planned | planned | planned | planned | planned | planned | SPDE/mesh: `spatial(..., mesh = mesh)` parses but the fit aborts ("planned but not implemented yet", R/drmTMB.R). Plan: adopt the GLLVM.jl#62 Matern-SPDE FEM core (MIT; provenance in inst/COPYRIGHTS). Not implemented. |
...
| Missing values | partial | planned | planned | planned | planned | planned | planned | planned | planned | planned | Use likelihood/FIML-style masks. Complete-data all-true masks must match current complete-data log-likelihood exactly. The narrower per-cell `gaussian_response_mask` Julia-bridge capability row is `covered` at univariate-Gaussian-response-mask granularity (engine-vs-engine point + logLik + coefficient + Wald-endpoint parity, not interval coverage); the aggregate Missing-values bridge cell stays `planned` (missing predictors, non-Gaussian masks, bivariate-mask parity, and EM/FIML remain open). |
