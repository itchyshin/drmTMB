...
- `drmTMB#547` fixed q4 Julia REML option forwarding. SEPARATELY, the native TMB
  ...
  block), plus the exact matching labelled fixed-covariance spatial and
  supplied-`K` `relmat()` bivariate q2 location-intercept cells. The supplied-`K`
  relmat cell reuses the existing native-TMB exact-Gaussian REML engine and is
  recovery-grade only; `Q`, `animal()`, slopes, q4+, scale-side structure,
  intervals, and coverage do not inherit that admission. The scale-side REML is
  real, not a phantom row. Authoritative ML-vs-REML
