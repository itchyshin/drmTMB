...
| Gaussian `sigma` | Historical sprint state: fitted for random intercepts and multiple independent numeric slopes on `log(sigma)`. Current 0.6.0 also fits unlabelled ordinary correlated intercept-slope and multi-slope blocks. | Keep labelled residual-scale slope covariance and cross-formula `mu`-`sigma` slope covariance planned. |
...
| `phylo()`, `spatial()`, `animal()`, `relmat()` | Historical sprint state, superseded by current Gaussian q1 `sigma` and exact non-Gaussian provider gates. | Keep multiple or labelled structured slopes, structured slope correlations, spatial sigma-slope intervals, structured `rho12`, and non-Gaussian structured slopes outside the exact gates planned. |
...
| Gaussian `sigma` slopes | Historical sprint evidence covered random intercepts and multiple independent numeric slopes on `log(sigma)`, such as `sigma ~ z + (0 + w | id)`. Current 0.6.0 also fits unlabelled ordinary correlated intercept-slope and multi-slope blocks. | `tests/testthat/test-gaussian-random-intercepts.R` checks independent and multiple residual-scale terms, prediction contribution on the `log(sigma)` scale, `sdpars$sigma`, and `profile_targets()`. `tests/testthat/test-phase18-gaussian-sigma-random-slope.R` checks the Phase 18 smoke runner. | Labelled residual-scale slope covariance and cross-formula slope-level mean-scale covariance remain planned. |
...
| Beta-family `mu` | Yes | Yes | `bounded_response_mu_random_intercept` artifact lane | Source-tested independent slopes cover beta and zero-one-beta `mu`; keep correlated bounded-response slopes, beta-family distributional-parameter random effects outside the exact beta `animal()` gate, and structured zero-one-beta effects planned. |
...
| Tweedie, zero-one beta, cumulative-logit ordinal, hurdle/zero-inflated counts, shape parameters | Current fitted status includes inference-ready-with-caveats exact `mu` slope cells for Tweedie, zero-one beta, and cumulative logit plus exact row-specific neighbours; this historical #441 table predates those gates | No #441 slope admission | Separate later source-test or row-specific lanes | Do not attribute the later non-Gaussian random-slope support to #441. |
...
| --- | --- | --- | --- | --- | --- |
| `phylo()` | Fitted for one numeric `mu` slope and the exact q1 `sigma` one-slope route. | Fitted for selected bivariate `mu1`/`mu2` location rows and q2 phylogenetic `corpair()` routes. | Fitted for selected location-scale blocks, but q4 rows remain diagnostic-heavy and interval-limited. | Manual `phylo_mu_slope` Actions task plus Poisson/NB2 q1 phylogenetic formal tasks. The count tasks are separate lanes, not Gaussian one-slope evidence. | Multiple or labelled phylogenetic slopes, phylogenetic slope correlations, non-Gaussian phylogenetic slopes beyond exact recovery routes, and structured `rho12`. |
| `spatial()` | Fitted for one coordinate-spatial `mu` slope and a q1 `sigma` one-slope point-fit/extractor route; mesh/SPDE slopes remain planned. | Fitted for constant bivariate spatial `mu1`/`mu2` q2 covariance with DGP, smoke, and grid helpers. | Fitted for constant coordinate-spatial q4 location-scale blocks as smoke/artifact evidence; recovery and interval-status evidence remain separate. | Manual `spatial_mu_slope` Actions task plus spatial q2 smoke/grid helpers; ordinary Poisson/NB2 q1 count `mu` intercept and unlabelled one-slope cells have focused source tests. | Mesh/SPDE, multiple or labelled spatial slopes, spatial slope correlations, the spatial sigma-slope interval gate, spatial direct-SD regression, spatial `corpair()` regression, and pure, multiple, or labelled count spatial slopes. |
| `animal()` | Fitted for one dense-pedigree or known-matrix Gaussian `mu` slope and the exact A-matrix q1 `sigma` one-slope route. | Fitted for known-matrix bivariate `mu1`/`mu2` q2 covariance with smoke/grid artifacts. | Fitted for constant all-four q4 location-scale blocks as point-estimate smoke; derived q4 intervals remain unavailable. | Manual `animal_mu_slope` Actions task. Animal/`relmat()` q2 and q4 smoke/grid helpers remain separate; ordinary Poisson/NB2 q1 count `mu` intercept and unlabelled one-slope cells have focused source tests. | Sparse large-pedigree speed claims, multiple or labelled animal slopes, slope correlations, predictor-dependent `corpair()` regression, pure, multiple, or labelled count animal slopes, and direct-SD grammar. |
| `relmat()` | Fitted for one known-matrix Gaussian `mu` slope and the exact K/Q q1 `sigma` one-slope route. | Fitted for known-matrix bivariate `mu1`/`mu2` q2 covariance with smoke/grid artifacts. | Fitted for constant all-four q4 location-scale blocks as point-estimate smoke; derived q4 intervals remain unavailable. | Manual `relmat_mu_slope` Actions task. Animal/`relmat()` q2 and q4 smoke/grid helpers remain separate; ordinary Poisson/NB2 q1 count `mu` intercept and unlabelled one-slope cells have focused source tests. | Multiple or labelled `relmat()` slopes, slope correlations, predictor-dependent `corpair()` regression, pure, multiple, or labelled count `relmat()` slopes, and direct-SD grammar. |

The current registry handles this split in
`inst/sim/registry/phase18_structured_workflow_registry.csv`: Gaussian
`phylo()`, `spatial()`, `animal()`, and `relmat()` one-slope rows are
`ready_grid` and route through manual opt-in Actions tasks. The task names are
`phylo_mu_slope`, `spatial_mu_slope`, `animal_mu_slope`, and
`relmat_mu_slope`; all are excluded from `task = "all"` and are not recovery,
coverage, or power evidence. Structured q2 rows are `ready_or_smoke`;
...
| --- | --- | --- |
| Fitted | Ordinary Gaussian `mu` random intercepts/slopes, independent Gaussian `sigma` slopes, selected bivariate Gaussian covariance, residual `rho12`, Gaussian structured `phylo()`/`spatial()`/`animal()`/`relmat()` intercept and one-slope `mu` and q1 `sigma` rows, ordinary Poisson/NB2 `mu` slopes, exact q1 NB2 structured `sigma` intercept-plus-one-slope routes, selected ordinary non-Gaussian `mu` slopes, diagnostic-only Poisson `zi ~ spatial()`, diagnostic-only fixed-`zi` Poisson `mu ~ spatial()`, and diagnostic-only fixed-`zi` NB2 `mu ~ spatial()`. | `docs/design/46-pre-simulation-readiness-matrix.md`, `docs/design/59-structural-slope-and-non-gaussian-map.md`, `vignettes/implementation-map.Rmd`, and the current README/model-map status rows. |
...
| Diagnostic-only | Ordinary or structured q=4 `corpairs()` rows, q=4 spatial/animal/`relmat()` location-scale point-estimate lanes, count q=1 spatial/animal/`relmat()` structured rows before formal recovery, the exact fixed-`zi` NB2 `mu ~ spatial()` intercept, and Ayumi hard real-data q2/q4 cases. | Registry rows marked `diagnostic_only`, plus hard-fit diagnostics in `docs/design/46-pre-simulation-readiness-matrix.md`. |
...
| Unsupported or blocked | Count labelled q2/q4 covariance, random effects in `rho12`, correlated non-Gaussian slopes, pure or multiple structured count slopes, zero-inflation beyond the exact Poisson q1 `zi ~ spatial()`, fixed-`zi` Poisson `mu ~ spatial()`, and fixed-`zi` NB2 `mu ~ spatial()` gates, hurdle random effects beyond the exact truncated-NB2 q1 `hu ~ relmat(K/Q)` gate, Student-t `nu` random effects beyond the exact q1 `nu ~ phylo()` gate, ordinal random effects beyond ordinary `mu` intercepts/slopes and the exact q1 `mu ~ phylo()` gate, ordinary NB2 `sigma` slopes, and richer or labelled structured `sigma` beyond the exact fitted q1 Gaussian and NB2 routes. | Registry rows marked `blocked`, failure-ledger text in the readiness matrix, and the implementation-map planned-neighbour rows. |
...
labelled residual-scale and cross-formula `mu`-`sigma` slope covariance planned;
unlabelled ordinary correlated sigma slope blocks are now fitted.
