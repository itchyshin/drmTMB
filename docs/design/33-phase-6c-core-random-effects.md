...
| Non-Gaussian families | Fixed-effect non-Gaussian families are implemented; ordinary binomial/Poisson/NB2 and selected other families fit `mu` random intercepts and independent slopes; binomial also has one experimental complete-data unlabelled q2 point-fit route; ordinary NB2, lognormal, and Gamma have separate log-`sigma` random-intercept gates | Revisit correlated or labelled slopes outside the exact binomial q2 route and family-specific scale slopes after the first intercept gates; keep combined lognormal/Gamma `mu`+`sigma`, shape, inflation, hurdle, ordinal, structured, and cross-parameter covariance blocks later | family-specific simulations show convergence, boundary behaviour, recovery, and useful failure messages on both model and response scales |
...
| Non-Gaussian families | Fixed-effect likelihoods plus bounded ordinary `mu` random-effect gates including binomial, one experimental complete-data unlabelled binomial q2 point-fit route, separate ordinary NB2/lognormal/Gamma log-`sigma` random intercepts, and ordinary Poisson/NB2 q=1 structured `mu` routes in bounded source-test and smoke gates | Correlated/labelled non-Gaussian `mu` slopes outside the exact binomial q2 route, `sigma` slopes or structured scale effects outside NB2's bounded gate, combined positive-continuous `mu`+`sigma`, other scale/shape/inflation/hurdle/ordinal random effects, cross-parameter covariance, and broader structured non-Gaussian paths |
...
- matching q2 `sigma1`/`sigma2` scale-slope covariance and same-response q2
  `mu`/`sigma` slope covariance were completed later; p8/q8 bivariate
  location-scale slope endpoints remain outside the fitted surface after the
  matching slope-only and q=4/q=6 `mu1`/`mu2` location slices;
...
independent numeric slopes, q=1 structured `mu` intercept terms through
`phylo()`, `phylo_interaction()`, `spatial()`, `animal()`, or `relmat()`, and
unlabelled intercept-plus-one-slope terms through `phylo()`, `spatial()`,
`animal()`, or `relmat()`. For percentage or proportion data, zero-one inflation is a bounded
response likelihood problem first; `zoi`
...
- all-four bivariate location-scale slope blocks and broader bivariate
  residual-scale slope endpoints beyond the matching q2 same-response and
  scale-scale slices;
