...
| Non-Gaussian families | Fixed-effect non-Gaussian families are implemented; ordinary binomial/Poisson/NB2 and selected other families fit `mu` random intercepts and independent slopes; ordinary NB2, lognormal, and Gamma have separate log-`sigma` random-intercept gates | Revisit correlated or labelled slopes and family-specific scale slopes after the first intercept gates; keep combined lognormal/Gamma `mu`+`sigma`, shape, inflation, hurdle, ordinal, structured, and cross-parameter covariance blocks later | family-specific simulations show convergence, boundary behaviour, recovery, and useful failure messages on both model and response scales |
