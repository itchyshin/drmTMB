...
| Skew-normal and skew-t | `skew_normal()` now has a fitted univariate fixed-effect `mu`/`sigma`/`nu` first slice with density, normal-limit, deterministic recovery, Gaussian-limit false-positive, interval-status, method, simulation, and malformed-neighbour source tests. Skew-t remains planned. | A small fixed-effect skew-normal example can now use `bf(y ~ x, sigma ~ z, nu ~ w)` inside the first-slice boundary. Keep random effects, structured effects, known covariance, bivariate skew-normal, residual `rho12`, latent `skew(id)`, skew-t, and broad formal-grid claims out of worked examples. |
