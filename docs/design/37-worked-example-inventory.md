...
| --- | --- | --- |
| Animal and `relmat()` known matrices | The focused animal and `relmat()` articles now have runnable examples for univariate `animal(Ainv = Ainv)` / `relmat(Q = Q)` in `mu` and/or `sigma`, one-slope Gaussian `mu` routes, bivariate q=2 location covariance, and constant q=4 location-scale covariance, with `check_drm()`, `corpairs()`, and profile-target status in the reader path. The first sigma-only and matched `mu+sigma` one-slope runtime cells for A-matrix `animal()` and K/Q `relmat()` are source-tested as native point-fit/extractor cells, but reader-facing example prose for those residual-scale and matched slope cells still needs a focused article refresh. Pedigree construction at scale, multiple slopes, labelled structured slope covariance, bridge/inference evidence for matched slope cells, predictor-dependent `corpair()`, non-Gaussian relatedness effects, and direct-SD grammar remain planned. | Use the focused structural-dependence articles for precomputed relatedness-matrix intercept and `mu`-slope examples; treat the sigma-only and matched one-slope cells as source-tested native runtime evidence until the examples are refreshed, and next add an ADEMP q=4 addendum before admitting broad animal/`relmat()` q=4 grids. |
