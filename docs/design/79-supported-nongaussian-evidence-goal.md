...
implemented fixed-effect family suite, the ordinary count random-effect first
...
| --- | --- | --- | --- | --- |
| `student()` | Fitted `mu`, `sigma`, and `nu`; likelihood, interval, malformed-input, and fixed-effect shape simulation evidence exist. | Blocked for `sigma`, `nu`, and `mu` random effects. | Blocked for phylogenetic, spatial, animal, `relmat()`, and bivariate Student-t paths. | Supported fixed-effect heavy-tail route; no random-effect or structured claim. |
| `lognormal()` | Fitted `mu` on the log-response scale and `sigma`; likelihood, fitted-response, and boundary tests exist. | Blocked. | Blocked. | Supported fixed-effect positive-response route. |
| `Gamma(link = "log")` | Fitted mean-CV route with `log(mu)` and `log(sigma)`; likelihood, recovery, prediction, and non-log-link boundary tests exist. | Blocked. | Blocked. | Supported fixed-effect positive-response route. |
| `beta()` | Fitted strict `(0, 1)` response route with `logit(mu)` and public `sigma`; likelihood, prediction, Wald-row, and boundary tests exist. | Blocked. | Blocked. | Supported fixed-effect bounded-response route; exact 0/1 mass remains planned. |
| `beta_binomial()` | Fitted denominator-aware route for `cbind(successes, failures)` with `mu` and `sigma`; likelihood, denominator, prediction, Wald-row, and boundary tests exist. | Blocked. | Blocked. | Supported fixed-effect success-rate route. |
...
| `truncated_nbinom2()` | Fitted positive-count fixed-effect route with `mu` and `sigma`; likelihood, prediction, and boundary tests exist. | Blocked. | Blocked. | Supported fixed-effect positive-count route. |
...
- ordinary Poisson and NB2 `mu` random intercepts and independent numeric
  slopes are admitted as first non-Gaussian random-effect slices;
...
- zero-truncated NB2 random effects, zero-inflation or hurdle random effects,
  correlated count slopes, labelled non-Gaussian covariance blocks, structured
  count slopes, NB2 `sigma` slopes, and non-Gaussian spatial/animal/`relmat()`
  effects remain planned or blocked.
...
> suite, plus first-slice ordinary Poisson/NB2 count random effects, the narrow
> NB2 log-`sigma` random-intercept gate, and smoke/formal-admission
