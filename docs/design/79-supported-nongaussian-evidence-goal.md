...
| `stats::binomial(link = "logit")` | Fitted fixed-effect ordinary Bernoulli/binomial route for 0/1 and `cbind(successes, failures)` responses with `logit(mu)` and no public `sigma`; likelihood, malformed-input, method-surface, simulation, and `stats::glm()` parity tests exist. | Blocked; the first slice is fixed-effect only. | Blocked, including Julia bridge support until separate parity evidence promotes it. | Supported fixed-effect event-probability route for ordinary binomial sampling variation. |
