...
| `animal()` models | Admitted for the known-matrix Gaussian `mu` intercept and matching bivariate `mu1`/`mu2` q=2 location covariance design lanes; the q=2 lane now has a first DGP, summariser, smoke runner, CSV grid writer, and interval-status plan | `docs/design/54-phase-18-animal-relmat-known-matrix-ademp.md` records the intercept and q=2 condition tables before broad grids; `docs/design/55-phase-18-animal-relmat-q2-interval-status.md` gates interval artifacts | Pedigree construction, structured slopes, `sigma`, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain failure-ledger rows |
| `relmat()` models | Admitted for the known-matrix Gaussian `mu` intercept and matching bivariate `mu1`/`mu2` q=2 location covariance design lanes; the q=2 lane now has a first DGP, summariser, smoke runner, CSV grid writer, and interval-status plan | `docs/design/54-phase-18-animal-relmat-known-matrix-ademp.md` records the intercept and q=2 condition tables before broad grids; `docs/design/55-phase-18-animal-relmat-q2-interval-status.md` gates interval artifacts | Structured slopes, `sigma`, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain failure-ledger rows |
