...
| NB2 phylogenetic q=1 `mu` | Fitted smoke/formal-admission surface with local audit hold | log-mean overdispersed count model with `phylo(1 | species, tree = tree)` in `mu`; `sigma` remains fixed-effect overdispersion; ordinary grouped species-intercept comparator recorded beside target rows; local Slices 541-555 artifacts pass QA but keep the route at `hold_smoke_only` until the 500-replicate formal grid runs | species, observations per species, mean count, overdispersion, phylogenetic SD, tree shape |
...
still need a separate runtime and artifact-review decision before they are
treated as routine evidence. The failure ledger in
