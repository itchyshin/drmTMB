...
| `animal()` models | Admitted for the known-matrix Gaussian `mu` intercept and matching bivariate `mu1`/`mu2` q=2 location covariance design lanes; simulation code still needs a DGP helper and runner | `docs/design/54-phase-18-animal-relmat-known-matrix-ademp.md` records the intercept and q=2 condition tables before broad grids | Pedigree construction, structured slopes, `sigma`, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain failure-ledger rows |
| `relmat()` models | Admitted for the known-matrix Gaussian `mu` intercept and matching bivariate `mu1`/`mu2` q=2 location covariance design lanes; simulation code still needs a DGP helper and runner | `docs/design/54-phase-18-animal-relmat-known-matrix-ademp.md` records the intercept and q=2 condition tables before broad grids | Structured slopes, `sigma`, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain failure-ledger rows |
