...
| Phylogenetic structured effects | Admitted for fitted Gaussian `mu` and `sigma` intercepts, matching univariate `mu`/`sigma` correlation, one numeric `mu` slope, direct-SD, and selected bivariate covariance subsets; ordinary Poisson q=1 phylogenetic `mu` is fitted but remains a separate smoke/ADEMP-planning lane | Write small tree-size and phylogenetic-signal condition tables for the fitted Gaussian intercept/direct-SD/one-slope surfaces; keep the Poisson q=1 lane separate until the runner contract in `docs/design/72-poisson-phylo-q1-runner-contract.md` is implemented and recovery grids exist | Keep multiple phylogenetic slopes, residual-scale structured slopes, Poisson phylogenetic slopes, NB2 or zero-inflated phylogenetic effects, direct-SD formulas combined with structured `sigma`, structured `rho12`, and predictor-dependent q=4 correlations out |
