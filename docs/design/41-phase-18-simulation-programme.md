...
| `animal()` models | Admit only the known-matrix Gaussian `mu` intercept first slice for now; matching bivariate q=2 is fitted but still needs its own ADEMP sheet | Write a tiny `A`/`Ainv` condition table after the fitted example, diagnostics, and profile-target checks are stable; add a separate q=2 sheet before admitting bivariate grids | Pedigree construction, structured slopes, `sigma`, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain failure-ledger rows |
| `relmat()` models | Admit only the known-matrix Gaussian `mu` intercept first slice for now; matching bivariate q=2 is fitted but still needs its own ADEMP sheet | Write a tiny `K`/`Q` condition table after matrix-validation, dense-likelihood comparison, diagnostics, and profile-target checks are stable; add a separate q=2 sheet before admitting bivariate grids | Structured slopes, `sigma`, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain failure-ledger rows |
