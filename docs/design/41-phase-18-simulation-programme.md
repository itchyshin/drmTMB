...
| Counts | Admitted for fixed-effect count families and ordinary non-zero-inflated Poisson/NB2 `mu` random effects; ordinary Poisson q=1 phylogenetic `mu` is fitted but still smoke-level | `docs/design/49-phase-18-count-mu-random-effect-ademp.md` records the paired Poisson/NB2 `mu` random-effect ADEMP sheet; `docs/design/67-sdstar-p8-poisson-q1.md` records the first Poisson phylogenetic q=1 gate before any broad grid | Keep zero-inflated/hurdle random effects, zero-truncated NB2 random effects, correlated count slopes, NB2 structured effects, spatial/animal/`relmat()` count effects, and count-side structured slopes out |
