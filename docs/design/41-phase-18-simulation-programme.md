...
| `animal()` models | Admitted for the known-matrix Gaussian `mu` intercept and matching bivariate `mu1`/`mu2` q=2 location covariance design lanes; the q=2 lane now has a first DGP, summariser, smoke runner, and CSV grid writer | `docs/design/54-phase-18-animal-relmat-known-matrix-ademp.md` records the intercept and q=2 condition tables before broad grids; the next code step is a formal-condition runner with interval-status artifacts | Pedigree construction, structured slopes, `sigma`, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain failure-ledger rows |
| `relmat()` models | Admitted for the known-matrix Gaussian `mu` intercept and matching bivariate `mu1`/`mu2` q=2 location covariance design lanes; the q=2 lane now has a first DGP, summariser, smoke runner, and CSV grid writer | `docs/design/54-phase-18-animal-relmat-known-matrix-ademp.md` records the intercept and q=2 condition tables before broad grids; the next code step is a formal-condition runner with interval-status artifacts | Structured slopes, `sigma`, q=4 location-scale blocks, predictor-dependent `corpair()` regressions, and direct-SD grammar remain failure-ledger rows |
