...
| --- | --- | --- | --- |
| Continuous location-scale | Admitted for Gaussian location-scale first; fixed-effect lognormal, Gamma, Tweedie, and Student-t have family-specific estimands and artifact lanes; lognormal/Gamma have separate `mu` random-effect or `sigma` random-intercept gates, and Student-t has ordinary `mu` random effects | `docs/design/47-phase-18-gaussian-location-scale-ademp.md` is the first ADEMP sheet; `docs/design/111-phase-18-positive-continuous-fixed-effect-artifacts-slices-1299-1308.md` records the fixed-effect lognormal/Gamma artifact lane; `docs/design/118-phase-18-positive-continuous-mu-random-intercept-artifacts-slices-1369-1378.md` records the lognormal/Gamma ordinary `mu` random-intercept lane; `docs/design/119-phase-18-student-mu-random-intercept-artifacts-slices-1379-1388.md` records the Student-t ordinary `mu` random-intercept lane; `tests/testthat/test-nongaussian-mu-random-slopes.R` records the first CRAN-safe slope recovery smoke checks; `tests/testthat/test-tweedie-location-scale.R` records the first focused Tweedie fixed-effect recovery checks | Do not claim combined lognormal/Gamma `mu`+`sigma`, sigma slopes or labels, correlated positive-continuous or Student-t random slopes, `sigma` random effects beyond fitted Gaussian/NB2/lognormal/Gamma gates, shape/skewness random effects, phylogenetic shape, generalized Gamma, Tweedie extensions beyond fixed-effect intercept-only power, or latent ID-level skewness |
| Proportion and bounded responses | Admitted for fixed-effect beta, beta-binomial, plain binomial, and zero-one beta artifact lanes; beta, beta-binomial, and binomial have ordinary `mu` random intercept/slope first slices; binomial additionally has one experimental complete-data unlabelled q2 point-fit route; zero-one beta has exact ordinary q1 `zoi` and `coi` intercept/same-symbol slope gates at point-fit-recovery scope | `docs/design/50-phase-18-proportion-fixed-effect-ademp.md` records the fixed-effect beta/beta-binomial ADEMP sheet; `docs/design/110-phase-18-proportion-fixed-effect-artifacts-slices-1289-1298.md` records the beta/beta-binomial DGP/smoke/grid artifact lane; `docs/design/175-phase-18-binomial-fixed-effect-artifacts.md` records the plain binomial fixed-effect artifact lane and `stats::glm()` parity table; `docs/design/114-phase-18-zero-one-bounded-response-design-gate-slice-d3.md` records the fixed-effect zero-one beta design gate; `docs/design/115-phase-18-zero-one-beta-fixed-effect-artifacts-slices-1339-1348.md` records the zero-one beta DGP/smoke/grid artifact lane; `docs/design/117-phase-18-bounded-response-mu-random-intercept-artifacts-slices-1359-1368.md` records the beta/beta-binomial ordinary `mu` random-intercept lane; `tests/testthat/test-nongaussian-mu-random-slopes.R` records the first bounded-response slope smoke checks; the MSPL design and quadrature receipts record the q2 route | Keep zero-one beta random effects outside the exact named ordinary q1 gates, correlated bounded-response random slopes outside the exact binomial q2 route, broader bounded-response atom/`sigma` random effects, labelled/multiple or structured binomial routes and bivariate binomial models, binomial weights-as-trials, and bounded-response `meta_V(V = V)` in the failure ledger |
| Counts | Admitted for fixed-effect count families; ordinary Poisson/NB2 and zero-truncated-NB2 `mu` random effects; the ordinary NB2 log-`sigma` intercept; Poisson/NB2 q1 structured `mu` intercept and one-slope routes; exact recovery-grade NB2 q1 structured `sigma` routes; exact Poisson `zi ~ spatial()` and truncated-NB2 `hu ~ relmat(K/Q)` intercepts; one exact diagnostic-only fixed-`zi` Poisson `mu ~ spatial()` route; one exact diagnostic-only fixed-`zi` NB2 `mu ~ spatial()` route; and one exact crossed NB2 `mu ~ spatial(1 | site, coords = coords) + relmat(1 | id, Q = Q)` two-provider route at recovery-only grade | Family-specific ADEMP sheets, focused source tests, count structured q1 artifacts, the NB2 phylogenetic formal audit, and live-ledger diagnostic/recovery evidence record these distinct gates. The fixed-`zi` NB2 spatial-`mu` route has no recovery, interval, or coverage promotion; the fixed-`zi` Poisson spatial-`mu` route has no interval or coverage promotion. The crossed two-provider route recovers both variance components only on the crossed design and has no interval or coverage evidence. | Keep `zi`/`hu` random effects beyond the exact gates, correlated count slopes, ordinary NB2 `sigma` slopes, richer or labelled structured sigma blocks, structured-sigma intervals/coverage, simultaneous structured count types beyond the exact crossed NB2 gate, labelled q2/q4 count covariance, pure or multiple count-side structured slopes, COM-Poisson, and generalized Poisson out until a separate decision opens them |
| Ordinal | Admitted for fixed-effect `cumulative_logit()` location models, ordinary `mu` random intercepts and independent slopes at recovery grade, and one exact diagnostic-only q1 `mu ~ phylo(1 | id, tree = tree)` intercept with local point-fit/extractor evidence only | `docs/design/51-phase-18-ordinal-fixed-effect-ademp.md` records the fixed-effect ordinal ADEMP sheet; `docs/design/112-phase-18-ordinal-fixed-effect-artifacts-slices-1309-1318.md` records the DGP/smoke/grid artifact lane; `tests/testthat/test-arc2a-mu-random-intercept.R` and the ordinal slope tests cover ordinary random effects; `tests/testthat/test-cumulative-logit.R` records the exact phylogenetic intercept gate | Keep the exact phylogenetic gate diagnostic-only; other structured ordinal effects, interval/coverage promotion, scale/discrimination formulas, bivariate ordinal, and mixed-response ordinal models remain out |
...
| Bivariate Gaussian | Admitted for residual `rho12`, selected intercept covariance blocks, matching slope-only `mu1`/`mu2`, same-response `mu`/`sigma`, and `sigma1`/`sigma2` blocks, the q4 and q6 `mu1`/`mu2` location smoke lanes, and the q8 all-endpoint diagnostic smoke/recovery/staged-start lanes | `docs/design/52-phase-18-bivariate-rho12-ademp.md` records the residual-correlation ADEMP sheet; group-level `corpairs()`, slope-only covariance, q4 location covariance, q6 location covariance, q2 same-response smoke/recovery, q2 scale-slope covariance, and q8 endpoint covariance have separate task routes before broader coverage or power claims; `docs/design/161-phase-18-bivariate-q8-recovery-audit.md` records the 2026-06-07 q8 diagnostic hold audit; `biv_gaussian_q8_endpoint_staged_diagnostic` writes cold-versus-staged fit metrics, deltas, provenance, and scope as a diagnostic-only artifact | Keep mixed-response families, random effects in `rho12`, broad p8/q8 slope covariance, broad recovery, coverage, and power claims out |
| Random slopes | Admitted for ordinary Gaussian `mu` q > 2, independent and unlabelled correlated Gaussian `sigma` slope blocks, ordinary Poisson/NB2 `mu` independent slopes, selected Student-t/lognormal/Gamma/beta/beta-binomial/zero-truncated NB2 `mu` independent slopes, one experimental complete-data unlabelled binomial q2 point-fit route, coordinate-spatial, phylogenetic, animal-model, and `relmat()` Gaussian `mu` one-slope paths, the matching bivariate Gaussian `mu1`/`mu2` slope-only lane, the matching same-response bivariate Gaussian `mu`/`sigma` q2 slope lane, the matching bivariate Gaussian `sigma1`/`sigma2` q2 scale-slope lane, the q4 and q6 bivariate location smoke lanes, and the q8 bivariate endpoint diagnostic smoke/recovery/staged-start lanes | Give each admitted slope class its own condition table for group count, repeats, slope SD, and covariate spread; selected non-Gaussian slope rows remain focused source tests, the binomial q2 route has local objective/quadrature evidence only, and same-response q2, scale-slope q2, and q8 bivariate rows now have smoke/recovery/staged-diagnostic artifact decisions; the 2026-06-07 q8 audit explicitly keeps q8 out of coverage/power use | Keep p8/q8 coverage and power, correlated non-Gaussian routes outside the exact binomial q2 point-fit cell, multiple structured, structured slope-correlation, labelled or cross-formula residual-scale slope covariance, and broad recovery, coverage, and power claims outside named lanes |
| Shape and skewness | Admitted for fixed-effect Student-t `nu` smoke, artifact-path, Wald interval, profile-smoke, and bootstrap-smoke evidence; one exact q1 Student-t `nu ~ phylo(1 | id, tree = tree)` intercept is fitted at diagnostic-only local fit/extractor grade; fixed-effect skew-normal residual slant is admitted through the standalone `skew_normal_fixed_effect` smoke/grid lane | Extend fixed-effect Student-t and skew-normal from smoke/grid evidence to deliberately sized formal coverage grids after the interval evidence schema is stable; retain the exact Student-t phylogenetic `nu` gate as single-smoke diagnostic-only, with no recovery denominator | Keep every other `nu` random effect, future `tau` random effects, skewness random effects, skew-normal random/structured/bivariate routes, and latent `skew(id) ~ ...` out |
| Phylogenetic structured effects | Admitted for fitted Gaussian `mu` and `sigma` intercepts, matching univariate `mu`/`sigma` correlation, one numeric `mu` slope, the exact q1 `sigma` one-slope route, direct-SD, selected bivariate covariance subsets, ordinary Poisson/NB2 q1 phylogenetic `mu` intercept and one-slope terms, and the exact NB2 q1 structured `sigma` route at recovery grade | Write small tree-size and phylogenetic-signal condition tables for the fitted Gaussian intercept/direct-SD/one-slope surfaces; keep the Poisson and NB2 q1 lanes separate until their formal artifacts are run and audited beyond the current smoke/formal-admission evidence; `tests/testthat/test-count-structured-mu.R` covers the count q1 one-slope source gate | Keep multiple or labelled phylogenetic slopes, structured slope correlations, pure or multiple count slopes, zero-inflated phylogenetic effects, richer NB2 structured sigma, direct-SD formulas combined with structured `sigma`, structured `rho12`, and predictor-dependent q4 correlations out |
| Coordinate spatial structured effects | Admitted for univariate Gaussian `mu` and `sigma` intercepts, matching univariate `mu`/`sigma` correlation, one numeric coordinate-spatial `mu` slope, a q1 `sigma` one-slope point-fit/extractor route, constant bivariate q2/q4 covariance, ordinary Poisson/NB2 q1 spatial `mu` intercept and one-slope terms, the exact NB2 q1 structured `sigma` route at recovery grade, the exact diagnostic-only Poisson `zi ~ spatial()` intercept, the exact diagnostic-only fixed-`zi` Poisson `mu ~ spatial()` route, and the exact diagnostic-only fixed-`zi` NB2 `mu ~ spatial()` route | Extend the existing spatial one-slope and q=2 smoke design with site count, observations per site, field SD, covariate-spread, and q=4 endpoint conditions; `docs/design/56-phase-18-spatial-q2-ademp.md` records the q=2 spatial design sheet, while `phase18_run_spatial_q2_smoke()` and `phase18_write_spatial_q2_grid_outputs()` supply the seeded smoke runner, repeatable CSV artifacts, fixed-effect Wald tables, and profile-status ledgers before broad paired-response reports; `tests/testthat/test-count-structured-mu.R` covers the count q=1 source gate; the fixed-`zi` NB2 spatial-`mu` gate has no recovery, interval, or coverage promotion, and the fixed-`zi` Poisson route has no interval or coverage promotion | Keep mesh/SPDE, multiple or labelled slopes, the Gaussian spatial sigma-slope interval gate, slope correlations, richer structured sigma, spatial direct-SD, spatial `corpair()`, pure or multiple count slopes, and zero-inflated spatial effects outside the exact Poisson `zi`, fixed-`zi` Poisson `mu`, and fixed-`zi` NB2 `mu` gates out |
| `animal()` models | Admitted for the small dense-pedigree and known-matrix Gaussian `mu` and `sigma` intercept paths, one-slope `mu` paths, matching bivariate `mu1`/`mu2` q=2 location covariance design lanes, the first constant all-four q=4 location-scale smoke lane, and ordinary Poisson/NB2 q=1 `mu` intercept and unlabelled one-slope terms; the q=2 lane now has a first DGP, summariser, smoke runner, CSV grid writer, fixed-effect Wald artifacts, opt-in profile-status artifacts, and an animal-only pedigree smoke cell; the q=4 lane now has a DGP, summariser, smoke runner, CSV grid writer, and derived-correlation interval-status artifacts | `docs/design/54-phase-18-animal-relmat-known-matrix-ademp.md` records the intercept and q=2 condition tables before broad grids; `docs/design/55-phase-18-animal-relmat-q2-interval-status.md` defines which q=2 rows receive Wald intervals, profile intervals, or `not_requested` status; `docs/design/58-phase-18-animal-relmat-q4-ademp.md` records the q=4 addendum and derived-unavailable correlation policy; `tests/testthat/test-count-structured-mu.R` covers the count q=1 source gate | Sparse large-pedigree construction, multiple structured slopes, slope correlations, predictor-dependent `corpair()` regressions, pure, multiple, or labelled count animal slopes, and direct-SD grammar remain failure-ledger rows |
| `relmat()` models | Admitted for the known-matrix Gaussian `mu` and `sigma` intercept paths, one-slope `mu` paths, matching bivariate `mu1`/`mu2` q=2 location covariance design lanes, the first constant all-four q=4 location-scale smoke lane, and ordinary Poisson/NB2 q=1 `mu` intercept and unlabelled one-slope terms; the q=2 lane now has a first DGP, summariser, smoke runner, CSV grid writer, fixed-effect Wald artifacts, and opt-in profile-status artifacts; the q=4 lane now has a DGP, summariser, smoke runner, CSV grid writer, and derived-correlation interval-status artifacts | `docs/design/54-phase-18-animal-relmat-known-matrix-ademp.md` records the intercept and q=2 condition tables before broad grids; `docs/design/55-phase-18-animal-relmat-q2-interval-status.md` defines which q=2 rows receive Wald intervals, profile intervals, or `not_requested` status; `docs/design/58-phase-18-animal-relmat-q4-ademp.md` records the q=4 addendum and derived-unavailable correlation policy; `tests/testthat/test-count-structured-mu.R` covers the count q=1 source gate | Multiple structured slopes, slope correlations, predictor-dependent `corpair()` regressions, pure, multiple, or labelled count `relmat()` slopes, and direct-SD grammar remain failure-ledger rows |
...
| Binomial fixed effects | Fitted for plain binomial | `Y_i ~ Binomial(n_i, mu_i)`, `logit(mu_i) = beta0 + beta1 x_i`; 0/1 rows use `n_i = 1`, while `cbind(success, failure)` rows sample integer trial totals | sample size, trial count, mean contrast, `stats::glm()` parity, convergence and `pdHess` |
| Beta ordinary `mu` random effects | Source-tested first slice | `logit(mu_ij) = beta0 + beta1 x_ij + b0_j + x_ij b1_j`, with independent normal intercept and slope effects, fixed-effect `log(sigma_i)`, and strict `(0, 1)` responses | groups, observations per group, `sd_mu`, slope SD, covariate spread, mean contrast, precision, strict-boundary stress |
...
| Skew-normal fixed effects | Fitted for univariate residual asymmetry | public-moment skew-normal responses with `mu ~ x`, `sigma ~ z`, and `nu ~ w`; the DGP transforms to native skew-normal `xi`, `omega`, and `alpha = nu` before simulating | sample size, skew direction and strength, `nu` slope, scale slope, predictor correlation |
...
ordinary `mu` random-effect rows are admitted as focused first slices; selected
independent numeric non-Gaussian `mu` slopes have only focused source tests so
far. Larger grids still need a separate runtime and artifact-review decision
before they are treated as routine evidence. The failure ledger in
...
`sigma` random-slope, coordinate spatial `mu` slope, Student-t shape,
skew-normal fixed-effect, binomial fixed-effect, and bivariate residual
`rho12` smoke runners are wired through this helper. Student-t shape,
skew-normal fixed-effect, and
bivariate `rho12` use the helper's closure-aware
...
contract. Gaussian location-scale, Student-t shape, skew-normal fixed-effect,
and bivariate residual `rho12` grid writers forward replicate-runner `cores`
and `backend`; the paired count pilot and count-gallery smoke wrapper do the
same. Student-t shape, skew-normal fixed-effect, and bivariate residual `rho12`
keep separate `bootstrap_cores` and
...
26. Slice 236 reconciles the random-slope pre-simulation promise: ordinary
    ...
    endpoint route now has diagnostic smoke/recovery/staged-start artifacts,
    while q8 coverage/power evidence and non-Gaussian scale/shape random-effect
    slopes remain outside Phase 18 Wave A until their recovery gates close.
...
28. Slice 238 added a CRAN-safe smoke surface for Gaussian `sigma` independent
    one-slope random effects on `log(sigma)`. Later work fitted unlabelled
    correlated intercept-slope and multi-slope blocks; labelled and
    cross-formula scale-slope covariance remain outside Wave A.
...
161. Slices 1309-1318 add the fixed-effect ordinal artifact lane in
     ...
     `cumulative_logit()`. This historical artifact claim predates the later
     ordinary `mu` intercept/slope recovery gate and exact phylogenetic
     intercept gate; it still excludes correlated/labelled or other structured
     ordinal covariance, scale or discrimination formulas, bivariate ordinal,
     known-covariance ordinal, and mixed-response ordinal models.
...
168. Slices 1369-1378 add the positive-continuous ordinary `mu`
     ...
     `(1 | id)` in `mu`; independent numeric slopes now have focused source
     tests. It records fixed-effect Wald rows plus direct-SD profile rows and
     keeps correlated positive-continuous random slopes, labelled
...
169. Slices 1379-1388 add the Student-t ordinary `mu` random-intercept
     ...
     rows for `mu`, `sigma`, and `nu` plus direct-SD profile rows; independent
     numeric slopes now have focused source tests, while correlated Student-t
     random slopes, labelled covariance, `sigma` random
     effects, `nu` random effects, structured effects, known covariance, and
     bivariate Student-t models remain outside the fitted artifact claim.
170. Slices 1389-1398 add the zero-truncated NB2 ordinary `mu`
     ...
     `sigma ~ z`; independent numeric slopes now have focused source tests. It
     records fixed-effect Wald rows plus direct-SD profile rows, while keeping
     correlated zero-truncated NB2 random slopes, labelled
...
185. Slices 1689-1702 add the skew-normal implementation gate in
     ...
     slice now covers the source-level subset, and the
     `skew_normal_fixed_effect` Phase 18 lane adds repeatable DGP/smoke/grid
     artifacts. Formal high-replicate operating-characteristic evidence
     remains future work.
...
193. Slices 1753-1760 add the follow-up condition-set split for the
     `count_structured_q1` lane. The new helper records the pilot role for
     each run `26631771105` cell and the manual Actions runner accepts
     `condition_set = "stable"`, `"stable_watch"`, `"boundary_stress"`, or
     `"all"`. Only the clean high-SD `stable` cells can propose a later
     formal-pilot design, and a passing run still cannot make recovery or
     coverage claims without direct intervals and MCSE targets.
194. Slices 1761-1762 audit the first stable-set diagnostic from GitHub
     Actions run `26638116979`. The 10-cell x 20-replicate artifact passed the
     boundary gate with 3/200 SD-boundary warnings, no Hessian warnings, no
     warning-ledger rows, and decision `propose_next_pilot`. This permits a
     separate formal-pilot design note for the stable count structured q1
     cells, not a recovery or coverage claim.
195. Slices 1763-1770 add that formal-pilot design note. The proposed manual
     Actions run stays on `condition_set=stable`, requests the direct
     `log_sd_phylo` profile target at `profile_level=0.70`, uses 100
     replicates per stable cell, leaves bootstrap disabled, names MCSE
     expectations for the 70% profile coverage target, and adds boundary,
...
224. Slice 1816 adds `phase18_random_slope_workflow_plan()`, which filters the
     ...
     Actions routing. The later q6 bivariate location route adds an eleventh
     admitted row with direct `biv_gaussian_q6_location` Actions routing.
...
233. Slice 1825 adds `biv_gaussian_mu_slope` as a manual-only Phase 18 Actions
     task in `inst/sim/run/sim_run_actions_cell.R` and
...
247. Slice 1840 adds the plain binomial fixed-effect artifact lane in
     `docs/design/175-phase-18-binomial-fixed-effect-artifacts.md`.
     `phase18_dgp_binomial_fe()`, `phase18_run_binomial_fe_smoke()`,
     `phase18_write_binomial_fe_grid_outputs()`, and the manual
     `binomial_fixed_effect` Actions task write replicate, aggregate, Wald
     interval, Wald coverage, failure-ledger, manifest, and `stats::glm()`
     parity CSVs for both 0/1 and `cbind(success, failure)` encodings. This
     lane supports the first fixed-effect glm-parity claim only; interval
     calibration, random/structured/bivariate binomial, weights-as-trials, and
     Julia bridge claims remain separate gates.
