...
| Phylogenetic structured effects | Admitted for fitted Gaussian intercept, one numeric `mu` slope, direct-SD, and selected bivariate covariance subsets | Write small tree-size and phylogenetic-signal condition tables for the fitted intercept/direct-SD/one-slope surfaces | Keep multiple phylogenetic slopes, slope correlations, non-Gaussian phylogenetic effects, structured `rho12`, and predictor-dependent q=4 correlations out |
