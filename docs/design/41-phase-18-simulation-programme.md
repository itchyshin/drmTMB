...
| --- | --- | --- | --- |
| Continuous location-scale | Admitted for Gaussian location-scale first; fixed-effect lognormal, Gamma, and Student-t can receive one-response artifact lanes after family-specific estimands are named | `docs/design/47-phase-18-gaussian-location-scale-ademp.md` is the first ADEMP sheet; `docs/design/111-phase-18-positive-continuous-fixed-effect-artifacts-slices-1299-1308.md`, `phase18_run_positive_continuous_fe_smoke()`, and `phase18_write_positive_continuous_fe_grid_outputs()` record the fixed-effect lognormal/Gamma artifact path | Do not claim shape/skewness random effects, phylogenetic shape, or latent ID-level skewness |
| Proportion and bounded responses | Admitted for fixed-effect beta and beta-binomial design; no bounded-response known-`V` or zero-one-inflated route yet | `docs/design/50-phase-18-proportion-fixed-effect-ademp.md`, `phase18_run_proportion_fe_smoke()`, and `phase18_write_proportion_fe_grid_outputs()` record the fixed-effect beta/beta-binomial ADEMP and artifact path | Keep `zoi`, `coi`, zero-one inflation, one inflation, random effects, and bounded-response `meta_V(V = V)` in the failure ledger |
| Counts | Admitted for fixed-effect count families, ordinary non-zero-inflated Poisson/NB2 `mu` random effects, and the first ordinary NB2 log-`sigma` random intercept; ordinary Poisson/NB2 q=1 phylogenetic `mu` routes are fitted but still smoke-level | `docs/design/49-phase-18-count-mu-random-effect-ademp.md` records the paired Poisson/NB2 `mu` random-effect ADEMP sheet; `phase18_run_poisson_phylo_q1_smoke()`, `phase18_write_poisson_phylo_q1_grid_outputs()`, optional direct `log_sd_phylo` profile artifacts, formal-grid QA helpers, and the manual `poisson_phylo_q1_formal` Actions task now exercise the Poisson phylogenetic q1 gate before any broad grid; NB2 `sigma` and NB2 q1 phylogeny have focused extractor/diagnostic smoke tests but still need their own overdispersion-aware grids | Keep zero-inflated/hurdle random effects, zero-truncated NB2 random effects, correlated count slopes, NB2 `sigma` slopes, NB2 `sigma` phylogeny, spatial/animal/`relmat()` count effects, and count-side structured slopes out |
| Ordinal | Admitted only for fixed-effect `cumulative_logit()` location models | `docs/design/51-phase-18-ordinal-fixed-effect-ademp.md`, `docs/design/112-phase-18-ordinal-fixed-effect-artifacts-slices-1309-1318.md`, `phase18_run_ordinal_fe_smoke()`, and `phase18_write_ordinal_fe_grid_outputs()` record the fixed-effect ordinal ADEMP and artifact path | Keep ordinal random effects, scale/discrimination formulas, bivariate ordinal, and mixed-response ordinal models out |
...
| Beta and beta-binomial fixed effects | Fitted smoke surface | strict proportions `prop ~ Beta(mu * phi, (1 - mu) * phi)` or successes `success ~ BetaBinomial(trials, mu, phi)`, with `logit(mu) ~ x`, `log(sigma) ~ z`, and `phi = 1 / sigma^2` | sample size, denominator band, mean-scale predictor correlation, public `sigma` slope |
| Positive-continuous fixed effects | Fitted smoke surface | lognormal `y ~ LogNormal(meanlog = eta_mu, sdlog = sigma)` or Gamma `y ~ Gamma(shape = 1 / sigma^2, scale = mu * sigma^2)`, with family-specific `mu ~ x` and `log(sigma) ~ z` | sample size, public `sigma` slope, baseline coefficient of variation, mean-scale predictor correlation |
...
| NB2 `sigma` random intercepts | Fitted focused surface | log-overdispersion count model with ordinary grouped random intercepts and fixed-effect `mu` | groups, observations per group, mean count, overdispersion, true `sigma` SD |
| NB2 phylogenetic q=1 `mu` | Fitted smoke surface | log-mean overdispersed count model with `phylo(1 | species, tree = tree)` in `mu`; `sigma` remains fixed-effect overdispersion | species, observations per species, mean count, overdispersion, phylogenetic SD, tree shape |
...
only after their focused gates are closed. The NB2 `mu` and NB2 `sigma`
ordinary random-effect rows are admitted as focused first slices; larger grids
still need explicit condition tables before they are treated as routine
evidence. The failure ledger in
...
random-effect, NB2 `mu` random-effect, fixed-effect beta/beta-binomial
proportion, fixed-effect lognormal/Gamma positive-continuous, fixed-effect
ordinal, Gaussian `mu` random-slope, Gaussian `sigma` random-slope, coordinate
spatial `mu` slope, Student-t shape, and bivariate residual `rho12` smoke
runners are wired through this helper.
