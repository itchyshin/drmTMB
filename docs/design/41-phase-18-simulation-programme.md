...
| Proportion and bounded responses | Admitted for fixed-effect beta and beta-binomial design; beta and beta-binomial ordinary `mu` random intercepts have source-level first-slice evidence; zero-one beta is design-only | `docs/design/50-phase-18-proportion-fixed-effect-ademp.md` records the fixed-effect beta/beta-binomial ADEMP sheet; `docs/design/110-phase-18-proportion-fixed-effect-artifacts-slices-1289-1298.md` records the DGP/smoke/grid artifact lane; `docs/design/114-phase-18-zero-one-bounded-response-design-gate-slice-d3.md` records the fixed-effect zero-one beta design gate; bounded-response `mu` random intercepts should get a separate ADEMP sheet before broad grids | Keep runnable `zoi` and `coi` formulas, zero-one inflation, one inflation, bounded-response random slopes, bounded-response `sigma` random effects, and bounded-response `meta_V(V = V)` in the failure ledger |
...
164. Slice D3 records the zero-one bounded-response design gate in
     ...
     `beta_binomial()` responses, and future fixed-effect zero-one beta responses
     with exact 0/1 mass. It opens no likelihood, formula grammar, TMB code, or
     `zoi`/`coi` random-effect route.
