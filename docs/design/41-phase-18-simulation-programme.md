...
| NB2 phylogenetic q=1 `mu` | Fitted smoke/formal-admission surface with local audit hold | log-mean overdispersed count model with `phylo(1 | species, tree = tree)` in `mu`; `sigma` remains fixed-effect overdispersion; ordinary grouped species-intercept comparator recorded beside target rows; local Slices 541-555 artifacts pass QA, and Slices 561-575 add sharded formal dispatch after the singleton Actions shape proved too large; keep the route at `hold_smoke_only` until all 500-replicate shards are audited together | species, observations per species, mean count, overdispersion, phylogenetic SD, tree shape |
...
162. Slices 1319-1328 close Slice C as the count first-wave review lane in
     ...
     routes; revalidates the focused count and first-wave smoke tests; keeps
