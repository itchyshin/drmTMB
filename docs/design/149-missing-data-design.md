...
| MD9e Gamma response plus binary `mi()`, fixed only | Implemented: one binary missing predictor inside a Gamma mean-CV response location-scale model (model_type 5); the `mi()` 2-point sum carries the CV `sigma` (shape `= 1/sigma^2`, scale `= mu*sigma^2`) through the shared leaf, added as `drm_response_log_density` case 5 (extracted verbatim from the model_type 5 block). | Stop before missing Gamma responses combined with `mi()`, response random effects or structured terms with `mi()`, non-binary missing predictors, multiple missing predictors, and simulation-based imputation summaries. |
| MD9f lognormal response plus binary `mi()`, fixed only | Implemented: one binary missing predictor inside a lognormal response location-scale model (model_type 4); the `mi()` 2-point sum carries the log-scale dispersion `sigma` through the shared leaf, added as `drm_response_log_density` case 4 (`dnorm(log(y), mu, sigma, log = TRUE) - log(y)`, extracted verbatim from the model_type 4 block). | Stop before missing lognormal responses combined with `mi()`, response random effects or structured terms with `mi()`, non-binary missing predictors, multiple missing predictors, and simulation-based imputation summaries. Student-t (model_type 3) is explicitly NOT admitted here: its density needs a per-row shape parameter `nu` that the shared leaf's fixed six-argument signature cannot carry without widening every existing caller. |
