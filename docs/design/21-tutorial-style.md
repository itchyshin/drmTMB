...
| Continuous proportions | Does leaf damage proportion change with treatment, and does among-leaf variability also change? | Implemented in `vignettes/proportion-beta-binomial.Rmd` with `cover ~ grazing`, `sigma ~ grazing`, and `family = beta_family()` for values strictly between 0 and 1 |
