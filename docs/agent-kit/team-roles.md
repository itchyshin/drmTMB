...
processes. Use them when they sharpen the work, and keep every claim grounded
in files, commands, tests, citations, or explicit design assumptions.
...
Fisher checks simulation evidence. Pat checks whether a new user can interpret
the result.
```
...
```text
Darwin checks whether the example answers a real domain question. Pat checks
the learning path. Rose checks stale claims. Grace checks site and release
impact.
```
...
```text
Boole checks the data and prediction contracts. Fisher checks metrics and
leakage. Curie checks test fixtures. Grace checks reproducibility and
dependency versions.
```

## Failure Modes

Do not use the names as decoration. If a role is named, it should have a
...
mistake, convert it into `AGENTS.md`, a design note, a local skill, a test, or
a check-log rule.
