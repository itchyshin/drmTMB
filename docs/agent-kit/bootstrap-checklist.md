...
6. Keep `docs/design/10-after-task-protocol.md` unless the project already has
...
- define canonical parameter names and link scales;
- require equations, syntax, and implementation to match;
- require simulation recovery tests for new likelihoods or estimators;
- record comparison targets such as `lme4`, `glmmTMB`, `brms`, `mgcv`,
  `rstanarm`, `torch`, or custom baselines when relevant.
