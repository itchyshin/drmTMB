...
This kit packages the collaboration habits that made the recent `drmTMB`
slices work well enough to copy into another R project. It is deliberately
project-neutral: a statistical modelling package, a machine-learning package,
an agent-based simulation package, or a data-wrangling package can all start
from the same skeleton.

The core idea is simple. Put the durable operating rules in the repository,
not in one agent's private context. Then ask each agent to read the same rules,
use the same named review perspectives, leave the same check-log evidence, and
close each meaningful task with an after-task note.
...
- `MEMORY.seed.md`: optional seed note for durable external memory systems.
- `docs/design/00-vision.md`: the project vision and scope boundary.
- `docs/design/10-after-task-protocol.md`: the after-task and after-phase
  report contract.
...
- `.agents/skills/*/SKILL.md`: local skills for prose review, after-task
  audit, simulation planning, model implementation review, and release
  readiness.
...
Do not copy `drmTMB` statistical claims into another project. Copy the
operating pattern, then rewrite the scope, parameter names, validation rules,
and examples for the target package.
...
- `rho12` is the canonical correlation name;
- TMB likelihoods are always involved;
- ecology examples are always the right examples;
