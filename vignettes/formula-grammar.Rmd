...
Phylogenetic random slopes, phylogenetic scale terms, structured effects in
`rho12`, and spatial terms are still planned:
...
The planned default should be `coords = coords`, because coordinates are the
data-level object users already have. A mesh is the computational scaffold for
the scalable SPDE/GMRF approximation; it may be built internally from
coordinates or supplied explicitly by advanced users who need boundary,
barrier, or reproducibility control. A coordinate-only dense covariance is a
possible small-data comparator, but not the planned scalable default.
For structured slopes, the intended staged path is one `mu` slope first and at
most two `mu` slopes as an advanced path. Multiple random factors should remain
separate additive blocks. The first slope models should not estimate
intercept-slope `corpair()` rows; the more interesting future bivariate target
is a slope1-slope2 correlation for the same covariate across responses.
...
phylogenetic mean-mean random-intercept correlations.
