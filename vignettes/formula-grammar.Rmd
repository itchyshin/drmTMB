...
| Bivariate random slopes, cross-parameter covariance blocks, or `rho12` random effects | Planned | These need a larger covariance parameterization, simulation, and naming checks before fitting. |
| `corpairs(fit)` | Implemented for current fitted correlations | Reports residual `rho12`, ordinary univariate Gaussian `mu` random-effect correlations, the first univariate `mu`/`sigma` mean-scale covariance row, and the implemented bivariate `mu1`/`mu2` and `sigma1`/`sigma2` random-intercept correlations in a long table; future phylogenetic and spatial pair classes remain planned. |
...
for the two residual-scale random intercepts on the log-`sigma` scale.
Bivariate random slopes, cross-parameter covariance blocks, and `rho12` random
...
mean-mean and scale-scale random-intercept correlations.
