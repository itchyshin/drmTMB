...
| Bivariate random slopes, full cross-parameter covariance blocks spanning more than one pair, or `rho12` random effects | Planned | These need a larger covariance parameterization, simulation, and naming checks before fitting. |
| `corpairs(fit)` | Implemented for current fitted correlations | Reports residual `rho12`, ordinary univariate Gaussian `mu` random-effect correlations, the first univariate and same-response bivariate `mu`/`sigma` mean-scale covariance rows, and the implemented bivariate `mu1`/`mu2` and `sigma1`/`sigma2` random-intercept correlations in a long table; future phylogenetic and spatial pair classes remain planned. |
...
in `mu1` and `sigma1`, or in `mu2` and `sigma2`. Larger cross-parameter blocks,
...
mean-mean, scale-scale, and same-response mean-scale random-intercept
correlations.
