...
| `corpair(id, level = "group", block = "p", from = "mu1", to = "sigma2") ~ x` | Reserved | Planned endpoint-specific predictor-dependent latent random-effect correlation syntax; `level = "phylogenetic"` and `level = "spatial"` are reserved sibling targets. `drmTMB()` rejects it until the likelihood and tests exist. |
...
reserved `corpair()` formula wording. The first planned fitted `corpair()`
route is endpoint-specific, for example
...
Bivariate random slopes, spatial q=4 blocks, predictor-dependent phylogenetic
