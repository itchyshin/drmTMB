...
| Bivariate random slopes, spatial q=4 blocks, predictor-dependent phylogenetic/spatial correlations, or `rho12` random effects | Planned | These need a larger covariance parameterization, simulation, and naming checks before fitting. |
...
interpretation. For future phylogenetic life-history trade-off models, the same
grammar should say
`corpair(species, level = "phylogenetic", block = "p", from = "mu1", to = "mu2") ~ ecology`.
That future phylogenetic formula has an extra design gate: because the tree
couples all species, `drmTMB` needs a positive-definite covariance contract for
the full species block before a species-level predictor can change the latent
...
scale-scale correlation regressions remain q=4 extensions. Bivariate random slopes, spatial q=4 blocks,
predictor-dependent phylogenetic correlations, and `rho12` random effects remain
planned.
