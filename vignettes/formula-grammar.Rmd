...
| `y ~ x1 + (1 | id)`, `family = poisson(link = "log")` | Implemented first slice | Ordinary Poisson `mu` random intercept on the log-mean scale. Poisson random slopes, labelled covariance blocks, zero-inflated Poisson random effects, and cross-parameter covariance remain planned. |
...
`rho12`, multiple spatial slopes, and mesh-based spatial terms are still
planned:
