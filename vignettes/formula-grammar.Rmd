...
| `corpairs(fit)` | Implemented for current fitted correlations | Reports residual `rho12` and ordinary univariate Gaussian `mu` random-effect correlations in a long table; future bivariate, phylogenetic, spatial, and cross-parameter pair classes remain planned. |
| Bivariate group-level correlation pairs | Planned | Future output should identify the level, group, block, distributional parameters, responses, and coefficients; residual `rho12` remains separate. |
...
Future double-hierarchical bivariate fits should report correlation pairs in a
long table, not just as unlabeled matrices. The design note
