...
`~ habitat`, or `~ habitat_score * habitat`, and `predict()` can evaluate the
fitted association link or bounded `eta` at compatible `newdata`. Those
predictions remain point-only; coefficient-level `vcov()` and `confint()` do
not create simultaneous or eta-scale prediction bands.
