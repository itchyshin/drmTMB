...
implemented, but only as point-estimate-only or diagnostic-only routes: they
fit as point estimates and are not coverage-validated, so trust the point
estimate, not the interval, and treat them as exploratory. Correlated slopes, labelled covariance
...
remain planned extensions. Start with this fixed-effect path when you need a
...
recovery, false-positive, and interval-visibility checks. Random effects, known covariance, structured effects, bivariate
