...
also supported. This first tutorial keeps `nu` constant so the comparison
focuses on location, scale, and residual-tail assumptions.
...
location-scale model. Future skew-normal and skew-t families should start as
fixed-effect residual-shape models; ID-level skewness such as `skew(id) ~ x`
is a different latent-effect question, not an alias for Student-t `nu`.
