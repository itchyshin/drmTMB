...
Skew-normal residual asymmetry is now a separate experimental fixed-effect
family, not a substitute for the Student-t heavy-tail check above:

```r
drmTMB(
  bf(y ~ x1, sigma ~ x2, nu ~ 1),
  family = skew_normal(),
  data = dat
)
```

Use the skew-normal route as a first-slice residual-asymmetry diagnostic until
formal recovery, comparator, and false-positive evidence improve. The current
pilot artifacts show that fitted skewness can appear under true `nu = 0` and
that positive-Hessian evidence is still weak, so fit the Gaussian and Student-t
models above when the scientific question is robustness to heavier tails.
ID-level skewness such as future `skew(id) ~ x` is a different latent-effect
question, not an alias for residual Student-t `nu` or skew-normal residual
`nu`.
