...
Skew-normal residual asymmetry is a separate planned family, not a fitted
option today:
...
Use this planned syntax only in design notes until `skew_normal()` has a
density implementation, a normal-limit check at `nu = 0`, recovery for
positive and negative skew, and tests showing that `nu ~ x` is not merely
absorbing heteroscedasticity or outliers. For now, fit the Gaussian and
Student-t models above and report whether the location and scale conclusions
are robust to heavier tails. ID-level skewness such as future `skew(id) ~ x`
is a different latent-effect question, not an alias for residual Student-t
`nu` or planned skew-normal `nu`.
