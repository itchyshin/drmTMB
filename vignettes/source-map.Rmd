...
| Skew-normal location-scale-shape | `drm_formula(y ~ x, sigma ~ z, nu ~ w)` with `family = skew_normal()` | `skew_normal()` in `R/family.R`; `drm_build_skew_normal_ls_spec()` in `R/drmTMB.R`; post-fit public moment semantics in `R/methods.R` | `model_type = 17`; public `mu` and `sigma` transform internally to native `xi` and `omega`, with `alpha = nu` | `tests/testthat/test-skew-normal-density-contract.R`, `tests/testthat/test-skew-normal-location-scale.R` | `vignettes/distribution-families.Rmd`, `docs/design/02-family-registry.md`, `docs/design/03-likelihoods.md`, `docs/design/19-family-link-contract.md`, `docs/design/123-phase-18-skew-normal-source-map-slices-1519-1538.md`, `docs/design/132-phase-18-skew-normal-implementation-gate-slices-1689-1702.md` |
...
| Bivariate Gaussian location-coscale | `mu1 = y1 ~ x`; `mu2 = y2 ~ x`; `sigma1 ~ z1`; `sigma2 ~ z2`; `rho12 ~ w`; optional matching `(1 \| p \| id)` random intercepts in `mu1`/`mu2`, `sigma1`/`sigma2`, one same-response `mu`/`sigma` pair, the all-four q4 `mu1`/`mu2`/`sigma1`/`sigma2` block, or the first all-endpoint q8 `(1 + x \| p \| id)` block across `mu1`, `mu2`, `sigma1`, and `sigma2` | `biv_gaussian()` in `R/family.R`; `drm_build_biv_gaussian_spec()` in `R/drmTMB.R`; `rho12()` and `corpairs()` in `R/methods.R`; `check_drm()` in `R/check.R`; q8 smoke/recovery writers under `inst/sim/*/sim_*_biv_gaussian_q8_endpoint*` | `model_type = 2`; `rho12 = 0.99999999 * tanh(eta_rho12)`; bivariate group-level correlations use `eta_cor_mu` for `mu1`/`mu2`, `eta_cor_sigma` for `sigma1`/`sigma2`, `eta_cor_mu_sigma` for same-response mean-scale pairs, and `u_re_cov` for all-four q4 and q8 endpoint blocks; q8 has eight endpoint SDs and 28 derived latent correlations | `tests/testthat/test-biv-gaussian.R`, `tests/testthat/test-phase18-biv-gaussian-q8-endpoint.R`, `tests/testthat/test-phase18-biv-gaussian-q8-endpoint-recovery.R` | `vignettes/bivariate-coscale.Rmd`, `docs/design/15-location-coscale-phylogenetic-extension.md`, `docs/design/20-coscale-correlation-pairs.md`, `docs/design/157-capability-completion-worklist.md`, `docs/design/161-phase-18-bivariate-q8-recovery-audit.md` |
