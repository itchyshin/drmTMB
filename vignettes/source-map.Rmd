...
| Zero-inflated negative-binomial 2 | `drm_formula(count ~ x, sigma ~ z, zi ~ w)` with `family = nbinom2()` | `nbinom2()` in `R/family.R`; `drm_build_nbinom2_spec()` in `R/drmTMB.R`; `zi` formula adds the structural-zero block | `model_type = 9`; count component `Var(y) = mu + sigma^2 * mu^2`, `Pr(y = 0) = zi + (1 - zi) NB2(0 \| mu, sigma)`, and `Pr(y > 0) = (1 - zi) NB2(y \| mu, sigma)` | `tests/testthat/test-zi-nbinom2.R` | `vignettes/distribution-families.Rmd`, `docs/design/02-family-registry.md`, `docs/design/03-likelihoods.md`, `docs/design/19-family-link-contract.md` |
