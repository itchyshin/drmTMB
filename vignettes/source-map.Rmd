...
| Gaussian animal and lower-level relatedness effects | `animal(1 \| id, Ainv = Ainv)` or `relmat(1 \| id, Q = Q)` in `mu`; matching labelled bivariate q=2 or all-four q=4 endpoint blocks | `animal()` and `relmat()` marker parsing plus `build_known_relatedness_mu_structure()` and bivariate structured q4 admission inside `R/drmTMB.R` | `model_type = 1` and `model_type = 2`; known covariance or precision matrices reuse the structured `u_phylo`, `log_sd_phylo`, and `theta_phylo` backend with `animal_mu` or `relmat_mu` labels in R output | `tests/testthat/test-animal-relmat-gaussian.R` | `vignettes/animal-models.Rmd`, `vignettes/relmat-known-matrices.Rmd`, `vignettes/structural-dependence.Rmd`, `docs/design/57-structural-parity-next-slices.md` |
...
| Beta mean-scale | `drm_formula(prop ~ x + (1 \| id), sigma ~ z)` with `family = beta()` | `beta()` in `R/family.R`; `drm_build_beta_ls_spec()` in `R/drmTMB.R`; `extract_random_mu_terms()` and `build_random_mu_structure()` route ordinary `mu` intercepts | `model_type = 10`; `mu = logit^{-1}(eta_mu + Z b)`, `phi = 1 / sigma^2`, `alpha = mu * phi`, `beta_shape = (1 - mu) * phi`; ordinary `mu` random intercepts use `u_mu` and `log_sd_mu` | `tests/testthat/test-beta-location-scale.R` | `vignettes/proportion-beta-binomial.Rmd`, `vignettes/distribution-families.Rmd`, `docs/design/02-family-registry.md`, `docs/design/03-likelihoods.md`, `docs/design/19-family-link-contract.md` |
...
| Beta-binomial mean-overdispersion | `drm_formula(cbind(successes, failures) ~ x + (1 \| id), sigma ~ z)` with `family = beta_binomial()` | `beta_binomial()` in `R/family.R`; `drm_build_beta_binomial_spec()` in `R/drmTMB.R`; `extract_random_mu_terms()` and `build_random_mu_structure()` route ordinary `mu` intercepts | `model_type = 14`; `mu = logit^{-1}(eta_mu + Z b)`, `phi = 1 / sigma^2`, successes follow a beta-binomial distribution with known trial totals; ordinary `mu` random intercepts use `u_mu` and `log_sd_mu` | `tests/testthat/test-beta-binomial.R` | `vignettes/proportion-beta-binomial.Rmd`, `vignettes/distribution-families.Rmd`, `docs/design/02-family-registry.md`, `docs/design/03-likelihoods.md`, `docs/design/19-family-link-contract.md` |
...
The ordinary Poisson/NB2 q=1 phylogenetic `mu` routes were added inside
...
`log_sd_phylo`, `Q_phylo`, profile-target, `ranef()`, and `check_drm()` labels
...
- the future `meta_V()` umbrella, including proportional sampling-variance
  models such as `meta_V(value, w = w, scale = "proportional")`;
- bivariate Student-t or bivariate skew-normal families;
- broader bivariate random slopes, `rho12` random effects, and
  cross-parameter covariance blocks beyond the currently implemented ordinary
  slope-only, ordinary intercept, phylogenetic, animal, and `relmat()`
  random-effect slices;
- mesh/SPDE spatial fields, residual-scale structured slopes, multiple spatial
  ...
  regressions, and non-Gaussian spatial effects;
