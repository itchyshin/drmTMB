...
| Gaussian residual scale and mean-scale random effects | `sigma ~ z + (1 \| id)`; `sigma ~ z + (1 + z \| id)`; matching labelled `y ~ x + (1 \| p \| id)`, `sigma ~ z + (1 \| p \| id)`; matching labelled `y ~ x + (1 + x \| p \| id)`, `sigma ~ x + (1 + x \| p \| id)` | `extract_random_sigma_terms()`, `build_random_sigma_structure()`, and `build_mu_sigma_random_covariance()` inside the Gaussian builder | `model_type = 1`; `u_sigma`, `log_sd_sigma`, `eta_cor_sigma`, and `eta_cor_mu_sigma` blocks | `tests/testthat/test-gaussian-random-intercepts.R`, `tests/testthat/test-corpairs.R` | `docs/design/04-random-effects.md`, `vignettes/which-scale.Rmd`, `vignettes/location-scale.Rmd` |
...
| Bivariate Gaussian location-coscale | `mu1 = y1 ~ x`; `mu2 = y2 ~ x`; `sigma1 ~ z1`; `sigma2 ~ z2`; `rho12 ~ w`; optional matching `(1 \| p \| id)` random intercepts in `mu1` and `mu2`, matching `(1 \| q \| id)` random intercepts in `sigma1` and `sigma2`, and matching `phylo(1 \| species, tree = tree)` terms in `mu1` and `mu2` | `biv_gaussian()` in `R/family.R`; `drm_build_biv_gaussian_spec()` in `R/drmTMB.R`; `rho12()` and `corpairs()` in `R/methods.R` | `model_type = 2`; `rho12 = 0.99999999 * tanh(eta_rho12)`; bivariate group-level correlations `rho_mu = 0.999999 * tanh(eta_cor_mu)` and `rho_sigma = 0.999999 * tanh(eta_cor_sigma)`; bivariate phylogenetic mean correlation `rho_phylo = 0.999999 * tanh(eta_cor_phylo_mu)` | `tests/testthat/test-biv-gaussian.R`, `tests/testthat/test-phylo-gaussian.R` | `vignettes/bivariate-coscale.Rmd`, `vignettes/phylogenetic-spatial.Rmd`, `docs/design/15-location-coscale-phylogenetic-extension.md`, `docs/design/16-phylo-spatial-common-math.md`, `docs/design/20-coscale-correlation-pairs.md` |
...
- bivariate random slopes and random effects in `rho12`;
