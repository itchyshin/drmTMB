...
in the [current reporting boundary](capability-and-limits.html). Planned features belong in design notes until implementation,
...
beta_family()                      -> drm_build_beta_ls_spec()
...
| Beta mean-scale | `drm_formula(prop ~ x + (1 \| id) + (0 + x \| id), sigma ~ z)` with `family = beta_family()` | `beta_family()` in `R/family.R`; `drm_build_beta_ls_spec()` in `R/drmTMB.R`; `extract_random_mu_terms()` and `build_random_mu_structure()` route ordinary `mu` intercepts and independent numeric slopes | `model_type = 10`; `mu = logit^{-1}(eta_mu + Z b)`, `phi = 1 / sigma^2`, `alpha = mu * phi`, `beta_shape = (1 - mu) * phi`; ordinary `mu` random effects use `u_mu` and `log_sd_mu` | `tests/testthat/test-beta-location-scale.R`, `tests/testthat/test-nongaussian-mu-random-slopes.R` | `vignettes/proportion-beta-binomial.Rmd`, `vignettes/distribution-families.Rmd`, `docs/design/02-family-registry.md`, `docs/design/03-likelihoods.md`, `docs/design/19-family-link-contract.md` |
