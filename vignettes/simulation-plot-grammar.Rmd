...
```{r bias-rmse-display, fig.width=8.2, fig.height=5.1, fig.alt="Two simulation accuracy plots faceted by simulation surface: a raincloud-style bias display with replicate-level errors, mean bias points, and MCSE intervals, followed by an aggregate RMSE display with MCSE intervals for four estimands."}
...
replicates that contributed to it and a Monte Carlo uncertainty interval. Real
reports should plot stored replicate rows or stored replicate-block summaries
when they are available; the fixture below illustrates that grammar with
block-level proportions plus a binomial MCSE approximation for the aggregate.
...
ggplot(interval_present, aes(value, estimand, colour = surface)) +
  ...
  labs(
    ...
    subtitle = "Faint dots illustrate replicate-block proportions; large points and bars are means with 95% binomial MCSE",
    x = "Proportion",
    y = NULL,
    colour = "Surface"
  ) +
