...
Simulation checks used the `A` representation shown here, with
`n_each = 20` and one fixed `M = 8` relationship matrix. Pedigree and `Ainv`
inputs have deterministic representation-parity evidence only; their interval
coverage has not been tested across simulated datasets. These models do not admit slope-only, labelled,
...
| `summary(fit)$parameters` | Read the fitted latent animal scale `s`; individual `i` has marginal SD `s sqrt(A[i, i])`. |
...
- richer non-Gaussian animal structured effects: labelled covariance blocks,
  ...
  model, and q=2 or q=4 non-Gaussian animal covariance blocks. The following q=1
  animal models already fit today for `poisson()`, `nbinom2()`, and `beta_family()`:
