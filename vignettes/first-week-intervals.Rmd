...
- `conf.status` is `profile_failed`, `clamp_limited`, `wald_at_boundary`,
  `bootstrap_at_boundary`, `newdata_required`, or `derived_interval_unavailable`:
  there is no usable calibrated interval on that row — follow the action implied
  by the status. `bootstrap_at_boundary` fires when at least 5% of the retained
  bootstrap draws land on the target's bound: resampling does not repair a
  boundary, so treat the interval as reporting the constraint, not the sampling
  distribution.
