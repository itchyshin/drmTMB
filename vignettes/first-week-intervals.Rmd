...
[Checking and using fitted models](model-workflow.html). For which cells have
coverage evidence, read
[What drmTMB can and can't do](capability-and-limits.html).
...
```{r}
ci_site[, c("estimate", "conf.low", "conf.high", "conf.status", "profile.boundary")]
```
...
- `profile.boundary == TRUE` on a usable interval: treat the interval as
  ...
  0.0732 against a nominal 0.95 when that flag was set; the same behaviour
  appears in `lme4` on the same seeds (see `?confint.drmTMB` Boundary
  intervals). Prefer more groups when the design allows it.
...
| --- | --- |
| Full post-fit toolbox and `conf.status` table | [Checking and using fitted models](model-workflow.html) |
| Which family × effect rows have coverage evidence | [What drmTMB can and can't do](capability-and-limits.html) |
