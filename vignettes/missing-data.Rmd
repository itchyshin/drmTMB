...
The main missing-predictor module covers one missing predictor at a time in a
...
semi-continuous predictors with exact zeros. The first non-Gaussian response
route also supports a Poisson count response with one binary missing predictor.
Complete covariates used to explain the missing predictor can still be ordinary
model-matrix inputs. Transformed `mi()` terms and multiple missing predictors
are later work.
...
| Poisson response with one missing binary predictor | `bf(count ~ z + mi(treatment))`, `family = poisson()`, with `impute = list(treatment = impute_model(treatment ~ z, family = binomial()))` | The response must be complete counts; the missing predictor must be binary; zero inflation, random effects, and structured response terms are not implemented for this route. |
...
`miss_control()`, `mi()`, `impute`, `fit$missing_data`, `predict_missing()` for
responses, and `imputed()` for predictors. Its multivariate trait engine needs
...
## Poisson responses with binary missing predictors

The first non-Gaussian response route uses the same binary predictor model, but
the response likelihood is Poisson rather than Gaussian. This is useful when the
outcome is a count and a two-level covariate is partly missing:
...
are (the `version` codes such as `"MD1"`, `"MD3a"`, `"MD6a"`, and `"MD9a"` are
internal slice identifiers for specific missing-data routes):

| Field | Meaning |
| --- | --- |
| `version` | The fitted implementation slice, such as `"MD1"`, `"MD2"`, `"MD3a"`, `"MD3b"`, `"MD4"`, `"MD6a"`, `"MD6b"`, `"MD6c"`, `"MD7a"`, `"MD7b"`, `"MD7c"`, `"MD7d"`, `"MD7e"`, `"MD7f"`, `"MD8a"`, `"MD8b"`, `"MD8c"`, or `"MD9a"`. |
...
- Missing predictors in non-Gaussian response models are implemented only for the
  MD9a Poisson-response plus binary-predictor route.
