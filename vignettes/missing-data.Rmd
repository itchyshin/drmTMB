...
`binomial()`, `poisson()`, `nbinom2()`, and `beta()` fits. Student-t,
lognormal, and Gamma include ordinary random-intercept recovery evidence;
skew-normal is a fixed-effect route. These ticks do not automatically extend to
...
| all other families (`tweedie()`, `truncated_nbinom2()`, `zero_one_beta()`, `beta_binomial()`, `cumulative_logit()`, zero-inflated and hurdle routes) | — | — |
...
| Missing non-Gaussian response (`student()`, `skew_normal()`, `lognormal()`, `Gamma()`, `binomial()`, `poisson()`, `nbinom2()`, `beta()`) | `missing = miss_control(response = "include")` | Missing response cells are marginalised out; predictors and other inputs must be complete. Lognormal/Gamma observed responses must be positive; `beta()` needs interior `(0, 1)` responses; skew-normal is fixed-effect only. |
