...
`binomial()`, `poisson()`, `nbinom2()`, and `beta()` fits.
...
| all other families (`gamma()`, `tweedie()`, `truncated_nbinom2()`, `zero_one_beta()`, `beta_binomial()`, `cumulative_logit()`, `student()`, `skew_normal()`, `lognormal()`, …) | — | — |
...
| Missing non-Gaussian response (`binomial()`, `poisson()`, `nbinom2()`, `beta()`) | `missing = miss_control(response = "include")` | Missing response cells are marginalised out; predictors and other inputs must be complete. `beta()` needs interior `(0, 1)` observed responses. |
