...
support a `poisson()`, `binomial()`, `nbinom2()`, or `beta_family()` response with one
...
`beta_family()` fits, plus whole-row `beta_binomial()` and ordered-factor
...
Student-t, lognormal, Gamma, beta-binomial, and truncated NB2 have also been
checked with ordinary random intercepts;
...
The two missing-data modes differ by response family. A checkmark means the
model can fit that combination; a dash means it cannot, and the fit gives a
family-specific error rather than silently using a different likelihood.
...
| `beta_family()` | ✓ | ✓ (one binary predictor) |
...
The table records the response-model combinations currently supported for
missing responses. Only the Gaussian response has the broad predictor-family
catalogue tabulated below; the non-Gaussian responses can currently model one
binary (Bernoulli/logit) missing predictor each.
...
| Missing non-Gaussian response (`student()`, `skew_normal()`, `lognormal()`, `Gamma()`, `tweedie()`, `zero_one_beta()`, `binomial()`, `poisson()`, `nbinom2()`, `beta_family()`) | `missing = miss_control(response = "include")` | Missing response cells are marginalised out; predictors and other inputs must be complete. Lognormal/Gamma observed responses must be positive; Tweedie allows zero and positive values; zero-one beta needs observed interior data plus optional 0/1 atoms; strict `beta_family()` needs interior `(0, 1)` responses. |
...
| Non-Gaussian response with one missing binary predictor | `bf(y ~ z + mi(treatment))`, `family = poisson()`/`binomial()`/`nbinom2()`/`beta_family()`, with `impute = list(treatment = impute_model(treatment ~ z, family = binomial()))` | The response must be complete; the missing predictor must be binary; zero inflation, random effects, and structured response terms are not implemented for these routes. |
...
| One missing strict proportion predictor | `bf(y ~ z + mi(cover), sigma ~ 1)` with `impute = list(cover = impute_model(cover ~ z, family = beta_family()))` | The missing `mi()` variable must be numeric and strictly inside `(0, 1)`, then modelled with one fixed-effect beta predictor model. |
...
| Continuous proportion in `(0, 1)` | `impute_model(x ~ z, family = beta_family())` | deterministic beta quadrature nodes |
...
The worked examples below show one model for each supported predictor-family
route.
...
The fitted and residual vectors preserve the input row order, so you can check
row accounting directly:
...
wrapped in `drm_formula()`. For each missing treatment value, the likelihood
considers the two possible states. `imputed(fit_binary)` reports the fitted
conditional probability that the missing predictor is in the second factor
level.
...
The overdispersed-count (`nbinom2()`) and proportion (`beta_family()`) responses reuse
...
- Missing predictors in non-Gaussian response models are implemented for the
  `poisson()`, `binomial()`, `nbinom2()`, and `beta_family()` responses, each with one
...
- Measurement-error models are not part of this missing-data interface.
