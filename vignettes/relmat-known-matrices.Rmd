...
- non-Gaussian known-matrix structured effects beyond the first
  models with point-estimate recovery evidence noted below.

Several non-Gaussian families already fit a first q1 unlabelled `relmat()`
structured model with route-specific evidence. The recovery-grade routes support
...
diagnostic-only intercept-only `hu` model. Use that route only to confirm
fit/extractor feasibility; it does not establish point-estimate recovery.
The `beta_family()` family and the families with no structured layer still
reject `relmat()`.
