...
Several non-Gaussian families already fit a first `relmat()` structured gate
at recovery grade. Trust the point estimate, not the interval: these routes
are not coverage-validated. They are `Gamma()` on `mu` (intercept or one
slope), `poisson()` and `nbinom2()` on `mu`, `nbinom2()` on `sigma` (one
slope), and `truncated_nbinom2()` (with its hurdle NB2 alias) on `hu`
(intercept). The `beta()` family and the families with no structured layer
still reject `relmat()`.
