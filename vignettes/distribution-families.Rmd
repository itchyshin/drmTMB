...
`(0 + effort | site)`, when there is no zero-inflation formula. Other
non-Gaussian random-effect requests are still boundary checks: non-Gaussian
`sigma` random effects, Student-t `nu` random effects, zero-inflation or
hurdle random effects, ordinal mixed models, structured non-Gaussian effects,
and correlated or labelled count random-slope blocks need their own likelihood
and recovery evidence before they become
runnable syntax. Fixed-effect Wald intervals are available where the fixed
