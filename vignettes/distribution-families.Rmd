...
| Continuous with heavy tails | `student()` | `mu`, `sigma`, `nu` | Student-t scale, not exactly residual SD | `mu` random intercepts and independent slopes implemented (recovery grade); `sigma`/`nu` random effects and bivariate models planned |
| Continuous with asymmetric residuals | `skew_normal()` | `mu`, `sigma`, `nu` | response standard deviation; `nu` is residual slant | fixed effects only |
| Positive multiplicative response | `lognormal()` | `mu`, `sigma` | SD on the log-response scale | `mu` random intercepts (and slopes) at recovery grade; `sigma` random effects and bivariate models planned |
| Positive response with mean-CV interpretation | `Gamma(link = "log")` | `mu`, `sigma` | coefficient of variation | `mu` random intercepts and independent slopes implemented (recovery grade); `sigma` random effects and bivariate models planned |
...
| Event indicator or successes out of known trials | `stats::binomial(link = "logit")` | `mu` | no modelled `sigma`; ordinary binomial sampling variation | fixed effects only; use `cbind(successes, failures)` for trial totals |
...
| Overdispersed count or rate | `nbinom2()` | `mu`, `sigma`, optional `zi` | extra-Poisson dispersion scale | ordinary non-zero-inflated `mu` random intercepts, independent slopes, grouped `sigma` random intercepts, q=1 phylogenetic `mu` intercept, and q=1 `sigma` structured one-slope routes (`phylo`/`spatial`/`animal`/`relmat`, recovery grade) implemented; plain `sigma` random slopes, `sigma` structured routes beyond one slope, and `zi` random effects planned |
| Positive count, zeros absent by design | `truncated_nbinom2()` | `mu`, `sigma` | NB2 dispersion for the untruncated component | `mu` random intercepts and independent slopes implemented (recovery grade; not with `hu`); `hu`/`sigma` random effects and bivariate models planned |
...
`(0 + effort | site)`, when there is no zero-inflation formula. Student-t,
Gamma, beta-binomial, and truncated NB2 (without a hurdle formula) `mu` models
likewise accept unlabelled random intercepts and independent numeric slopes at
recovery grade, and lognormal `mu` accepts a random intercept (slopes at
recovery grade). Ordinary Poisson
...
`nu ~ 1`; skew-normal is currently fixed-effect only, with `nu` modelling
residual slant. Other non-Gaussian random-effect requests are still boundary
checks: Tweedie random effects or predictor-dependent power, skew-normal random
effects or structured effects, NB2 plain `sigma` random slopes and `sigma`
structured routes beyond the one-slope gate,
Student-t `nu` random effects, zero-inflation or hurdle random effects,
ordinal mixed models, other non-count `spatial()`/`animal()`/`relmat()`
...
finite. A `mu` random intercept runs at recovery grade, with random slopes also
at recovery grade (trust the point estimate, not the interval). Scale (`sigma`)
random effects, known sampling covariance, phylogenetic terms, and bivariate
lognormal models are planned later.
...
scientific summary is the expected number of successes. The first slice is
fixed-effect only and rejects factor responses, proportions with trial weights,
random effects, structured effects, bivariate responses, and `engine =
"julia"`.
