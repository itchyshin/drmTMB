...
| Continuous proportion in `[0, 1]` with structural exact 0 or 1 values | `zero_one_beta()` | `mu`, `sigma`, `zoi`, `coi` | public interior beta scale mapped to precision | ordinary `mu` random intercepts and independent slopes are fitted; exact ordinary `zoi` q1 intercept and same-raw-symbol slope routes plus one exact `coi` q1 intercept are fitted at point-fit recovery only; `coi` slopes and broader atom random effects are planned |
...
`zoi ~ 1 + (1 | id)`, same-raw-symbol `zoi ~ x + (0 + x | id)`, and
`coi ~ 1 + (1 | id)` q1 routes are also fitted at point-fit recovery only.
For the `coi` intercept, retained population-level recovery used `M = 64` and
50 observations per group; sparse observed zeroes or ones can weaken
individual conditional modes, so inspect atom counts before interpreting them.
Their limitations are reported as warnings: direct profiles, intervals,
coverage, joint atom effects, `coi` slopes, broader `zoi` forms, structured
