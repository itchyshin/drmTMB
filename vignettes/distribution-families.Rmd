...
```r
drmTMB(
  bf(count ~ habitat, zi ~ survey_method),
  family = poisson(link = "log"),
  data = dat
)
```
...
```r
drmTMB(
  bf(count ~ habitat, sigma ~ treatment, zi ~ survey_method),
  family = nbinom2(),
  data = dat
)
```
...
## Planned bounded and positive-count families
...
The next proportion family should be `beta()` for continuous responses strictly
inside `(0, 1)`, such as a continuously measured cover proportion or a rate
that is not generated as successes out of known trials. The planned public
grammar uses `sigma`, not `phi`, so the scale formula reads like the rest of
`drmTMB`:
...
Here larger `sigma` means more variation around the mean proportion. This is
the opposite direction from beta precision, because `phi = 1 / sigma^2`.
Once implemented, `fitted()` should return `mu`. Responses equal to 0 or 1
will need a zero/one-inflated beta or ordered-beta route rather than the first
strict beta implementation.
...
Positive-count models should come before full hurdle models. A truncated NB2
model describes counts after zero has been removed by design, such as clutch
size among breeding individuals:
...
Hurdle models then add `hu ~ predictors` for the probability of zero:
...
The design preference is to use `hu` as a formula component, parallel to the
implemented `zi` component, instead of immediately adding separate
`hurdle_nbinom2()` and `hurdle_poisson()` constructors.
...
distribution; then `hu` is the overall probability that the observed response
is zero.
...
The practical next-family order is `beta()`, `truncated_nbinom2()`, hurdle
NB2 via `hu`, and then univariate ordinal models. COM-Poisson and generalized
Poisson remain valuable, but they should wait until the mean-dispersion
contract and comparator checks are designed.
