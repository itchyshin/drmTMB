...
positive count, `mu / (1 - Pr_NB2(0))`. Hurdle syntax with `hu ~ predictors`
remains planned; the current implementation rejects zeros and rejects `hu`
clearly.
...
Hurdle models add `hu ~ predictors` for the probability of zero and combine a
zero model with a positive-count truncated distribution:
...
The practical next-family order is hurdle NB2 via `hu`, and then univariate
ordinal models. COM-Poisson and generalized Poisson remain valuable, but they
should wait until the mean-dispersion contract and comparator checks are
designed.
