...
with `meta_V(V = V)`, while `meta_known_V(V = V)` remains a compatibility
alias.
...
also has the first q=1 phylogenetic `mu` intercept route,
`phylo(1 | species, tree = tree)`, on the log-mean scale. Other non-Gaussian
random-effect requests are still boundary checks: non-Gaussian `sigma` random
effects, Student-t `nu` random effects, zero-inflation or hurdle random effects,
ordinal mixed models, NB2/spatial/animal/`relmat()` structured effects, count
