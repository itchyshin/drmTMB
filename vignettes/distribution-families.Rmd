...
| Two continuous Gaussian responses | `c(gaussian(), gaussian())` | `mu1`, `mu2`, `sigma1`, `sigma2`, `rho12` | residual SDs for each response | matching labelled `mu1`/`mu2` random intercepts implemented; richer covariance planned |
...
```text
larger fitted sigma -> larger modelled variability
```
...
q_i = 1 - Pr_NB2(0)
Var[y_i | y_i > 0] =
  (mu_i + (1 + sigma_i^2) * mu_i^2) / q_i - (mu_i / q_i)^2
```
...
Var[y_i] = (1 - hu_i) Var_trunc_i + hu_i * (1 - hu_i) * E_trunc_i^2
```
