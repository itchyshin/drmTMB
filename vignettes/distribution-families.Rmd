...
| Continuous with asymmetric residuals | `skew_normal()` | `mu`, `sigma`, `nu` | response standard deviation; `nu` controls residual slant | fixed-effect first slice only |
...
| `skew_normal()` | response SD `sigma` | native Azzalini scale `omega` after moment transform | larger `sigma` means larger response variance |
...
| `skew_normal()` | residual SD is public `sigma`; native `omega` is internal after converting from `mu`, `sigma`, and `nu` |
...
Skew-normal location-scale-shape models add `nu`, the residual slant or
asymmetry parameter:

\[
\begin{aligned}
y_i \mid \mu_i, \sigma_i, \nu_i
  &\sim \operatorname{SkewNormal}(\mu_i, \sigma_i, \nu_i),\\
\mu_i
  &= \beta_0 + \beta_1 \text{temperature}_i,\\
\log(\sigma_i)
  &= \gamma_0 + \gamma_1 \text{treatment}_i,\\
\nu_i
  &= \delta_0 + \delta_1 \text{season}_i.
\end{aligned}
\]

```r
drmTMB(
  bf(y ~ temperature, sigma ~ treatment, nu ~ season),
  family = skew_normal(),
  data = dat
)
```

Use this first skew-normal path when a continuous response has residual
asymmetry after the expected response and residual SD have been modelled.
Public `mu` is `E[y]`, public `sigma` is `SD[y]`, and `nu` is the residual
slant: positive values give right-skewed residuals and negative values give
left-skewed residuals. The implementation transforms internally to native
`xi`, `omega`, and `alpha = nu`, but those native quantities are not the
user-facing `fitted()` or `sigma()` results. Random effects, structured
effects, known sampling covariance, bivariate skew-normal models, residual
`rho12`, and latent `skew(id)` syntax remain planned.
