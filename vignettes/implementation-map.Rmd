...
| Student-t | fixed-effect `mu`, `sigma`, and `nu` | ordinary unlabelled `mu` random intercepts | none | `sigma` or `nu` random effects, random slopes, skewness, and structured Student-t effects |
| Lognormal, Gamma, beta | fixed-effect `mu` and `sigma` or the family-specific scale | ordinary unlabelled `mu` random intercepts | none | scale random effects, random slopes, bounded-response known covariance, and structured dependence |
...
| Beta-binomial | fixed-effect `mu` and `sigma` with row trials | ordinary unlabelled `mu` random intercepts | none | random slopes, `sigma` random effects, zero-one inflation, and structured dependence |
...
| Non-Gaussian ordinary `mu` | Yes for count slopes; intercept-only for selected continuous, positive, and bounded families | Poisson and NB2 independent numeric slopes; ordinary Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial `mu` random intercepts | correlated slopes, zero-inflated or hurdle random effects, structured count slopes, and bounded-response random slopes |
