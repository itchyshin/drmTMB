...
| Phylogenetic direct SD | `sd(..., level = "phylogenetic")`, `sd1(..., level = "phylogenetic")`, and `sd2(..., level = "phylogenetic")` direct-SD surfaces where documented | fixed-effect summaries, prediction helpers, profile targets where direct | `sd*()` direct-SD levels for spatial, animal, and `relmat()` remain future unification lanes |
...
| Non-Gaussian ordinary `mu` | Poisson and NB2 q=1 random intercepts, independent numeric slopes, and ordinary q=1 phylogenetic `mu` intercepts | `sdpars$mu`, `ranef()`, `ranef("phylo_mu")`, direct `log_sd_mu` or `log_sd_phylo` profile targets, `check_drm()` | correlated non-Gaussian slopes, labelled covariance, structured slopes, spatial/animal/`relmat()` structured dependence, zero-inflated/hurdle random effects, and cross-parameter covariance |
| Non-Gaussian scale, shape, zero-inflation, and hurdle | fixed-effect formulas where the family supports the component; ordinary NB2 log-`sigma` random intercepts | fixed-effect coefficient tables and Wald intervals where implemented; NB2 `sdpars$sigma`, `random_effects$sigma`, direct `log_sd_sigma`, and `check_drm()` for the first scale gate | NB2 `sigma` slopes, structured scale effects, and random effects in `nu`, `zi`, `hu`, future `zoi`, future `coi`, or latent skewness |
...
| --- | --- | --- |
| Generic `sd*()` direct-SD design | Users should not need separate direct-SD names for phylo, spatial, animal, and `relmat()` routes forever. | The grammar, compatibility route for legacy phylogenetic helper names, examples, tests, and reference-index plan are explicit before parser work. |
