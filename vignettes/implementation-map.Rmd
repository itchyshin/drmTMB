...
| Bivariate Gaussian | `mu1`, `mu2`, `sigma1`, `sigma2`, and fixed-effect `rho12` | ordinary bivariate random-intercept slices, same-response `mu`/`sigma` intercept covariance, all-four q=4 intercept blocks, matching slope-only `mu1`/`mu2` blocks, and source-tested matching q=4/q=6 `mu1`/`mu2` location blocks | residual `rho12()` and group-level `corpairs()` stay separate | random effects in `rho12`, mixed-response bivariate families, residual-scale slope blocks, same-response location-scale slope covariance, and all-four p8/q8 slope endpoints |
...
| Ordinary bivariate Gaussian | Yes | matching slope-only `mu1`/`mu2` q=2 block plus source-tested q=4/q=6 location blocks | same-response slope covariance, residual-scale slope covariance, and p8/q8 all-endpoint slopes |
