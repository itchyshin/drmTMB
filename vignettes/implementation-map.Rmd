...
| Lognormal, Gamma, beta | fixed-effect `mu` and `sigma` or the family-specific scale | ordinary unlabelled `mu` random intercepts and independent numeric slopes | recovery grade: Gamma `relmat()` q=1 `mu` intercept/one-slope; beta `animal()` q=1 `mu` intercept/one-slope and `sigma` intercept; lognormal none | scale random effects, correlated slopes, bounded-response known covariance, and further structured dependence beyond the Gamma `relmat()` and beta `animal()` recovery gates |
...
| Non-Gaussian scale, shape, zero-inflation, and hurdle | fixed-effect formulas where the family supports the component; ordinary NB2 log-`sigma` random intercepts, plus recovery-grade NB2 q=1 structured `sigma` one-slope slices from `phylo()`/`spatial()`/`animal()`/`relmat()` | fixed-effect coefficient tables and Wald intervals where implemented; NB2 `sdpars$sigma`, `random_effects$sigma`, direct `log_sd_sigma`, and `check_drm()` for the first scale gate | plain NB2 `sigma` slopes, structured scale routes beyond the q=1 one-slope gate, and random effects in `nu`, `zi`, `hu`, `zoi`, `coi`, or latent skewness |
