...
| Student-t | fixed-effect `mu`, `sigma`, and `nu` | ordinary unlabelled `mu` random intercepts and independent numeric slopes | recovery/diagnostic grade only: one `spatial()` q=1 `mu` intercept and one `phylo()` `nu` intercept; no labelled covariance | `sigma` or `nu` random effects, correlated slopes, skewness, and structured Student-t effects beyond those two recovery gates |
...
| Poisson and NB2 | fixed-effect `mu`; NB2 also has fixed-effect `sigma` and the first ordinary log-`sigma` random-intercept gate | ordinary non-zero-inflated `mu` random intercepts and independent numeric slopes; ordinary NB2 `sigma` random intercepts; ordinary Poisson/NB2 also fit one q=1 structured `mu` intercept from `phylo()`, `spatial()`, `animal()`, or `relmat()` | Poisson/NB2 q=1 structured `mu` intercepts, plus recovery-grade NB2 q=1 structured `sigma` one-slope slices from `phylo()`, `spatial()`, `animal()`, or `relmat()`; no labelled count covariance | correlated count slopes, zero-inflated or hurdle random effects, plain NB2 `sigma` slopes and structured `sigma` routes beyond the q=1 one-slope gate, structured count slopes, simultaneous structured types, and broader count covariance |
| Zero-inflated Poisson and zero-inflated NB2 | fixed-effect `mu`; `zi ~ ...`; NB2 also has fixed-effect `sigma` | none | recovery grade: Poisson `zi ~ spatial(1 | id, coords = coords)` q=1 structured intercept only (no NB2 `zi` structure) | random effects in `zi`, structured zero inflation beyond the Poisson `zi ~ spatial()` intercept, NB2 `zi` structure, and `corpairs()` for count latent effects |
| Truncated NB2 and hurdle NB2 | fixed-effect `mu`; fixed-effect `sigma`; hurdle NB2 also has `hu ~ ...` | ordinary unlabelled zero-truncated NB2 `mu` random intercepts and independent numeric slopes for non-hurdle models | recovery grade: one `relmat()` q=1 `hu` intercept (hurdle NB2, inherited by the alias) | random effects in `hu`, random effects in hurdle count `mu`, correlated zero-truncated slopes, and structured hurdle effects beyond the `relmat()` `hu` intercept |
| Cumulative-logit ordinal | fixed-effect ordinal location with estimated cutpoints | none | recovery grade: one `phylo()` q=1 `mu` intercept | ordinal random effects, scale or discrimination formulas, other structured ordinal effects (`spatial()`/`animal()`/`relmat()`, structured slopes), and bivariate ordinal models |
...
| Non-Gaussian structured dependence | Poisson and NB2 q=1 `phylo()`, `spatial()`, `animal()`, and `relmat()` `mu` intercepts fitted; NB2 also q=1 structured `sigma` one-slope slices | recovery-grade NB2 q=1 structured `sigma` one-slope (`1 + x`) blocks; no structured count `mu` slopes yet | one ordinary non-zero-inflated count structured intercept at a time, on the log-mean (`mu`) scale or, for NB2, the log-`sigma` scale | focused tests cover extraction, diagnostics, direct profile-target rows, and planned-neighbour errors; broader simulation evidence remains separate for the older phylogenetic lanes | Direct `log_sd_phylo` profile targets; no derived correlations |
...
| Ordinal, shape, bounded scale | fixed effects only where supported | no random effects | recovery-grade cumulative-logit `phylo()` q=1 `mu` intercept only; otherwise none | fixed-effect or family tests | fixed-effect/profile only where implemented |
...
| Non-Gaussian ordinary `mu` | Poisson and NB2 q=1 random intercepts, independent numeric slopes, ordinary q=1 `phylo()`, `spatial()`, `animal()`, and `relmat()` `mu` intercepts, ordinary Student-t/zero-truncated NB2/lognormal/Gamma/beta/beta-binomial `mu` random intercepts and independent numeric slopes, plus recovery-grade Gamma `relmat()` and beta `animal()` q=1 structured `mu` intercepts and one-slopes | `sdpars$mu`, `ranef()`, marker-specific structured `ranef()` blocks, direct `log_sd_mu` or `log_sd_phylo` profile targets, `check_drm()` | correlated non-Gaussian slopes, labelled covariance, structured slopes, simultaneous structured count types, zero-inflated/hurdle random effects, and cross-parameter covariance |
| Non-Gaussian scale, shape, zero-inflation, and hurdle | fixed-effect formulas where the family supports the component; ordinary NB2, lognormal, and Gamma log-`sigma` random intercepts, plus recovery-grade NB2 q=1 structured `sigma` one-slope slices from `phylo()`/`spatial()`/`animal()`/`relmat()` | fixed-effect coefficient tables and Wald intervals where implemented; NB2/lognormal/Gamma `sdpars$sigma`, `random_effects$sigma`, direct `log_sd_sigma`, and `check_drm()` for the intercept gates; only the exact lognormal ledger domain is inference-ready with caveats | `sigma` slopes, structured scale routes outside the NB2 q=1 one-slope gate, and random effects in `nu`, `zi`, `hu`, `zoi`, `coi`, or latent skewness |
...
| Non-Gaussian scale, shape, `zi`, `hu`, ordinal, or bounded-response components | First NB2/lognormal/Gamma scale-intercept slices | fixed effects where implemented; ordinary NB2, lognormal, and Gamma log-`sigma` random intercepts | random effects and structured dependence outside those ordinary `sigma` intercepts and ordinary Poisson/NB2 q=1 structured `mu` |
...
| --- | --- | --- |
| zero-inflated counts with predictors in the zero process | fixed-effect `zi ~ predictors` in the supported count family; Poisson `zi ~ spatial(1 | id, coords = coords)` for a q=1 structured intercept at recovery grade | random effects in `zi`, NB2 `zi` structure, and Poisson `zi` structure beyond the first `spatial()` intercept |
...
| NB2 structured count model | ordinary NB2 q=1 `phylo()`, `spatial()`, `animal()`, or `relmat()` in `mu` when fixed-effect `sigma` is enough, ordinary NB2 `mu` random effects if a plain group is enough, or recovery-grade NB2 q=1 structured `sigma` one-slope slices (`sigma ~ phylo(1 + x | sp)`, and `spatial()`/`animal()`/`relmat()`) | NB2 structured `mu` slopes, zero-inflated NB2 structure, NB2 structured `sigma` beyond the q=1 one-slope gate, simultaneous structured types, and labelled count covariance |
