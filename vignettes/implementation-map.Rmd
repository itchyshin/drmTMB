...
| Student-t | fixed-effect `mu`, `sigma`, and `nu` | ordinary unlabelled `mu` random intercepts and independent numeric slopes | none | `sigma` or `nu` random effects, correlated slopes, skewness, and structured Student-t effects |
| Lognormal, Gamma, beta | fixed-effect `mu` and `sigma` or the family-specific scale | ordinary unlabelled `mu` random intercepts and independent numeric slopes | none | scale random effects, correlated slopes, bounded-response known covariance, and structured dependence |
| Tweedie | fixed-effect `mu`, `sigma`, and intercept-only `nu ~ 1` | none | none | predictor-dependent power, random effects, zero-inflation aliases, hurdle aliases, structured dependence, and bivariate or mixed-response Tweedie |
| Zero-one beta | fixed-effect `mu`, `sigma`, `zoi`, and `coi` | none | none | zero-one beta random effects, covariance blocks, known covariance, denominator syntax, bivariate bounded responses, and structured dependence |
...
| Poisson and NB2 | fixed-effect `mu`; NB2 also has fixed-effect `sigma` and the first ordinary log-`sigma` random-intercept gate | ordinary non-zero-inflated `mu` random intercepts and independent numeric slopes; ordinary NB2 `sigma` random intercepts; ordinary Poisson/NB2 also fit one q=1 structured `mu` intercept from `phylo()`, `spatial()`, `animal()`, or `relmat()` | Poisson/NB2 q=1 structured `mu` intercepts only; no labelled count covariance | correlated count slopes, zero-inflated or hurdle random effects, NB2 `sigma` slopes or structured `sigma` effects, structured count slopes, simultaneous structured types, and broader count covariance |
...
| Truncated NB2 and hurdle NB2 | fixed-effect `mu`; fixed-effect `sigma`; hurdle NB2 also has `hu ~ ...` | ordinary unlabelled zero-truncated NB2 `mu` random intercepts and independent numeric slopes for non-hurdle models | none | random effects in `hu`, random effects in hurdle count `mu`, correlated zero-truncated slopes, and structured hurdle effects |
...
| Bivariate Gaussian | `mu1`, `mu2`, `sigma1`, `sigma2`, and fixed-effect `rho12` | ordinary bivariate random-intercept slices, same-response `mu`/`sigma` intercept covariance, all-four q=4 intercept blocks, matching slope-only `mu1`/`mu2` and `sigma1`/`sigma2` blocks, and matching q4/q6 `mu1`/`mu2` location blocks with smoke artifact routing | residual `rho12()` and group-level `corpairs()` stay separate | random effects in `rho12`, mixed-response bivariate families, same-response location-scale slope covariance, and all-four p8/q8 slope endpoints |
...
| Bivariate ordinary Gaussian | q=2 intercepts fitted | matching slope-only, q4 location smoke, and source-tested q6 location `mu1`/`mu2` fitted | selected q=2, same-response `mu`/`sigma`, all-four q=4 intercept blocks, and q > 2 location blocks | q=2 admitted; slope-only artifact-ready; q4 location smoke-ready; q6 location source-tested only | q=2 direct profile; q > 2 location SDs direct; q > 2 correlations derived-unavailable |
...
| NB2 ordinary `sigma` | fitted for independent log-`sigma` random intercepts | no `sigma` slopes yet | no labelled, joint `mu`/`sigma`, zero-inflated, truncated, hurdle, or structured scale blocks | separate overdispersion-random-intercept smoke lane | Direct `log_sd_sigma` profile target |
| Non-Gaussian structured dependence | Poisson and NB2 q=1 `phylo()`, `spatial()`, `animal()`, and `relmat()` `mu` intercepts fitted | no structured count slopes yet | only one ordinary non-zero-inflated count structured `mu` intercept at a time on the log-mean scale | focused tests cover extraction, diagnostics, direct profile-target rows, and planned-neighbour errors; broader simulation evidence remains separate for the older phylogenetic lanes | Direct `log_sd_phylo` profile targets; no derived correlations |
| `zi`, `hu`, `zoi`, and `coi` | fixed effects only where supported | no random effects | no covariance layer | fixed-effect tests, with zero-one beta now carrying Phase 18 artifact helpers | Wald fixed effects where implemented |
...
| Ordinary Gaussian `sigma` | q=1 residual-scale intercepts and multiple independent numeric slopes on `log(sigma)` | `sdpars$sigma`, `profile_targets()`, `check_drm()`, `sigma()` | correlated or labelled univariate residual-scale slope covariance |
...
| Ordinary bivariate `mu1`/`mu2` | q=2 matching labelled random intercepts, q=2 matching slope-only random-slope blocks, and q4/q6 location blocks with smoke artifact routing | `corpairs(class = "mean-mean")`, `summary()$covariance`, `profile_targets()`, `check_drm()` | same-response location-scale slope covariance, all-four p8/q8 endpoints, and predictor-dependent slope `corpair()` regressions |
| Ordinary bivariate `sigma1`/`sigma2` | q=2 matching labelled random intercepts and slope-only scale-slope blocks | `corpairs(class = "scale-scale")`, `summary()$covariance`, `profile_targets()` | same-response location-scale slopes and p8/q8 endpoint covariance |
...
| Coordinate spatial structure | q=1 univariate `mu` and `sigma` intercepts with optional matching `mu`/`sigma` correlation; one numeric `mu` slope; q=2 bivariate `mu1`/`mu2` intercept covariance; constant q=4 location-scale intercept block; ordinary Poisson/NB2 q=1 `mu` intercepts | `sdpars$mu`, `sdpars$sigma`, marker-specific `ranef()` blocks, `corpairs(level = "spatial")`, `summary()$covariance`, `profile_targets()`, `check_drm()` | mesh/SPDE, multiple slopes, residual-scale structured slopes, slope correlations, direct-SD surfaces, spatial `corpair()` regression, count spatial slopes, and zero-inflated spatial effects |
| Animal-model structure | q=1 univariate `mu` and `sigma` intercepts with optional matching `mu`/`sigma` correlation; one numeric `mu` slope for `pedigree`, `A`, or `Ainv`; q=2 bivariate `mu1`/`mu2`; constant q=4 location-scale intercept block; ordinary Poisson/NB2 q=1 `mu` intercepts | `sdpars$mu`, `sdpars$sigma`, marker-specific `ranef()` blocks, `corpairs(level = "animal")`, `profile_targets()`, `check_drm()` | sparse large-pedigree construction, multiple slopes, residual-scale structured slopes, slope correlations, predictor-dependent `corpair()` regression, direct-SD grammar, count animal slopes, and labelled count covariance |
| `relmat()` known latent relatedness | q=1 univariate `mu` and `sigma` intercepts with optional matching `mu`/`sigma` correlation; one numeric `mu` slope for `K` or `Q`; q=2 bivariate `mu1`/`mu2`; constant q=4 location-scale intercept block; ordinary Poisson/NB2 q=1 `mu` intercepts | `sdpars$mu`, `sdpars$sigma`, marker-specific `ranef()` blocks, `corpairs(level = "relmat")`, `profile_targets()`, `check_drm()` | multiple slopes, residual-scale structured slopes, slope correlations, predictor-dependent `corpair()` regression, direct-SD grammar, count `relmat()` slopes, and labelled count covariance |
| Non-Gaussian ordinary `mu` | Poisson and NB2 q=1 random intercepts, independent numeric slopes, ordinary q=1 `phylo()`, `spatial()`, `animal()`, and `relmat()` `mu` intercepts, and ordinary Student-t/zero-truncated NB2/lognormal/Gamma/beta/beta-binomial `mu` random intercepts and independent numeric slopes | `sdpars$mu`, `ranef()`, marker-specific structured `ranef()` blocks, direct `log_sd_mu` or `log_sd_phylo` profile targets, `check_drm()` | correlated non-Gaussian slopes, labelled covariance, structured slopes, simultaneous structured count types, zero-inflated/hurdle random effects, and cross-parameter covariance |
| Non-Gaussian scale, shape, zero-inflation, and hurdle | fixed-effect formulas where the family supports the component; ordinary NB2 log-`sigma` random intercepts | fixed-effect coefficient tables and Wald intervals where implemented; NB2 `sdpars$sigma`, `random_effects$sigma`, direct `log_sd_sigma`, and `check_drm()` for the first scale gate | NB2 `sigma` slopes, structured scale effects, and random effects in `nu`, `zi`, `hu`, `zoi`, `coi`, or latent skewness |
...
| Ordinary bivariate Gaussian | Yes | matching slope-only `mu1`/`mu2` and `sigma1`/`sigma2` q=2 blocks plus q4/q6 location blocks with smoke artifact routing | same-response slope covariance and p8/q8 all-endpoint slopes |
...
| Non-Gaussian ordinary `mu` | Yes | Poisson, NB2, Student-t, zero-truncated NB2, lognormal, Gamma, beta, and beta-binomial independent numeric slopes plus ordinary random intercepts; ordinary Poisson/NB2 q=1 structured `mu` intercepts | correlated slopes, zero-inflated or hurdle random effects, structured count slopes, simultaneous structured types, and structured bounded-response dependence |
| Non-Gaussian scale, shape, `zi`, `hu`, ordinal, or bounded-response components | First NB2 scale slice only | fixed effects where implemented; ordinary NB2 log-`sigma` random intercepts | random effects and structured dependence outside ordinary NB2 `sigma` intercepts and ordinary Poisson/NB2 q=1 structured `mu` |
...
| phylogenetic or spatial count dependence | ordinary Poisson or NB2 q=1 `phylo(1 | species, tree = tree)` or `spatial(1 | site, coords = coords)` when one structured effect is a log-mean intercept; ordinary Poisson/NB2 `mu` random effects when a plain group is enough | structured count slopes, zero-inflated structured counts, NB2 structured `sigma`, simultaneous structured types, and labelled count covariance |
...
| NB2 structured count model | ordinary NB2 q=1 `phylo()`, `spatial()`, `animal()`, or `relmat()` in `mu` when fixed-effect `sigma` is enough, or ordinary NB2 `mu` random effects if a plain group is enough | NB2 structured slopes, zero-inflated NB2 structure, NB2 structured `sigma`, simultaneous structured types, and labelled count covariance |
| Poisson structured count slopes | ordinary Poisson/NB2 independent numeric `mu` slopes when the grouping is exchangeable | structured count slopes need their own recovery and diagnostic evidence |
| Poisson `animal()` or `relmat()` count dependence | ordinary non-zero-inflated Poisson/NB2 q=1 `animal()` or `relmat()` in `mu`, ordinary Poisson/NB2 `mu` random effects, or Gaussian `animal()`/`relmat()` when the response is Gaussian | known-relatedness count slopes, labels, zero-inflation, and simultaneous structured types remain planned |
| non-Gaussian structured scale or shape effects | fixed-effect `sigma`, `nu`, `zi`, or `hu` formulas where the family supports them; ordinary NB2 log-`sigma` random intercepts only for plain grouping | structured scale, shape, zero-inflation, and hurdle random effects need family-specific likelihood, extractor, diagnostic, and recovery contracts |
