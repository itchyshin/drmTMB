...
Wald intervals as a shortcut for derived q=4 correlations, `total_variance_share`,
`phylo_total_variance_share`, unavailable rows, or weak-Hessian fits that need
...
| Phylogenetic structure | Gaussian q=1 univariate `mu` and `sigma` intercepts with optional matching `mu`/`sigma` correlation; one numeric `mu` slope; the exact q1 `sigma` one-slope route; a local-fit/oracle Gaussian location-only OU alternative; and an ML-only joint unlabelled OU intercept pair in `mu`/`sigma` with independent `alpha_mu` and `alpha_sigma`; Gaussian q=2 bivariate `mu1`/`mu2`; Gaussian constant q=4 location-scale intercept block; ordinary Poisson/NB2 q=1 `mu` intercept-plus-one-slope routes; separate recovery-grade NB2 q1 structured `sigma` intercept-plus-one-slope | `summary()$parameters`, marker-specific `ranef()$terms`, `corpairs(level = "phylogenetic")` for Gaussian q>=2 except the independent joint OU pair, `profile_targets()`, `check_drm()`; its rates are `decay_phylo` (`alpha_mu`) and `decay_phylo:sigma` (`alpha_sigma`), with no interval/recovery claim | OU slopes, cross-field OU correlation, labels, direct-SD formulas combined with joint OU, known sampling covariance, REML, pure, multiple, or labelled Poisson/NB2 phylogenetic slopes, zero-inflated phylogenetic effects, multiple or labelled Gaussian phylogenetic slopes, phylogenetic slope correlations, and structured `rho12` |
