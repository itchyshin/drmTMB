...
| Poisson and NB2 | fixed-effect `mu`; NB2 also has fixed-effect `sigma` and the first ordinary log-`sigma` random-intercept gate | ordinary non-zero-inflated `mu` random intercepts and independent numeric slopes; ordinary NB2 `sigma` random intercepts; ordinary Poisson/NB2 also fit q=1 `phylo(1 | species, tree = tree)` in `mu` | Poisson/NB2 q=1 phylogenetic `mu` intercepts only; no labelled count covariance | correlated count slopes, zero-inflated or hurdle random effects, NB2 `sigma` slopes or structured `sigma` effects, spatial/animal/`relmat()` structured count effects, and broader count covariance |
...
| Non-Gaussian structured dependence | Poisson and NB2 q=1 phylogenetic `mu` intercepts fitted; other routes planned | no structured count slopes yet | only ordinary Poisson/NB2 `phylo(1 | species, tree = tree)` on the log-mean scale | smoke/artifact only for Poisson phylogenetic q=1; focused NB2 smoke tests now cover extraction, diagnostics, and planned-neighbour errors | Direct `log_sd_phylo` profile targets; no derived correlations |
...
| Non-Gaussian ordinary `mu` | Poisson and NB2 q=1 random intercepts, independent numeric slopes, ordinary q=1 phylogenetic `mu` intercepts, and ordinary Student-t/zero-truncated NB2/lognormal/Gamma/beta/beta-binomial `mu` random intercepts | `sdpars$mu`, `ranef()`, `ranef("phylo_mu")`, direct `log_sd_mu` or `log_sd_phylo` profile targets, `check_drm()` | correlated non-Gaussian slopes, labelled covariance, structured slopes, spatial/animal/`relmat()` structured dependence, zero-inflated/hurdle random effects, and cross-parameter covariance |
...
| Non-Gaussian scale, shape, `zi`, `hu`, ordinal, or bounded-response components | First NB2 scale slice only | fixed effects where implemented; ordinary NB2 log-`sigma` random intercepts | random effects and structured dependence outside ordinary NB2 `sigma` intercepts and ordinary Poisson/NB2 q=1 phylogenetic `mu` |
...
| phylogenetic or spatial count dependence | ordinary Poisson or NB2 q=1 `phylo(1 | species, tree = tree)` when the structured effect is a phylogenetic log-mean intercept; ordinary Poisson/NB2 `mu` random effects when a plain group is enough | phylogenetic count slopes, zero-inflated phylogenetic counts, NB2 `sigma` phylogeny, and `spatial()`, `animal()`, or `relmat()` inside non-Gaussian likelihoods |
...
| NB2 phylogenetic or spatial count model | ordinary NB2 q=1 `phylo(1 | species, tree = tree)` in `mu` when fixed-effect `sigma` is enough, or ordinary NB2 `mu` random effects if a plain group is enough | NB2 phylogenetic slopes, zero-inflated NB2 phylogeny, NB2 `sigma` phylogeny, and spatial/animal/`relmat()` count routes |
| Poisson structured count slopes | ordinary Poisson/NB2 independent numeric `mu` slopes when the grouping is exchangeable | structured count slopes wait until the q1 phylogenetic intercept has recovery and diagnostic evidence |
| Poisson `animal()` or `relmat()` count dependence | ordinary Poisson/NB2 `mu` random effects, or Gaussian `animal()`/`relmat()` when the response is Gaussian | known-relatedness count routes need their own q1 issue after Poisson phylogenetic recovery |
