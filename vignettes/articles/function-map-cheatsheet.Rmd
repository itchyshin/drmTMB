...
| Record which build produced a result, when the version number cannot tell two builds apart | `drm_provenance()` | [Errors, warnings, and convergence](convergence.html) |
...
`drmTMB()` is the primary R workflow shown here. Julia is an optional
current fitting route for the documented admitted model cells. The native TMB
engine remains the default for unsupported models, controls, and inference
routes.
