...
validated per response family against two single-source-of-truth allow-lists
(`drm_missing_response_families()`, `drm_missing_predictor_families()`), and a
parametrized test asserts that every family outside them rejects loudly — so an
unsupported request is a clear error, never a silent wrong likelihood. The full
worked walkthrough is in `vignette("missing-data")`.

| Response family | Missing response (`response = "include"`) | Missing predictor `mi()` (`predictor = "model"`) |
| --- | :---: | :---: |
...
| every other family | — (rejects) | — (rejects) |

Two details behind the checkmarks:
...
terms; missing responses for families outside the six above; and EM, profile, or
REML missing-data engines.
