...
| REML for non-Gaussian families; non-phylogenetic mean-side REML outside that pure-`mu` Arc 1a boundary; any bivariate non-phylogenetic structured REML | Not available (rejected by design) | Use `REML = FALSE` (maximum likelihood) |
...
only with constant `sigma ~ 1` and no sigma random effect. It rejects
non-phylogenetic mean-side shapes outside that boundary, every bivariate
non-phylogenetic structured route, and every non-Gaussian family outright.
