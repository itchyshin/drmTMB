...
| Ordinary independent `mu` random slope `(0 + x \| id)` -- binomial, cumulative-logit, skew-normal, Tweedie, zero-one-beta | Inference-ready with caveats | ML-Laplace profile interval for the natural-scale slope SD at the certified deployment floor `M >= 16` per family, each under its own recorded design; ML only, not `supported`, and no Wald or point-bias claim. Zero-one-beta is generator-qualified: its executed generator leaked rare machine-exact boundary values, so its coverage holds only for that generator (see the zero-one-beta caveat below), and cumulative-logit uses adaptive Gauss-Hermite with a Cox-Reid restricted likelihood |
...
zero-one-beta. The claim is narrow and identical in shape across them: the
ML-Laplace profile interval for the *natural-scale* slope SD, at a certified
deployment floor of `M >= 16` groups, each family under its own recorded
simulation design. These are ML only -- not REML -- and make no Wald-interval or
point-bias claim. The cumulative-logit cell reaches this tier through adaptive
Gauss-Hermite quadrature with a Cox-Reid restricted likelihood; the other four
use the standard Laplace approximation.
