...
Evidence accumulates through the following tiers. A higher tier retains the
capabilities below it.

1. **Supported.** The route is inference-ready without a material caveat in
   its declared domain. This is the strongest, narrowest status.
...
## Capability skim (family × dpar × RE × interval tier)

Use this skim when you need a **parseable** first answer without opening the
capability ledger TSV. Counts and highest-evidence labels are a snapshot of the
generated model surface dated **2026-08-05**
(`docs/dev-log/dashboard/capability-surface.md`). They are **not** a promotion,
and they are not a claim that every cell in a family sits at the family's
highest tier — read the exact cell and claim boundary before reporting an
interval as inference-ready.

**Evidence ladder (same definitions as above):** supported >
inference-ready with caveats > interval-feasible > point-fit recovery >
diagnostic-only. Model-surface snapshot: **699** cells; evidence counts
**4** supported / **29** inference-ready / **187** interval-feasible /
**55** recovery-grade; runtime **339** implemented / **350** not currently
supported / **10** actionable not implemented.
...
| `binomial` | `mu` | `mu` intercept + slope | not generally available | inference-ready with caveats |
...
imply coverage. For the long caveat table, stay on this page; for the full
generated census, see the dashboard snapshot linked above.
...
| --- | --- | --- |
| Frozen-margin latent-normal association -- all five admitted pair classes, plus the Bernoulli x ordinary-NB2 fixed-effect association formula | Interval-feasible | Alpha-scale `vcov()` and Wald `confint()`, plus derived eta standard errors and transformed pointwise intervals, when fit-specific Godambe diagnostics pass; coverage is uncalibrated outside the next row and the method warns accordingly |
...
`conf.status = "newdata_required"` and no bounds until you supply the
covariate values. Use `confint(fit, parm = "rho12", newdata = grid,
...
post-0.6 development association route, see the frozen-margin article; it is a
separate staged interface and is not a Julia fit.

### Frozen-margin association intervals

All five admitted fixed-effect, complete-pair association classes are
