...
| REML for non-Gaussian families; non-phylogenetic mean-side REML outside the pure-`mu` Arc 1a boundary and the exact bivariate-spatial q2 intercept above; all other bivariate non-phylogenetic structured REML | Not available (rejected by design) | Use `REML = FALSE` (maximum likelihood) |
...
only with constant `sigma ~ 1` and no sigma random effect. Arc 1b-S1 also
accepts matching labelled fixed-covariance
...
random, direct-SD, or `corpair()` layer, at recovery-only grade. It rejects
non-phylogenetic mean-side shapes outside those boundaries,
all other bivariate non-phylogenetic structured routes, and every non-Gaussian
family outright.
