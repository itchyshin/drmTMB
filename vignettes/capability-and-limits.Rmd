...
If you are deciding what to report from an ecology or evolution analysis, use
this page before putting a fitted `drmTMB` model in a manuscript. A successful
fit is not, by itself, evidence that its point estimate or interval is ready
to report. The summary below gives five distinct answers: whether the model can
be fitted, whether its point estimate can be reported, whether its named
interval can be reported, what simpler model or other tool to try when it
cannot, and the exact scope that was tested.

`mu` is the **location** parameter: it describes the family-specific centre
of the response. `sigma` is the **scale** parameter: it describes residual
variability. `nu` is a **shape** parameter for distributions such as the
Student-t, describing features beyond location and scale. **Coscale** models
residual correlation such as `rho12`: association between two responses left
after their locations and scales are modelled. `sd(group)` describes
among-group random-effect variation, not residual `sigma`; `phylo()` and
`spatial()` specify phylogenetic and coordinate-structured random effects;
and `meta_V(V = V)` supplies known sampling covariance for the specialist
meta-analysis route. `meta_known_V(V = V)` is a deprecated compatibility
alias. This page does not use `tau`, because it is not a general drmTMB
parameter name.

## Can I fit and report this model?

The current ledger supplies this summary when the article is built. Read the
scope and fallback fields before generalising a result to a different family,
parameter, grouping structure, or sample size.
...
None of these structured cells carries the "supported" `fit_status`
