...
`beta_family()`, and `nbinom2()` across `n` in `{50, 150, 500}`, 400 seeds per
...
calibrated (coverage 0.93-0.97 across `n`), and to `beta_family()` location-scale
models on interior proportions, where mean and `sigma ~ x` coefficients are
also calibrated (coverage 0.93-0.95). The `beta_family()` result carries one hard
...
`zero_one_beta()` instead of `beta_family()`.
...
**Julia cross-family fitting is reachable but experimental.**
`drmTMB(bf(...), c(fam1, fam2), engine = "julia")` reaches DRM.jl's
`fit_mixed_family` and is tested, but its capability-ledger status is
`experimental`/`partial` (`inst/extdata/julia-capabilities.tsv`,
`cross_family_latent`) -- one family pair, one fixture, no simulation-recovery
evidence yet. Its extractors return only `u = 0` response-scale means and
response residuals for `mu1` / `mu2`; `vcov()` and fixed-effect Wald intervals
explicitly report unavailable rather than returning an empty value. What to
try next: for models this route does not cover, use the native TMB engine,
`drmTMB`'s default. For the bounded post-0.7 development association route,
see the frozen-margin article; it is a
...
| `binomial()`, `poisson()`, `nbinom2()`, `beta_family()`, `lognormal()`, `Gamma(link = "log")`, `student()`, `beta_binomial()` | ✓ (one binary predictor) |
