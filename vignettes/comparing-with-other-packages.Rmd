...
article is not a catalogue of every available model.**
...
other model, assess its assumptions and inference separately. See
[Capability and limits](capability-and-limits.html) when choosing a model
...
| Other `drmTMB` models and structures | none | not compared here; see [Capability and limits](capability-and-limits.html) | nothing to classify |
...
engine, but we have not yet assessed how independent its estimation route is.

## Checked against a separate estimation engine

The three comparisons in this section are against `lme4` and `metafor`,
which share no estimation code with `drmTMB`. Agreement here is a genuine
...
**Independence: STRONG** (`metafor` shares no estimation code with `drmTMB`).
...
`ordinal` is a separate estimation engine, but we have not yet assessed how
independent its estimation route is. The two comparisons in this section are
therefore reported without an independence label.
...
In particular, this comparison does not establish confidence-interval
calibration, coverage, or support for other ordinal random-effect structures.
It fits an ordinary random intercept, not a random slope, correlated effect,
or labelled effect.

**Independence:** `ordinal` is a separate estimation engine, but its
independence has not yet been assessed.
...
`drmTMB`. Agreement here is a consistency check between related
implementations, not a cross-implementation check. It is also, uncomfortably,
where `drmTMB`'s
...
has not yet been assessed. It is worth naming
where the
...
implements beyond these examples is summarized in
[Capability and limits](capability-and-limits.html).
